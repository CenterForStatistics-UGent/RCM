...
#   residList$abund = abunds[residList$taxon]
  ...
  weightsMat =  if(weighted){
    ...
    cMatNewList = glm.nb.col(X = X, reg = rMatPsi, current.theta = thetas, abunds = abunds, libSizes = libSizes, weights = weightsMat, cMatInit = cMat, global = global, nleqslv.control = nleqslv.control, k=k, lambdaCol = lambda, method=method)
    #Standard errors

#     cMatNew = sapply(cMatNewList, function(x){x$cMat})
#     cMatSE = sapply(cMatNewList, function(x){x$cMatSE})
    colScoresNorm = cMatNewList$cMat
...
#   weightsMat=cbind(weightsMat, rep(1,nrow(weightsMat)))
  #define the offsets
    cMatPsi = psis*cMat
    rMatNewList = glm.nb.row(X = X, reg = cMatPsi, current.theta = thetas, abunds = abunds, libSizes = libSizes, weights = weightsMat, global =global, nleqslv.control = nleqslv.control, rMatInit = rMat, lambdaRow=lambda, method=method)
    
#   rMatNew = t(sapply(rMatNewList, function(x){x$rMat}))
#   rMatSE = t(sapply(rMatNewList, function(x){x$rMatSE}))
  # rMatNew = rMatNew/rMatSE
  
  rowScoresNorm = rMatNewList$rMat
...
glm.nb.col = function (X, reg, current.theta, abunds, libSizes, weights, cMatInit, nleqslv.control, k , lambdaCol, global = "dbldog", method){
...
globalInd = 1
while(keepGoing){
tmp = try(nleqslv(fn = dNBllcol, x = beta, current.theta = current.theta, y = X, reg = reg, abunds =abunds, libSizes = libSizes, weights =weights, k=k, global = global[globalInd], control = nleqslv.control, n=n, p=p, jac = NBjacobianCol, method=method), silent=TRUE)
if(class(tmp)=="try-error" || tmp$termcd %in% c(2,3,4,5,6)){
 ...
 if(class(tmp)=="list" && tmp$termcd %in% c(2,3,4,5,6)){
  ...
  lambdaCol = tmp$x[(k*(p+2)+1):length(tmp$x)]
  beta = c(as.vector(t(cMat)), lambdaCol)
    }
...
#   rep(1,k)
# } else {diag(SEmat)}
return(list(cMat = cMat, cMatSE = 1, lambdaCol = c(lambda1, lambda2,lambdaCol)))
}
  ...
  score = as.vector(sapply(1:k, function(K){
    sapply(1:p, function(P){
      sum(reg[,K]*(y[,P]-mu[,P])/(1+t(t(mu[,P])/current.theta[P]))*weights[,P])  + 
            sum(lambda3Mat[K,]*cMat[,P]*abunds[P]) + lambda1[K]*abunds[P] + 2*lambda2[K]*abunds[P]
          })})) 
...
glm.nb.row = function (X, reg, current.theta, abunds, libSizes, weights, rMatInit, nleqslv.control, lambdaRow, global ="dbldog", method){
...
while(keepGoing){
tmp = try(nleqslv(fn = dNBllrow, x = beta, current.theta = current.theta, y = X, reg = reg, abunds = abunds, libSizes = libSizes, weights =weights, k=k, global=global[globalInd], control= nleqslv.control, p=p, n=n, jac= NBjacobianRow, method=method), silent=TRUE)
if(class(tmp)=="try-error" || tmp$termcd %in% c(2,3,4,5,6)){
 globalInd = globalInd + 1
 if(globalInd > length(global)){
   keepGoing = FALSE
 ...
 if(class(tmp)=="list" && tmp$termcd %in% c(2,3,4,5,6)){
  ...
  lambdaRow = tmp$x[(k*(n+2)+1):length(tmp$x)]
beta = c(as.vector(rMat), lambda1, lambda2,lambdaRow)
    }
} else if(tmp$termcd==1){
    keepGoing=FALSE
  }
...
#   rep(1,k)
# } else {diag(SEmat)}
return(list(rMat = rMat, rMatSE = 1, lambdaRow=c(lambda1, lambda2,lambdaRow)))
}
...
#Orthogonality restrictions sum(abunds*r_{ik}*r_{ik'}) = 0. First all of dimension 1, then the remaining of dimension 2 etc. #+2*k
  ...
  return(c(score*lambdaWeights/n,centers*lambdaWeights, unitSums*lambdaWeights, orthogons*lambdaWeights))#centers, unitSums,
}
#-------------------------------------------#
...
outerLoop = function(X, k, tol = 1e-8, maxItOut = 50, maxItML = 15, maxItNB = 25, maxItIn=10, maxItScore = 200,
                     tolML = 1e-8, tolDisp = 1e-5, tolScore = 1e-5, verbose = TRUE, mC = 1, libSizes = NULL, MLweighted =FALSE, rMatInit = NULL, cMatInit = NULL, psiInit = NULL,  global ="dbldog", nleqslv.control=list(),init="SVD",method="Broyden", ...){
...
# @return converged: a boolean indicating if the algorithm converged
 ...
 ## 1) Initialization
  ...
  lambdaRow = lambdaCol = rep(0,2*k+k*(k-1)/2)
...
#   abunds[is.na(abunds)] = (colSums(X)/sum(X))[is.na(abunds)] #Resort to simpler estimation if glm.nb fails
  ...
  while((iterOut ==1) || ((iterOut <= maxItOut) && ((any(abs(1-psi/psiOld) > tol)) || 
...
plotRCM = function(psis, rMat, cMat, Dim = c(1,2), X = NULL, thetas = NULL, 
                   ...
                   libLoc ="topright",libLegend=TRUE, ...){
...
rowScores1 = rnorm(Nsamples-Nsamples%%2) + c(rep(5,10), rep(0, Nsamples-20), rep(5,10))
rowScores2 = rnorm(Nsamples-Nsamples%%2) + c(rep(0,5),rep(8,15), rep(0, Nsamples-40), rep(8,15), rep(0,5))

#Taxa 1-10 and Nsamples - 1 - -10 have increased col scores 1
#Taxa 6-15 and Nsamples -6 - -15 have increased col scores 2
colScores1 = rnorm(Ntaxa-Ntaxa%%2) + c(rep(2,10), rep(0, Ntaxa-20), rep(2,10))
colScores2 = rnorm(Ntaxa-Ntaxa%%2) + c(rep(0,5),rep(2,15), rep(0, Ntaxa-40), rep(2,15), rep(0,5))
...
plotRCM(psis = c(psi1, psi2), rMat = rowScoresMat, cMat = colScoresMat, X = dataMat4, biplot = FALSE, libLegend=FALSE)
points(col="green", pch=2, c(rowScoresMat[1:10,1],rowScoresMat[(Nsamples-10):Nsamples,1])*psi1, c(rowScoresMat[1:10,2],rowScoresMat[(Nsamples-10):Nsamples,2])*psi2)
points(col="black", pch=3, c(rowScoresMat[6:15,1],rowScoresMat[(Nsamples-15):(Nsamples-5),1])*psi1, c(rowScoresMat[6:15,2],rowScoresMat[(Nsamples-15):(Nsamples-5),2])*psi2)
```
...
tmp3 =outerLoop(dataMat4, k=2, maxItML = 10, maxItOut = maxItOut, printN = 1, mC = 1, MLweighted=FALSE,nleqslv.control=list(xtol=1e-8, ftol=1e-12, btol=1e-3, sigma=0.5, trace=TRUE, allowSingular=TRUE, maxit=1e2), tol = 1e-6, global = c("dbldog"), method="Broyden")

tmp3c = outerLoop(dataMat4, k=2, maxItML = 10, maxItOut = maxItOut, printN = 1, mC = 1, MLweighted=FALSE, rMat = tmp3$rMat, cMat =  tmp3$cMat, psiInit = tmp3$psis, nleqslv.control=list(xtol=1e-10, ftol=1e-12, btol=1e-3, sigma=0.5, trace=TRUE, allowSingular=FALSE, maxit=5e2), tol = 1e-4, global = c("dbldog"),maxItNB = 25)
