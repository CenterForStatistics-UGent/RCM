...
6. Repeat steps 2-5 until convergence. Convergence is assumed if between two iterations
 ...
 and analogously for all the scores. Other norms, such as L1 or infinity norms can also be used.
...
plotRCM(psis = c(psi1, psi2), rMat = rowScoresMat, cMat = colScoresMat, X = dataMat4, biplot = TRUE, libLegend=TRUE, libInset = c(0,-0.2), dispInset = c(0,-0.25), abInset = c(0,-0.25), arrowFrac=0.01, main="True underlying deviations from independence")
...
#Taxa
arrowLengthsTrue = apply(colScoresMat, 2,function(x){sum(x^2)})
...
# plot(arrowLengthsTrue, col=colo)
# legend("topright",legend=c("Signal in two dimensions","Signal in one dimensions", "taxa without signal"), pch=1, col=c("red","orange", "green"), ylab="True signal strengths")

df = data.frame(arrowLength = arrowLengthsTrue, signals = mapply(id1,id2,FUN= function(one, two){3-((one==0) + (two==0)) }))
...
Convergence after `r sum(syntNB$psiRec[1,]!=0)` iterations
...
```{r monoplot samples}
plotRCM(syntNB$psis, syntNB$rMat, syntNB$cMat, X = dataMat4, thetas = thetas,abunds = rhos, arrowFrac = 0.05, biplot =FALSE, main="Estimated deviations from independence (NB data)")
...
```{r NB biplot samples}
plotRCM(syntNB$psis, syntNB$rMat, syntNB$cMat, X = dataMat4, arrowFrac = 0.05, biplot =TRUE, main="Estimated deviations from independence (NB data)")
points(y=syntNB$psis[2]*syntNB$rMat[c(1:10),2], syntNB$psis[1]*syntNB$rMat[c(1:10),1], col="green", pch=2)
points(y=syntNB$psis[2]*syntNB$rMat[,2][c((Nsamples-9):Nsamples)], syntNB$psis[1]*syntNB$rMat[,1][c((Nsamples-9):Nsamples)], col="black", pch=3)
```

Are the taxa whose abundance deviate from 

```{r NB plots: taxa}
#Taxa
arrowLengthsNBest = apply(syntNB$cMat, 2,function(x){sum(x^2)})
id1 =  c(rep(0,10),rep(25,10), rep(0, Ntaxa-30), rep(25,10))
id2 = c(rep(0,5),rep(2,15), rep(0, Ntaxa-40), rep(2,15), rep(0,5))
...
df = data.frame(arrowLength = arrowLengthsNBest, signals = mapply(id1,id2,FUN= function(one, two){(one==0) + (two==0) }))
boxplot(arrowLength ~ signals, data=df, main="Estimated impact on means vs. true number of dimensions modified", xlab="True number of dimensions affected", ylab="Estimated strength of signal")
```
...
We now generate data from another distribution, the Dirichlet multinomial, and see if the algortihm still converges
...
```{r DM plots}
#Samples
plotRCM(DMsol$psis, DMsol$rMat, DMsol$cMat, X = dataMat4DMtrim, arrowFrac = 0.05, biplot =TRUE, main="Dirichlet multinomial")#, abunds=colSums(DMsol$X)/sum(DMsol$X), thetas=DMsol$thetas)
...
The two signals are being picked up, one in both dimensions(green traingles) and one in the second dimension(black crosses). Are the signals also assigned to the right taxa? 
...
dimAf = rep(0,length(arrowLengthsDM))
dimAf[c((1:x/2),(NtaxaDM-x/2+1))] = dimAf[c((1:x/2),(NtaxaDM-x/2+1))]+1

dfDM = data.frame(arrowLength = arrowLengthsDM, signals = dimAf)
boxplot(arrowLength ~ signals, data=dfDM, main="Estimated impact on means vs. true number of dimensions modified", xlab="True number of dimensions affected", ylab="Estimated strength of signal")
```

Because of compositionality or high dispersion, the algorithm cannot distinguish abundance and signal in the taxa
...
```{r Own Method: loglinear ZIP, eval=FALSE}
maxItOut = 1e4
...
save(syntZIP,dataMatZIP, colscoresMatZIP, rowscoresMatZIP, psi1ZIP, psi2ZIP, libSizesZIP, rhosZIP, file="toyDataResZIP.RData")
...
```{r ZIP plots}
#Samples
plotRCM(syntZIP$psis, syntZIP$rMat, syntZIP$cMat, X = syntZIP$X, arrowFrac = 0.01, biplot =TRUE, main="Dirichlet multinomial")#, abunds=colSums(syntZIP$X)/sum(syntZIP$X), thetas=syntZIP$thetas)
...
The two signals are being picked up, one in both dimensions(black crosses) and one in the second dimension(green triangles). Are the signals also assigned to the right taxa? 
...
dfZIP = data.frame(arrowLength = arrowLengthsZIP, signals = dimAf)
boxplot(arrowLength ~ signals, data=dfZIP, main="Estimated impact on means vs. true number of dimensions modified", xlab="True number of dimensions affected", ylab="Estimated strength of signal")
```
...
```{r Kostic Data: Age}
#Effect of age
plotRCM(DDLL$psis, DDLL$rMat, DDLL$cMat, X = otu_table(phyloDD), arrowFrac=0.01, cex= as.integer(sample_data(phyloDD)$Age_at_Collection/356)+1)#, abunds=colSums(DDLL$X)/sum(DDLL$X)
...
This is a tough one, presumably because it's very large and noisy and no clear biological signal is present

## Zeller 2014 metagenomics data
...
Zellerjob16S = mcparallel(phyloWrapper(zellerSphy, k=2, nleqslv.control=list(trace=TRUE, maxit=250), method="Broyden", tol=1e-4, maxItOut=1000, round=TRUE))
...
```{r Zeller metaPlot}
plotRCM(ZellerRCmeta$psis, ZellerRCmeta$rMat, ZellerRCmeta$cMat, X=ZellerRCmeta$X, libLoc = "topright", libInset = c(0,-0.25))
```
...
legend("topright",legend=c("Cancer","Normal","Small adenoma"), pch=1, col=c("darkred","green", "blue"), inset=c(0,-0.3), xpd=TRUE)
```

The first dimension seems to have a Cancer signal although it is not very strong

```{r Zeller metaPlot Age}
plotRCM(ZellerRCmeta$psis, ZellerRCmeta$rMat, ZellerRCmeta$cMat, cex=sample_data(zellerMphy)$Age/50)
legend("topright",legend=c("Younger","Older"), pch=1, pt.cex=c(0.9,1.6), inset=c(0,-0.3), xpd=TRUE)
```

```{r Zeller metaPlot Age}
plotRCM(ZellerRCmeta$psis, ZellerRCmeta$rMat, ZellerRCmeta$cMat, cex=sample_data(zellerMphy)$BMI/20)
legend("topright",legend=c("Lower BMI","Higher BMI"), pch=1, pt.cex=c(0.9,1.6), inset=c(0,-0.3), xpd=TRUE)
```
