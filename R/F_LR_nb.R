...
#'  at the given alpha
LR_nb <- function(Alpha, X, CC,
                responseFun = c("linear","quadratic","nonparametric","dynamic"),
                muMarg, psi, nleqslv.control = list(trace=FALSE), n, NB_params,
                NB_params_noLab, thetaMat, ncols, nonParamRespFun,
                envGradEst,...){
  ...
  if(responseFun %in% c("linear","quadratic","dynamic")){
    ...
    muT = exp(nonParamRespFun$rowMat) * muMarg
    if(envGradEst=="LR") mu0 = c(exp(nonParamRespFun$rowVecOverall)) * muMarg
  }
  logDensj = dnbinom(X, mu = muT, size = thetaMat, log = TRUE)
  #Likelihoods under species specific model
  #Immediately return log likelihoods

  if(envGradEst=="LR") logDens0 = dnbinom(X, mu = mu0, size = thetaMat,
                                          log = TRUE) #Likelihoods of null model

  lr <- switch(envGradEst, "LR" = sum(logDensj-logDens0), "ML" = sum(logDensj))
