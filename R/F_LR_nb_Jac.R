...
LR_nb_Jac <- function(
    ...
    responseFun <- switch(responseFun,
        dynamic = "quadratic",
        responseFun
    )
    ...
    Jac[did, did] <- switch(responseFun,
        ...
        quadratic = switch(envGradEst,
                ...
                colSums((tensor(vapply(did,
                    FUN.VALUE = tmp, function(x) {
                        CC[, x] * (-psi^2 * (tmp * (matrix(NB_params[2, ]^2, n, ncols,
                            byrow = TRUE
                        ) + 4 * outer(cSam, NB_params[2, ] * NB_params[3, ]) + 4 * outer(cSam2, NB_params[3, ]^2)) - tmp0 *
                            (NB_params_noLab[2] +
            ...
            ML = colSums(tensor(vapply(did,
                FUN.VALUE = tmp,
                function(x) {
                    CC[, x] * (-psi^2 * (tmp * (matrix(NB_params[2, ]^2, n, ncols,
                        byrow = TRUE
                    ) + 4 * outer(cSam, NB_params[2, ] * NB_params[3, ]) + 4 * outer(cSam2, NB_params[3, ]^2)) + 2 * psi *
                        (rowMultiply((X -
