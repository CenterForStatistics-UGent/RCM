...
NBalphaInfl = function(rcm, Dim) {
    ...
    thetaMat = matrix(rcm$thetas, byrow = TRUE, 
        n, p)
    NB_params = rcm$NB_params[, , Dim]
    NB_params_noLab = rcm$NB_params_noLab[, 
        Dim]
    psi = rcm$psis[Dim]
    ...
    muMarg = extractE(rcm, seq_len(Dim - 
        1))
    ...
    if (envGradEst == "LR") {
        mu0 = muMarg * c(exp(buildDesign(sampleScore, 
            responseFun) %*% NB_params_noLab * 
            psi))
        tmp0 = (X - mu0)/(1 + mu0/thetaMat)
    }
    # A n-by-p-by-d array
    score = switch(responseFun, linear = if (envGradEst == 
        "LR") {
        psi * (vapply(seq_len(d), FUN.VALUE = tmp, 
            function(i) {
                tmp2 * CC[, i]
            }) - NB_params_noLab[2] * vapply(seq_len(d), 
            FUN.VALUE = tmp0, function(i) {
                tmp0 * CC[, i]
            }))
    } else {
        psi * (vapply(seq_len(d), FUN.VALUE = tmp, 
        ...
        psi * (vapply(seq_len(d), FUN.VALUE = tmp, 
            function(i) {
                tmp2 * CC[, i]
            }) + 2 * vapply(seq_len(d), FUN.VALUE = tmp, 
            function(i) {
                rowMultiply(tmp, NB_params[3, 
                  ]) * CC[, i] * c(sampleScore)
            }) - NB_params_noLab[2] * vapply(seq_len(d), 
            FUN.VALUE = tmp0, function(i) {
                tmp0 * CC[, i]
            }) - 2 * NB_params_noLab[3] * 
            vapply(seq_len(d), FUN.VALUE = tmp, 
                function(i) {
    ...
    # function
    rep(c(lambda1s %*% centMat) + lambda2 * 
        2 * alpha + if (Dim > 1) rowSums(rcm$alpha[, 
        seq_len(Dim - 1), drop = FALSE] %*% 
        lambda3) else 0, each = n * p)
    
    JacobianInv = -solve(LR_nb_Jac(Alpha = c(alpha, 
        lambda1s, lambda2, lambda3), X = X, 
        CC = CC, responseFun = responseFun, 
        psi = psi, NB_params = NB_params, 
        NB_params_noLab = NB_params_noLab, 
        d = d, k = Dim, centMat = centMat, 
        nLambda = nLambda1s + Dim, nLambda1s = nLambda1s, 
        thetaMat = thetaMat, muMarg = extractE(rcm, 
            seq_len(Dim - 1)), n = n, ncols = p, 
        envGradEst = envGradEst, alphaK = rcm$alpha[, 
            seq_len(Dim - 1), drop = FALSE], 
        preFabMat = 1 + X/thetaMat))[seq_len(d), 
        seq_len(d)]
    # Return only alpha indices, don't forget
    # the minus sign!
