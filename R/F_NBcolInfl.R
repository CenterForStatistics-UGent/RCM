...
NBcolInfl = function(rcm, Dim = 1){
  ...
  score = (reg *(rcm$X-mu)/(1+mu/thetaMat)) +
    rcm$colWeights*(lambdaCol[1] + lambdaCol[2]*2*rcm$cMat + tmp)

  JacobianInv = solve(NBjacobianCol(beta = c(rcm$cMat[Dim,], lambdaCol),
                                    X = rcm$X, reg = reg, thetas = thetaMat,
                                    muMarg = mu, k = Dim, p = nrow(rcm$X),
                                    n=ncol(rcm$X) ,colWeights = rcm$colWeights,
                                    nLambda = length(lambdaCol), cMatK = cMatK))
  #Inverse Jacobian
