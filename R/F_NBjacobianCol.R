...
NBjacobianCol = function(beta, X, reg, thetas, muMarg, k, n, p, colWeights,
                         nLambda, cMatK, preFabMat, Jac){
  cMat = beta[seq_len(p)]

  #Calculate the mean
  mu = exp(reg %*% cMat) * muMarg

  #The symmetric jacobian matrix. The upper part is filled first,
  #then mirror image is taken for lower triangle
  ...
  #dLag²/dr_{ik}²
  diag(Jac)[seq_len(p)] = -crossprod(preFabMat*mu/(1+mu/thetas)^2, reg^2) +
    2*beta[p+2]*colWeights
  Jac
}
