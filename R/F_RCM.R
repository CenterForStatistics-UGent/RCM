...
#' @rdname RCM
#' @export
setMethod("RCM", "phyloseq", function(
      dat, covariates = NULL,
      confounders = NULL, ...
) {
    ...
    tmp <- RCM(X,
        confModelMat = confModelMat, confTrimMat = confTrimMat,
        covModelMat = covModelMat, centMat = centMat,
        ...
    )
    tmp$physeq <- prune_samples(rownames(tmp$X), prune_taxa(
        colnames(tmp$X),
        dat
    ))
    return(tmp)
