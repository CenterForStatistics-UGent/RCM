...
#' @export
setGeneric("RCM", 
           function(dat, ...) standardGeneric("RCM"))
...
setMethod("RCM",
          ...
          function(dat, k = 2, round=FALSE, 
            ...
            #Check X type
            if(!all(floor(X)==X)){
              stop("Please provide integer count matrix (not a matrix of proportions!), or set 'round' to TRUE! \n")
            } else{
              X=matrix(as.integer(X), ncol=p, nrow=n)
              }
