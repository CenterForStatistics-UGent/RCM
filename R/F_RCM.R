...
setMethod("RCM",
          "phyloseq",
          function(dat, covariates = NULL, confounders = NULL, ...) {
            # Safer than using @.Data, as this uses the standard conversion
            # function of the package as opposed to the internal representation.
            ...
            if (!is.null(covariates)) {
              ...
              if (anyNA(datFrame)) {
                ...
                warning(
                  paste(
                    "Some covariates contain missing values.
                    We removed samples \n",
            ...
            tmp = RCM(
              ...
              confTrimMat = confTrimMat,
              covModelMat = covModelMat,
              centMat = centMat,
              ...
            )
...
setMethod("RCM",
          ...
          function(dat,
            ...
            if (!all(floor(dat) == dat)) {
              stop(
                "Please provide integer count matrix
            ...
            #Remove taxa with low prevalence and those who do not meet the
            #minFraction requirement
            if (any(rowSums(dat) == 0)) {
              warning(
                immediate. = TRUE,
                paste0(
                  "Samples \n",
                  paste(collapse = ", ", rownames(dat)[rowSums(dat) == 0]),
