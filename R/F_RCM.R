...
#' @param round a boolean, whether to round to nearest integer.
#'  Defaults to FALSE.
#' @param prevCutOff a scalar, the prevalance cutoff for the trimming.
#' Defaults to 2.5e-2
#' @param minFraction a scalar, each taxon's total abundance should equal
#'  at leat the number of samples n times minFraction, otherwise it is trimmed.
#'   Defaults to 10\%
#' @param rowWeights,colWeights character strings, the weighting procedures
#'  for the normalization of row and column scores.
#'  Defaults to 'uniform' and 'marginal' respectively
#' @param covariates In case 'dat' is a phyloseq object,
#' the names of the sample variables to be used as covariates
#'  in the constrained analysis, or 'all' to indicate all variables to be used.
#'   In case 'dat' is a matrix, a nxf matrix or dataframe of covariates.
#'    Character variables will be converted to factors, with a warning.
#'    Defaults to NULL, in which case an unconstrained analysis is carried out.
#' @param confounders In case 'dat' is a phyloseq object,
#' the names of the sample variables to be used as confounders
#'  to be filtered out. In case 'dat' is a matrix,
#'   a nxf matrix or dataframe of confounders.
#'    Character variables will be converted to factors, with a warning.
...
RCM = function(dat, k = 2, round = FALSE, 
    prevCutOff = 0.05, minFraction = 0.1, 
    ...
    if (classDat == "matrix") {
        ...
        X = if (taxa_are_rows(dat)) {
            t(otu_table(dat)@.Data)
        } else {
            otu_table(dat)@.Data
        }
    } else {
        stop("Please provide a matrix or a phyloseq object! \n")
    }
    p = ncol(X)
    n = nrow(X)
    ...
    rownames(X) = row
    mes
    X = X[, (colMeans(X == 0) < (1 - prevCutOff)) & 
        (colSums(X) > (n * minFraction))]
    # Remove txa with low prevalence and
    # those who do not meet the minFraction
    # requirement
    if (any(rowSums(X) == 0)) {
        warning(immediate. = TRUE, paste0("Samples \n", 
            paste(collapse = ", ", rownames(X)[rowSums(X) == 
    ...
    if (!is.null(confounders)) {
        tmp = buildConfMat(confounders, n, 
            dat)
    ...
    if (!is.null(covariates)) {
        tmp = buildCovMat(covariates, n, 
            dat)
        covModelMat = tmp$covModelMat
        covariates = tmp$covariates
        # Already prepare the matrix that defines
        # the equations for centering the
        ...
        # Remove rows with NA's, we might want to
        # find something better or leave it to
        # the end user
        if (anyNA(datFrame)) {
    ...
    tmp = RCM_NB(X, rowWeights = rowWeights, 
        colWeights = colWeights, k = k, confounders = list(confounders = confModelMat, 
            confoundersTrim = confModelMatTrim), 
        covariates = covModelMat, prevCutOff = prevCutOff, 
        minFraction = minFraction, centMat = centMat, 
        ...)
    if (classDat == "phyloseq") {
        tmp$physeq = prune_samples(rownames(tmp$X), 
            prune_taxa(colnames(tmp$X), dat))
    }
    tmp = within(tmp, {
