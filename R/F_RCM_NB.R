...
RCM_NB = function(X, k, rowWeights = "uniform",
    colWeights = "marginal", tol = 0.001,
    maxItOut = 1000L, Psitol = 0.001, verbose = FALSE,
    global = "dbldog", nleqslv.control = list(maxit = 500L,
        cndtol = 1 - 16), jacMethod = "Broyden",
    dispFreq = 10L, convNorm = 2, prior.df = 10,
    marginEst = "MLE", confounders = NULL,
    prevCutOff, minFraction = 0.1, covariates = NULL,
    centMat = NULL, responseFun = c("linear",
        "quadratic", "dynamic", "nonparametric"),
    record = FALSE, control.outer = list(trace = FALSE),
    control.optim = list(), envGradEst = "LR",
    dfSpline = 3, vgamMaxit = 100L, degree = switch(responseFun[1],
    ...
    thetas = matrix(0, p, k + 1 + (!is.null(confounders$confounders)),
        dimnames = list(colnames(X), c("Independence",
            if (!is.null(confounders$confounders)) "Filtered" else NULL,
    ...
    # Get the trended-dispersion estimates.
    # Very insensitive to the offset so only
    # need to be calculated once per
    # dimension
    trended.dispersion <- edgeR::estimateGLMTrendedDisp(y = t(X),
        design = NULL, method = "bin.loess",
        offset = t(log(outer(libSizes, abunds))),
        weights = NULL)
    ...
    if (marginEst == "MLE") {
        ...
        while ((initIter == 1) || ((initIter <=
            maxItOut) && (!convergenceInit))) {
            logLibsOld = logLibSizesMLE
            logAbsOld = logAbundsMLE

            thetas[, "Independence"] = estDisp(X = X,
                rMat = as.matrix(rep(0, n)),
                cMat = t(as.matrix(rep(0,
                  p))), muMarg = exp(outer(logLibSizesMLE,
                  logAbundsMLE, "+")), psis = 0,
                prior.df = prior.df, trended.dispersion = trended.dispersion)
            thetasMat = matrix(thetas[, "Independence"],
                n, p, byrow = TRUE)

            logLibSizesMLE = nleqslv(fn = dNBlibSizes,
                x = logLibSizesMLE, theta = thetasMat,
                X = X, reg = logAbundsMLE,
                global = global, control = nleqslv.control,
                jac = NBjacobianLibSizes,
                method = jacMethod)$x
            logAbundsMLE = nleqslv(fn = dNBabunds,
                x = logAbundsMLE, theta = thetasMat,
                X = X, reg = logLibSizesMLE,
                global = global, control = nleqslv.control,
                jac = NBjacobianAbunds, method = jacMethod)$x
            initIter = initIter + 1

            convergenceInit = ((initIter <=
                maxItOut) && ((sum(abs(1 -
                logLibSizesMLE/logLibsOld)^convNorm)/n)^(1/convNorm) <
                tol) && ((sum(abs(1 - logAbundsMLE/logAbsOld)^convNorm)/p)^(1/convNorm) <
                tol))
        }
        # Converges very fast, even when
        # dispersions are re-estimated. For the
        # library sizes there is a big
        # difference, for the abundances less so
        muMarg = exp(outer(logLibSizesMLE,
            logAbundsMLE, "+"))
        # The marginals to be used as
        # expectation. These are augmented with
        # the previously estimated dimensions
        # every time
    ...
    rowWeights = switch(paste(marginEst,
        rowWeights, sep = "_"), marginSums_marginal = libSizes/sum(libSizes),
        MLE_marginal = exp(logLibSizesMLE)/sum(exp(logLibSizesMLE)),
        rep.int(1/n, n)  #For uniform weights
)
    colWeights = switch(paste(marginEst,
        colWeights, sep = "_"), marginSums_marginal = abunds,
    ...
    if (record) {
        ...
        colRec = thetaRec = array(0, dim = c(k,
            p, maxItOut))
    ...
    iterOut = rep(1, k)
    if (!is.null(confounders$confounders)) {
        ## Filter out the confounders by adding
        ## them to the intercept, also adapt
        ## overdispersions
        filtObj = filterConfounders(muMarg = muMarg,
            confMat = confounders$confounders,
            p = p, X = X, thetas = thetas[,
                1], nleqslv.control = nleqslv.control,
            n = n, trended.dispersion = trended.dispersion)
        muMarg = muMarg * exp(confounders$confounders %*%
            filtObj$NB_params)
    ...
    ## 1) Initialization
    svdX = svd(diag(1/libSizes) %*% (X -
        muMarg) %*% diag(1/colSums(X)))
    ...
    # Redistribute some weight to fit the
    # constraints
    ...
    if (is.null(covariates)) {
        ...
        for (KK in seq_len(k)) {
            ...
            if (KK > 1) {
                muMarg = muMarg * exp(rMat[,
                  (KK - 1), drop = FALSE] %*%
                  (cMat[(KK - 1), , drop = FALSE] *
                    psis[(KK - 1)]))
            }
            idK = seq_k(KK)  #prepare an index

            # Re-estimate the trended dispersions,
            # once per dimensions
            trended.dispersion <- edgeR::estimateGLMTrendedDisp(y = t(X),
                design = NULL, method = "bin.loess",
                offset = t(log(muMarg)),
            ...
            JacR = matrix(0, nrow = n + KK +
                1, ncol = n + KK + 1)  #Prepare sparse Jacobians,
            # and prefill what we can
            JacR[seq_len(n), n + 1] = rowWeights
            if (KK > 1) {
                JacR[seq_len(n), (n + 3):(n +
                  KK + 1)] = rMat[, seq_len(KK -
                  1), drop = FALSE] * rowWeights
            }
            # Symmetrize
            JacR = JacR + t(JacR)

            JacC = matrix(0, nrow = p + KK +
                1, ncol = p + KK + 1)
            JacC[seq_len(p), p + 1] = colWeights
            if (KK > 1) {
                JacC[seq_len(p), (p + 3):(p +
                  KK + 1)] = t(cMat[seq_len(KK -
                  1), , drop = FALSE]) *
                  colWeights
            }
            # Symmetrize
            ...
            while ((iterOut[KK] == 1) ||
                ...
                # iterations to speed things up, the
                # estimates do not change a lot anyway)
                if ((iterOut[KK]%%dispFreq) ==
                  ...
                  thetasMat = matrix(thetas[,
                    paste0("Dim", KK)], n,
                    p, byrow = TRUE)
                  # Make a matrix for numerical reasons, it
                  # avoids excessive use of the t()
                  # function
               ...
               the prevCutOff parameter.\n")
                ...
                regCol = rMat[, KK, drop = FALSE] *
                  psis[KK]
                tmpCol = nleqslv(fn = dNBllcol,
                  x = c(cMat[KK, ], lambdaCol[idK]),
                  thetas = thetasMat, X = X,
                  reg = regCol, muMarg = muMarg,
                  k = KK, global = global,
                ...
                if (verbose)
                  cat(ifelse(tmpCol$termcd ==
                    1, "Column scores converged \n",
                    "Column scores DID NOT converge \n"))
                cMat[KK, ] = tmpCol$x[seq_len(p)]
                lambdaCol[idK] = tmpCol$x[p +
                  seq_along(idK)]

                # Normalize (speeds up algorithm if
                # previous step had not converged)
                cMat[KK, ] = cMat[KK, ] -
                  sum(cMat[KK, ] * colWeights)/sum(colWeights)
                cMat[KK, ] = cMat[KK, ]/sqrt(sum(colWeights *
                  cMat[KK, ]^2))
                # Row scores
                if (verbose)
                  cat("\n Estimating row scores \n")
                regRow = cMat[KK, , drop = FALSE] *
                  psis[KK]
                tmpRow = nleqslv(fn = dNBllrow,
                  x = c(rMat[, KK], lambdaRow[idK]),
                  thetas = thetasMat, X = X,
                  reg = regRow, muMarg = muMarg,
                  k = KK, global = global,
                  control = nleqslv.control,
                ...
                if (verbose)
                  cat(ifelse(tmpRow$termcd ==
                    1, "Row scores converged \n",
                    "Row scores DID NOT converge \n"))
                rMat[, KK] = tmpRow$x[seq_len(n)]
                lambdaRow[idK] = tmpRow$x[n +
                  seq_along(idK)]

                # Normalize (speeds up algorithm if
                # previous step had not converged)
                rMat[, KK] = rMat[, KK] -
                  sum(rMat[, KK] * rowWeights)/sum(rowWeights)
                ...
                convergence[KK] = ((iterOut[KK] <=
                  ...
                  Psitol) && ((mean(abs(1 -
                  rMat[, KK]/rMatOld)^convNorm))^(1/convNorm) <
                  tol) && ((mean(abs(1 -
                  cMat[KK, ]/cMatOld)^convNorm))^(1/convNorm) <
                  tol))
        ...
        returnList = list(rMat = rMat, cMat = cMat,
            rowRec = rowRec, colRec = colRec,
            psiRec = psiRec, thetaRec = thetaRec,
            fit = "RCM_NB", lambdaRow = lambdaRow,
            lambdaCol = lambdaCol)

    } else {
        # If covariates provided, do a
        # constrained analysis
        d = ncol(covariates)
        CCA = vegan::cca(X = X, Y = covariates)$CCA
        # Constrained correspondence analysis for
        # starting values
        if (sum(!colnames(covariates) %in%
            CCA$alias) < k) {
            k = sum(!colnames(covariates) %in%
                CCA$alias)
        ...
        returnList = list(fit = "RCM_NB_constr",
            lambdaCol = lambdaCol, alpha = alpha,
            alphaRec = alphaRec, covariates = covariates,
            NB_params = NB_params, NB_params_noLab = NB_params_noLab,
            responseFun = responseFun, nonParamRespFun = nonParamRespFun,
            envGradEst = if (is.null(covariates)) NULL else envGradEst,
    ...
    return(c(returnList, list(converged = convergence,
        ...
        thetaRec = thetaRec, iter = iterOut -
            1, X = X, Xorig = Xorig, rowWeights = rowWeights,
        colWeights = colWeights, libSizes = switch(marginEst,
            MLE = exp(logLibSizesMLE), marginSums = libSizes),
        abunds = switch(marginEst, MLE = exp(logAbundsMLE),
            marginSums = abunds), confounders = confounders,
