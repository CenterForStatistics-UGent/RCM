...
RCM_NB = function(X,
  ...
  if(!is.null(confModelMat)){ #First and foremost: filter on confounders
    ...
    #Track the overdispersions, also the one associated to the independence
    #model
    ...
    if (marginEst == "MLE") {
        ...
        while ((initIter == 1) || ((initIter <=
            ...
            convergenceInit = ((initIter <= maxItOut) &&
                                 ((sum(
                                   abs(1 - logLibSizesMLE / logLibsOld)^convNorm
                                 ) / n) ^ (1 / convNorm) < tol) &&
                                 ((sum(
                                   abs(1 - logAbundsMLE / logAbsOld)^convNorm
                                 ) / p) ^ (1 / convNorm) < tol))
        }
    ...
    if(!is.null(confModelMat)){
      ...
      filtObj = filterConfounders(
        muMarg = muMarg,
        confMat = confModelMat,
        p = p,
        X = X,
        thetas = thetas[, 1],
  ...
  if(is.null(covModelMat)){
    ...
    for (KK in seq_len(k)){
      ...
      if (KK > 1) {
        muMarg = muMarg * exp(rMat[, (KK - 1), drop = FALSE] %*%
      ...
      trended.dispersion <-
        edgeR::estimateGLMTrendedDisp(y = t(X), design = NULL,
                                      method = "bin.loess",
                                      offset = t(log(muMarg)), weights = NULL)

      JacR = matrix(0, nrow = n+KK+1, ncol = n+KK+1)
                                  ...
                                  (!convergence[KK])))
      {
        ...
        #Overdispersions (not at every iterations to speed things up, the
        ...
        if((iterOut[KK] %% dispFreq) ==0 || (iterOut[KK]==1)){
          ...
          #Make a matrix for numerical reasons, it avoids excessive use of the
          #t() function
    ...
    alpha = matrix(0,d,k)
    alpha[!colnames(covModelMat) %in% CCA$alias,] = CCA$biplot[,seq_len(k)]
    #Leave the sum constraints for the factors alone for now, may or may not
    ...
    #Initiate parameters of the response function, taxon-wise. No zeroes or
    #trivial fit! Improved starting values may be possible.
    NB_params = if (responseFun != "nonparametric")
      vapply(seq_len(k), FUN.VALUE = matrix(0, v, p), function(x) {
    ...
    NB_params_noLab = if (responseFun != "nonparametric" &&
      ...
      NULL #Initiate parameters of the response function, ignoring taxon-labels
    if(responseFun == "nonparametric") {
      nonParamRespFun = lapply(seq_len(k), function(x) {
        list(taxonWise = lapply(integer(p), function(d) {
    ...
    for (KK in seq_len(k)){
        ...
        muMarg = if(responseFun %in% c("linear","quadratic", "dynamic")){
          exp(
            getRowMat(
              ...
              sampleScore = covModelMat %*% alpha[, KK - 1, drop = FALSE],
              NB_params = NB_params[, , KK - 1]
            ) * psis[KK - 1]
          ) * muMarg
        } else {
      ...
      ## 2) Propagation
      while ((iterOut[KK] == 1) ||
             ((iterOut[KK] <= maxItOut) && (!convergence[KK])))
      {
        ...
        #Overdispersions (not at every iterations to speed things up, doesn't
        #change a lot anyway)
        if((iterOut[KK] %% dispFreq) == 0 || iterOut[KK] == 1){
          if (verbose) cat(" Estimating overdispersions \n")
          thetas[, paste0("Dim", KK)] = estDisp(
            X = X,
            muMarg = muMarg,
            psis = psis[KK],
            prior.df = prior.df,
        ...
        if(responseFun %in% c("linear","quadratic", "dynamic")){
          ...
          NB_params[, , KK] = estNBparams(
            ...
            nleqslv.control = nleqslv.control,
            ncols = p,
            initParam = NB_params[, , KK],
            v = v,
            dynamic = responseFun == "dynamic",
            envRange = envRange
          )
          NB_params[,,KK] = NB_params[,,KK]/sqrt(rowSums(NB_params[,,KK]^2))

          if (envGradEst == "LR") {
            NB_params_noLab[, KK] = estNBparamsNoLab(
              design = design,
              thetasMat = thetasMat,
              muMarg = muMarg,
              psi = psis[KK],
          ...
          AlphaTmp = nleqslv(
            x = c(alpha[, KK], lambdasAlpha[seq_k(KK, nLambda1s)]),
            fn = dLR_nb,
            jac = LR_nb_Jac,
            X = X,
            CC = covModelMat,
          ...
          nonParamRespFun[[KK]] = estNPresp(
            ...
            thetas = thetas[, paste0("Dim", KK)],
            n = n,
            coefInit = nonParamRespFun[[KK]]$taxonCoef ,
        ...
        convergence[KK] = ((iterOut[KK] <= maxItOut) &&
                             ...
                             ((mean(abs(1-alpha[,KK]/alphaOld)^convNorm))^
                                (1/convNorm) < tol) && #Env gradient
                             if (responseFun == "nonparametric")
                               TRUE
                           else
                             (mean(abs(1 - (NB_params[, , KK] / NBparamsOld)[
                               NBparamsOld * NB_params[, , KK] != 0]) ^
                                     convNorm) ^ (1 / convNorm) < tol)
