...
RCM_NB <- function(
      X, k, rowWeights = "uniform", colWeights = "marginal",
      tol = 0.001, maxItOut = 1000L, Psitol = 0.001,
      verbose = FALSE, global = "dbldog", nleqslv.control = list(
          maxit = 500L,
          cndtol = 1e-16
      ), jacMethod = "Broyden", dispFreq = 10L,
    ...
    if (!is.null(confModelMat)) {
        ...
        Xorig <- X
        X <- trimOnConfounders(X,
            confounders = confTrimMat,
    ...
    trended.dispersion <- edgeR::estimateGLMTrendedDisp(
        y = t(correctXMissingness(
            X, outer(libSizes, abunds),
            allowMissingness, naId
        )),
        design = NULL, method = "bin.loess", offset = t(log(outer(
            libSizes,
            abunds
        ))), weights = NULL
    )
    if (marginEst == "MLE") {
        ...
        while ((initIter == 1) || ((initIter <= maxItOut) &&
            ...
            thetas[, "Independence"] <- estDisp(
                X = X,
                rMat = as.matrix(rep(0, n)), cMat = t(as.matrix(rep(
                    0,
                    p
                ))), muMarg = exp(outer(
                    logLibSizesMLE,
            ...
            logAbundsMLE <- vapply(seq_len(p), FUN.VALUE = double(1), function(j) {
                nleqslv(
                    ...
                    method = jacMethod, allowMissingness = allowMissingness, naId = naId[, j]
                )$x
            })
    ...
    if (is.null(covModelMat)) {
        ...
        for (KK in seq_len(k)) {
            if (verbose) {
                message("Dimension", KK, "is being esimated")
            }
            ...
            while ((iterOut[KK] == 1) || ((iterOut[KK] <=
                ...
                if (verbose && iterOut[KK] %% 1 == 0) {
                    ...
                    if (iterOut[KK] != 1) {
                      message(
                            "Old psi-estimate: ", psisOld)
                      message(
                            "New psi-estimate: ", psis[KK],
                            "\n"
                        )
                    }
                }
                ...
                if ((iterOut[KK] %% dispFreq) == 0 ||
                    (iterOut[KK] == 1)) {
                    if (verbose) {
                      message(" Estimating overdispersions")
                    }
                    ...
                    thetasMat <- matrix(thetas[, paste0(
                        "Dim",
                        KK
                    )], n, p, byrow = TRUE)
                ...
                # Psis
                if (verbose) {
                  message("Estimating psis")
                }
                regPsis <- outer(rMat[, KK], cMat[KK, ])
                psiTry <- try(abs(nleqslv(
                    fn = dNBpsis,
                    ...
                    preFabMat = preFabMat, allowMissingness = allowMissingness, naId = naId
                )$x))
                ...
                if (verbose) {
                  message(ifelse(tmpRow$termcd == 1, "Row scores converged",
                ...
                cMat[KK, ] <- vapply(seq_len(p), FUN.VALUE = double(1), function(j) {
                    nleqslv(
                        ...
                        Jac = JacC, allowMissingness = allowMissingness, naId = naId[, j]
                    )$x
                })
        ...
        d <- ncol(covModelMat)
        CCA <- constrCorresp(
            X = correctXMissingness(X, muMarg, allowMissingness, naId),
        ...
        if (sum(!colnames(covModelMat) %in% CCA$alias) <
            ...
            warning(
                immediate. = TRUE, "Can only fit an ordination with ",
        ...
        NB_params <- if (responseFun != "nonparametric") {
            vapply(seq_len(k), FUN.VALUE = matrix(
                0,
                v, p
            ), function(x) {
        ...
        for (KK in seq_len(k)) {
            ...
            while ((iterOut[KK] == 1) || ((iterOut[KK] <=
                ...
                # Overdispersions (not at every iterations to speed
                # things up, don't change a lot anyway)
                if ((iterOut[KK] %% dispFreq) == 0 ||
                    iterOut[KK] == 1) {
                    if (verbose) {
                      message("Estimating overdispersions")
                ...
                if (responseFun %in% c("linear", "quadratic", "dynamic")) {
                    ...
                    AlphaTmp <- nleqslv(
                        ...
                        envGradEst = envGradEst, allowMissingness = allowMissingness, naId = naId
                    )$x
        ...
        if (responseFun == "nonparametric") {
            ...
            names(nonParamRespFun) <- names(psis) <- paste0(
                "Dim",
                seq_len(k)
            )
        }
        ...
        returnList <- list(
            fit = "RCM_NB_constr", lambdaCol = lambdaCol,
