    ...
    for (KK in minK:k){
      ...
      if(KK>1){
        modMat = if(responseFun %in% c("linear","quadratic", "dynamic")){
          ...
          exp(nonParamRespFun[[KK]]$taxonWise*psis[KK-1])
        }
      ...
      while((iterOut[KK] ==1) || ((iterOut[KK] <= maxItOut) && (!convergence[KK])))
      {
        ...
        if(responseFun %in% c("linear","quadratic", "dynamic")){
          ...
          rowMat = nonParamRespFun[[KK]]$taxonWise
        }
        ...
        NB_params[,,KK] = NB_params[,,KK]/sqrt(rowSums(NB_params[,,KK]^2)*colWeights) #The post-hoc normalization is much more efficient, since the equations are easier to solve. Crucially, we do not need orthogonality with other dimensions, which makes this approach feasible
        ...
        cat("Initial values", c(alpha[,KK],lambdasAlpha), "\n")
