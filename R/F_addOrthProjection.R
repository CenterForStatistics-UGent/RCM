...
addOrthProjection <- function(RCMplot, sample = NULL, species = NULL,
    ...
    RCMplot$plot <- RCMplot$plot + geom_segment(
        inherit.aes = FALSE,
        mapping = aes_string(
            x = 0, y = 0,
            xend = "Dim1", yend = "Dim2"
        ),
        data = data.frame(t(mat1))
    ) # The sample or variable vector
    ...
    dfTip <- data.frame(
        x = mat2[grep(names(mat2),
            pattern = "end"
        )][1], y = mat2[grep(names(mat2),
            pattern = "end"
        )][2], xend = IntCoordsXTip,
        yend = IntCoordsYTip
    )
    dfStart <- data.frame(
        x = mat2[grep(names(mat2),
