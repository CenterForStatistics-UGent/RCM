...
if(is.data.frame(object)){
  nFactorLevels = vapply(object, FUN.VALUE = double(1),
                         function(x){if(is.factor(x)) nlevels(x) else 1})
  ...
  nFactorLevels = vapply(FUN.VALUE = integer(1),
                         unique(attr(object$covariates, "assign")),
                      function(x){sum(attr(object$covariates, "assign") == x)})
  #Number of levels per factor

} else {stop("Invalid object supplied! \n")}
  #Already prepare the matrix that defines the equations
  #for centering the coefficients of the dummy variables
  ...
  if(is.data.frame(object)) {return(list(centMat = centMat, datFrame  = object))
    }else {return(centMat)}
}
