...
in the analyses because they have only one value: \n",
        ...
        nFactorLevels <- vapply(
            FUN.VALUE = integer(1), unique(object$attribs),
            function(x) {
    ...
    centMat <- t(vapply(
        FUN.VALUE = numeric(sum(nFactorLevels)),
        seq_along(nFactorLevels), function(i) {
            c(
                rep.int(0L, sum(nFactorLevels[seq(0, i - 1)])),
                rep.int(
                    if (nFactorLevels[i] == 1) 0L else 1L,
                    nFactorLevels[i]
                ), rep.int(0L, sum(nFactorLevels[-seq(
                    1,
                    i
                )]))
            )
