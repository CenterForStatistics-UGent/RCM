...
estNBparams = function(design, thetas, muMarg, psi, X, nleqslv.control, ncols, initParam,v, dynamic = FALSE, envRange){
  vapply(seq_len(ncols), FUN.VALUE = vector("numeric",v), function(i){
    ...
    nleq = c(nleqslv(initParam[-3,i] , reg = design[,-3],  fn = dNBllcol_constr, theta = thetas[i], muMarg = muMarg[,i], psi = psi, X = X[,i], control = nleqslv.control, jac = JacCol_constr)$x,0)
    }
    return(nleq)
  })
}