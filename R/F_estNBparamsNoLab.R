...
estNBparamsNoLab = function(design, thetas, muMarg, psi, X, nleqslv.control, initParam, n, v, dynamic, envRange){
  #Without taxon Labels
  nleq = nleqslv(x = initParam , reg = design,  fn = dNBllcol_constr_noLab, thetas = thetas, muMarg = muMarg, psi = psi, X = X, control = nleqslv.control, jac = JacCol_constr_noLab, n=n, v=v)$x
  if(dynamic && ((-nleq[2]/(2*nleq[3]) < envRange[1]) || (-nleq[2]/(2*nleq[3]) > envRange[2]))){ #If out of observed range, fit a linear model
  nleq = c(nleqslv(initParam[-3] , reg = design[,-3],  fn = dNBllcol_constr, theta = thetas, muMarg = muMarg, psi = psi, X = X, control = nleqslv.control, jac = JacCol_constr)$x,0)
  }
  return(nleq)
}