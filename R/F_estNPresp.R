...
estNPresp = function(sampleScore, muMarg, X, ncols, thetas, n, coefInit,
  ...
  #The model matrix of the first degree
    ...
    df = data.frame(x = X[,i], sampleScore = sampleScore,
                    logMu = log(muMarg[,i]))
    #Going through a dataframe slows things down,
    #so ideally we should appeal directly to the vgam.fit function
      tmp = try(suppressWarnings(vgam(data = df,
                                      x ~ s(sampleScore, df = dfSpline),
                                      offset = logMu,
                                      family = negbinomial.size(
                                        lmu = "loge", size = thetas[i]),
    ...
    if(inherits(tmp,"try-error")) { #If this fails turn to parametric fit
      ...
      tmp = try(nleqslv(fn =  dNBllcolNP,
                        x = if(length(coefInit[[i]])==2) {rep(1e-4, degree+1)}
                        else coefInit[[i]], X = X[,i], reg = MM,
                        theta = thetas[i], muMarg = muMarg[,i],
                        jac = NBjacobianColNP)$x)
    ...
    #Report failed fits
  sumFit = sum(!vapply(FUN.VALUE = TRUE, taxonWise, is.list))
  if(verbose && sumFit) warning("A total number of ",sumFit,
                                " response functions did not converge! \n")
#Overall fit
  samRep = rep(sampleScore, ncols)
  overall = vgam(c(X) ~ s(samRep, df = dfSpline), offset = c(logMu),
                 family = negbinomial.size(lmu = "loge",
...
rowMat = vapply(FUN.VALUE = numeric(n),taxonWise, function(x){
  if(is.list(x)) cbind(MM1, predict(x$spline, x = sampleScore)$y) %*%
    c(x$coef,1) else MM %*% x})
rowVecOverall = cbind(MM1, predict(overallList$spline, x = sampleScore)$y) %*%
  c(overallList$coef,1)
...
splinesList = lapply(taxonWise, function(x){if(is.list(x)) x$spline else NULL})
  ...
  list(taxonCoef = taxonCoef, splinesList = splinesList, rowMat = rowMat,
       overall = overallList, rowVecOverall = rowVecOverall)
}
