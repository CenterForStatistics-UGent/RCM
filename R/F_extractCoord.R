...
extractCoord <- function(RCM, Dim = c(1, 2)) {
    ...
    dataSam <- if (constrained) {
        data.frame(RCM$covariates %*% RCM$alpha[,
            Dim,
            drop = FALSE
        ] %*% diag(RCM$psis[Dim], nrow = length(Dim)))
    } else {
    ...
    if (constrained) {
        if (RCM$responseFun == "linear") {
            dataTax <- data.frame(origin1 = -RCM$NB_params[
                1, ,
                Dim[1]
            ] / RCM$NB_params[
                2, ,
                Dim[1]
            ], origin2 = -RCM$NB_params[
                1, ,
                Dim[2]
            ] / RCM$NB_params[
                2, ,
                Dim[2]
            ], slope1 = RCM$NB_params[
                2, ,
                Dim[1]
            ], slope2 = RCM$NB_params[
                2, ,
                Dim[2]
            ])
            ...
            dataTax <- data.frame(apply(RCM$NB_params[c(
                2,
                3
            ), , Dim, drop = FALSE], c(2, 3), function(x) {
                a <- x[2]
                b <- x[1]
                -b / (2 * a)
            ...
            peakHeights <- apply(RCM$NB_params[, , Dim, drop = FALSE], 2, function(x) {
                ...
                C <- x[1, ]
                vapply(
                    FUN.VALUE = numeric(1), exp(B^2 - 4 * A * C) / (4 * A),
                    function(y) {
                        max(y, 1 / y)
                    }
                )
            ...
            dataEllipse <- t(Reduce(x = lapply(
                Dim,
                function(x) {
                    RCM$NB_params[, , x]
                }
            ), f = rbind))
            colnames(dataEllipse) <- c(vapply(
                FUN.VALUE = character(length(Dim)),
                Dim, function(x) {
                    paste0(c("c", "b", "a"), x)
                }
            ))
    ...
    list(
        samples = dataSam, species = dataTax,
        variables = dataVar
    )
}
