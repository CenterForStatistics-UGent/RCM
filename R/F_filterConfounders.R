...
filterConfounders = function(muMarg, confMat, X, thetas, p, n, nleqslv.control,
                             trended.dispersion, tol = 1e-3, maxIt = 20){
  ...
  while((iter ==1) || ((iter <= maxIt) && (!convergence))){
    ...
    NB_params = vapply(FUN.VALUE = numeric(nrow(NB_params)), seq_len(p),
                       function(i){
      nleq = try(nleqslv(NB_params[,i] , reg = confMat,  fn = dNBllcol_constr,
                         theta = thetas[i], muMarg = muMarg[,i], X = X[,i],
                         control = nleqslv.control, jac = JacCol_constr,
                         psi = 1)$x)
      # Fit the taxon-by taxon NB with given overdispersion parameters
      #and return predictions
      if(inherits(nleq,"try-error")| anyNA(nleq)| any(is.infinite(nleq))){
        nleq = nleqslv(NB_params[,i] , reg = confMat,  fn = dNBllcol_constr,
                       theta = thetas[i], muMarg = muMarg[,i], X = X[,i],
    ...
    if(anyNA(NB_params)) {stop("Filtering on confounders failed because of
                               failed fits. Consider more stringent filtering by
                               increasing the prevCutOff parameter.\n")}

    thetas = estDisp(X = X, cMat = matrix(0, ncol=p), rMat = matrix(0, nrow=n),
