...
plot.RCM <- function(x, ..., Dim = c(1, 2),
        ...
        if (samColour == "Influence") {
            if (is.null(inflVar)) {
               ...
               on which the influence needs to be calculated")
                ...
                stop(
                    "Provide as 'inflVar' either 'psi' or a variable name. ",
                    if (!is.null(x$covariates)) {
                        c("Variable names are:\n", paste(colnames(x$covariates)))
                    }
                )
            }
        } else if (!samColour %in% c(
            richSupported, "Deviance",
    ...
    if ("samples" %in% plotType) {
        ...
        dataSam$colourPlot <- if (length(samColour) == 1) {
            if (samColour == "Influence") {
                ...
                estimate_richness(x$physeq, measures = samColour)[[1]]
            } else {
        ...
        plot <- ggplot(dataSam, aes_string(
            x = names(dataSam)[1],
            y = names(dataSam)[2],
        ...
        if (!is.null(colLegend) & is.factor(dataSam$colourPlot)) {
            plot <- plot +
                scale_colour_manual(
                    name = colLegend,
                    values = colorRampPalette(brewer.pal(max(
                        3,
            ...
            plot <- plot + scale_colour_continuous(
                name = colLegend,
                low = contCol[1],
                high = contCol[2]
            )
    ...
    if ("species" %in% plotType) {
        idTaxRegExp <- if (!is.null(taxRegExp)) {
            ...
            apply(
                vapply(
                    FUN.VALUE = logical(nrow(coords$species)),
        ...
        if (constrained) {
            if (x$responseFun == "linear") {
                dataTax$arrowLength <- apply(
                    dataTax[, c("slope1", "slope2")],
                    1, function(x) {
                        sqrt(sum(x^2))
                    }
                )
                id <- dataTax$arrowLength >= quantile(
                    dataTax$arrowLength,
                    1 - taxFrac
                )
                    ...
                    scalingFactorTmp <- apply(dataSam[, Dimnames], 2, range) /
                        apply(
                            dataTax[
                                ,
                                c("end1", "end2")
                            ] - dataTax[, c("origin1", "origin2")],
                            2, range
                        )
                    scalingFactor <- min(scalingFactorTmp[scalingFactorTmp >
                        0]) * 0.975
                    # Scale the arrows
                    dataTax[, c("end1", "end2")] <- dataTax[, c(
                        "origin1",
                        "origin2"
                    )] + dataTax[, c("slope1", "slope2")] *
                        scalingFactor
                ...
                dataTax$colour <- apply(
                    coords$species[, paste0("a", Dim)],
                    1, function(x) {
                        if (all(x > 0)) {
                            return("green")
                        } else if (all(x < 0)) {
                ...
                dataID <- data.frame(
                    meanPeakHeights = rowMeans(dataTax[
                        ,
                        paste0("peak", Dim)
                ...
                dataEllipse <- data.frame(apply(
                    dataEllipseTmp[, , id],
                    2, c
                ), colour = as.character(dataTax$colour))
            } else {
        ...
        if ((!constrained || x$responseFun == "linear")) {
                ...
                if ("samples" %in% plotType | (length(taxCol) == 1 &&
                    ...
                    plot <- plot + geom_segment(
                        ...
                        arrow = arrow(length = unit(
                            0.1,
                            "cm"
                        )), inherit.aes = FALSE, size = arrowSize
                    ) +
                        guides(alpha = "none")
                } else {
                    plot <- plot + geom_segment(
            ...
            plot <- plot + geom_tile(
                ...
                if (plotEllipse) {
                    geom_path(
                        inherit.aes = FALSE, data = dataEllipse,
                        mapping = aes_string(
                            x = "x",
                            y = "y", group = "taxon"
                        ), colour = "grey50",
            ...
            if (is.null(dataTax$taxCol)) {
                ...
                plot <- plot + geom_point(
                    ...
                    scale_colour_manual(values = c(brewer.pal(length(
                        ...
                        1, Palette), "Grey90"), name = colLegend)
                } else {
        ...
        if (is.null(varPlot)) {
            ...
            varID <- attr(x$covariates, "dimnames")[[2]] %in%
                unlist(lapply(varPlot,
                    grep,
                    value = TRUE, x = attr(x$covariates, "dimnames")[[2]]
                ))
        }
    ...
    ## AXIS LABELS
    if (plotPsi == "psi") {
        plot <- plot + xlab(bquote(psi[.(Dim[1])] == .(round(
            x$psis[Dim[1]],
            1
        )))) + ylab(bquote(psi[.(Dim[2])] == .(round(
            x$psis[Dim[2]],
            1
        ))))
        ...
        if (length(x$confModelMat)) {
            ...
            plot <- plot + ggtitle(paste0(
                "Confounders' deviance explained: ",
                liksTab["logLikExplained", "filtered"] * 100, "%"
            ))
        }
        plot <- plot + xlab(paste0(Dimnames[1], ": ", liksTab[
            "logLikExplained",
            Dimnames[1]
        ] * 100, "%")) + ylab(paste0(
            Dimnames[2], ": ",
        ...
        if (length(x$confModelMat)) {
            # If filtered on confounders, print in title.
            plot <- plot + ggtitle(paste0(
                "Confounders' inertia explained: ",
                inertTab["inertiaExplained", "filtered"] * 100, "%"
            ))
        }
        plot <- plot + xlab(paste0(Dimnames[1], ": ", inertTab[
            "inertiaExplained",
            Dimnames[1]
        ] * 100, "%")) + ylab(paste0(
            Dimnames[2], ": ",
    ...
    plot <- plot + geom_point(data = data.frame(x = 0, y = 0), aes_string(
        x = "x",
        y = "y"
    ), size = crossSize, inherit.aes = FALSE, shape = 3)
    # Enlarge most text
    plot <- plot + theme_bw() +
        theme(
...
setting aspRatio = 1 and axesFixed = TRUE.")
    ...
    if (returnCoords) {
        list(
            plot = plot, samples = dataSam, species = dataTax,
            variables = varData
        )
    } else {
        plot
