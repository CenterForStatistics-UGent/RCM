...
plotRespFun <- function(
    ...
    predictFun <- function(taxon, x) {
        if (length(RCM$nonParamRespFun[[
            ...
            cbind(1, x, predict(
                ...
                x = x
            )$y) %*% c(RCM$nonParamRespFun[[
                paste0("Dim", Dim)
            ]]$taxonCoef[[taxon]], 1)
        } else {
            getModelMat(x, RCM$degree) %*% RCM$nonParamRespFun[[paste0(
                "Dim",
                Dim
            )]]$taxonCoef[[taxon]]
        }
    ...
    if (is.null(taxa)) {
        ...
        # The integrals
        taxa <- taxa_names(RCM$physeq)[intsNonParam > quantile(
            intsNonParam,
    ...
    plot <- ggplot(data = dfMolt, aes_string(
        x = "sampleScore",
        y = "responseFun",
    ...
    textDf$y <- (if (is.null(yLocVar)) {
        (max(dfMolt$responseFun) + min(dfMolt$responseFun)) / 2
    } else {
        yLocVar
    })
    plot <- plot + geom_text(
        data = textDf, mapping = aes_string(
            x = "x",
            y = "y", label = "text"
        ), inherit.aes = FALSE,
    ...
    if (addSamples) {
        ...
        dfSam$y <- (if (is.null(yLocSam)) {
            min(dfMolt$responseFun) * 0.8
        } else {
            yLocSam
        }) + if (addJitter) {
            runif(min = -1, max = 1, n = nrow(RCM$alpha)) *
                diff(range(dfMolt$responseFun)) / 20
        } else {
            0
        }
        if (!is.null(samSize)) {
