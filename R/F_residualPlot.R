...
#'
#'If whichTaxa is "run" or "response" the taxa with the highest run statistics or responses are plotted, numTax indicates the number. If whichTaxa is a character vector, these are interpreted as taxon names to plot. This function is mainly meant for linear response functions, but can be used for others too.
...
if(!whichTaxa %in% c("run","response")){
  numTaxa = length(whichTaxa)
  idTaxa = whichTaxa
} else {
  sizes = if(whichTaxa =="run") getRunStats(resMat, sampleScore) else RCM$NB_params[2,,Dim] #Select taxa with longest runs or strongest responses
