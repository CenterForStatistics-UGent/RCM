...
residualPlot = function(RCM, Dim = 1, whichTaxa = "response",
                        ...
                        axisTitleSize = 16, taxTitle = TRUE){
  ...
  if(resid == "Deviance"){
    resMat = getDevianceRes(RCM, seq_len(Dim))
      ...
      thetaMat = matrix(byrow = TRUE, nrow = nrow(RCM$X), ncol = ncol(RCM$X),
                        data = RCM$thetas[,switch(as.character(Dim),
                                                  "0" = "Independence",
                                                  "0.5" = "Filtered",
                                                  paste0("Dim", Dim))])
...
if(!whichTaxa %in% c("runs","response")){
  ...
  sizes = if(whichTaxa =="runs") apply(resMat>0,2,
                                       function(x){
                                         runs.test(factor(x))$statistic}) else
                                           RCM$NB_params[2,,Dim]
  #Select taxa with longest runs or strongest responses
  ...
  lapply(colnames(resMat), function(tax){
    plot(x = sampleScore, y = resMat[,tax], ylab = paste(resid,"residuals"),
         xlab = paste("Environmental score in dimension",Dim), main = tax)
  })
  } else {
    Plot = ggplot(data.frame(x = c(sampleScore), y = c(resMat),
                             samColour = if(is.null(samColour)) "black" else
                               get_variable(RCM$physeq, samColour),
                             samShape = if(is.null(samShape)) "none" else
                               get_variable(RCM$physeq, samShape)),
                  mapping = aes_string(x="x", y="y", colour = "samColour",
                                       shape = "samShape"))+
      ylab(paste(resid,"residuals")) +
      xlab(paste("Environmental score in dimension",Dim)) +
      geom_point() + ggtitle(ifelse(taxTitle,colnames(resMat), ""))
    Plot = Plot + if(is.null(samShape)) guides(shape = FALSE) else
      scale_shape_discrete(name = samShape)
    Plot = Plot + if(is.null(samColour)) {guides(colour = FALSE)
      }else if(is.factor(get_variable(RCM$physeq, samColour))) {
        scale_colour_discrete(name = samColour)} else
          scale_colour_continuous(name = samColour)
