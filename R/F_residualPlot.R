...
residualPlot <- function(
    ...
    if (length(idTaxa) > 1) {
        ...
        Plot <- ggplot(
            ...
            ylab(paste(resid, "residuals")) +
            xlab(paste(
                "Environmental score in dimension",
                Dim
            )) +
            geom_point() +
            ggtitle(ifelse(taxTitle,
                colnames(resMat), ""
            ))
        ...
        Plot <- Plot + if (is.null(samColour)) {
            guides(colour = FALSE)
        } else if (is.factor(get_variable(
            RCM$physeq,
            samColour
        ))) {
