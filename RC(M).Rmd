...
RCM_NB = function(X, k, tol = 1e-3, maxItOut = 500, Psitol = 1e-3, verbose = TRUE, libSizes = NULL, NBRCM=NULL,  global ="dbldog", nleqslv.control=list(),method=c("Broyden"), dispFrec=10, convNorm = 2, rowWeights, colWeights, prior.df=10){
 ...
 ## 1) Initialization
  svdX = svd(diag(sqrt(1/libSizes)) %*% (X-muMarg) %*% diag(1/sqrt(colSums(X)))) #Weight for numerical reasons here
  rMat = svdX$u[,1:k]
  cMat = t( svdX$v[,1:k] )
  psis = svdX$d[1:k]
...
RCM_ZIP = function(X, k, tol = 1e-3, maxItOut = 500, psiTol = 1e-4, chiTol=psiTol, verbose = TRUE, global ="dbldog", nleqslv.control=list(),method=c("Broyden"), twoCores=TRUE, convNorm = 2,  maxItMean = 20, maxItZeroes=20, ZIPRCM=NULL, rowWeights,colWeights){
...
RCM_ZINB = function(X, k, tol = 1e-3, maxItOut = 500, psiTol = 1e-4, chiTol=psiTol, verbose = TRUE, ZINBRCM = NULL, global ="dbldog", nleqslv.control=list(),method=c("Broyden"), twoCores=TRUE, convNorm = 2, maxItMean = 20, maxItZeroes=20, dispFreq = 5, rowWeights , colWeights){
...
RCM = function(dat, round=FALSE, method= c("NB","ZIP", "ZINB"),prevCutOff=0,rowWeights = "uniform", colWeights = "marginal",...){
  if (class(dat)=="matrix"){
...
colWeightsNum = colWeightsNum[colnames(dat)]
  switch(method, NB=RCM_NB(dat, rowWeights=rowWeightsNum, colWeights=colWeightsNum,...), ZIP=RCM_ZIP(dat, rowWeights=rowWeightsNum, colWeights=colWeightsNum, ...), ZINB=RCM_ZINB(dat,  rowWeights=rowWeightsNum, colWeights=colWeightsNum,...))
}
...
if(!file.exists("toyDataUnifNS.RData")){
  syntNBNSunifJob = mcparallel(RCM(dataMatNBNS, method="NB", k=2, nleqslv.control= nleqslv.control, maxItOut=2e3, prevCutOff=0.1))
 ...
 #Marginal
    syntNBNSmargJob = mcparallel(RCM(dataMatNBNS, method="NB", k=2, nleqslv.control= nleqslv.control, maxItOut=2e3, colWeights = "marginal", prevCutOff=0.1))
...
nleqslv.control = list(trace=TRUE, maxit = 250, cndtol=.Machine$double.eps)
if(!file.exists("toyDataUnifSig.RData")){
  syntNBSigunifJob = mcparallel(RCM(dataMatSigNB, method="NB", k=2, nleqslv.control= nleqslv.control, maxItOut=2e3, prevCutOff=0.1, colWeights ="uniform"))
  syntNBSigunif=mccollect(syntNBSigunifJob, FALSE)[[1]] 
  
    syntNBSigmargJob = mcparallel(RCM(dataMatSigNB, method="NB", k=2, nleqslv.control= nleqslv.control, maxItOut=2e3, prevCutOff=0.1, colWeights="marginal"))
  syntNBSigmarg=mccollect(syntNBSigmargJob, FALSE)[[1]] 
  load("toyData.RData")
      syntNBSigunifJob2 = mcparallel(RCM(syntNB$X, method="NB", k=2, nleqslv.control= nleqslv.control, maxItOut=2e3, prevCutOff=0.1, colWeights="uniform"))
  syntNBSigunif2=mccollect(syntNBSigunifJob2, FALSE)[[1]] 
  
    syntNBSigmargJob2 = mcparallel(RCM(syntNB$X, method="NB", k=2, nleqslv.control= nleqslv.control, maxItOut=2e3, prevCutOff=0.1, colWeights="marginal"))
  syntNBSigmarg2=mccollect(syntNBSigmargJob2, FALSE)[[1]] 
  taxaSigNB2 = factor(mapply(cid1,cid2,FUN=paste), levels=c("FALSE FALSE", "TRUE FALSE","FALSE TRUE","TRUE TRUE"), labels=c("Reference", "Signal 1","Signal2", "Signal 1 and 2"))
  sampleSigNB2 = factor(mapply(rid1,rid2,FUN=paste), levels=c("FALSE FALSE", "TRUE FALSE","FALSE TRUE","TRUE TRUE"), labels=c("Reference", "Signal 1","Signal2", "Signal 1 and 2"))
  #Not the same as the original solution?? Needs further investigation
  save(syntNBSigunif,syntNBSigmarg, taxaSigNB, sampleSigNB, syntNBSigunif2, syntNBSigmarg2, taxaSigNB2, sampleSigNB2,file="toyDataUnifSig.RData")
} else {load("toyDataUnifSig.RData")}

RCMres2DF = function(RCM,  sampleGroups){
...
NBunifSigPlotDF = RCMres2DF(syntNBSigunif,  sampleSigNB)
NBmargSigPlotDF = RCMres2DF(syntNBSigmarg,  sampleSigNB)
...
ggplot(data=NBunifSigPlotDF2$rows, aes(x=r1, y=r2, col=sampleGroups)) + geom_point() + scale_colour_manual(values=c("grey50","blue","purple","red"))
ggplot(data=NBmargSigPlotDF2$rows, aes(x=r1, y=r2, col=sampleGroups)) + geom_point() + scale_colour_manual(values=c("grey50","blue","purple","red"))
...
#The column scores are very sensitive to high zero frequencies and dito overdispersions. the abundances are normal but very high fractio of zeroes. Try with less prior.df? Or inverse weight by overdispersions?
...
#Prio df does not matter

#DataMat4
data4Job = mcparallel(RCM(dataMat4, method="NB", k=2, nleqslv.control= list(maxit=250), maxItOut=2e3, colWeights = "marginal", prevCutOff=0.1))
data4RCM=mccollect(data4Job, FALSE)[[1]]
with(data4RCM, plot(rMat, col=rowSignal)); legend("topleft", legend=levels(rowSignal), pch=1, col=1:4)

data4Job2 = mcparallel(RCM(dataMat4, method="NB", k=2, nleqslv.control= list(maxit=250), maxItOut=2e3, colWeights = "marginal", prevCutOff=0.1))
data4RCM2=mccollect(data4Job2, FALSE)[[1]]
with(data4RCM2, plot(rMat, col=rowSignal)); legend("topleft", legend=levels(rowSignal), pch=1, col=1:4)

data4Job3 = mcparallel(RCM(dataMat4, method="NB", k=2, nleqslv.control= list(maxit=250), maxItOut=8e3, colWeights = "marginal", prevCutOff=0.1, NBRCM=data4RCM2))
data4RCM3=mccollect(data4Job3, FALSE)[[1]]
...
if(!file.exists("syntZIPnoSig.RData")){
  syntZIPnoSigJob = mcparallel(RCM(dataMatZIPnoSig, method = "ZIP",k=2, nleqslv.control = nleqslv.controlZIP))
...
taxaSigNBtmp[idSig2TaxZIP] = "Signal 2"
taxaSigNB =factor(taxaSigNBtmp) 
names(taxaSigNB) = colnames(dataMatSigNB) = names(rhosNBSigref)
names(sampleSigNB) = rownames(dataMatSigNB) = 1:NsamplesNBNS
...
if(!file.exists("syntZIPSig.RData")){
  syntZIPSigMargJob = mcparallel(RCM(dataMatZIPSig, method = "ZIP",k=2, nleqslv.control = list(trace=TRUE, maxit=250)))
  syntZIPSigMarg =  mccollect(syntZIPSigMargJob, FALSE)[[1]] 
  
  save(syntZIPSig,sampleSigZIP, taxaSigNB, file="syntZIPSig.RData")
