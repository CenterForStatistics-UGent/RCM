...
PCbiplot = function(RCMfit, Dim=c(1,2), ..., colour = NULL, colLegend = colour, shapeLegend = NULL, size=3, arrowFrac = 0.025, scalingFactor=NULL, Palette=NULL, arrowCol="black", plotArrows =TRUE) {
    ...
    if(!is.null(colour) && !is.factor(colour)){
      ...
      dataSam$colourPlot = colour} else {dataSam$colourPlot=1}
...
Palette=rainbow(length(unique(dataSam$colourPlot)))
    ...
    arrowLengths = apply(dataTax,2,function(x){sqrt(sum(x^2))})
    id = arrowLengths >= quantile(arrowLengths,1-arrowFrac)
    ...
    if(constrained){
      ...
      plot = plot + geom_text(data = varData, mapping = aes_string(x=Dim1, y = Dim2, label = label))
    }
...
datList1mat = lapply(RCMresList1, function(x){x$X})
CAresList1 = lapply(datList1mat, caSVD)
CodaResList1 = lapply(datList1mat, CoDa)
CodaScaledResList1 = lapply(datList1mat, CoDa, scale = TRUE)
BCresList1 = lapply(datList1mat, function(x){ordinate(otu_table(x, taxa_are_rows = FALSE), method="PCoA", distance = "bray")})
JSDresList1 = lapply(datList1mat, function(x){ordinate(otu_table(x, taxa_are_rows = FALSE), method="PCoA", distance = "jsd")})
...
obj = resList1Rows[[ID]]
lapply(names(obj),function(x){plot(obj[[x]], col = groupFactor, main=x, xlab = "Dim 1",ylab = "Dim 2")})
...
rm(ID, obj)
par(mfrow = c(1,1))
# It seems like our method performs better, only two groups do not get separated very well. But then again this is under the model assumption of the NB, although one might expect the other methods to indentify the clusters. Jensen Shannon divergence also performs rather well, separating all (but one) clusters in two dimensions. Look in first and third dimension. 

par(mfrow = c(2,3), mar = c(3,3,3,3))
ID = sample(size=1, seq_along(resList1))
obj = resList1Rows[[ID]]
lapply(names(obj),function(x){plot(obj[[x]][,c(1,3)], col = groupFactor, main=x, xlab = "Dim 1",ylab = "Dim 3")})
# plot(resList1$RCM[[id]]$rMat[,1:2], col = groupFactor, main = "RC(M)")
# plot(resList1$CA[[id]]$u[,1:2], col = groupFactor, main = "CA", xlab = "Dim 1", ylab ="Dim2")
# plot(resList1$CoDA[[id]]$rotation[,1:2], col = groupFactor, main = "CoDA")
# plot(resList1$BC[[id]]$vectors[,1:2], col = groupFactor, main = "PCoA: Bray-Curtis")
...
#For each population, modify a fraction TPR of the abundances (compensation is not so badly needed here, we do not test individual species)
rhosSampled = sapply(1:nPop, function(x, rhos){
  id = sample(seq_len(p), round(TPR*p))
  rhos[id] = rhos[id]*10
  rhos/sum(rhos) #renormalize
}, rhos = rhosSampled0)[,groupInd]
...
obj = resList2Rows[[ID]]
lapply(names(obj),function(x){plot(obj[[x]], col = groupFactor2, main=x, xlab = "Dim 1",ylab = "Dim 2")})
rm(ID, obj)
par(mfrow = c(1,1))
...
ID = sample(size=1, seq_along(resList1))
obj = resList2Rows[[ID]]
lapply(names(obj),function(x){plot(obj[[x]][,c(1,3)], col = groupFactor2, main=x, xlab = "Dim 1",ylab = "Dim 3")})
# plot(resList1$RCM[[id]]$rMat[,1:2], col = groupFactor, main = "RC(M)")
# plot(resList1$CA[[id]]$u[,1:2], col = groupFactor, main = "CA", xlab = "Dim 1", ylab ="Dim2")
# plot(resList1$CoDA[[id]]$rotation[,1:2], col = groupFactor, main = "CoDA")
# plot(resList1$BC[[id]]$vectors[,1:2], col = groupFactor, main = "PCoA: Bray-Curtis")
