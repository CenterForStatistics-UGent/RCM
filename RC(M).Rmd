...
reqpkg <- c("phyloseq","MASS", "parallel","nleqslv", "edgeR", "VGAM","HMP", "ggplot2", "pscl", "vegan", "alabama", "zCompositions", "DirFactor","reshape2", "tensor", "locfit", "SimSeq","fdrtool", "cluster")
...
par(mfrow = c(1,1))
rm(filtRes, physeqListV13, physeqListV13_SC, phyList)
```
...
with $\mathbb{1}$ a column matrix of one of the appropriate dimension

```{r contrTaxa}
contrTaxa = function(rcm, groupFactor, idTax){
  ...
  sapply(seq_along(levels(groupFactor)), function(i){
    crossprod(rcm$cMat[,idTax[,i]], centroids[, i])
  })
}
```
...
covariates using generalized UniFrac distances", 2012), and use the dataset "Global Patterns" from Caporaso _et al._ available in the _phyloseq_ package.

```{r UniFrac, purl = FALSE}
data(GlobalPatterns) # The Caporaso et al. (2011) dataset is available in the phyloseq package
getTreeClust = function(physeq, k = 20){
  Tree = phy_tree(physeq)
  dd = as.dist(cophenetic.phylo(Tree)) #Get distances
