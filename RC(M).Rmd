...
plotRCM = function(RCM, X = NULL, thetas = NULL, abunds = NULL, arrowFrac = 0.04, biplot = TRUE,
  ...
  if(!is.null(RCM$physeq) && ! is.null(samColour)){
...
realNames = c("AGP", "Hard.palate","Buccal.mucosa","Anterior.nares","Left.Antecubital.fossa", "Posterior.fornix", "Mid.vagina" )
...
resListRCM = lapply(realNames, function(x){
  tmp = resListRCM[[x]]
  tmp$physeq = phyList[[x]]
  tmp
}) 
...
par(mfrow = c(2,3))
lapply(names(resListRCM)[-1], function(x){plotRCM(resListRCM[[x]], biplot=FALSE, samColour = "sex", main=x)})
par(mfrow = c(2,3))
lapply(names(resListRCM)[-1], function(x){plotRCM(resListRCM[[x]], biplot=FALSE, samColour = "RUNCENTER", main=x, libInset = c(-0.9,-0.1))})
```
...
whereby $h_{ik} = \boldsymbol{\alpha}_k^t \mathbf{c}_{i.}$ the optimal linear combination of $\mathbf{c}_{i.}$ in dimension $k$. $\boldsymbol{\alpha}_k$ is called the _environmental gradient_. $\alpha_{ky}$ is element corresponding to covariate $y$ of the environmental gradient in dimension $k$. $a_{jk}$ is the maximal log-departure from independence achieved when the linear combination of covariates $\boldsymbol{\alpha}_k^t \mathbf{c}_{i.}$ equals the optimum $q_{jk}$. $m_{jk}$ is the tolerance, it determines how fast the expected value decreases as $\mathbf{c}_{i.}$ moves away from the optimal values $\boldsymbol{\alpha}_k^{-t}h_{ik}$. The scales of $h_{ik}$, $q_{jk}$ and $m_{jk}$ are undefined, so we need an additional restriction to render the method identifiable. Zhu _et al._, 2005 propose setting $\sum_{j=1}^p m_{jk}^2/p=1$. Note that $a_{jk}$, $q_{jk}$ and $m_{jk}$ are species specific parameters whereas $\boldsymbol{\alpha}_k$ is specific for the dimension but common to all species.

The estimation iterates between estimating $\boldsymbol{\alpha}_k$ and $\beta_{jk} = \big(a_{jk}, q_{jk}, m_{jk})$.

This model was designed with the ecological context in mind, whereby (continuous) environmental variables such as pH, temperature, sunlight intensity, ion concentrations, distance to landscape elements etc. were measured. However for microbiome data, we usually do not have direct measurements of variables in the ecosystem, but rather charactersitics of the patient, which are very often categorical variables. Even for a continuous variable like age the Gaussian assumption seems weird to me. Keeping the other covariates fixed, this would mean there is an optimal age for bacteria to grow and their expected abundance would decrease for older otr younger people. This may or may not be a realistic model.

Let's take a look at the nature of the covariate data.

```{r Nature of the covariates, eval=TRUE}
load("/home/stijn/PhD/American Gut/AGphylo.RData")
...
with $f$ the response function and the usual restrictions on $\mathbf{s}$:

$$\mathbf{s^tu_{tax}} = \mathbf{0}$$
$$\mathbf{s_k^tUs_{k'}} = I(k=k')$$

with $u_{tax}$ the vector of $u_j$'s and  $\mathbf{U}$ a diagonal p-by-p matrix with $u_{tax}$ on the diagonal.

For $\alpha$ (a d-by-k matrix) we could enforce the following restrictions

$$\boldsymbol{\alpha}\mathbf{1} = \mathbf{0}$$
$$\boldsymbol{\alpha}^t\boldsymbol{\alpha} = \mathbb{1}_k$$

with $\mathbf{1}$ a vector of ones of length $n$ and $\mathbb{1}_k$ the identity matrix of dimension $k$.. However to maintain maximal comparability with the unconstrained RC(M)-model we prefer to set 
...
For the response function previously a Gaussian has been proposed but this may not be realistic. In a first stage we will try fit the linear model with $f(x) = x$ but later we might estimate $f$ non-parametrically as in Zhu _et al._ (2005).
...
$$LR(\boldsymbol{\alpha}) = log \frac{\prod_{i=1}^n \prod_{j=1}^p \big(p_k^{(\alpha)}(\boldsymbol{\alpha}^T \mathbf{x}_i) \big)^x_{ij}}{\prod_{i=1}^n \prod_{j=1}^p \big(p^{(\alpha)}(\boldsymbol{\alpha}^T \mathbf{x}_i) \big)^x_{ij}}$$

with $p^{(\alpha)}$ and $p^{(\alpha)}_k$ estimated probability density functions without and with taxon labels. In the linear case ($f(x) = x$) this becomes $p^{(\alpha)}(\boldsymbol{\alpha}^T \mathbf{x}_i) = \frac{\sum_{j=1}^p x_{ij}}{\sum_{j=1}^p \sum_{i=1}^n x_{ij}}$ and $p^{(\alpha)}_k(\boldsymbol{\alpha}^T \mathbf{x}_i) = \frac{x_{ij}}{\sum_{j=1}^p x_{ij}}$.
