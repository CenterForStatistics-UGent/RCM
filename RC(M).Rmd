...
for (i in funFilesPub) {source(file.path("pubFun",i))}
```
...
if(verbose)  cat(ifelse(tmpAlpha$convergence, "Environmental gradient converged \n", "Environmental gradient DID NOT converge \n"))
...
NB_params_tmp = nleqslv(x = c(NB_params, lambda2), fn = respFunScoreMat, jac = respFunJacMat, aX = X, reg = design, thetaMat = thetas, muMarg = muMarg, v=v, p=p, psi = psi)
...
Palette=rainbow(length(unique(dataSam$colourPlot)))
  ...
  taxFrac = taxNum/ncol(RCMfit$X)
    ...
    id = arrowLengths >= quantile(arrowLengths,taxFrac)
...
We clearly have two groups in the second dimension, partly explained by the _Reactor.phase_ and _collection.date_ variables. In the first dimension we have a strong time signal.
...
This came up during our own researhc into a version of correspondence analysis that would accurately model the variance of our count data. Still it is designed for distances between distributions rather than between real data ratios.
...
$$d_{hellinger}^2(a,b) = \sum_{j=1}^p (\sqrt{\frac{x_{aj}}{x_{a.}}}-\sqrt{\frac{x_{bj}}{x_{b.}}})^2 $$
