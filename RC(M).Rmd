...
We can do this for the taxa that respond strongest to the environmental gradient, and make residual plots with deviance or Pearson residuals. An alternative is to try to detect systematic trends through series of positive or negative residuals using Ward and Wolfowitz' runs test, and plot the taxa with the largest test statistic. The drawback of this approach is that these runs statistic are very sensitive to the number of zeroes, since zeroes always have negative residuals. Therefore I prefer the first approach, more so because for these strong responders receive most attention so their assumptions should be verified.  
...
with $\mathbf{Z}$ a diagonal matrix with the column weights $\mathbf{z}$ on hte diagonal, i.e. $Z_{ab} = \delta_{ab}z_a$. Since the parameters of the response functions are species specific, they should undergo weighted normalization, following the same argumant as for the species scores in the unconstrained case.
