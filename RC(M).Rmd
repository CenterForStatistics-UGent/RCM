...
mapply(RCMlist, CAresList0, CodaResList0, CodaScaledResList0, BCresList0, JSDresList0, BCresList0Rel, FUN = function(RCM, CA, CoDa, CoDaScale, BC, JSD, BCrel){
                    list(RCM = RCM, CA = CA, 
...
#-----------------------------------------#
# A functions to plot the correlations with the library sizes of the row scores
plotRowCor = function(resListRows, datList, Dims = 1:3, dataMat = FALSE){
par(mar = c(4,4,4,4))
if(!dataMat){
  datList = lapply(datList, function(x){x$dataMat})
}
  libSizes = rowSums(datList[[1]])
cor0 = lapply(Dims, function(Dim){
  rbind(
  mapply(resListRows, datList, FUN = function(x,z){
  libSizes = rowSums(z)
  sapply(x, function(y){libCor(y, libSizes = libSizes, Dim = Dim)}) 
 }, SIMPLIFY = TRUE),
 "Control" = replicate(length(datList),cor(rnorm(length(libSizes)), libSizes)))}) #Include a control

cor0df = lapply(cor0, melt, value.name = "Correl", varnames = c("Method"))
par(mfrow = c(1,max(Dims)))
for (i in Dims){
data = cor0df[[i]]
boxplot(Correl ~ Method, main = paste("Correlations of library sizes \n with row scores of dimension",i), data = data, ylim = c(-1,1), ylab = "Pearson correlation")
...
plotRowCor(resList0Rows, datList0)
```
...
plotRowCor(resList1Rows, datList1, dataMat = TRUE)
```
...
plotRowCor(resList2Rows, datList2)
```
