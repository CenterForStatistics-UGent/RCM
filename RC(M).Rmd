...
dNBllrow_1B1 = function(beta, X, reg, thetas, muMarg, k, n , p, rowWeights, nLambda, rMatK) {
  ...
  lambda1 = beta[k+1] #Centering restrictions sum(abunds*r_{ik}) = 0
  lambda2 = beta[k+2] #normalization restrictions sum(abunds*r^2_{ik}) = 1
#Orthogonality restrictions sum(abunds*r_{ik}*r_{ik'}) = 0. First all of dimension 1, then the remaining of dimension 2 etc. #+2*k
  if(k==1){lambda3Mat=0} else {
  lambda3Mat = matrix(0,ncol=k-1, nrow=k-1)
...
rMat = apply(rMat, 2, function(rowS){
  ...
  cat("\n","Outer Iteration", iterOut, "\n","\n")
    if(iterOut!=1){
  cat("Old psi-estimates: ", psisOld[KK], "\n")
...
#Overdispersions (not at every iterations to speed things up, doesn't change a lot anyway)
    if((iterOut %% dispFrec) ==0  || iterOut==1){
  if (verbose) cat("\n Estimating overdispersions \n")
 thetas = estDisp(X = X, rMat = rMat[,1:KK,drop=FALSE], cMat = cMat[1:KK,,drop=FALSE],  muMarg=muMarg, psis = psis[KK], k=1:KK, prior.df = prior.df)
  }
    
#Psis
    if (verbose) cat("\n Estimating psis \n")
regPsis = outer(rMat[,KK] ,cMat[KK,]*psis[KK])
...
if(class(tmpCol)=="list"){
  cMat = matrix(tmpCol$x[1:p], byrow=TRUE, ncol=p, nrow=k)
  lambdaCol = tmpCol$x[p + seq_along(idK)]
}
#Normalize (speeds up algorithm if previous step had not converged)
cMat = t(apply(cMat,1, function(colS){
    colS - sum(colS * colWeights)/sum(colWeights) 
...
weightsChar = paste0(rowWeights, colWeights)
  switch(method, NB=RCM_NB(dat, rowWeights=rowWeightsNum, colWeights=colWeightsNum, weightsChar = weightsChar,...), ZIP=RCM_ZIP(dat, rowWeights=rowWeightsNum, colWeights=colWeightsNum,weightsChar = weightsChar, ...), ZINB=RCM_ZINB(dat,  rowWeights=rowWeightsNum, colWeights=colWeightsNum,weightsChar = weightsChar,...))
}
