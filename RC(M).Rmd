...
reqpkg <- c("phyloseq","MASS", "parallel","nleqslv", "edgeR", "VGAM","HMP", "ggplot2", "pscl", "vegan", "alabama", "zCompositions", "DirFactor","reshape2", "tensor", "locfit")
...
Filtering out the effect of confounders brings the problems of the zeroes to the fore again. If a taxon has only zero counts in one of the subgroups defined by the confounding variables, the model will fail to fit, since the parameters $\zeta_{jl}$ would need to equal infinity. For continuous confounders this is of course not an issue. Maybe there exists a more efficient solution, but for now we will have to trim the taxa for which this occurs from further analysis in order to render the model identifiable. This trimming occurs already before fitting the independence model.
...
This approach differs from the one proposed in Zhu _et al._, 2005 in the intercepts $u_i$ and $u_j$. These intercepts will still be estimated by maximum likelihood in the first step, just as in the unconstrained case. This is a special case of the fourth method.

3) Constrained RC(M), overparametrized version
...
A linear response function may be most appropriate for problems with __short gradients__ i.e. whereby the difference in observed environmental variables is too short to devine more than an increase or decrease in departure from independence. For problems with __long gradients__, i.e. whereby species' departures of independence rise and drop (or drop and rise) within the score of the observed environmental scores, quadratic response functions may be more appropriate. This corresponds e.g. with the scenario whereby a species' abundance departs a lot from independence (e.g. by being very low) for extreme values of the environmental score, and behaves approximately like the independence model for a regular value of the environmental score. If the user is unsure and has enough data, he may use non-parametric response funcion. This may improve the sample and covariate ordination, although the result for the species becomes harder to interpret.
...
One restriction I intended to include but which turns out to be a very bad idea is to center the $\alpha$'s:
...
Again, only the $\psi$ parameters can grow in size to reflect the importance of the dimension.
...
For continuous variables the previous discussion suffises, but for categorical variables special attention is required. In a regular, unrestricted setting one would use treatment coding, setting one of the levels of the variable to be the reference level and estimate no parameter for it. However, the above centering, normalization and orthogonality constraints render the result dependent on the choice of reference level, which is of course highly undesirable. Also for quick understanding of the biplot it would be convenient to plot all the values of the categorical variable, and save the reader the trouble of looking up the reference value. Because of these two reasons we will need a coding scheme that includes all parameters.
...
For imposing the restrictions and for plotting we want a representation whereby all covariate levels are represented. We therefore include all parameters, and impose that the parameters sum to 0 within each covariate.
...
To render the values of the continuous variables in the environmental gradient comparable, it is clear that they need to be centered and scaled prior to model fitting, as in PCA. We might have to do the same for the categorical variables, for complete comparability? There are a lot of possible solutions (see e.g. SOCIOECONOMIC STATUS MEASUREMENT WITH DISCRETE PROXY VARIABLES: IS PRINCIPAL COMPONENT ANALYSIS A RELIABLE ANSWER? by Kolenikov and Angeles, 2009) for the PCA case. For now we do not normalize the categorical variables, but this means we may not be able to compare components of $\boldsymbol{\alpha}$ from continuous and categorical variables.

### Environmental gradient

#### Naive approach

The naive approach to optimize $LR(\alpha)$ would be to use constrained optimization (constraining the $\alpha$'s to have variance one etc.) and estimate $p_j^{(\alpha)}$ and $p^{(\alpha)}$ in every evaluation of this function. This implies a analytical jacobian cannot be estimated and requires a huge amount of re-estimation of the response functions. This naive approach is computationally unfeasible

#### Iterative approach

A smarter approach (also the one proposed by Zhu and Hastie, 2005) is to iterate between estimating $\alpha$ by optimizing $LR(\alpha)$ given $p_j^{(\alpha)}$ and $p^{(\alpha)}$, and estimating $p_j^{(\alpha)}$ and $p^{(\alpha)}$ given $\alpha$. This results in much fewer estimations of $p_j^{(\alpha)}$ and $p^{(\alpha)}$ and allows to optimize $LR(\alpha)$ by finding the roots of $\frac{\partial LR(\alpha)}{\partial \alpha} = 0$ (and analytically specifying the second order derivative).
...
with $p^{(\alpha)}$ and $p^{(\alpha)}_j$ estimated probability density functions without and with taxon labels and $\boldsymbol{\beta}_l$ and $\boldsymbol{\beta}$ the parameters of the response functions. $p^{(\alpha)}$ and $p^{(\alpha)}_j$ are calculated under a certain error distribution (e.g. negative binomial).
...
$$ Lag_{LR}(\boldsymbol{\alpha}_k| \boldsymbol{\beta}_{jk}, \boldsymbol{\beta}_{k}) =  \sum_{i=1}^n  \sum_{j=1}^p \Big(log \big(\frac{f_{NB}(x_{ij}|s_{kj}, \psi_k, \mathbf{C}_i, \boldsymbol{\alpha}_k, \phi_j, f_j)}{f_{NB}(x_{ij}|s_{kj}, \psi_k, \mathbf{C}_i, \boldsymbol{\alpha}_k, \phi_j, f_0)} \big) \Big) + \lambda_{1k} \boldsymbol{\alpha}_k^t\mathbf{M} + \lambda_{2k} (\boldsymbol{\alpha}_k^t \boldsymbol{\alpha}_k - 1) + \boldsymbol{\lambda}_{orth,k} \boldsymbol{\alpha}_k^t \boldsymbol{\alpha}_{-k}$$

with $\boldsymbol{\alpha}_{-k}$ the $\boldsymbol{\alpha}$-matrix from the first to the (k-1)th column, and $f_{NB}$ the density function of the negative binomial distribution.

##### Linear response function

The first order derivatives becomes (in case of a linear response function, and remember we set $s_{jk} = 1$)

$$\frac{\partial Lag_{LR}(\boldsymbol{\alpha}_k)}{\partial \boldsymbol{\alpha}_k} = \psi_k \Big( \sum_{i=1}^n  \sum_{j=1}^p s_{jk} \mathbf{C}_i \beta_{1j} \frac{x_{ij} - \mu_{ij}}{1 + \frac{\mu_{ij}}{\theta_j}} - \sum_{i=1}^n  \sum_{j=1}^ps_{jk} \mathbf{C}_i \beta_{1} \frac{x_{ij} - \mu_{ij0}}{1 + \frac{\mu_{ij0}}{\theta_j}} \Big) + \boldsymbol{\alpha}_k^t\mathbf{M}\boldsymbol{\lambda}_{1k} + 2\lambda_{2k} \boldsymbol{\alpha}_k + \boldsymbol{\lambda}_{orth,k}^t \boldsymbol{\alpha}_{-k} = \mathbf{0}$$
...
and for the quadratic response function these second order derivatives are:
...
The whole estimation of the constrained RC(M)-model would then proceed as follows:
 ...
 6. Repeat steps for every dimension $k$ 4-5 until convergence
...
The interpretation of the triplot is different from the one from Zhu _et al._ (2005), and also different from that of the unconstrained RC(M).
 ...
 - The components of $\alpha_k$ are plotted as arrows and represent the extent to which the covariates contribute to the departure from independence in dimension $k$. Categorical variables are centered, all their levels are shown. For a scale-free solution, the continuous variables should be centered and scaled. Should the dummy variables not also be normalized then? QUESTION!
 - For the samples, the environmental scores are plotted as points and represent the values of the product of the environmental gradient with their covariates. Samples close together in one dimension come from similar environments, considering that environmental gradient.
 - The plot for the taxa depends on the response function used. 
  * In case of a linear response functionwe can plot arrows originating from $-\frac{\beta_{0j}}{\beta_{1j}}$, which represents the point of no departure from independence. The direction and size of the arrow then depends purely on $\beta_{1j}$. The projection of this vector onto $\boldsymbol{alpha}^t\mathbf{C}_i$ then corresponds to the departure from independence for taxon $j$ in sample $i$ explained by its environmental variables $\mathbf{C}_i$. The arrow is appropriate since a larger environmental score always results in a larger departure from indepedence under this model. As often in statistics, linearity may not be a very credible assumption, but it leads to models that are easy to interpret.
  * In case of quadratic response function we should plot the extremum of the response functions in each dimension as a point. If this is a minimum, this represents the value of the environmental score with the smallest departure from indepence, in case of a maximum the largest departure. With colour and shape codes we can show which kind of extremum it is, and which kind of departure from indepedence (negative or positive effect on the expectation).
  * In case of non-parametric response function I do not see I clear way to plot the species. The benefit of this method will have to be the improved ordination of the environmental gradient and the samples.
...
estDisp = function (X, cMat = NULL, rMat = NULL, muMarg, psis, trended.dispersion, prior.df = 10, dispWeights = NULL, rowMat = NULL, nonParamsRespFun = NULL)
{
  # @param X: the data matrix of dimensions nxp
  ...
  logMeansMat = 
    if(!is.null(rMat)){ #Unconstrained
      ...
      if(is.null(nonParamsRespFun)){ #Parametric response function
      t(log(muMarg) + psis* rowMat)
      } else { #Parametric response function
        t(log(nonParamsRespFun$taxonWise*psis))
      }
    }
...
nonParamsRespFun = if(responseFun == "nonparametric") {list(taxonWise = matrix(0.1,n,p), overall = rep(0.1,n))} else {NULL}
rowMat = NULL
...
nLambda1s = NROW(centMat)
 ...
 lambdasAlpha = rep(0,nLambda1s +KK)
  ...
  if(KK>1){
    ...
    modMat = exp(getRowMatConstr(responseFun = responseFun, NB_params = NB_params[,,KK-1], covariates = covariates, alpha = alpha[, KK-1]))
    muMarg = muMarg * if(KK==2) {modMat} else {apply(modMat, 3, prod)}
    } else {
    muMarg = nonParamsRespFun$taxonWise
    }
  }
  ...
  #The matrix of row scores
   if(responseFun %in% c("linear","quadratic")){
  rowMat = getRowMatConstr(responseFun = responseFun, NB_params = NB_params[,,KK], covariates = covariates, alpha = alpha[, KK])
   }

#Overdispersions (not at every iterations to speed things up, doesn't change a lot anyway)
  if((iterOut[KK] %% dispFrec) == 0 || iterOut[KK] == 1){
    if (verbose) cat("\n Estimating overdispersions \n")
 thetas = estDisp(X = X, muMarg = muMarg, psis = psis[KK], prior.df = prior.df, trended.dispersion = trended.dispersion, rowMat = rowMat, nonParamsRespFun = nonParamsRespFun)
 thetasMat = matrix(thetas, n, p, byrow=TRUE)
  }
...
psis[KK]  = abs(nleqslv(fn = dNBpsis, x = psis[KK], theta =  thetasMat , X = X, reg=rowMat, muMarg=muMarg, global=global, control = nleqslv.control, jac=NBjacobianPsi, method = jacMethod, nonParamsRespFun = nonParamsRespFun)$x)

#Row scores
  if (verbose) cat("\n Estimating environmental gradient \n")
tmpAlpha = estAlpha(Alpha = alpha[, KK,drop=FALSE], X = X, CC = covariates, d = d, ncols = p, v = v, responseFun = responseFun, muMarg = muMarg, thetas = thetasMat , alphaK = alpha[,1:(KK-1),drop=FALSE], k=KK, centMat = centMat, n=n, NB_params = NB_params[,,KK], NB_params_noLab = NB_params_noLab[,KK], psi = psis[KK], nLambda1s = nLambda1s, nLambda = KK + nLambda1s, analytic = analytic, lambdasAlpha = lambdasAlpha, nleqslv.control = nleqslv.control, nonParamsRespFun = nonParamsRespFun) #Provide previous parameters of the response function since they might provide good initial estimates
...
Palette=rainbow(length(unique(dataSam$colourPlot)))
      ...
      if(constrained) {
        if(RCMfit$responseFun=="linear"){
          dataTax = data.frame(
            ...
            slope1 = RCMfit$NB_params[2,,Dim[1]], #The gradient
            slope2 = RCMfit$NB_params[2,,Dim[2]]
            ) 
    ...
    plot = ggplot(dataSam, aes_string(x="Dim1", y="Dim2", col = "colourPlot", dotList)) + 
    ...
    if(plotTaxa && (!constrained || RCMfit$responseFun=="linear")){
    ...
    if(constrained){
      ...
      plot = plot + geom_text(data = varData, mapping = aes_string(x = "Dim1", y = "Dim2", label = "label"), inherit.aes = FALSE)
    }
    squarePlot(plot)
}
...
LR_nb <- function(Alpha, X, CC, responseFun = c("linear","quadratic","nonparametric"), muMarg, psi, nleqslv.control = list(trace=FALSE), NB_params, NB_params_noLab, thetaMat, n, ncols, nonParFit, nonParamRespFun,...){#
  ...
  # @return: a scalar, the evaluation of the log-likelihood ratio at the given alpha
    ...
    mu0 = muMarg * exp(nonParamRespFun$overall)
    }
...
dLR_nb <- function(Alpha, X, CC, responseFun = c("linear", "quadratic", "nonparametric"), psi, NB_params, NB_params_noLab, d, alphaK, k, centMat, nLambda, nLambda1s, thetaMat, muMarg, ncols, ...){
  ...
  lag = switch(responseFun, #The lagrangian depends on the shape of the response function
               ...
               "quadratic" = psi * ( #ANALYTIC SCORE FUNCTION IS WRONG!!
...
estNPresp = function(sampleScore, muMarg, X, ncols, psis, ...){
  taxonWiseFits = lapply(seq_len(ncols), function(i){
  locfit.raw(x = sampleScore, y = (X[,i]-muMarg[,i])/psis,link = "log",...)
})
  overall = fitted.locfit(locfit.raw(x = sampleScore, y = (X- muMarg)/psis,link = "log",...))
  taxonWise = sapply(taxonWiseFits, fitted.locfit)
  list(taxonWise = taxonWise, overall = overall, taxonWiseFits = taxonWiseFits)
  }
#--------------------------------------#
...
NB_params_noLab = estNBparamsNoLab(design = design, thetas = thetas, muMarg = muMarg, psi = psi, X = X, nleqslv.control = nleqslv.control, initParam = NB_params_noLab, n = n, v = v)
 ...
 if(analytic){
  ...
  #New strategy: estimate the parameters and then optimize alpha, iterate between both.
  sampleScore = CC %*% Alpha
  nonParamRespFun = estNPresp(sampleScore = sampleScore, muMarg = muMarg, X = X, ncols = ncols, ...)
  Alpha = constrOptim.nl(par = Alpha, fn = LR_nb, gr = NULL, heq = heq_nb, heq.jac = heq_nb_jac, alphaK = alphaK, X=X, CC=CC, responseFun = responseFun, muMarg = muMarg, d = d, ncols=ncols, control.outer = control.outer, control.optim = control.optim, nleqslv.control = nleqslv.control, k =k, centMat = centMat, n=n, nonParamRespFun = nonParamRespFun, psis = psis)
...
con1Dmat = RCM(physeq, k=1, covariates =  sample_variables(physeq), analytic = TRUE, responseFun = "quadratic")
# The zeller data?
con1Dmat = RCM(physeq, k=1, covariates =  sample_variables(physeq), analytic = TRUE, responseFun = "nonparametric")
```
...
```{r RC(M)_NB real datasets, purl = FALSE, eval = FALSE}
...
```{r HMP plot, purl = FALSE, eval = FALSE}
...
```{r HMP filter out variable effect, purl=FALSE, eval = FALSE}
K=2
...
```{r zeller country vs diagnosis, purl = FALSE}
...
zellerSphyRCMconstr = RCM(zellerSphy, k=2, round = TRUE, covariates = c("Age","Gender","BMI","Country", "Diagnosis")) # Use smallest possible cndtol!
save(zellerSphyRCMconstr, file="zellerSphyRCMconstr.RData")
...
```{r ZellerConstrainedBiplot, purl=FALSE}
PCbiplot(zellerSphyRCMconstr)
PCbiplot(zellerSphyRCMconstr, colour = "Diagnosis")
PCbiplot(zellerSphyRCMconstr, colour = "Gender")
PCbiplot(zellerSphyRCMconstr, colour = "Country")
PCbiplot(zellerSphyRCMconstr, colour = "Age")
...
```{r LRnbcheck, purl=FALSE, eval = FALSE}
...
testLR = LR_nb(alphaK, X, t(regMat), responseFun ="linear", muMarg = muMargObs, thetaMat = matrix(thetasCon[idCol], byrow = TRUE, nrow = nrow(X), ncol = ncol(X)), cReg = cVec2, d= nrow(regMat), ncols=ncol(X),v=2)
...
```{r LRnbchack2, purl=FALSE, eval = FALSE}
...
## Unconstrained RC(M)
 ...
 - PCoA
   ...
   * UniFrac distance (Microbial ecology, uses phylogenetic information) => This will be difficult since it does not rely on counts, or only partially in case of weighted unifrac
   * Jenssen-Shannon divergence because of the enterotypes publication
...
location and dispersion effects
   ...
   * PCoA also has the drawback of not yielding any species scores, it is an eigenvalue rather than a singular value method. The information on the individual species is lost in the distance calculation 
...
# A bayesian approach, gets a bit complicated
...
nonParamBayes = function(dat, hyper = list( nv = 3, a.er = 1, b.er = 0.3, a1 = 3, a2 = 4, m = 10, alpha = 10, beta = 0 )){
  ...
  DirFactor(t(dat), hyper, step = 100)
}
```
...
Pearson correlations of row scores with library sizes could be another criterion to evaluate the quality of the biplot.

```{r libSizeCorrelation}
libCor = function(coord, libSizes, Dim = c(1,2)){
  apply(coord[, Dim, drop=FALSE], 2, cor, libSizes)
}
```
...
For the methods that do yield taxon scores, we can also verify if the correct taxa contribute to the separation of the clusters. A useful measure in this case seems the relative contribution of the taxa with signal in the direction of the cluster centroid.
...
extractRows = function(x){
  with(x, list(
    ...
    CoDaScale = CoDaScale$rotation %*% diag(CoDaScale$sdev^2),
    ...
    JSDrel = JSDrel$vectors %*% diag(JSDrel$values$Eigenvalues[seq_len(ncol(JSDrel$vectors))])
    ))
}
```
...
datList0 = mclapply(mc.cores=4, 1:reps, function(i){
## Sample parameters
...
datList0mat = lapply(RCMresList0, function(x){x$X})
CAresList0 = lapply(datList0mat, caSVD)
CodaResList0 = lapply(datList0mat, CoDa)
CodaScaledResList0 = lapply(datList0mat, CoDa, scale = TRUE)
...
JSDresList0Rel = lapply(datList0mat, function(x){ordinate(otu_table(x/rowSums(x), taxa_are_rows = FALSE), method="PCoA", distance = "jsd")})
resList0 = mapply(RCMresList0, CAresList0, CodaResList0, CodaScaledResList0, BCresList0, JSDresList0, BCresList0Rel, JSDresList0Rel, FUN = function(RCM, CA, CoDa, CoDaScale, BC, JSD, BCrel, JSDrel){
                    list(RCM = RCM, CA = CA, 
                         CoDa = CoDa, CoDaScale = CoDaScale, 
                         BC = BC, JSD = JSD, 
                         BCrel = BCrel, JSDrel = JSDrel)
                  }, 
                  SIMPLIFY = FALSE
)
...
```{r resList0 sampleBiplot, eval = FALSE, purl = FALSE}
#A sample biplot
resList0Rows = lapply(resList0, extractRows)
par(mfrow = c(3,3), mar = c(3,3,3,3))
ID = sample(size=1, seq_along(resList0))
obj = resList0Rows[[ID]]
lapply(names(obj),function(x){plot(obj[[x]], main=x, xlab = "Dim 1",ylab = "Dim 2", pty = "s")})
# plot(resList1$RCM[[id]]$rMat[,1:2], col = groupFactor, main = "RC(M)")
# plot(resList1$CA[[id]]$u[,1:2], col = groupFactor, main = "CA", xlab = "Dim 1", ylab ="Dim2")
# plot(resList1$CoDA[[id]]$rotation[,1:2], col = groupFactor, main = "CoDA")
# plot(resList1$BC[[id]]$vectors[,1:2], col = groupFactor, main = "PCoA: Bray-Curtis")
# plot(resList1$JSD[[id]]$vectors[,1:2], col = groupFactor, main = "PCoA: Jensen-Shannon divergence")
rm(ID, obj)
par(mfrow = c(1,1))
...
ID = sample(size=1, seq_along(resList0))
obj = resList0Rows[[ID]]
lapply(names(obj),function(x){plot(obj[[x]][,c(1,3)], main=x, xlab = "Dim 1",ylab = "Dim 3", pty = "s")})
# plot(resList1$RCM[[id]]$rMat[,1:2], col = groupFactor, main = "RC(M)")
# plot(resList1$CA[[id]]$u[,1:2], col = groupFactor, main = "CA", xlab = "Dim 1", ylab ="Dim2")
# plot(resList1$CoDA[[id]]$rotation[,1:2], col = groupFactor, main = "CoDA")
# plot(resList1$BC[[id]]$vectors[,1:2], col = groupFactor, main = "PCoA: Bray-Curtis")
...
```{r libSizeCorr0, purl = FALSE, fig.cap = "Correlations of row scores with library sizes, results of 100 simulations shown for first two dimensions. Red diamonds indicate means. A silhouette of 1 point to perfect classification, 0 to borderline cases, -1 to perfect wrong classification \\label{fig:libCor1}"}
par(mar = c(4,4,4,4))
cor0 = lapply(1:3, function(Dim){
  rbind(
  mapply(resList0Rows, datList0, FUN = function(x,z){
  libSizes = rowSums(z$dataMat)
  sapply(x, function(y){libCor(y, libSizes = libSizes, Dim = Dim)}) 
 }, SIMPLIFY = TRUE),
 "Control" = replicate(length(datList0),cor(rnorm(length(libSizes)), libSizes)))}) #Include a control

cor0df = lapply(cor0, melt, value.name = "Correl", varnames = c("Method"))
par(mfrow = c(1,3))
for (i in 1:3){
data = cor0df[[i]]
boxplot(Correl ~ Method, main = paste("Correlations for row scores \n with library sizes for dimension",i), data = data, ylim = c(-1,1), ylab = "Pearson correlation")
...
save(datList1,groupFactor,  file = "RCMsim/datList1.RData")
...
CAresList1 = lapply(datList1, caSVD)
CodaResList1 = lapply(datList1, CoDa)
CodaScaledResList1 = lapply(datList1, CoDa, scale = TRUE)
BCresList1 = lapply(datList1, function(x){ordinate(otu_table(x, taxa_are_rows = FALSE), method="PCoA", distance = "bray")})
JSDresList1 = lapply(datList1, function(x){ordinate(otu_table(x, taxa_are_rows = FALSE), method="PCoA", distance = "jsd")})
#With relative abundances
BCresList1Rel = lapply(datList1mat, function(x){ordinate(otu_table(x/rowSums(x), taxa_are_rows = FALSE), method="PCoA", distance = "bray")})
JSDresList1Rel = lapply(datList1mat, function(x){ordinate(otu_table(x/rowSums(x), taxa_are_rows = FALSE), method="PCoA", distance = "jsd")})
resList1 = mapply(RCMresList1, CAresList1, CodaResList1, CodaScaledResList1, BCresList1, JSDresList1, BCresList1Rel, JSDresList1Rel,
                  FUN = function(RCM, CA, CoDa, CoDaScale, BC, JSD, BCrel, JSDrel){
                    list(RCM = RCM, CA = CA, CoDa = CoDa, CoDaScale = CoDaScale, BC = BC, JSD = JSD, BCrel = BCrel, JSDrel  =JSDrel)
                  }, 
                  SIMPLIFY = FALSE
)
save(resList1, file = "RCMsim/resList1.RData")
} else {load(file = "RCMsim/resList1.RData")}
```

```{r resList1 sampleBiplot, eval = FALSE, purl = FALSE}
#A sample biplot
resList1Rows = lapply(resList1, extractRows)
par(mfrow = c(3,3), mar = c(3,3,3,3))
ID = sample(size=1, seq_along(resList1))
obj = resList1Rows[[ID]]
lapply(names(obj),function(x){plot(obj[[x]], col = groupFactor, main=x, xlab = "Dim 1",ylab = "Dim 2", pty = "s")})
# plot(resList1$RCM[[id]]$rMat[,1:2], col = groupFactor, main = "RC(M)")
# plot(resList1$CA[[id]]$u[,1:2], col = groupFactor, main = "CA", xlab = "Dim 1", ylab ="Dim2")
# plot(resList1$CoDA[[id]]$rotation[,1:2], col = groupFactor, main = "CoDA")
# plot(resList1$BC[[id]]$vectors[,1:2], col = groupFactor, main = "PCoA: Bray-Curtis")
...
ID = sample(size=1, seq_along(resList1))
obj = resList1Rows[[ID]]
lapply(names(obj),function(x){plot(obj[[x]][,c(1,3)], col = groupFactor, main=x, xlab = "Dim 1",ylab = "Dim 3", pty = "s")})
# plot(resList1$RCM[[id]]$rMat[,1:2], col = groupFactor, main = "RC(M)")
# plot(resList1$CA[[id]]$u[,1:2], col = groupFactor, main = "CA", xlab = "Dim 1", ylab ="Dim2")
# plot(resList1$CoDA[[id]]$rotation[,1:2], col = groupFactor, main = "CoDA")
# plot(resList1$BC[[id]]$vectors[,1:2], col = groupFactor, main = "PCoA: Bray-Curtis")
...
sil1  = lapply(resList1Rows, function(x){
...
boxplot(Silhouette~Method, main = "Mean silhouettes by method", data = meanSil1df)
meansSil = tapply(meanSil1df$Silhouette, meanSil1df$Method, mean)
points(meansSil, col="red", pch=18, cex = 1.6)
```

```{r resList1Evaldist, purl = FALSE, fig.cap = "Mean ratios of within to overal distance by method, results of 100 simulations shown for first two dimensions. Red diamonds indicate means. A silhouette of 1 point to perfect classification, 0 to borderline cases, -1 to perfect wrong classification \\label{fig:dist1}"}
dist1  = sapply(resList1Rows, function(x){
  sapply(x, function(y){distanceFun(y, clusters = groupFactor)$ratio})
})
meanDist1df = melt(dist1, value.name = "DistanceRatio", varnames = c("Method"))
...
```{r libSizeCorr1, purl = FALSE, fig.cap = "Correlations of row scores with library sizes, results of 100 simulations shown for first two dimensions. Red diamonds indicate means. A silhouette of 1 point to perfect classification, 0 to borderline cases, -1 to perfect wrong classification \\label{fig:libCor1}"}
par(mar = c(4,4,4,4))
cor1 = lapply(1:3, function(Dim){mapply(resList1Rows, datList1, FUN = function(x,z){
  libSizes = rowSums(z)
  sapply(x, function(y){libCor(y, libSizes = libSizes, Dim = Dim)})
}, SIMPLIFY = TRUE)})
cor1df = lapply(cor1, melt, value.name = "Correl", varnames = c("Method"))
par(mfrow = c(1,3))
for (i in 1:3){
data = cor1df[[i]]
boxplot(Correl ~ Method, main = paste("Correlations for row scores \n with library sizes for dimension",i), data = data, ylim = c(-1,1), ylab = "Pearson correlation")
...
datList2 = mclapply(mc.cores=4, 1:reps, function(i){
## Sample parameters
...
RCMresList2 = mclapply(mc.cores=4, datList1, function(y){
  RCM(y$dataMat, k = nDim, distribution = "NB")
})
datList2mat = lapply(datList2, function(x){x$dataMat})
CAresList2 = lapply(datList2mat, caSVD)
CodaResList2 = mclapply(mc.cores=1,datList2mat, CoDa)
CodaScaledResList2 = lapply(datList2mat, CoDa, scale = TRUE)
BCresList2 = lapply(datList2mat, function(x){ordinate(otu_table(x, taxa_are_rows = FALSE), method="PCoA", distance = "bray")})
JSDresList2 = lapply(datList2mat, function(x){ordinate(otu_table(x, taxa_are_rows = FALSE), method="PCoA", distance = "jsd")})
#With relative abundances
BCresList2Rel = lapply(datList2mat, function(x){ordinate(otu_table(x/rowSums(x), taxa_are_rows = FALSE), method="PCoA", distance = "bray")})
#For JSD the relative abundances do not make any difference, it works on relative abundances anyway!
resList2 = mapply(RCMresList2, CAresList2, CodaResList2, CodaScaledResList2, BCresList2, JSDresList2, 
                  BCresList2Rel, 
                  FUN = function(RCM, CA, CoDa, CoDaScale, BC, JSD, BCrel){
                    list(RCM = RCM, CA = CA, CoDa = CoDa, CoDaScale = CoDaScale, BC = BC, JSD = JSD, BCrel = BCrel)
                  }, 
                  SIMPLIFY = FALSE
)
save(resList2, file = "RCMsim/resList2.RData")
} else {load(file = "RCMsim/resList2.RData")}
```

```{r resList2 sampleBiplot, eval = FALSE, purl = FALSE}
#A sample biplot
resList2Rows = lapply(resList2, extractRows)
par(mfrow = c(3,3), mar = c(3,3,3,3))
ID = sample(size=1, seq_along(resList2))
obj = resList2Rows[[ID]]
lapply(names(obj),function(x){plot(obj[[x]], col = groupFactor2, main=x, xlab = "Dim 1",ylab = "Dim 2", pty = "s")})
rm(ID, obj)
par(mfrow = c(1,1))
...
```{r resList2Evalsil, purl = FALSE, fig.cap = "Mean silhouettes by method, results of 100 simulations shown for first two dimensions. Red diamonds indicate means. A silhouette of 1 point to perfect classification, 0 to borderline cases, -1 to perfect wrong classification \\label{fig:silhouette1}"}
sil2  = lapply(resList2Rows, function(x){
  sapply(x, silhouette, clusters = groupFactor2)
})
meanSil2 = sapply(sil2, function(x){
  colMeans(x)
})
meanSil2df = melt(meanSil2, value.name = "Silhouette", varnames = c("Method"))
boxplot(Silhouette~Method, main = "Mean silhouettes by method", data = meanSil2df)
meansSil2 = tapply(meanSil2df$Silhouette, meanSil2df$Method, mean)
points(meansSil2, col="red", pch=18, cex = 1.6)
```

The RC(M) method has the highest silhouettes, PCoA is second best but far below.

```{r resList2Evaldist, purl = FALSE, fig.cap = "Mean ratios of within to overal distance by method, results of 100 simulations shown for first two dimensions. Red diamonds indicate means. A silhouette of 1 point to perfect classification, 0 to borderline cases, -1 to perfect wrong classification \\label{fig:dist1}"}
dist2  = sapply(resList2Rows, function(x){
  sapply(x, function(y){distanceFun(y, clusters = groupFactor2)$ratio})
})
meanDist2df = melt(dist2, value.name = "DistanceRatio", varnames = c("Method"))
boxplot(DistanceRatio~Method, main = "Mean ratios of within to overal distance by method", data = meanDist2df)
meansDist = tapply(meanDist2df$DistanceRatio, meanDist2df$Method, mean)
points(meansDist, col="red", pch=18, cex = 1.6)
```

RCM comes out as best, CA as worst

```{r libSizeCorr2, purl = FALSE, fig.cap = "Correlations of row scores with library sizes, results of 100 simulations shown for first two dimensions. Red diamonds indicate means. A silhouette of 1 point to perfect classification, 0 to borderline cases, -1 to perfect wrong classification \\label{fig:libCor1}"}
par(mar = c(4,4,4,4))
cor2 = lapply(1:3, function(Dim){rbind(mapply(resList2Rows, datList2mat, FUN = function(x,z){
  libSizes = rowSums(z)
  sapply(x, function(y){libCor(y, libSizes = libSizes, Dim = Dim)})
}, SIMPLIFY = TRUE), "Control" = replicate(length(datList0),cor(rnorm(length(libSizes)), libSizes)))}) #Include a control
cor2df = lapply(cor2, melt, value.name = "Correl", varnames = c("Method"))
par(mfrow = c(1,3))
for (i in 1:3){
data = cor2df[[i]]
boxplot(Correl ~ Method, main = paste("Correlations fo row scores \n with library sizes for dimension",i), data = data, ylim = c(-1,1), ylab = "Pearson correlation")
meansCor = tapply(data$Correl, data$Method, mean)
points(meansCor, col="red", pch=18, cex = 1.6)
abline(h=0, lty = "dashed")
rm(data, meansCor)
}
#Write standard functions for these things!
```

The CoDa method is clearly affected by the library sizes, as is CA and Bray-Curtis on absolute abundances, but not PCoA.
