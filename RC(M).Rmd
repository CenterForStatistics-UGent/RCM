...
knitr::opts_chunk$set(cache = TRUE, autodep = TRUE,  warning=FALSE, message=FALSE, echo=FALSE, eval=TRUE, tidy = TRUE, fig.width=9, fig.height=6, root.dir=WD, purl=TRUE, fig.show = "hold", cache.lazy = FALSE)
# The required package list:
reqpkg <- c("phyloseq","MASS", "parallel","nleqslv", "edgeR", "VGAM","HMP", "ggplot2", "pscl", "vegan", "zCompositions", "DirFactor","reshape2", "tensor", "locfit", "SimSeq","fdrtool", "cluster", "ape")
...
levelsMeth = c("CA","Hellinger","BC","BCrel","JSD","CoDa","RCM")
labelsMeth = c("CA","Hellinger","Bray-Curtis-Abs","Bray-Curtis","JSD","CoDa","RCM")
groupsMeth = factor(c("Independence","Independence","PCoA","PCoA","PCoA","CoDa","RCM"), levels = c("Independence","PCoA","CoDa","RCM"), ordered = TRUE)
```
...
The rationale is that each observation x is the best estimate of its expectation E(X), and under the Poisson model also of its variance. Still also these plots are susceptible to differences in library sizes. See Rao (1997), who suggests this method as an alternative to correspondence analysis. The drawback of correspondence analysis is that the estimate for the expected abundances used to standardize departures from independence $X-E$ is based on the taxon sums, and thus on other samples. With the Hellinger distance only, the sample itself is used to estimate the expected abundances, namely by the count itself.
...
Redundancy analysis extracts variation between variables of a n-by-p dataset __X__ that can be explained by variables from another n-by-d dataset __Y__. It is a constrained version of principal components analysis (PCA), whereby principal components are restricted to be linear combinations of the set of constraining variables.
...
CCA can be seen as regular CA, with the restrictions that the row scores must be linear combinations of the environmental variables. Also, it is Gaussian ordination with the scores equally restricted under the same conditions as the unrestricted Gaussian ordination.
...
Palette=rainbow(length(unique(dataSam$colourPlot)))
    ...
    plot = ggplot(dataSam, aes_string(x=names(dataSam)[1], y=names(dataSam)[2], col = "colourPlot", dotList)) + 
      ...
      xlab(paste0(names(dataSam)[1],": psi = ",RCMfit$psis[Dim[1]])) + #xlabel
      ylab(paste0(names(dataSam)[2],": psi = ",RCMfit$psis[Dim[2]])) + #ylabel
    #add legend names
...
save(zellerSphyRCMconstrLin, zellerSphyRCMconstrQuad, file="zellerSphyRCMconstr.RData")
...
plot.RCM(zellerSphyRCMconstrLin, samColour = "Diagnosis", taxFrac = 0.015)
...
plot.RCM(zellerSphyRCMconstrQuad, samColour = "Diagnosis", taxFrac = 0.015, Palette = rainbow(5))
```
...
if(!file.exists(file = "armpitRCM.RData")){
  ...
  armpitRCMconstrLin = RCM(armpit, sample_variables(armpit)[!sample_variables(armpit) %in% c("subject")], k=3, responseFun = "linear")
  save(armpitRCM, armpitRCMconstrLin, file = "armpitRCM.RData")
} else {load(file = "armpitRCM.RData")}
```

```{r ArmpitPlot1, purl = FALSE}
plot(armpitRCM, taxFrac = 0.015)
plot(armpitRCM, taxFrac = 0.015, samColour = "shaving")
plot(armpitRCM, taxFrac = 0.015, samColour = "odor_panel")
plot(armpitRCM, taxFrac = 0.015, samColour = "gender")
plot(armpitRCM, taxFrac = 0.015, samColour = "age")
plot(armpitRCM, taxFrac = 0.015, samColour = "antimicrobial")
```
...
Non-metric multidimensional scaling (NMDS) can be considered the little brother of MDS, since it also represents distance matrices in lower dimensions. Still there are some important differences: 
 ...
 The ordination is fitted iteratively, by minimizing the stress between the original distances and the distances of the actual ordination.
...
distanceFun = function(coord, clusters, Dim = c(1,2)){
  ...
  ratio = mean(unlist(sapply(unique(clusters), function(x){withinDist[[x]]/overalDist[clusters==x]})))
...
contrTaxa = function(rowMatPsi, colMat, groupFactor, idTax, sigFrac = 0.5, Dim = 1:3, upDown = FALSE, centerFun = median){
  ...
  allContr = if(upDown) {sapply(seq_along(levels(groupFactor)), function(i){
    ...
    sapply(seq_along(levels(groupFactor)), function(i){
    centerFun( apply(contrib[!colnames(colMat) %in% idTax[,i],],2,sort, decreasing = TRUE)[seq_len(round(sigFrac*(ncol(colMat)-nrow(idTax)))), ])
  })
...
mclapply(mc.cores = cores, RCMlist, function(x){
  ...
  list(
    RCM = x, CA = caSVD(tmp), DCA = ordinate(otuTab, method = "DCA"), methCoDa = CoDa(tmp),
    ...
    BCrelNMDS = ordinate(otuTabRel, method = "NMDS", distance = "bray"),
...
boxplot(Silhouette~Method, main = "Mean silhouettes per Monte Carlo simulation by method", data = meanSil1df, col = groupsMeth, ...)
meansSil = tapply(meanSil1df$Silhouette, meanSil1df$Method, mean)
points(meansSil, col="red", pch=18, cex = 1.6)
addLegend()
}
#---------------------------------#
distRatioPlot = function(resListRows, groupFactor, method = NULL, ...) {
...
contrPlot = function(contr, method = NULL, ...) {
...
orderDF = function(df){
  ...
  df$groupMethod = groupsMeth
  df
}
#-------------------------------#
#A function to add the legend
addLegend = function(yloc = 0.2){
legend(legend = levels(groupsMeth), col = factor(levels(groupsMeth), levels = levels(groupsMeth), ordered=TRUE),pch = 1, x = length(groupsMeth)+1, y = yloc, xpd = TRUE)
}
```
