...
NB_params[,,KK] = estNBparams(design = design, thetas = thetasMat[1,], muMarg = muMarg, psi = psi, X = X, nleqslv.control = nleqslv.control, ncols =ncols, initParam = NB_params[,,KK], v = v)

NB_params[,,KK] = NB_params[,,KK]/sqrt(rowSums(NB_params[,,KK]^2)) #Normalize the parameters

NB_params_tmp = nleqslv(x = c(NB_params[,,KK], lambdaResp), fn = respFunScoreMat, jac = respFunJacMat, aX = X, reg = design, thetaMat = thetasMat, muMarg = muMarg, v=v, p = ncols, psi = psi)$x
NB_params[,,KK] = matrix(NB_params[,,KK], nrow = v, ncol = ncols)
lambdaResp = NB_params_tmp[p*v + seq_len(v)]

NB_params_noLab[, KK] = nleqslv(x = NB_params_noLab[, KK] , reg = design,  fn = dNBllcol_constr_noLab, thetas = thetas, muMarg = muMarg, psi = psi, X = X, control = nleqslv.control, jac = JacCol_constr_noLab, n=n, v=v)$x

if (verbose) cat("\n Estimating environmental gradient \n")  
 if(analytic){
   AlphaTmp = nleqslv(x = c(alpha[,KK],lambdasAlpha), fn = dLR_nb, jac = LR_nb_Jac, X = X, CC = covariates, responseFun = responseFun, cMat = cMat, psi = psi, NB_params = NB_params[,,KK], NB_params_noLab = NB_params_noLab[, KK], alphaK = alpha[, seq_len(KK-1)], k = KK, d = d, centMat = centMat, nLambda = lambdasAlpha, nLambda1s = nLambda1s, thetaMat = thetas, muMarg = muMarg, control = nleqslv.control, n=n, v=v, ncols = ncols)$x
  alpha[,KK] = AlphaTmp[seq_len(d)]
  lambdasAlpha = AlphaTmp[d+seq_len(lambdasAlpha)]
} else {
   AlphaTmp = constrOptim.nl(par = alpha[,KK], fn = LR_nb, gr = NULL, heq = heq_nb, heq.jac = heq_nb_jac, alphaK = alpha[, seq_len(KK-1)], X=X, CC=covariates, responseFun = responseFun, muMarg = muMarg, psi = psi, d = d, ncols=ncols, control.outer = control.outer, control.optim = control.optim, nleqslv.control = nleqslv.control, k =KK, centMat = centMat, NB_params = NB_params[,,KK], NB_params_noLab = NB_params_noLab[, KK], v = v, thetaMat = thetas, n=n) #
  ...
  AlphaTmp = constrOptim.nl(par = alpha[,KK], fn = LR_nb, gr = NULL, heq = heq_nb, heq.jac = heq_nb_jac, alphaK = alpha[, seq_len(KK-1)], X=X, CC=covariates, responseFun = responseFun, muMarg = muMarg, d = d, ncols=ncols, control.outer = control.outer, control.optim = control.optim, nleqslv.control = nleqslv.control, k =KK, centMat = centMat, n=n, nonParamRespFun = nonParamRespFun, psi = psi, thetaMat = thetas)
  alpha[,KK] = AlphaTmp$par
  lambdasAlpha = AlphaTmp$lambda
     }
...
NB_params_tmp = nleqslv(x = c(NB_params, lambda2), fn = respFunScoreMat, jac = respFunJacMat, aX = X, reg = design, thetaMat = thetas, muMarg = muMarg, v=v, p = ncols, psi = psi)$x
NB_params = matrix(NB_params, nrow = v, ncol = ncols)
lambda2 = NB_params_tmp[p*v + seq_len(v)]

NB_params_noLab = nleqslv(x = NB_params_noLab , reg = design,  fn = dNBllcol_constr_noLab, thetas = thetas, muMarg = muMarg, psi = psi, X = X, control = nleqslv.control, jac = JacCol_constr_noLab, n=n, v=v)$x
