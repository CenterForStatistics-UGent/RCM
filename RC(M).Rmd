...
Ter Braak (1987) proposes a method of direct gradient analysis based on CA: canonical correspondence analysis (CCA). It basically assumes that species have a Gaussian response curve in function of an environmental score which is a _linear_ conbination of environmental variables. The linear projection vector $\boldsymbol{\alpha}$ is called the __environmental gradient__. The row scores of the ordination are restricted to be linear combinations of the environmental variables. Prior to the analysis the environmental variables are centered and scaled to avoid arbitrariness in the choice of the units and render the elements of the envrionmental gradient comparable. The rows of the chi-square transformed matrix $\mathbf{R^{-1/2}(X-E)C^{-1/2}}$ are subjected to weighted least squares regression on the environmental variables, with the sample totals $\mathbf{R}$ as weights. The matrix of environmental variables undergoes weighted centering prior to model fitting, with the library sizes as weights. The row-weight-centered matrix $C'$ is thus calculated as $C' = C(I - \frac{1}{c_{..}d}R\mathdd{1}$. Then singular value decomposition is applied to the matrix of fitted values $F = Y (C^T R C)^{-1} C' R R^{-1/2}(X-E)C^{-1/2}$ of this regression, such that $F = U'\Sigma'V'$. Using the fitted values assures that only the systematic part explained by the covariates plays a role. The scores of the environmental variables are then  CCA is related to weighted Redundancy analysis (RDA) of the Chi-squared distances, just like regular CA is a PCoA of Chi-squared distances.

An interesting reference on the topic is _"The history of canonical correspondence analysis"_ by ter Braak, 2014, where he also references Goodman's work.
...
A linear response function may be most appropriate for problems with __short gradients__ i.e. whereby the difference in observed environmental variables is too short to devine more than an increase or decrease in departure from independence. Also in this case it is easy to interpret the effect of each of the environmental variables on the departure from independence. As so often in statistics, the linearity assumption may not be realistic, but renders models that are easy to interpret ("All models are wrong, but some models are useful"). For problems with __long gradients__, i.e. whereby species' departures of independence rise and drop (or drop and rise) within the score of the observed environmental scores, quadratic response functions may be more appropriate. This corresponds e.g. with the scenario whereby a species' abundance does bot depart a lot from independence for extreme values of the environmental score, and but does depart the independence model for an intermediate value of the environmental score, e.g. it thrives in this environment and its abundance is higher than under independence. In essence this is the same as the approach of Zhu and Hastie, only now the baseline is the independence model rather than 0. Every taxon thereby has its own baseline, the response function models departures from this baseline. If the user is unsure and has enough data, he may use non-parametric response funcion. This may improve the sample and covariate ordination, although the result for the species becomes harder to interpret.
...
# @return: the singular value decomposition of the matrix of pearson residuals
  ...
  Ycent = t(t(Y) - rowMeans(t(Y)*R/sum(R) ))#Weighted centering  
  Goal = Ycent %*% solve(a =  crossprod(Ycent,  diag(R)) %*% Ycent,
        b = crossprod(Ycent, diag(R) %*% diag(1/sqrt(R))) %*% (X-E) %*% diag(1/sqrt(C))) 
  ...
  dimnames(Goal) = dimnames(X)
  Svd = svd(Goal)
  envScores = 1/sqrt(colSums(Ycent^2))*crossprod(Ycent, Svd$u)
  list(Svd = Svd, envScores = envScores)
}
#-------------------------------------------------#
cca2 = function (X, Y, Z, ...) 
{
  ...
  if (!missing(Y) && !is.null(Y)) {
    ...
    if (rank) {
      ...
      CCA$biplot <- (1/sqrt(colSums(xx^2))) * crossprod(xx, 
        sol$u[, seq_len(rank), drop = FALSE]) #Standardize variables
      CCA$rank <- rank
      CCA$qrank <- qrank
...
#Our solution equals the plot.cca(display = "lc" option)

anacor2 = function (tab, ndim = 2, row.covariates, col.covariates, scaling = c("Benzecri", 
    "Benzecri"), ellipse = FALSE, eps = 1e-06) 
{
    tab <- as.matrix(tab)
    if (length(scaling) == 1) 
        scaling <- rep(scaling, 2)
    scaling[1] <- match.arg(scaling[1], c("standard", "Benzecri", 
        "Goodman"))
    scaling[2] <- match.arg(scaling[2], c("standard", "Benzecri", 
        "Goodman"))
    if (missing(row.covariates)) {
        ...
        if (nrow(row.covariates) != nrow(tab)) 
            stop("Matrix with row covariates does not match table dimensions!")
    }
    if (missing(col.covariates)) {
        ...
        if (nrow(col.covariates) != ncol(tab)) 
            stop("Matrix with column covariates does not match table dimensions!")
    }
    ...
    if (COL) {
        ...
        z <- z/outer(rep(1, dim(z)[1]), sqrt(c))
    }
    sv <- svd(z, nu = qdim, nv = qdim)
    sval <- N * ((sv$d[-1])^2)
    sigmavec <- (sv$d)[2:qdim]
    dimlab <- paste("D", 1:ndim, sep = "")
    Tr <- Xstar <- isetCorRow <- NULL
    Tc <- Ystar <- isetCorCol <- NULL
    if (ROW) {
        x <- (xr %*% or %*% (sv$u))[, -1]
        U <- or %*% (sv$u)[, -1]
        Tr <- U %*% (diag(sigmavec))
        dimnames(Tr) <- list(paste("beta", 1:(dim(Tr)[1]), sep = ""), 
            dimlab)
        if (COL) 
            y <- (yr %*% oc %*% (sv$v))[, -1]
        else y <- ((sv$v)/sqrt(c))[, -1]
        Xstar <- (diag(1/r) %*% tab %*% y)
        dimnames(Xstar) <- list(rownames(tab), dimlab)
        isetCorRow <- cor(Xstar, row.covariates)
    }
    else {
        x <- ((sv$u)/sqrt(r))[, -1]
    }
    if (COL) {
        ...
        V <- oc %*% (sv$v)[, -1]
        Tc <- V %*% (diag(sigmavec))
        dimnames(Tc) <- list(paste("beta", 1:(dim(Tc)[1]), sep = ""), 
            dimlab)
        Ystar <- (diag(1/c) %*% t(tab) %*% x)
        dimnames(Ystar) <- list(colnames(tab), dimlab)
        isetCorCol <- cor(Ystar, col.covariates)
    }
    else {
        y <- ((sv$v)/sqrt(c))[, -1]
    }
    ...
    if (scaling[2] == "Goodman") 
        y <- y * outer(rep(1, m), sqrt(sigmavec))
    if (scaling[1] == "Goodman") 
        x <- x * outer(rep(1, n), sqrt(sigmavec))
    if (scaling[2] == "Benzecri") 
        y <- y * outer(rep(1, m), sigmavec)
    if (scaling[1] == "Benzecri") 
        x <- x * outer(rep(1, n), sigmavec)
    benzres <- benzdist(scaling, x, y, z, tab, n, m, r, c, row.covariates, 
        col.covariates)
    prob <- sval/chisq
    pcum <- cumsum(prob)
    ...
    eigenall <- sval/N
    if (ellipse) {
        res.deriv <- gsvdDer(tab, ndim)
        res.deriv.scaling <- gsvdScal(res.deriv, scaling)
        res.acov <- acovuv(res.deriv.scaling, prop, n, m, N, 
            ndim)
        se.sigma <- sqrt(diag(res.acov$acovd))
    }
    else {
        res.acov <- NULL
        se.sigma <- NULL
    ...
    if (!is.null(se.sigma)) {
        ...
        colnames(stestmat) <- c("Singular Value", "Standard Error", 
            "p-value")
    ...
    result <- list(datname = name, tab = tab, ndim = ndim, row.covariates = row.covariates, 
        ...
        chisq.decomp = cs.mat, chisq = chisq, singular.values = sigmavec, 
        se.singular.values = se.sigma, stestmat = stestmat, left.singvec = sv$u[, 
            -1], right.singvec = sv$v[, -1], eigen.values = sigmavec^2, 
        eigenall = eigenall, scaling = scaling, bdmat = benzres[1:4], 
        rmse = benzres[5:6], row.acov = res.acov$acovu, col.acov = res.acov$acovv, 
        cancoef = list(rows = Tr, columns = Tc), sitescores = list(rows = Xstar, 
            columns = Ystar), isetcor = list(rows = isetCorRow, 
            columns = isetCorCol), call = match.call())
    class(result) <- "anacor"
    result
}
```
...
# A ggplot2-version of the biplot function
plot.RCM = function(RCMfit, Dim = c(1,2), 
                    samColour = NULL, colLegend = samColour, samShape = NULL, shapeLegend = samShape, samSize = 3,
                    taxFrac = 0.05, scalingFactor = NULL, plotTaxa = TRUE, quadDrop = 0.995, nPoints = 1e3, plotEllipse = TRUE, taxaScale = 0.5,
                    Palette = NULL, arrowCol = "blue", nudge_y = -0.08,...) {
  # @param RCMfit: an RCM object
  # @param Dim: two dimensions to plot, defaults to the first two
  ...
  # @param taxFrac: a scalar between 0 and 1, the fraction of taxa to be plotted
...
Palette=rainbow(length(unique(dataSam$colourPlot)))
    ...
    dataTax = within(dataTax, { #Scale the arrows
          ...
          dataEllipseTmp = vapply(seq_along(taxa_names(RCMfit$physeq)), FUN.VALUE = matrix(0,nPoints,3),function(tax){
              ...
              cbind(ellipseCoord(a = x[3,] * RCMfit$psis, b = x[2,] * RCMfit$psis, c = x[3,] * RCMfit$psis, quadDrop = quadDrop, nPoints = nPoints), taxon = tax)
              })
  
          peakHeights = apply(RCMfit$NB_params[,,Dim],2, function(x){
            A = x[3,]; B = x[2,]; C = x[1,];
            sapply(exp(B^2 * RCMfit$psis-4*A*C)/(4*A), function(y){max(y,1/y)}) #select largest relative departure
          })
          ...
          dataEllipse = data.frame(
            apply(dataEllipseTmp[,,id], 2, c), colour = dataTax$colour
          )
    ...
    if(!constrained || RCMfit$responseFun == "linear"){ #Scaling needed
    ...
    } else if(RCMfit$responseFun=="quadratic"){ #quadratic responsef unctions
    plot <- plot + 
      ...
      if(plotEllipse) {geom_path(inherit.aes = FALSE, data = dataEllipse, mapping = aes_string(x = "x", y = "y", group = "taxon", colour = "colour"))}
    } else {
...
#--------------------------------#
# A function to enforce squareness
...
plot.RCM(zellerSphyRCMconstrLin, colour = "Diagnosis")
plot.RCM(zellerSphyRCMconstrLin, colour = "Gender")
plot.RCM(zellerSphyRCMconstrLin, colour = "Country")
plot.RCM(zellerSphyRCMconstrLin, colour = "Age")
...
plot.RCM(zellerSphyRCMconstrLin, colour = "Diagnosis", plotTaxa = TRUE)
```
...
plot.RCM(zellerSphyRCMconstrLin, colour = "Age", plotTaxa = TRUE)
rm(zellerSphyRCMconstrLin)
```
...
plot.RCM(HumMiceRCM, colour = "Diet") #Diet is a main factor
plot.RCM(HumMiceRCM, colour = "Individual") #Individuals play less of a role
plot.RCM(HumMiceRCM, colour = "Generation_p") #Microbiome changes quickly with feed pattern
plot.RCM(HumMiceRCM, colour = "Time") #Later samples also form a cluster
```

```{r humMicePlotExec, purl = FALSE}
plot.RCM(HumMiceRCM, colour = "Diet", main =  "Humanized mice") #Diet is a main factor
plot.RCM(HumMiceRCM, colour = "Time", main =  "Humanized mice") #Later samples also form a cluster
rm(HumMiceRCM)
```

```{r humMicePlot13D, purl = FALSE, eval = FALSE}
plot.RCM(HumMiceRCM, colour = "Diet", Dim = c(1,3)) #Diet is a main factor
plot.RCM(HumMiceRCM, colour = "Individual", Dim = c(1,3)) #Individuals play less of a role
plot.RCM(HumMiceRCM, colour = "Generation_p", Dim = c(1,3)) #Microbiome changes quickly with feed pattern
plot.RCM(HumMiceRCM, colour = "Time", Dim = c(1,3)) #Later samples also form a cluster
plot.RCM(HumMiceRCM, colour = "DTG", Dim = c(1,3))
plot.RCM(HumMiceRCM, colour = "DGS", Dim = c(1,3))
```
...
We could perhaps look for the distance based method that minimizes $\frac{d_{m, within}}{d_{m, overall}}$? This is related to Ward's minimum variance method, but with scaling to the overall distance. This takes differences in the scales of solutions of different methods into account.
...
distanceFun = function(coord, clusters, Dim = c(1,2)){
  ...
  ratio = mean(sapply(unique(clusters), function(x){withinDist[[x]]/overalDist[clusters==x]}))
...
```{r contrTaxa, purl = FALSE}
contrTaxa = function(rowMatPsi, colMat, groupFactor, idTax, sigFrac = 0.5, Dim = 1:3, singleID = FALSE){
  rowMatPsi = rowMatPsi[,Dim]
  colMat = t(colMat)[Dim,]
  if(singleID){ #Replicate columns of idTax if needed 
    idTax = matrix(idTax, nrow = length(idTax), ncol = length(levels(groupFactor)))
  }
  ...
  }) # The mean contribution of the taxa with signal
  allContr = sapply(seq_along(levels(groupFactor)), function(i){
    mean(
      apply(contrib[!colnames(colMat) %in% idTax[,i],],2,sort, decreasing = TRUE)[seq_len(round(sigFrac*(ncol(colMat)-nrow(idTax)))), ])
  }) # The mean contribution of the 50% taxa with highest contribution among non-significant taxa
...
for (i in Dims){
data = cor0df[[i]]
data$Method = gsub(paste0(".Dim",i), "", data$Method) #remove annoying suffices
...
contrTaxaWrap = function(resListRows, resListCols, IDlist, groupFactor, singleID = FALSE){
dat = mapply(resListRows, resListCols, IDlist, SIMPLIFY = TRUE, FUN = function(row, col, id){
  mapply(row, col, SIMPLIFY = TRUE, MoreArgs = list(idTax = id, groupFactor = groupFactor, singleID = singleID), FUN = contrTaxa)
})
as.data.frame(t(dat))
}
#-----------------------------#
# A function to plot the result of this as boxplots
contrPlot = function(contr) {
moltenContr = melt(contr, variable.name = "Method", value.name = "contrRatio", id.vars = NULL)
boxplot(contrRatio~Method, main = "Mean ratios of contribution of DA taxa \n relative to 50% most contributing non-DA taxa by method", data = moltenContr, ylab = "Mean contribution ratio")
meansDist = tapply(moltenContr$contrRatio, moltenContr$Method, mean)
points(meansDist, col="red", pch=18, cex = 1.6)
abline(h=1, lty = "dashed")
}
#-----------------------------#
...
startFun = function(list){
  ...
  cMat = list$RCM$cMat
  rMat = list$RCM$rMat
  list(corRows1 = getCor(svd1$u, rMat), corRows2 = getCor(svd2$u, rMat), corRows3 = getCor(svd3$u, rMat), corRows4 = getCor(svd4$u, rMat), corRows5 = getCor(svd5$u, rMat) ,corCols1 = getCor(svd1$v, t(cMat)), corCols2 = getCor(svd2$v, t(cMat)), corCols3 = getCor(svd3$v, t(cMat)), corCols4 = getCor(svd4$v, t(cMat)), corCols5 = getCor(svd5$v, t(cMat)))
} 
getCor = function(a,b){
...
save(resList3, groupFactor3, IDlist3, file = "RCMsim/resList3.RData")
...
# ID = sample(size=1, seq_along(resList3))
# obj = resList3Rows[[ID]]
# lapply(names(obj),function(x){plot(obj[[x]][,c(1,3)], col = groupFactor3, main=x, xlab = "Dim 1",ylab = "Dim 3", pty = "s")})
# rm(ID, obj)
# par(mfrow = c(1,1))
```
...
resListPhy = otherAnalyses(RCMresListPhy, unifrac = TRUE, treeList = )
...
save(RCMresListSimSeqHum, RCMresListSimSeqAG, RCMresListSimSeqZeller, file = "RCMsim/resListSimSeqRCM.RData")
  ...
  IDlistHum = mapply(HumSimSeqList,resListSimSeqHum, FUN =  function(y,x){y$DEtaxa[y$DEtaxa %in% colnames(x$RCM$X)]}, SIMPLIFY = FALSE)
  groupFactorHum = HumSimSeqList[[1]]$treatment
save(resListSimSeqHum, IDlistHum, groupFactorHum, file = "RCMsim/resListSimSeq.RData")
...
plotCor(resListSimSeqHumCols, lapply(resListSimSeqHum, function(x){x$RCM$X}), scoreDim = "columns", dataMat = TRUE)
```
...
contrHum = contrTaxaWrap(resListSimSeqHumRows, resListSimSeqHumCols, IDlistHum, groupFactorHum, singleID = TRUE)
contrPlot(contrHum)
```
