...
$$\frac{\partial^2 Lag_{LR}(\boldsymbol{\alpha}_k)}{\partial \boldsymbol{\alpha}_k^2} = - {\psi_k}^2 \Big( \sum_{i=1}^n  \sum_{j=1}^p \Big( (s_{jk} \mathbf{C}_i (\beta_{1j}+ 2\beta_{2j}\boldsymbol{\alpha}_k^t\mathbf{C}_i))^2 \frac{\mu_{ij}(1+ \frac{x_{ij}}{\theta_j})}{(1 + \frac{\mu_{ij}}{\theta_j})^2} + 2\beta_{2j}s_{jk} \mathbf{C}_i^2 \frac{x_{ij} - \mu_{ij}}{1 + \frac{\mu_{ij}}{\theta_j}}\Big) \\- \sum_{i=1}^n  \sum_{j=1}^p \Big( (s_{jk} \mathbf{C}_i (\beta_{1}+ 2\beta_{2}\boldsymbol{\alpha}_k^t\mathbf{C}_i))^2 \frac{\mu_{ij0}(1+ \frac{x_{ij0}}{\theta_j})}{(1 + \frac{\mu_{ij0}}{\theta_j})^2} + 2\beta_{2}s_{jk} \mathbf{C}_i^2 \frac{x_{ij} - \mu_{ij0}}{1 + \frac{\mu_{ij0}}{\theta_j}}\Big) + 2\lambda_{2k} $$
...
LR_nb <- function(Alpha, X, CC, responseFun = c("linear","quadratic","nonparametric"), muMarg, psi, nleqslv.control = list(trace=FALSE), NB_params, NB_params_noLab, thetaMat, n, ncols, ...){#
  ...
  # @return: a scalar, the evaluation of the log-likelihood ratio at the given alpha
   ...
   # NB_params_mat = matrix(nleqslv(matrix(1,v,p) , reg = design,  fn = dNBllcol_constr_mat, theta = thetas[1:p], muMarg = muMarg, cReg = cReg, aX = X, control = nleqslv.control, jac = JacCol_constr_mat)$x,v,p)
    ...
    logDensj = dnbinom(X,mu = muMarg * c(exp(design %*% NB_params *psi)), size = thetaMat, log = TRUE) #Likelihoods under species specific model
    #Immediately return log likelihoods

    logDens0 = dnbinom(X,mu =muMarg * c(exp(design %*% NB_params_noLab *psi)), size = thetaMat, log = TRUE) #Likelihoods of null model
...
LR_nb_Jac <- function(Alpha, X, CC, responseFun = c("linear", "quadratic", "nonparametric"), cMat, psi, NB_params, NB_params_noLab, d, alphaK, k, nLambda, nLambda1s, centMat, thetaMat, muMarg, n, v, ncols, ...){
  ...
  Jac[did,did] = switch(responseFun,
                        ...
                        colSums(tensor(vapply(did, FUN.VALUE = tmp, function(x){CC[,x]*tmp}),CC,1,1)*NB_params_noLab[2]^2) + colSums(tensor(vapply(did, FUN.VALUE = tmp, function(x){CC[,x]*tmp*c(sampleScore)}),CC,1,1)*NB_params_noLab[2]*NB_params_noLab[3]*2) + 
                        colSums(tensor(vapply(did, FUN.VALUE = tmp, function(x){CC[,x]*tmp*c(sampleScore)}),CC*c(sampleScore),1,1)*NB_params_noLab[3]^2*4)
) + psi*(colSums(tensor(vapply(did, FUN.VALUE = tmp, function(x){CC[,x]*(X-mu)/(1+mu/thetaMat)}),CC,1,1)*2*NB_params[3,])-
...
vapply(seq_len(ncols), FUN.VALUE = vector("numeric",v), function(i){
     nleq = nleqslv(initParam[,i] , reg = design,  fn = dNBllcol_constr, theta = thetas[i], muMarg = muMarg[,i], psi = psi, X = X[,i], control = nleqslv.control, jac = JacCol_constr)$x#try(, silent=TRUE)  # , ... Fit the taxon-by taxon NB with given overdispersion parameters and return predictions
     # if(class(nleq)=="try-error" |anyNA(nleq) | any(is.infinite(nleq))){
...
#--------------------------------------#
# A function to estimate the taxon-wise response functions non-parametrically
estNPresp = function(sampleScore, muMarg, X, ncols,...){
  vapply(seq_len(ncols), FUN.VALUE = vector("numeric",v), function(i){
  locfit.raw(x = sampleScore, y = X[,i], base = muMarg[,i],...)
})
  }
