...
rMat = apply(rMat, 2, function(rowS){
 ...
 if(!is.null(NBRCM)){ #If fit provided, replace lower dimension starting values
  ...
  if(KK>1){muMarg = muMarg * exp(rMat[,1:(KK-1), drop=FALSE] %*% (cMat[1:(KK-1),, drop=FALSE]*psis[1:(KK-1)]))}
...
EstepZIP  = function(X, rMat, cMat, tMat, vMat, muMarg,  Zcol, psis, chis){
...
RCM = function(dat, k,round=FALSE, method= c("NB","NB_1B1", "ZIP", "ZINB"),prevCutOff=0.01,rowWeights = "uniform", colWeights = "marginal",...){
  if (class(dat)=="matrix"){
...
tic = proc.time() #Time the calculation
  tmp = switch(method, NB=RCM_NB(dat, rowWeights=rowWeightsNum, colWeights=colWeightsNum, weightsChar = weightsChar, k=k,...),NB_1B1=RCM_NB_1B1(dat, rowWeights=rowWeightsNum, colWeights=colWeightsNum, weightsChar = weightsChar, k=k,...), ZIP=RCM_ZIP(dat, rowWeights=rowWeightsNum, colWeights=colWeightsNum,weightsChar = weightsChar, k=k,...), ZINB=RCM_ZINB(dat,  rowWeights=rowWeightsNum, colWeights=colWeightsNum,weightsChar = weightsChar, k=k,...))
  within(tmp, {
    runtime = (proc.time()-tic)[3]
...
#   #Refitting does not help, new taxa become outlying
 ...
 #With marginal weights
  #2D
    syntNBNSmargJob = mcparallel(RCM(dataMatNBNS, method="NB", k=2, nleqslv.control= nleqslv.control, maxItOut=1e3, colWeights = "marginal", prevCutOff=0.01))
  syntNBNSmarg=mccollect(syntNBNSmargJob, FALSE)[[1]]
  
      syntNBNSmarg_1B1Job = mcparallel(RCM(dataMatNBNS, method="NB_1B1", k=2, nleqslv.control= nleqslv.control, maxItOut=1e3, colWeights = "marginal", prevCutOff=0.01))
  syntNBNSmarg_1B1=mccollect(syntNBNSmarg_1B1Job, FALSE)[[1]]
  
  #3D
      syntNBNSmargJob_3 = mcparallel(RCM(dataMatNBNS, method="NB", k=3, nleqslv.control= nleqslv.control, maxItOut=1e3, colWeights = "marginal", prevCutOff=0.01))
  syntNBNSmarg_3=mccollect(syntNBNSmargJob_3, FALSE)[[1]]
  
      syntNBNSmarg_1B1_3Job = mcparallel(RCM(dataMatNBNS, method="NB_1B1", k=3, nleqslv.control= nleqslv.control, maxItOut=1e3, colWeights = "marginal", prevCutOff=0.01))
...
```{r Plot results, purl=FALSE}
with(syntNBNSmarg_1B1_3, {plot(rMat %*% diag(psis))})
with(syntNBNSmarg_1B1_3, {plot(t(cMat))}) # No signal in either plot
#Look at the loadings in function of the library sizes and abunds
par(pty = "m")
with(syntNBNSmarg_1B1_3, {plot(cMat[1,], log(colSums(X)), log="")})
with(syntNBNSmarg_1B1_3, {lines(lowess(cMat[1,], log(colSums(X))))})
with(syntNBNSmarg_1B1_3, {plot(cMat[2,], log(colSums(X)), log="")})
with(syntNBNSmarg_1B1_3, {lines(lowess(cMat[2,], log(colSums(X))))})
with(syntNBNSmarg_1B1_3, {plot(abs(cMat[2,]), colSums(X), log="y")})
#Maybe a slight abundance/ score relationship? Not very significant anyway
...
#   syntNBSigmarg_1B1=mccollect(syntNBSigmarg_1B1Job, FALSE)[[1]] 
  ...
  save(dataMatNBSig,  syntNBSigmarg, syntNBSigmarg_3, syntNBSigmarg_1B1_3, taxaSigNB, sampleSigNB, file="toyDataSig.RData")#syntNBSigunif
...
NBmargSigPlotDF = RCMres2DF(syntNBSigmarg_3,  sampleSigNB, taxaSigNB[colMeans(dataMatNBSig==0)<0.99])
NBmargSigPlotDF_1B1 = RCMres2DF(syntNBSigmarg_1B1_3,  sampleSigNB, taxaSigNB[colMeans(dataMatNBSig==0)<0.99])
...
```{r Nb sig 2, eval=FALSE, purl=FALSE}
...
```{r RC(M) Nb with signal results, purl=FALSE}
