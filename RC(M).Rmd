...
- Normalization restricts the size, only $\psi_k$ can grow in magnitude, and dimensions are orthogonal (if $k \geq 3$)
...
Note that the concept of a "row score" is lost here: since every taxon reacts differently to changes in the environment $C_{i.}$. Still this will not yield a perfect fit since $\boldsymbol{\alpha}_k^t$ is only unique to the dimension and the number of parameters $q$ estimated per response function $f_j$ is much smaller than the number of samples $n$. In practice in R we will work with a row score matrix.
...
To render the values of the continuous variables in the environmental gradient comparable, it is clear that they need to be
centered and scaled prior to model fitting, as in PCA. We might have to do the same for the categorical variables, for complete comparability? There are a lot of possible solutions (see e.g. SOCIOECONOMIC STATUS MEASUREMENT WITH DISCRETE PROXY VARIABLES: IS PRINCIPAL COMPONENT ANALYSIS A RELIABLE ANSWER? by Kolenikov and Angeles, 2009) for the PCA case. For now we do not normalize the categorical variables, but this means we may not be able to compare components of $\boldsymbol{\alpha}$ from continuous and categorical variables.
...
The first order derivatives becomes (in case of a linear response function)
...
The whole estimation of the constrained RC(M)-model would then proceed as follows:
 ...
 3. Find starting values for $\alpha$, $\psi_k$ and $s_{jk}$  based on CCA
 4. Iterate between
  - Estimate $f_j$'s and $f_0$ given $\boldsymbol{\alpha}$
  - Find $\boldsymbol{\alpha}$ by optimizing $LR(\boldsymbol{\alpha})$
 5. Estimate $\psi_k$ and $s_{jk}$ using ML and overdispersions using empirical Bayes
 6. Repeat steps for every dimension $k$ 4-5 until convergence

Filtering of rare taxa is not needed in this case, as all covariates are summarized in one "continuous" row score. to check for convergencen, for $\psi_k$  we apply an infinity norm, for $\mathbf{s}_k$ and $\boldsymbol{\alpha}_k$ the default is an L2-norm.
