...
with $LL_k$ the log-likelihoods of RC(k) model, $LL_0$ being the independence model. Scaling by $(LL_{sat} - LL_0)$ with $LL-sat$ the log-likelihood of the saturated model, will provide interpretable fractions of the log-likelihood explained(for fitting the saturated model, we need to set the means of the observed zeroes to a very small number to avoid overflow). This approach leads to weird results for the constrained case as well, because the environmental gradients are not estimated to maximize the likelihood. For the the order of the dimensions is dictated by optimal separation of the species' niches, for good and for ill.
...
Deviance (in relation to the saturated model) would be one option. The taxon- and samplewise deviances could be calculated and plotted using a colour code. In the end all samples and taxa will be "poorly" fitted, we just need a relative measure of how which ones are worst.

Since we are not in a full likelihood framework, we cannot use formal tests. Therefore I propose to use the number and extent of outlying values as a measure of lack of fit. In our setting outliers will always be values larger than expected, a zero is practically never outlying.
...
1) In a first approach we thought of was to normalize the 16S sequencing data to relative abundances through division by the library sizes, and then multiply them by the cell concentrations, and round to the nearest integer to achieve the counts. This is several drawbacks however. Firstly the information on the variance of the relative abundances is lost. Secondly the unit in which the concentration is expressed is arbitrary but will affect the analysis because of the assumed mean-variance analysis of count data.
