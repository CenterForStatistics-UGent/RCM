...
```{r ArmpitPlot1, purl = FALSE}
plot(armpitRCMconstrLin)
plot(armpitRCMconstrLin, samColour = "shaving")
plot(armpitRCMconstrLin, samColour = "odor_panel")
plot(armpitRCMconstrLin, samColour = "gender")
plot(armpitRCMconstrLin, samColour = "age")
plot(armpitRCMconstrLin, samColour = "antimicrobial")
rm(armpitRCMconstrLin)
```

OTU855 staphylococcus represents a clear outlier, in the other direction the Corynebacteria react most strongly to the environmental gradient. Fish and bread consumption appear to be drivers of the environmental gradient. Note how different the results of the constrained and unconstrained analyses are.

##### CMET data

This is a longitudinal dataset on growth in a water cooling system. It has the unique property of providing relative counts through 16S sequencing that can be multiplied by absolute counts from flow cytometry to get truly absolute abundances that reflect the number of cells. This type of data does not qualify for the term _compositional data_ anymore. The advent of these kind of approaches support the use of count models rather than compositional data approaches.
...
if(!file.exists(file = "CMETRCM.RData")){
  ...
  CMETRCMnonParam= RCM(CMETwater, k = 3, responseFun = "nonparametric", covariates = envVarsCMET)
...
with $W^TW = I$. This dimensionality reduction optimally represents Euclidean distances between samples in lower dimensions. Its use with dummies of categorical variables is problematic, since they are inherently correlated and affect the ordination of the PCA.

#### PCA on ranks

In the phyloseq manual it is proposed to perform PCA on ranked abundance data
...
extractCols = function(x, row){
  tmpList = with(x, list(
    ...
    Hellinger = Hellinger$v %*% 1/diag((crossprod(sqrt(RCM$X/rowSums(RCM$X))-sqrt(colSums(RCM$X)/sum(RCM$X)), diag(rowSums(RCM$X))) %*% (sqrt(RCM$X/rowSums(RCM$X))-sqrt(colSums(RCM$X)/sum(RCM$X)))))
    ))
  tmpList = tmpList[!sapply(tmpList, is.null)]
