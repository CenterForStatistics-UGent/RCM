...
RCM_NB = function(X, k, rowWeights = "uniform", colWeights = "marginal", tol = 1e-3, maxItOut = 500, Psitol = 1e-3, verbose = TRUE, NBRCM = NULL, global ="dbldog", nleqslv.control=list(), method=c("Broyden"), dispFrec=10, convNorm = 2, prior.df=10, marginEst = "MLE", estPsis = TRUE, confoundersFilt = NULL, confounders = NULL, covariates = NULL, factorInd = NULL, centMat = NULL, responseFun = "linear", prevCutOff = 1e-2, minFraction = 0.1){
...
tmpAlpha = estAlpha(Alpha = alpha[, KK], X = X, CC = covariates, d = d, ncols = p, responseFun = responseFun, muMarg = muMarg, thetas = thetas, cReg = regAlpha, alphaK = alpha[,1:(KK-1),drop=FALSE], k=KK , factorInd = factorInd, centMat = centMat)
...
estAlpha = function(Alpha, X, CC, d, k, ncols, factorInd, centMat, responseFun = c("linear", "quadratic", "nonparametric"), muMarg, thetas, cReg, alphaK, control.outer = list(trace=TRUE), control.optim=list(trace=TRUE), nleqslv.control = list() ){
  ...
  alphaNew = constrOptim.nl(par = Alpha, fn =LR_nb, gr = NULL, heq = heq_nb, heq.jac = heq_nb_jac, alphaK = alphaK, X=X, CC=CC, responseFun = c("linear"), muMarg = muMarg, thetas = thetas, cReg = cReg, d = d, ncols=ncols, control.outer = control.outer, control.optim = control.optim, nleqslv.control = nleqslv.control, k = k, factorInd = factorInd, centMat = centMat) #
}
#--------------------------------------#
# A linear equality constraints function for estAlpha (see ?alabama)
heq_nb = function(Alpha, alphaK, d, k, factorInd, centMat, ...){
  center = sum(Alpha) #Overall centering
  centerFactors = centMat %*% Alpha[factorInd]
  size = sum(Alpha^2)-1
  if(k==1) { return(c(center, centerFactors, size))}
  ortho = alphaK %*% Alpha
  c(center, centerFactors, size, ortho)
}
#--------------------------------------#
# It's jacobian, returns a matrix with d columns and at least 2 rows
heq_nb_jac = function(Alpha, alphaK, d, k, factorInd, centMat, ...){
  center = rep.int(1,d + sum(factorInd)) #Embed the centerfactors here
  size = 2*Alpha
  ...
  if(!is.null(covariates)){
    if(class(covariates) %in% c("vector","matrix")){
      ...
      if(class(covariates[,1]) %in% c("character", "factor")){ #If character or factor, build model matrix
        ...
        covModelMat = model.matrix(object = as.formula(paste("~" , paste(colnames(covariates), collapse="+"),"-1")), contrasts.arg = apply(colnames(covariates),2,contrasts, contrasts=FALSE)) #Without intercept
      } else {
      ...
      covModelMat = model.matrix( object = as.formula(paste("~", paste(names(covariates), collapse="+"),"-1")), contrasts.arg = lapply(names(covariates)[,sapply(covariates, is.factor),drop=FALSE],contrasts, contrasts=FALSE))
      ...
      if(covariates=="all"){covariates = sample_variables(dat)} #Enable the "all" option if phyloseq object is provided
      datFrame = data.frame(sample_data(dat))[,covariates] # The dataframe with the covariates
      nFactorLevels = sapply(datFrame[,sapply(datFrame, is.factor)], nlevels) #Number of levels per factor
      datFrame[,sapply(datFrame, is.factor)][,(nFactorLevels < 2)] = NULL #Drop variables with one level
      if(any(nFactorLevels < 2)){
        warning("The following variables were not included in the analyses because they are factors with only one level: \n", paste(covariates[nFactorLevels < 2], sep = " \n"))
      }
    ...
    centMat  = t(sapply(seq_along(nFactorLevels), function(i){
      c(rep.int(0, sum(nFactorLevels[seq(0,i-1)])), #Zeroes before
        rep.int(1, nFactorLevels[i]), #Ones within the categorical variable
        rep.int(0, sum(nFactorLevels[-seq(1,i)]))) #
    })) #Multiply this by the vector of coefficients of the factors
    # An indicator vector for factors
    factorInd = unlist(sapply(datFrame, function(x){
      if(is.factor(x)){
        rep(TRUE, nlevels(x))
      } else {FALSE}
    }))
    ...
    covModelMat = factorInd = centMat = NULL
  }
...
tic = proc.time() #Time the calculation
  tmp = switch(distribution, 
               NB=RCM_NB(X, rowWeights = rowWeights, colWeights = colWeights, k = k, 
                         ...
                         covariates = covModelMat, prevCutOff = prevCutOff, minFraction = minFraction, factorInd = factorInd, centMat = centMat,...),
