...
plot.RCM = function(RCMfit, Dim = c(1,2), 
                    ...
                    taxFrac = 0.025, scalingFactor = NULL, plotTaxa = TRUE, quadDrop = 0.95, nPoints = 100,
                    Palette = NULL, arrowCol = "blue", ...) {
  ...
  # @param quadDrop: a scalar, the value at which the response function has dropped to this fraction of its maximum on the log scale to be plotted as a halo
  # @param nPoints: an integer, number of points to evaluate for the ellipse
  
  # @param Palette: teh colour palette
  ...
  #Retrieve dots (will be passed on to aes())
    ...
    if(!is.null(samColour) && !is.factor(samColour)){
      ...
      dataSam$colourPlot = samColour} else {dataSam$colourPlot=rep(1, nrow(dataSam))}
...
Palette=rainbow(length(unique(dataSam$colourPlot)))
    ...
    dataTax = within(dataTax, { #Scale the arrows
          ...
          dataEllipseTmp = sapply(seq_along(taxa_names(RCMfit$physeq)), function(tax){
              x = RCMfit$NB_params[,tax,Dim]
              cbind(ellipseCoord(a = x[3,], b = x[2,], c = x[3,], quadDrop = quadDrop, nPoints = nPoints), taxon = tax)
              })
    ...
    plot <- plot + geom_point(data=dataTax, aes_string(x='end1', y='end2', fill="colour", size), pch = 21, show.legend=FALSE, inherit.aes = FALSE) 
    } else {
