...
setwd(WD)
knitr::opts_chunk$set(cache = TRUE, autodep = TRUE,  warning=FALSE, message=FALSE, echo=FALSE, eval=TRUE, tidy = TRUE, fig.width=9, fig.height=6, root.dir=WD, purl=FALSE, fig.show = "hold")
# The required package list:
reqpkg <- c("phyloseq","MASS", "parallel","nleqslv", "edgeR", "VGAM","HMP", "ggplot2", "pscl", "vegan", "alabama", "zCompositions", "DirFactor","reshape2", "tensor")
...
$$\frac{\partial Lag_{LR}(\boldsymbol{\alpha}_k)}{\partial \boldsymbol{\alpha}_k} = \psi_k \Big( \sum_{i=1}^n  \sum_{j=1}^p s_{jk} \mathbf{C}_i (\beta_{1j}+2 \beta_{2j}\boldsymbol{\alpha}_k) \frac{x_{ij} - \mu_{ij}}{1 + \frac{\mu_{ij}}{\theta_j}} - \sum_{i=1}^n  \sum_{j=1}^ps_{jk} \mathbf{C}_i (\beta_{1}+2 \beta_{2}\boldsymbol{\alpha}_k) \frac{x_{ij} - \mu_{ij0}}{1 + \frac{\mu_{ij0}}{\theta_j}} \Big) + \boldsymbol{\alpha}_k^t\mathbf{M}\boldsymbol{\lambda}_{1k} + 2\lambda_{2k} \boldsymbol{\alpha}_k + \boldsymbol{\lambda}_{orth,k}^t \boldsymbol{\alpha}_{-k} = \mathbf{0}$$
...
$$\frac{\partial^2 Lag_{LR}(\boldsymbol{\alpha}_k)}{\partial \boldsymbol{\alpha}_k^2} = - {\psi_k}^2 \Big( \sum_{i=1}^n  \sum_{j=1}^p \Big( (s_{jk} \mathbf{C}_i (\beta_{1j}+ 2\beta_{2j}\boldsymbol{\alpha}_k))^2 \frac{\mu_{ij}(1+ \frac{x_{ij}}{\theta_j})}{(1 + \frac{\mu_{ij}}{\theta_j})^2} + 2\beta_{2j}s_{jk} \mathbf{C}_i \frac{x_{ij} - \mu_{ij}}{1 + \frac{\mu_{ij}}{\theta_j}}\Big) \\- \sum_{i=1}^n  \sum_{j=1}^p \Big( (s_{jk} \mathbf{C}_i (\beta_{1}+ 2\beta_{2}\boldsymbol{\alpha}_k))^2 \frac{\mu_{ij0}(1+ \frac{x_{ij0}}{\theta_j})}{(1 + \frac{\mu_{ij0}}{\theta_j})^2} + 2\beta_{2}s_{jk} \mathbf{C}_i \frac{x_{ij} - \mu_{ij0}}{1 + \frac{\mu_{ij0}}{\theta_j}}\Big) + 2\lambda_{2k} $$
$$\frac{\partial^2 Lag_{LR}(\boldsymbol{\alpha}_k)}{\partial {\alpha}_{yk}{\alpha}_{y'k}} = - {\psi_k}^2 \Big( \sum_{i=1}^n  \sum_{j=1}^p s_{jk}^2 (\beta_{1j} + 2\beta_{2j}\alpha_{yk})(\beta_{1j} + 2\beta_{2j}\alpha_{y'k}) c_{iy} c_{iy'}\frac{\mu_{ij}(1+ \frac{x_{ij}}{\theta_j})}{(1 + \frac{\mu_{ij}}{\theta_j})^2} \\ 
- \sum_{i=1}^n  \sum_{j=1}^p s_{jk}^2 (\beta_{1} + 2\beta_{2}\alpha_{yk})(\beta_{1} + 2\beta_{2}\alpha_{y'k}) c_{iy} c_{iy'}\frac{\mu_{ij0}(1+ \frac{x_{ij}}{\theta_j})}{(1 + \frac{\mu_{ij0}}{\theta_j})^2} \Big) + 2\lambda_{2k} $$
...
estDisp = function (X, cMat, rMat = NULL, muMarg, psis, trended.dispersion, prior.df = 10, dispWeights = NULL, rowMat = NULL)
{
  ...
  logMeansMat = 
    if(!is.null(rMat)){ #Unconstrained
      ...
      t(log(muMarg) + psis* rowMultiply(rowMat,c(cMat)))
    }
...
if(!is.null(NBRCM)){
 newK = (Kprev+1):k
 cMat = rbind(cMat, t(CCA$v[,newK, drop=FALSE]))
 psis = c(psis, CCA$eig[newK])
} else {
  cMat = matrix(1,k,p)#t(CCA$v)[1:k,,drop=FALSE]
  psis = CCA$eig[1:k]
}
...
NB_params = if(responseFun=="linear") {vapply(seq_len(k),FUN.VALUE = matrix(0,v,p), function(x){x = NB_params[,,x, drop=FALSE];x/sqrt(rowSums(x^2))})}
...
nLambda1s = NROW(centMat)
 ...
 lambdasAlpha = rep(0,nLambda1s +KK)
  ...
  #Modify offset if needed
  if(KK>1){modMat = exp(getRowMatConstr(responseFun = responseFun, NB_params = NB_params[,,KK-1], covariates = covariates, alpha = alpha[, KK-1]))
...
regPsis = rowMultiply(rowMat,cMat[KK,])
...
# #This one has some difficulties converging, try some alternatives
# 
# if(tmpCol$termcd!=1){
#   tmpCol = nleqslv(fn = dNBllcol, x = tmpCol$x, thetas =  thetasMat , X = X, reg = regCol, muMarg = muMarg, k = KK,  global = "hook", control = nleqslv.control, n=n, p=p, jac = NBjacobianCol, method = jacMethod, colWeights = colWeights, nLambda = (KK+1), cMatK = cMat[1:(KK-1),,drop=FALSE], constrained = TRUE)
# }
# 
# if(tmpCol$termcd!=1){
#   tmpCol = nleqslv(fn = dNBllcol, x = tmpCol$x, thetas =  thetasMat , X = X, reg = regCol, muMarg = muMarg, k = KK,  global = "gline", control = nleqslv.control, n=n, p=p, jac = NBjacobianCol, method = jacMethod, colWeights = colWeights, nLambda = (KK+1), cMatK = cMat[1:(KK-1),,drop=FALSE], constrained = TRUE)
# }
# 
# if(tmpCol$termcd!=1){
#   tmpCol = nleqslv(fn = dNBllcol, x = tmpCol$x, thetas =  thetasMat , X = X, reg = regCol, muMarg = muMarg, k = KK,  global = "cline", control = nleqslv.control, n=n, p=p, jac = NBjacobianCol, method = jacMethod, colWeights = colWeights, nLambda = (KK+1), cMatK = cMat[1:(KK-1),,drop=FALSE], constrained = TRUE)
# }
# 
# if(tmpCol$termcd!=1){
#   tmpCol = nleqslv(fn = dNBllcol, x = tmpCol$x, thetas =  thetasMat , X = X, reg = regCol, muMarg = muMarg, k = KK,  global = "qline", control = nleqslv.control, n=n, p=p, jac = NBjacobianCol, method = jacMethod, colWeights = colWeights, nLambda = (KK+1), cMatK = cMat[1:(KK-1),,drop=FALSE], constrained = TRUE)
# }
# 
# 
# cat(ifelse(tmpCol$termcd==1, "Column scores converged \n", "Column scores DID NOT converge \n"))
...
LR_nb <- function(Alpha, X, CC, responseFun = c("linear","quadratic","nonparametric"), muMarg,   cReg, nleqslv.control = list(trace=FALSE), NB_params, NB_params_noLab, thetaMat, ...){#
  ...
  # @return: a scalar, the evaluation of the log-likelihood ratio at the given alpha
   ...
   # NB_params_mat = matrix(nleqslv(matrix(1,v,p) , reg = design,  fn = dNBllcol_constr_mat, theta = thetas[1:p], muMarg = muMarg, cReg = cReg, aX = X, control = nleqslv.control, jac = JacCol_constr_mat)$x,v,p)
    ...
    logDensj = dnbinom(X,mu = muMarg * exp(rowMultiply(design %*% NB_params, cReg)), size = thetaMat, log = TRUE) #Likelihoods under species specific model
    #Immediately return log likelihoods

    logDens0 = dnbinom(X,mu =muMarg * exp(outer(c(design %*% NB_params_noLab), cReg)), size = thetaMat, log = TRUE) #Likelihoods of null model
...
dLR_nb <- function(Alpha, X, CC, responseFun = c("linear", "quadratic", "nonparametric"), cMat, psi, NB_params, NB_params_noLab, d, alphaK, k, centMat, nLambda, nLambda1s, thetaMat, muMarg,  ...){
  ...
  mu = muMarg * exp(rowMultiply(design %*% NB_params, cMat*psi))
  mu0 = muMarg * exp(outer(c(design %*% NB_params_noLab), cMat*psi))

  lag = switch(responseFun, #The lagrangian depends on the shape of the response function
               "linear" = psi * (crossprod(CC, (X-mu)/(1+mu/thetaMat)) %*% (cMat*NB_params[2,]) - crossprod(CC, (X-mu0)/(1+mu0/thetaMat)) %*% (cMat*NB_params_noLab[2])) ,
               "quadratic" = psi * (
                   rowSums((tcrossprod(crossprod(CC, (X-mu)/(1+mu/thetaMat)),(rowMultiply(NB_params[c(2,3),],cMat)))) * cbind(1,2*alpha)) - 
                   rowSums((crossprod(CC, (X-mu0)/(1+mu0/thetaMat))%*%(outer(cMat, NB_params_noLab[c(2,3)]))) * cbind(1,2*alpha))),
...
LR_nb_Jac <- function(Alpha, X, CC, responseFun = c("linear", "quadratic", "nonparametric"), cMat, psi, NB_params, NB_params_noLab, d, alphaK, k, nLambda, nLambda1s, centMat, thetaMat, muMarg, n, v, ...){
  # @param Alpha: a vector of length d + k*(2+(k-1)/2), the environmental gradient plus the lagrangian multipliers
  # @param X: the n-by-p count matrix
  # @param CC: a n-by-d covariate vector
  # @param responseFun: a character string indicating the type of response function
  ...
  if(responseFun=="linear"){
    tmp = rowMultiply((1+X/thetaMat)*mu/(1+mu/thetaMat)^2,(cMat * NB_params[2,])^2)
    tmp0 = rowMultiply((1+X/thetaMat)*mu0/(1+mu0/thetaMat)^2,(cMat * NB_params_noLab[2])^2)
  } else if (responseFun=="quadratic"){
    tmp = tcrossprod((1+X/thetaMat)*mu/(1+mu/thetaMat)^2 , rowMultiply(NB_params[c(2,3),],cMat)^2) 
    tmp0 = ((1+X/thetaMat)*mu0/(1+mu0/thetaMat)^2) %*% outer(cMat, NB_params_noLab[c(2,3)])^2
    alphaArray = array(1, dim = c(v,d,d))
    alphaArray[2,,] = outer(alpha, alpha)
  }
  #dLag²/ds_{ik}dlambda_{3kk'}
  if(k>1){
  ...
  Jac[did,did] = switch(responseFun,
                           "linear" = #-psi^2 *(crossprod(CC*c(rowMultiply(tmp,(cMat*NB_params[2,])^2),CC) - crossprod(CC*c(tmp0 %*% (cMat*NB_params[2])^2),CC))
                           -psi^2 *(colSums(tensor(vapply(did, FUN.VALUE = tmp, function(x){CC[,x]*tmp}), CC,1,1)) - colSums(tensor(vapply(did, FUN.VALUE = tmp, function(x){CC[,x]*tmp0}), CC, 1, 1))) ,
                          "quadratic" = -psi^2 *(colSums(tensor(vapply(did, FUN.VALUE = tmp, function(x){CC[,x]*tmp}), CC,1,1)) - colSums(tensor(vapply(did, FUN.VALUE = tmp, function(x){CC[,x]*tmp0}), CC, 1, 1)*alphaArray)) 
...
vapply(seq_len(ncols), FUN.VALUE = vector("numeric",v), function(i){
     nleq = nleqslv(initParam[,i] , reg = design,  fn = dNBllcol_constr, theta = thetas[i], muMarg = muMarg[,i], cReg = cReg[i], X = X[,i], control = nleqslv.control, jac = JacCol_constr)$x#try(, silent=TRUE)  # , ... Fit the taxon-by taxon NB with given overdispersion parameters and return predictions
     # if(class(nleq)=="try-error" |anyNA(nleq) | any(is.infinite(nleq))){
...
JacCol_constr_noLab = function(betas, X, reg, thetas, muMarg, cReg, n ,v) {
  ...
  mu = exp(outer(c(reg %*% betas), cReg)) * muMarg
  tmp = rowMultiply((1+(X/thetas))*mu/(1+(mu/thetas))^2, cReg^2) #Don't forget to square cReg!
...
getRowMatConstr = function(v, NB_params, alpha, covariates, responseFun, ...){
