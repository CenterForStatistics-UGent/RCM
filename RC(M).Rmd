...
# @return: the singular value decomposition of the matrix of pearson residuals
  ...
  # 
  # GeigDec = geigen(t(X) %*% Y %*% diag(1/C) %*% t(Y) %*% X, t(Y) %*% diag(R) %*% Y)
...
Palette=rainbow(length(unique(dataSam$colourPlot)))
    ...
    plot = ggplot(dataSam, aes_string(x=names(dataSam)[1], y=names(dataSam)[2], col = "colourPlot", dotList)) + 
      ...
      xlab(paste0(names(dataSam)[1],": psi = ",round(RCMfit$psis[Dim[1]],1))) + #xlabel
      ylab(paste0(names(dataSam)[2],": psi = ",round(RCMfit$psis[Dim[2]],1))) + #ylabel
...
if(!file.exists(file = "CMETdata.RData")){
  ...
  otuTab = round(as.matrix(otuTab)*samData$`Cell.density.(cells/mL)`)
  ...
  CMETwater = phyloseq(otu_table(otuTab, taxa_are_rows = FALSE), sample_data(samData), tax_table(taxTab))
