...
Rdim = dim(R)
Xdim = dim(X)
Rrep = rep(c(R),times = Xdim[2])
Xrep = rep(c(X), times = Rdim[2])
tmp = Rrep *Xrep
dim(tmp) = c(prod(Rdim), Xdim[2])
tmp2 = rowSums(tmp)
dim(tmp2) = Rdim
tmp2
...
estAlpha = function(Alpha, X, CC, d, k, ncols, centMat, responseFun = c("linear", "quadratic", "nonparametric"), muMarg, thetas, cReg, alphaK, control.outer = list(trace=TRUE), control.optim=list(trace=TRUE), nleqslv.control = list() ){
  ...
  alphaNew = constrOptim.nl(par = Alpha, fn =LR_nb, gr = NULL, heq = heq_nb, heq.jac = heq_nb_jac, alphaK = alphaK, X=X, CC=CC, responseFun = responseFun, muMarg = muMarg, thetas = thetas, cReg = cReg, d = d, ncols=ncols, control.outer = control.outer, control.optim = control.optim, nleqslv.control = nleqslv.control, k = k, centMat = centMat) #
}
#--------------------------------------#
