...
reqpkg <- c("phyloseq","MASS", "parallel","nleqslv", "edgeR", "VGAM","HMP", "ggplot2", "pscl", "vegan", "alabama", "zCompositions", "DirFactor","reshape2")
...
dLR_nb <- function(Alpha, X, CC, responseFun = c("linear", "quadratic", "nonparametric"), cMat, psi, NB_params, NB_params_noLab, d, alphaK, k, centMat, nLambda, nLambda1s, thetaMat, muMarg,  ...){
  ...
  lag = switch(responseFun, #The lagrangian depends on the shape of the response function
               ...
               "quadratic" = psi * (
                 crossprod((X-mu)/(1+mu/thetaMat) ,CC) * (NB_params[2,] + 2*outer(NB_params[3,], alpha) %*% cMat
                - crossprod(
                   CC,(NB_params_noLab[2]+2*alpha*NB_params_noLab[3])),
                   (X-mu0)/(1+mu0/thetaMat) %*% cMat)),
               stop("Unknown response function provided! \n")) + #Restrictions do not depend on resonse function
                 c(lambda1s %*% centMat)  +
                 lambda2 * 2 * alpha + 
                 alphaK %*% lambda3
...
LR_nb_Jac <- function(Alpha, X, CC, responseFun = c("linear", "quadratic", "nonparametric"), cMat, psi, NB_params, NB_params_noLab, d, alphaK, k, nLambda, nLambda1s, centMat, thetaMat, muMarg, n,...){
  ...
  tmp = switch(responseFun,
               linear = {rowMultiply((1+X/thetaMat)*mu/(1+mu/thetaMat)^2,cMat * NB_params[2,])},
               quadratic = {(1+X/thetaMat)*mu/(1+mu/thetaMat)^2 %*% (NB_params[2,] + 2*outer(NB_params[3,], alpha))})
  tmp0 = switch(responseFun,
               linear = rowMultiply((1+X/thetaMat)*mu0/(1+mu0/thetaMat)^2,cMat * NB_params_noLab[2]),
               quadratic = (1+X/thetaMat)*mu0/(1+mu0/thetaMat)^2) 
  ...
  Jac[did,did] = switch(responseFun,
                           ...
                           -psi^2 *(crossprod(vapply(did, FUN.VALUE = vector("numeric", n), function(x){rowSums(CC[,x]*tmp)}), CC) - crossprod(vapply(did, FUN.VALUE = vector("numeric", n), function(x){rowSums(CC[,x]*tmp0)}), CC)),
                          "quadratic" = -psi^2 *(crossprod(vapply(did, FUN.VALUE = vector("numeric", n), function(x){rowSums(CC[,x]*tmp)}), CC) - crossprod(vapply(did, FUN.VALUE = vector("numeric", n), function(x){rowSums(CC[,x]*tmp0)}), rowMultiply(CC, (NB_params_noLab[2] + 2*NB_params_noLab[3] * alpha))))
...
NB_params_noLab = estNBparamsNoLab(design = design, thetas = thetas, muMarg = muMarg, cReg = cReg, X = X, nleqslv.control = nleqslv.control, initParam = NB_params_noLab, n = n, v = v)
 ...
 if(analytic){
  ...
  lambdasAlpha = AlphaTmp[d+seq_len(nLambda)]
   ...
   Alpha = constrOptim.nl(par = Alpha, fn = LR_nb, gr = NULL, heq = heq_nb, heq.jac = heq_nb_jac, alphaK = alphaK, X=X, CC=CC, responseFun = responseFun, muMarg = muMarg, cReg = cReg, d = d, ncols=ncols, control.outer = control.outer, control.optim = control.optim, nleqslv.control = nleqslv.control, k =k, centMat = centMat, NB_params = NB_params, NB_params_noLab = NB_params_noLab, v = v, thetaMat = thetas)$par #
}
  ...
  return(list(Alpha = Alpha, NB_params = NB_params, NB_params_noLab = NB_params_noLab, convergence = convergenceA, lambdasAlpha = if(analytic) lambdasAlpha  else NULL))
}
#--------------------------------------#
