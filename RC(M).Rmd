...
nonParamsRespFun = if(responseFun == "nonparametric") {list(taxonWise = matrix(0,n,p), overall = rep(0,n))} else {NULL}
rowMat = NULL
...
estNPresp = function(sampleScore, muMarg, X, ncols, psi, ...){
  ...
  locfit.raw(x = sampleScore, y = (X[,i]/muMarg[,i])/exp(psi),link = "log",...)
})
  overall = fitted(locfit.raw(x = rep(sampleScore,ncols), y = c((X/muMarg)/exp(psi)), link = "log",...))
  taxonWise = sapply(taxonWiseFits, fitted)
