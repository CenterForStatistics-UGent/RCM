...
- (\beta_{1} + 2\beta_{2}\alpha_{yk}c_{iy})(\beta_{1} + 2\beta_{2}\alpha_{y'k}c_{iy'}) c_{iy} c_{iy'}\frac{\mu_{ij}(1+ \frac{x_{ij}}{\theta_j})}{(1 + \frac{\mu_{ij}}{\theta_j})^2}  \Big) \Big] - 2\psi_k \Big[\sum_{i=1}^n \sum_{j=1}^p c_{iy}c_{iy'} \big(\beta_{2j} \frac{x_{ij} - \mu_{ij}}{1 + \frac{\mu_{ij}}{\theta_j}}-\beta_{2} \frac{x_{ij} - \mu_{ij0}}{1 + \frac{\mu_{ij0}}{\theta_j}}\big)\Big] $$
...
LR_nb_Jac = function(Alpha, X, CC, responseFun = c("linear", "quadratic", "nonparametric"), psi, NB_params, NB_params_noLab, d, alphaK, k, nLambda, nLambda1s, centMat, thetaMat, muMarg, n, v, ncols, ...){
  # @param Alpha: a vector of length d + k*(2+(k-1)/2), the environmental gradient plus the lagrangian multipliers
  # @param X: the n-by-p count matrix
  # @param CC: a n-by-d covariate vector
  # @param responseFun: a character string indicating the type of response function
  ...
  Jac[did,did] = switch(responseFun,
                        ...
                        colSums(tensor(vapply(did, FUN.VALUE = tmp, function(x){CC[,x]*rowMultiply(tmp,NB_params[2,]^2)}),CC,1,1)) + #b1b1
                          colSums(tensor(vapply(did, FUN.VALUE = tmp, function(x){CC[,x]*rowMultiply(tmp*c(sampleScore),NB_params[2,]*NB_params[3,]*2)}),CC,1,1)) + #b1b2
                        colSums(tensor(vapply(did, FUN.VALUE = tmp, function(x){CC[,x]*rowMultiply(tmp*c(sampleScore)^2,NB_params[3,]^2*4)}), CC,1,1)) - #b2b2
                        colSums(tensor(vapply(did, FUN.VALUE = tmp, function(x){CC[,x]*tmp0}),CC,1,1)*NB_params_noLab[2]^2) - #b1b1
                        colSums(tensor(vapply(did, FUN.VALUE = tmp, function(x){CC[,x]*tmp0 }), CC*c(sampleScore),1,1)*NB_params_noLab[2]*NB_params_noLab[3]*2) - #b1b2
                        colSums(tensor(vapply(did, FUN.VALUE = tmp, function(x){CC[,x]*tmp0 }),CC*c(sampleScore)^2,1,1)*NB_params_noLab[3]^2*4) #b2b2
) +2*# c_{iy} with the beta_2's are still missing
  psi*(
    colSums(tensor(vapply(did, FUN.VALUE = tmp, function(x){CC[,x]*rowMultiply((X-mu)/(1+mu/thetaMat),NB_params[3,])}),CC,1,1)) -
    colSums(tensor(vapply(did, FUN.VALUE = tmp, function(x){CC[,x]*(X-mu0)/(1+mu0/thetaMat)}),CC,1,1)*NB_params_noLab[3])
           )
)
  
  tmpMat  = array(0, dim = c(n,p,d))
...
$$-(2hb_2cy_2 + b_1cy_2)(2hb_2 +
b_1)cy \psi^2
\frac{\mu}{(\frac{\mu}{\theta} + 1)} \\ + (2hb_2cy_2 + b_1cy_2)(2hb_2 + b_1)(\mu - x) cy\psi^2 \frac{\mu}{\theta(\mu)/\theta + 1)^2}) - 2(\mu) - x)b_2cycy_2\frac{\psi}{\frac{\mu}{\theta} + 1})$$
