...
Filtering out the effect of confounders brings the problems of the zeroes to the fore again. If a taxon has only zero counts in one of the subgroups defined by the confounding variables, the model will fail to fit again. For continuous confounders this is of course not an issue. Maybe there exists a more efficient solution, but for now we will have to trim the taxa for which this occurs from further analysis in order to render the model identifiable. This trimming best occurs alrady before fitting the independence model.
...
trimOnConfounders = function(confounders, X, prevCutOff){
  ...
  trimmingID = apply(X, 2, function(x){ #Over taxa
    any(apply(confounders, 2, function(conf){ #Over confounding variables
      tapply(X = x, INDEX = conf, FUN = function(y){mean(y!=0) <= prevCutOff}) #Any all-zero subgroup?
    }))
  })
...
RCM_NB = function(X, k, rowWeights = "uniform", colWeights = "marginal", tol = 1e-3, maxItOut = 500, Psitol = 1e-3, verbose = TRUE, NBRCM = NULL, global ="dbldog", nleqslv.control=list(), method=c("Broyden"), dispFrec=10, convNorm = 2, prior.df=10, marginEst = "MLE", estPsis = TRUE, confounders = NULL, covariates = NULL, responseFun = "linear", prevCutOff = 1e-2){
  ...
  if(!is.null(confounders)){ #First and foremost: filter on confounders
    Xorig = X
    X = trimOnConfounders(X, confounders = confounders, prevCutOff = prevCutOff)
  }
 ...
 if(class(absTmp)!="try-error"){ logAbundsMLE = absTmp}
  ...
  muMarg = filterConfounders(muMarg = muMarg, confMat = confounders,p=p, X=X, thetas = thetas, nleqslv.control = nleqslv.control, n=n )
  }
 ## 1) Initialization
...
RCM = function(dat, k, round=FALSE, distribution= c("NB", "ZIP", "ZINB"), prevCutOff = 0.01, rowWeights = "uniform", colWeights = "marginal", covariates = NULL, confounders = NULL, ...){
  ...
  X=X[, colMeans(X==0)<(1-prevCutOff)] #Remove txa with low prevalence
  ...
  if(!is.null(confounders)){
    if(class(confounders) %in% c("vector","matrix")){
      ...
      if(class(confounders[,1]) %in% c("character", "factor")){ #If character or factor, build model matrix
        ...
        confModelMat = model.matrix(object = as.formula(~ paste(colnames(confounders), sep="+"),"-1")) #Without intercept
      } else {
        confModelMat = confounders
      }
    } else if(class(confounders) == "dataframe"){
      if(NROW(X)!=NROW(confounders)){ #Check dimensions
        stop("Data and confounder matrix do not have the same number of samples! \n")
      }
      confModelMat = model.matrix( object = as.formula(~ paste(names(confounders), sep="+"),"-1"))
      ...
      contrastsList = rep("contr.sum", length(confounders))
      names(contrastsList) = confounders
      confModelMat = model.matrix(object = formula(paste("~", paste(confounders, sep="+"),"-1")), data = data.frame(sample_data(dat)), contrasts.arg = as.list(contrastsList))
    } else{
...
tic = proc.time() #Time the calculation
  tmp = switch(distribution, 
               NB=RCM_NB(X, rowWeights = rowWeights, colWeights = colWeights, k = k, confounders  = confModelMat, covariates = covModelMat, prevCutOff = prevCutOff, ...),
               ZIP=RCM_ZIP(X, rowWeights = rowWeights, colWeights = colWeights, k = k, confounders  = confModelMat, covariates = covModelMat, prevCutOff = prevCutOff,...),
               ZINB=RCM_ZINB(X,  rowWeights = rowWeights, colWeights = colWeights, k = k, confounders  = confModelMat, covariates = covModelMat, prevCutOff = prevCutOff,...))
  if(classDat=="phyloseq"){tmp$physeq = physeq} 
  within(tmp, {
