...
RCM_NB = function(X, k, rowWeights = "uniform", colWeights = "marginal", tol = 1e-3, maxItOut = 500, Psitol = 1e-3, verbose = TRUE, NBRCM = NULL, global = "dbldog", nleqslv.control=list(), jacMethod = "Broyden", dispFrec = 20, convNorm = 2, prior.df=10, marginEst = "MLE", confounders = NULL, prevCutOff = 1e-2, minFraction = 0.1, covariates = NULL, centMat = NULL, responseFun = "linear"){
  ...
  trended.dispersion <- estimateGLMTrendedDisp(y = t(X), design = NULL, method = "bin.loess", offset = t(log(outer(libSizes, abunds))), weights = dispWeights)
...
#--------------------------------------#
# A function that returns the value of the partial derivative of the log-likelihood ratio of alpha to alpha, keeping the response functions fixed
LR_nb <- function(Alpha, X, CC, responseFun = c("linear", "quadratic", "nonparametric"), muMarg,   cReg, nleqslv.control = list(trace=FALSE), NB_params, NB_params_noLab, ...){
  # @param Alpha: a vector of length d, the environmental gradient
  # @param X: the n-by-p count matrix
  # @param CC: the n-by-d covariate matrix
