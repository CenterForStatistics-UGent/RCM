...
Since we are not in a full likelihood framework, we cannot use deviance residuals or formal tests. Therefore I propose to use the number and extent of outlying values as a measure of lack of fit. In our setting outliers will always be values larger than expected, a zero is practically never outlying.

If we look at the quantile functions, we could consider all observations of a sample from the $1-\frac{1}{n}$th quantile or higher as possible outliers. The ratio of $1-\frac{1}{n}$ to the geometric mean of all quantiles of observations above the $1-\frac{1}{n}$th quantile is then a measure of the severity of the outliers.
...
```{r CMETliks, purl = FALSE}
liksCMET = sapply(list("unconstrained" = CMETRCM, "linear" = CMETRCMlin, "nonParametric" = CMETRCMnonParam), liks)
