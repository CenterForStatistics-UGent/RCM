...
palStore = palette()
levelsMeth = c("CApearson","CAcontrRat","CAchisq","DCA", "Hellinger","BC","BCrel","BCrelNMDS", "JSD","UniFrac","wUniFrac", "CoDa","RCM", "Control")
labelsMeth = c("CApearson","CAcontrRat","CAchisq","DCA","Hellinger","Bray-Curtis-Abs","Bray-Curtis","Bray-Curtis NMDS", "JSD","UniFrac","weighted UniFrac","CoDa","RCM", "Control")
...
It is evident that the row scores of these biplots will be equal to PCoA biplots based on the chi-squared distance.

Correspondence analysis has been criticised for putting undue emphasis on rare species (Legendre and Gallagher, 2001).

#### Joint plots

Some researchers prefer to plot $U\Sigma$ vs $\SigmaV^T$ to represent distances between both sites and species corrrectly. However, this loses the biplot property (inner product interpration) and end their discussion here.
...
tmpList = with(x, list(
    ...
    CAcontRat = CAchisq  = daig(1/sqrt(rowSums(RCM$X))) %*% CA$u %*% diag(CA$d),
    DCA = DCA$rproj,
    CoDa = CoDa$rowScores,
...
# newHclust = list(merge = randomTree$edge, labels = randomTree$tip.label)
# class(newHclust) = "hclust"
# outgroup <- match(c("root1","root2"),randomTree$tip.label)
# sptree.rooted <- root(randomTree,outgroup,node = NULL)

sptree.rooted.ultra <- chronopl(randomTree, 0, age.min = 1, age.max = NULL,node = "root", S = 1, tol = 1e-8,CV = FALSE, eval.max = 500, iter.max = 500) #Make tree ultrametric


# dd = as.dist(cophenetic.phylo(randomTree)) #Get branch lenghts
# psclust = cutree(as.hclust(hclust(dd)), k = k) #Split into clusters
psclust = cutree(as.hclust(sptree.rooted.ultra), k = k) #Split into clusters
