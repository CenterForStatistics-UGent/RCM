...
Note also that when there is a very large overdispersion for a taxon _j_ ($\theta_j$ small), its observations carry little information and their weights are small in the calculation of the library sizes. However, when there is very little overdispersion ($\theta_j \rightarrow \infty$), the weights of the components of the score function equal 1, as with Poisson regression. Finally, we see that departures from the mean $\mu_{ij}$ are weighted down for large values of $\mu_{ij}$, acknowledging the fact that the variance increases faster than linear with the mean in the negative binomial model.
...
# A ggplot2-version
RCMbiplot = function(RCMfit, Dim=c(1,2), ..., colour = NULL, colLegend = colour, shapeLegend = NULL, size=3, arrowFrac = 0.025, scalingFactor=NULL, Palette=NULL, arrowCol="blue", plotTaxa = FALSE) {
...
RCMbiplot(con2DmatLin, colour = "BMI")
RCMbiplot(con2DmatQuad, colour = "BMI")
RCMbiplot(con2DmatNon, colour = "BMI")
...
lapply(c("Country","Diagnosis","Gender", "BMI","Age"), function(x){
  RCMbiplot(zellerSphyRCM, colour = x)
})
```
...
RCMbiplot(zellerSphyRCMconstrLin)
RCMbiplot(zellerSphyRCMconstrLin, colour = "Diagnosis")
RCMbiplot(zellerSphyRCMconstrLin, colour = "Gender")
RCMbiplot(zellerSphyRCMconstrLin, colour = "Country")
RCMbiplot(zellerSphyRCMconstrLin, colour = "Age")
...
RCMbiplot(zellerSphyRCMconstrLin, colour = "Diagnosis", plotTaxa = TRUE)
```
...
RCMbiplot(zellerSphyRCMconstrLin, colour = "Age", plotTaxa = TRUE)
rm(zellerSphyRCMconstrLin)
```
...
RCMbiplot(turntwinsRCm, colour = "Family_p")
RCMbiplot(turntwinsRCm, colour = "twin")
RCMbiplot(turntwinsRCm, colour = "Status_p")
rm(turntwinsRCm)
```
...
RCMbiplot(HumMiceRCM) # Two or three distinct groups
RCMbiplot(HumMiceRCM, colour = "Diet") #Diet is a main factor
RCMbiplot(HumMiceRCM, colour = "Individual") #Individuals play less of a role
RCMbiplot(HumMiceRCM, colour = "Generation_p") #Microbiome changes quickly with feed pattern
RCMbiplot(HumMiceRCM, colour = "Time") #Later samples also form a cluster
```

```{r humMicePlotExec, purl = FALSE}
RCMbiplot(HumMiceRCM, colour = "Diet", main =  "Humanized mice") #Diet is a main factor
RCMbiplot(HumMiceRCM, colour = "Time", main =  "Humanized mice") #Later samples also form a cluster
rm(HumMiceRCM)
```

```{r humMicePlot13D, purl = FALSE, eval = FALSE}
RCMbiplot(HumMiceRCM, colour = "Diet", Dim = c(1,3)) #Diet is a main factor
RCMbiplot(HumMiceRCM, colour = "Individual", Dim = c(1,3)) #Individuals play less of a role
RCMbiplot(HumMiceRCM, colour = "Generation_p", Dim = c(1,3)) #Microbiome changes quickly with feed pattern
RCMbiplot(HumMiceRCM, colour = "Time", Dim = c(1,3)) #Later samples also form a cluster
RCMbiplot(HumMiceRCM, colour = "DTG", Dim = c(1,3))
RCMbiplot(HumMiceRCM, colour = "DGS", Dim = c(1,3))
```
