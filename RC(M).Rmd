...
palStore = palette()
levelsMeth = c("CA","DCA", "Hellinger","BC","BCrel","BCrelNMDS", "JSD","UniFrac","wUniFrac", "CoDa","RCM", "Control")
labelsMeth = c("CA","DCA","Hellinger","Bray-Curtis-Abs","Bray-Curtis","Bray-Curtis NMDS", "JSD","UniFrac","weighted UniFrac","CoDa","RCM", "Control")
groupsMeth = factor(c("Independence","Independence","Independence","Distance","Distance","Distance","Distance","Distance","Distance", "CoDa", "RCM"), levels = c("Independence","Distance","CoDa","RCM", "Control"), ordered = TRUE)
funFiles = dir("R")
...
tmpList = with(x, list(
    ...
    UniFrac = UniFrac$vectors %*% diag(UniFrac$values$Eigenvalues[seq_len(ncol(UniFrac$vectors))]),
    wUniFrac = wUniFrac$vectors %*% diag(wUniFrac$values$Eigenvalues[seq_len(ncol(wUniFrac$vectors))]),
...
extractCols = function(x, row){
  tmpList = with(x, list(
    ...
    UniFrac = weightedTaxonScores(RCM$X, row$UniFrac),
    wUniFrac = weightedTaxonScores(RCM$X, row$wUniFrac),
...
#---------------------------------#
distRatioPlot = function(resListRows, groupFactor, method = NULL, groupMeth = droplevels(groupsMeth),log="",...) {
...
boxplot(DistanceRatio~Method, main = "Mean ratios of overal to within distance by method", data = meanDist1df, col = groupMeth, ylim = c(0, ceiling(max(meanDist1df$DistanceRatio))),las = 2,log = log,...)
meansDist = tapply(meanDist1df$DistanceRatio, meanDist1df$Method, mean)
points(meansDist, col="red", pch=18, cex = 1.6)
abline(h=switch(log,""=1, "y"=0), lty = "dashed")
...
```{r NBrelatedDistrCorr, purl = FALSE, eval = FALSE}
p = 1000 #Number of taxa
n = 60 #Number of samples
nPop = 4 #Number of underlying populations
nPerPop = n/nPop #Number of samples per population
...
distRatioPlot(resListPhyRows, groupFactorPhy)
```
...
Regular unifrac suffers from correlations with the library sizes
