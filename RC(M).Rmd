...
rMat = apply(rMat, 2, function(rowS){
  ...
  while((iterOut ==1) || ((iterOut <= maxItOut) && (!convergence[KK])))
    {
...
if(class(tmpRow)=="list"){
  cat(ifelse(tmpRow$termcd==1, "Column scores converged \n", "Column scores DID NOT converge \n"))
...
#Store intermediate estimates
  ...
  ##Check convergence  (any numbered norm for row and column scores)
  convergence[KK] = ((iterOut <= maxItOut) && 
                   (all(abs(1-psis[KK]/psisOld) < Psitol)) &&
                   ((sum((1-rMat[,KK]/rMatOld)^convNorm)/n)^(1/convNorm) < tol) && 
                   ((sum((1-cMat[KK,]/cMatOld)^convNorm)/p)^(1/convNorm) < tol) )
...
weightsChar = paste0(rowWeights, colWeights)
  switch(method, NB=RCM_NB(dat, rowWeights=rowWeightsNum, colWeights=colWeightsNum, weightsChar = weightsChar,...),NB_1B1=RCM_NB_1B1(dat, rowWeights=rowWeightsNum, colWeights=colWeightsNum, weightsChar = weightsChar,...), ZIP=RCM_ZIP(dat, rowWeights=rowWeightsNum, colWeights=colWeightsNum,weightsChar = weightsChar, ...), ZINB=RCM_ZINB(dat,  rowWeights=rowWeightsNum, colWeights=colWeightsNum,weightsChar = weightsChar,...))
}
...
if(!file.exists("toyDataUnifNS.RData")){
  ...
  syntNBNSunifJob = mcparallel(RCM(dataMatNBNS, method="NB", k=2, nleqslv.control= nleqslv.control, maxItOut=2e3, prevCutOff=0.01))
  syntNBNSunif=mccollect(syntNBNSunifJob, FALSE)[[1]] 
  #Some taxa are outliers. Should we leave them out?
...
#   #Refitting does not help, new taxa become outlying
 ...
 #With marginal weights
  ...
  #Save the results
  save(syntNBNSunif,syntNBNSmarg, file="toyDataUnifNS.RData" )
...
#Row scores
rChange1 = c(rep(25,10), rep(0, Nsamples-20), rep(25,10))
rChange2 =  c(rep(0,5),rep(8,15), rep(0, Nsamples-40), rep(8,15), rep(0,5))

#Logical indices for these
rid1 = rChange1!=0
rid2 = rChange2!=0

rowScores1 = rnorm(Nsamples-Nsamples%%2, sd=9)  + rChange1
rowScores2 = rnorm(Nsamples-Nsamples%%2, sd=6) + rChange2

#Column scores
cChange1 = c(rep(0,10),rep(25,20), rep(0, Ntaxa-50), rep(25,20))
cChange2 = c(rep(0,30),rep(20,25), rep(0, Ntaxa-100), rep(20,25), rep(0,20))

cid1 = cChange1!=0
cid2 = cChange2!=0

colScores1 = rnorm(Ntaxa-Ntaxa%%2, sd = 9) + cChange1
colScores2 = rnorm(Ntaxa-Ntaxa%%2, sd = 6) + cChange2
...
if(!file.exists("toyDataUnifSig.RData")){
  ...
  save(dataMatNBSig, syntNBSigunif,syntNBSigmarg, taxaSigNB, sampleSigNB, file="toyDataUnifSig.RData")
...
NBmargSigPlotDF = RCMres2DF(syntNBSigmarg,  sampleSigNB, taxaSigNB)
