...
RCM_NB = function(X, k, rowWeights, colWeights, weightsChar, tol = 1e-3, maxItOut = 500, Psitol = 1e-3, verbose = TRUE, NBRCM=NULL, global ="dbldog", nleqslv.control=list(), method=c("Broyden"), dispFrec=10, convNorm = 2, prior.df=10, marginEst = "marginSums"){
...
tmpCol = try(nleqslv(fn = dNBllcol, x = c(t(cMat[KK,]), lambdaCol[idK]), thetas=thetas, X = X, reg = regCol, muMarg=muMarg, k=KK,  global = global, control = nleqslv.control, n=n, p=p, jac = NBjacobianCol, method=method, colWeights=colWeights, nLambda=(KK+1), cMatK = cMat[1:(KK-1),,drop=FALSE]), silent=TRUE)
...
#--------------------------------------#
#Estimate the offsets for the zeroes
dZipZeroOffset = function(beta, Z){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param X: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param theta: a vector of length p with the dispersion parameters
...
#  if(class(zeroRowsTmp)!="try-error"){ logitZeroRows = zeroRowsTmp} # We don't let zero-inflation depend on the samples, there is no baseline absence rate for a sample
    zeroOffsetTmp = try(nleqslv(fn = dZipZeroOffset, x = logitZeroRows,   reg=logitZeroCols, global=global, control = nleqslv.control, jac=ZipJacobianZeroOffset, method=method, Z=Z)$x, silent=TRUE)
#  if(class(zeroRowsTmp)!="try-error"){ logitZeroRows = zeroRowsTmp}
  zeroColsTmp = try(nleqslv(fn = dZipZeroCol, x = logitZeroCols, reg=logitZeroRows, global=global, control = nleqslv.control, jac=ZipJacobianZeroCol, method=method, Z=Z)$x, silent=TRUE)
...
#   })
  ...
  if(!convergence[KK] ){
    ...
    return(list(converged = convergence,rMat=rMat, cMat=cMat, psis = psis, X=X,
                rowRec = rowRec, colRec = colRec, psiRec = psiRec, lambdaRow = lambdaRow, lambdaCol = lambdaCol, lambdaRowZero = lambdaRowZero, lambdaColZero = lambdaColZero, chis = chis, tMat = tMat, vMat = vMat, zeroMarg = zeroMarg, chiRec = chiRec, rowRecZero = rowRecZero, colRecZero = colRecZero, iter=iterOut-1, Z=Z, fit="RCM_ZIP"))
}
```
...
RCM_ZINB = function(X, k, rowWeights , colWeights, weightsChar, tol = 1e-3, maxItOut = 500, psiTol = 1e-4, chiTol = psiTol, verbose = TRUE, ZINBRCM = NULL, global = "dbldog", nleqslv.control = list(), method="Broyden", twoCores = FALSE, convNorm = 2, maxItMean = 20, maxItZeroes = 20, dispFreq = 5){
 ...
 if(class(zeroRowsTmp)!="try-error"){ logitZeroRows = zeroRowsTmp}
  zeroColsTmp = try(nleqslv(fn = dZinbZeroCol, x = logitZeroCols, reg=logitZeroRows, global=global, control = nleqslv.control, jac=ZinbJacobianZeroCol, method=method, Z=Z)$x, silent=TRUE)
 if(class(zeroColsTmp)!="try-error"){ logitZeroCols = zeroColsTmp}
...
#                 "marginaluniform" = svd(diag(1/libSizes) %*% (X-muMarg)),
#                 "uniformmarginal" = svd((X-muMarg) %*% diag(1/colSums(X))),
#                 "uniformuniform" = svd(X-muMarg)) svd((X-muMarg)*(1-zeroMarg)) 
  rMat = svdX$u[,1:k,drop=FALSE]
  cMat = t(svdX$v[,1:k,drop=FALSE])
  psis = svdX$d[1:k]
#  
...
minK = ifelse(is.null(ZINBRCM),1,Kprev+1)
  ...
  if(!convergence ){
    ...
    return(list(converged = convergence, rMat=rMat, cMat=cMat, psis = psis, X=X,
                 rowRec = rowRec, colRec = colRec, psiRec = psiRec, lambdaRow = lambdaRow, lambdaCol = lambdaCol, lambdaRowZero = lambdaRowZero, lambdaColZero = lambdaColZero, chis = chis, tMat = tMat, vMat = vMat, chiRec = chiRec, rowRecZero = rowRecZero, colRecZero = colRecZero, iter=iterOut-1, Z=Z, thetas=thetas))
}
```
...
There are subtle differences but less than with the library sizes.

##### Influence function

Take a look at the influence function values for the influence on the first psi parameters

```{r NS influence measures: psis, results="hide", eval=TRUE}
...
```{r X vs Infl(X), eval=TRUE, results="hide"}
#Expectations
par(mfrow=c(2,4))
lapply(names(infl1), function(x){
...
```{r ab plots, include=FALSE, eval=TRUE}
...
cols = c("grey", "red","blue","purple","green","brown","cyan","black", "orange", "magenta","yellow","pink", "darkblue","grey75","cadetblue") 
palette(cols)
...
The MLE estimate is generally larger than the rowSums estimate, this may explain the gradient of the row scores in function of the library sizes.
...
if(!file.exists("syntZIPnoSig.RData")){
  syntZIPnoSigUnifUnifJob = mcparallel(RCM(dataMatZIPnoSig, method = "ZIP",k=2, nleqslv.control = nleqslv.controlZIP, colWeights = "uniform", rowWeights = "uniform", maxItOut=5e2))
...
if(!file.exists("syntZINBnoSig.RData")){
    syntZINBSigunifunifJob = mcparallel(RCM(dataMatZINBnoSig, method = "ZINB", k=3, nleqslv.control = nleqslv.control.zinb, colWeights = "uniform", rowWeights = "uniform", twoCores=FALSE, maxItOut = 5e3))
...
So far we haven been doing unconstrained or unsupervised explorative visualization, or _indirect_ gradient analysis. We just visualized the strongest signals in the data, independent of sample covariates. A logical next step is to develop a constrained method in the same framework, i.e. with the same error distribution. We argued that PCoA with Bray-Curtis distance or regular correspondence analysis did not perform well, and we will proably conclude the same for canonical correspondence analysis since it relies on the same distributional assumptions. We will visualize those departures from the independence model that can be explained by the sample covariates. Each dimension will thus consist of a linear combination of sample covariates, the weight of each of these covariates in the dimensions relflecting how much they contribute to explaining the variation. For backgrung information see also Zhu _et al._ , 2005, and Zhang and Thas, 2012.

We call our n-by-d covariate matrix $C$, for every sample $d$ baseline covariates are measured. These covariates $\mathbf{c_i}$ of sample $i$ can be either discrete or continuous. The expected counts are then modelled as 

$$log(E(X_{ij})) = u + u_i + u_j  + \sum_{l=1}^k \big(a_{jk} - \frac{(h_{ik} - q_{jk})}{2p_{jk}^2}$$

whereby $h_{ik} = \mathbf{\alpha_k c_i}$ the optimal linear combination of $c_i$ in dimension $k$. $a_{jk}$ is the maximal log-departure from independence achieved when the linear combination of covariates $\mathbf{\alpha_k c_i}$ equals the optimum $q_{jk}$. $p_{jk}$ is the tolerance, it determines how fast the expected value decreases as $\mathbf{c_i}$ moves away from the optimal values $\mathbf{\alpha_k^{-1}}h_{ik}$.
