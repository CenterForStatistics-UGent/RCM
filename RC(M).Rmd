...
filterConfounders  =function(muMarg, confounders, X, thetas, p, n, nleqslv.control, tol = 1e-3){
  #@param muMarg: a nxp matrix, the current offset
  #@param confounders: a nxt confounder dataframe. Only numeric values and factors are allowed
  #@param X: the nxp data matrix
  ...
  if(!is.data.frame(confounders)){
    stop("Confounders must be supplied a dataframe! \n")
  }
#   confClass = sapply(confounders, class) # The variable classes
#   if(any(!confClass %in% c("factor","numeric"))){
#     stop("Variables must be either numeric or factors! \n")
#   }
  #Build the confounder matrix (treatment coding for the dummies)
  confMat = model.matrix(data = confounders, object = as.formula(paste0("~",paste(names(confounders), collapse = "+")))) #With intercept
  rep0 = rep(0, ncol(confMat))

  #Fit the NB models for every taxon
#   NB_params = sapply(seq_len(p), function(i){
#     tmp = try(glm.fit(family = negative.binomial(theta = thetas[i]), x=confMat, y= X[,i], offset = log(muMarg[,i]))$coef)
...
#     }
#   })
  NB_params_old = matrix(rep0, nrow = ncol(confMat), ncol = p)
  
  while(!convergence){
    
    NB_params_old = NB_params
    
    offset  = muMarg * exp(confMat %*% NB_params_old)
  
     NB_params = sapply(seq_len(p), function(i){
     nleq = try(nleqslv(rep0 , reg = confMat,  fn = dNBllcol_constr, theta = thetas[i], muMarg = offset[,i], cReg = 1, X = X[,i], control = nleqslv.control, jac = JacCol_constr)$x)  # , ... Fit the taxon-by taxon NB with given overdispersion parameters and return predictions
     if(class(nleq)=="try-error" |anyNA(nleq) | any(is.infinite(nleq))){
     nleq = nleqslv(rep0 , reg = confMat,  fn = dNBllcol_constr, theta = thetas[i], muMarg = muMarg[,i], cReg =1, X = X[,i], control = nleqslv.control)$x # If fails try with numeric jacobian
     }
     return(nleq)
     ...
     thetas = estDisp(X = X, cMat = matrix(0, ncol=p), rMat = matrix(0, nrow=n), psis = 0, muMarg = offset) #Estimate overdispersion
     
  convergence = sqrt(mean((NB_params_old-NB_params)^2)) < tol #Check for convergence
  
  NB_params_old = NB_params
  }
  
  list(muMarg = muMarg * exp(confMat %*% NB_params), thetas = thetas)
  }
#Filtering out the confounders is difficult in the case of discrete variables: some taxa may have only very few non-zero observations for certain factor levels.
...
RCM_NB = function(X, k, rowWeights = "uniform", colWeights = "marginal", tol = 1e-3, maxItOut = 500, Psitol = 1e-3, verbose = TRUE, NBRCM = NULL, global ="dbldog", nleqslv.control=list(), method=c("Broyden"), dispFrec=10, convNorm = 2, prior.df=10, marginEst = "MLE", estPsis = TRUE, confounders = NULL, covariates = NULL, responseFun = "linear"){
  ...
  # @param confounders: an nxg matrix with confounders
 ...
 if(class(absTmp)!="try-error"){ logAbundsMLE = absTmp}
  ...
  muMarg = filterConfounders(muMarg = muMarg, confounders = confounders,p=p, X=X, thetas = thetas, nleqslv.control = nleqslv.control )
  }
 ## 1) Initialization
...
alpha = Matrix::solve(covariates[1:d,], rMat[1:d,]) #invert definition to obtain starting values
...
tmpAlpha = estAlpha(Alpha = alpha[, KK], X = X, CC = covariates, d = d, ncols = p, responseFun = responseFun, muMarg = muMarg, thetas = thetas, cReg = regAlpha, alphaK = alpha[,1:(KK-1),drop=FALSE])
...
#Normalize (speeds up algorithm if previous step had not converged)
alpha[,KK] = alpha[,KK] - sum(alpha[,KK]* rowWeights)/sum(rowWeights) 
alpha[,KK] = alpha[,KK]/sqrt(sum(rowWeights * alpha[,KK]^2))
...
#Store intermediate estimates
  ...
  returnList=list(converged = convergence,alpha = alpha, cMat=cMat, thetas = thetas, psis = psis, X=X,
                 alphaRec= alphaRec, covariates = covariates, colRec = colRec, psiRec = psiRec, thetaRec = thetaRec, lambdaCol=lambdaCol, rowWeights=rowWeights, colWeights = colWeights, iter = iterOut-1, fit="RCM_NB_constr", libSizesMLE = switch(marginEst, "MLE" = exp(logLibSizesMLE), "marginSums" = libSizes), abundsMLE = switch(marginEst, "MLE" = exp(logAbundsMLE), "marginSums" = abunds))
...
RCM = function(dat, k, round=FALSE, method= c("NB","NB", "ZIP","ZIP", "ZINB"), prevCutOff=0.01, rowWeights = "uniform", colWeights = "marginal",...){
  if (class(dat)=="matrix"){
...
tic = proc.time() #Time the calculation
  tmp = switch(method, 
               NB=RCM_NB(dat, rowWeights=rowWeights, colWeights=colWeights, k=k,...),
               ZIP=RCM_ZIP(dat, rowWeights=rowWeights, colWeights=colWeights, k=k,...),
               ZINB=RCM_ZINB(dat,  rowWeights=rowWeights, colWeights=colWeights, k=k,...))
...
sample_data(x)[["RUNCENTER"]]  = runcenters
x})

physeq = RCMhmpFilt[["Buccal.mucosa"]]
conf = get_variable(physeq, "RUNCENTER")
confDrop = droplevels(conf)[[1]]

filtRes = lapply(RCMhmpFilt, function(x){
  RCM(x, method="NB", k=K, nleqslv.control= nleqslv.control, maxItOut=5e3, prevCutOff=0.01, colWeights="marginal", rowWeights = "uniform", marginEst = "MLE", round=TRUE, confounders = runcenters)})
```
...
$$log(E(X_{ij})) = log(\mu_{ij}) = u_i + u_j + \sum_{k=1}^K \Big(\psi_k s_{jk} f \big( \boldsymbol{\alpha}_k^t \mathbf{C}_{i.}|\boldsymbol{\beta}_j \big) \Big)$$
...
$$ \frac{\partial l_{NB}(X_{.j}|\boldsymbol{\beta}_j, \theta_j)}{\partial \beta_{jv}} = \sum_{i=1}^n \psi s_j \frac{\partial f \big( \boldsymbol{\alpha}^t \mathbf{C}_{i.}|\boldsymbol{\beta}_j \big)}{\partial \beta_{jv}} \frac{x_{ij}-\mu_{ij}}{1+\frac{\mu_{ij}}{\theta_j}}$$
...
$$ \frac{\partial^2 l_{NB}(X_{.j}|\boldsymbol{\beta}_j, \theta_j)}{\partial \beta_{jv} \partial \beta_{jv'}} = \sum_{i=1}^n \frac{\psi s_j}{1+\frac{\mu_{ij}}{\theta_j}} \Big( \frac{\partial^2 f \big( \boldsymbol{\alpha}^t \mathbf{C}_{i.}|\boldsymbol{\beta}_j \big)}{\partial \beta_{vj} \partial \beta_{v'j}} (x_{ij} - \mu_{ij}) + \frac{\partial f \big( \boldsymbol{\alpha}^t \mathbf{C}_{i.}|\boldsymbol{\beta}_j \big)}{\partial \beta_{vj}} \frac{\partial f \big( \boldsymbol{\alpha}^t \mathbf{C}_{i.}|\boldsymbol{\beta}_j \big)}{\partial \beta_{v'j}} \psi s_j \big(\frac{\mu_{ij}(1-\theta_j(1+\mu_{ij}))-x_{ij}}{\theta_j+\mu_{ij}} \big)  \Big)$$
...
We would again first estimate the independence model ($u_i$ and $u_j$) as before, and then estimate one dimension $k$ at the time, conditional on the previous ones. $\mathbf{\psi}$ and $\mathbf{s}$ will be estimated as before. $\boldsymbol{\alpha}$ could be estimated by ML as before, but in order to ensure maximum separation of the response functions of all the species we will consider a version of the likelihood ratio approach from Zhu _et al._ (2005):
...
with $p^{(\alpha)}$ and $p^{(\alpha)}_j$ estimated probability density functions without and with taxon labels. $p^{(\alpha)}$ and $p^{(\alpha)}_j$ are calculated as the expected counts given $\boldsymbol{\alpha}$ under a certain error distribution (e.g. negative binomial): $p_j^{(\alpha)}(\boldsymbol{\alpha}^T \mathbf{c}_i) = e_{ij}$ and $p^{(\alpha)}(\boldsymbol{\alpha}^T \mathbf{c}_i) = \frac{\sum_{j=1}^p e_{ij}}{p} = e_{i.}$ (averaging over the taxa).
...
The estimation would then proceed as follows:
 ...
 2. Find initial values for $\psi_k$ and $s_{jk}$ as before.
 3. Iterate between
...
4. Estimate $\psi_k$ and $s_{jk}$ using ML
...
$$\boldsymbol{\alpha}^t\boldsymbol{\alpha} = \mathbf{1}$$
...
estAlpha = function(Alpha, X, CC, d, ncols, responseFun = c("linear","quadratic","nonparametric"), muMarg, thetas, cReg, alphaK, control.outer = list(trace=TRUE), control.optim=list(trace=TRUE), nleqslv.control = list() ){
  ...
  alphaNew = constrOptim.nl(par = Alpha, fn =LR_nb, gr = NULL, heq = heq_nb, heq.jac = heq_nb_jac, alphaK = alphaK, X=X, CC=CC, responseFun = c("linear"), muMarg = muMarg, thetas = thetas, cReg = cReg, d = d, ncols=ncols, control.outer = control.outer, control.optim = control.optim, nleqslv.control = nleqslv.control) #
}
#--------------------------------------#
# A linear equality constraints function for estAlpha (see ?alabama)
heq_nb = function(Alpha, alphaK, d, ...){
  center = sum(Alpha)
  size = sum(Alpha^2)-1
  if(is.null(alphaK)) { return(c(center, size))}
  ortho = alphaK %*% Alpha
  c(center, size, ortho)
}
# It's jacobian, returns a matrix with d columns and at least 2 rows
heq_nb_jac = function(Alpha, alphaK, d, ...){
  center = rep.int(1,d)
  size = 2*Alpha
  if(is.null(alphaK)) {return(rbind(center, size))}
  ortho = alphaK
...
testRCMconstr = RCM_NB(X = X, k = 2, covariates = regMat, rowWeights)
```
