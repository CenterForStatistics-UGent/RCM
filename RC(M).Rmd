...
Palette=rainbow(length(unique(dataSam$colourPlot)))
    ...
    plot = ggplot(dataSam, aes_string(x=names(dataSam)[1], y=names(dataSam)[2], col = "colourPlot", dotList)) + 
      ...
      if(is.null(samColour)) {guides(color=FALSE)} 
    #add legend names
...
contrTaxa = function(rowMatPsi, colMat, groupFactor, idTax, sigFrac = 0.5, Dim = 1:3){
  ...
  sigContr = sapply(seq_along(levels(groupFactor)), function(i){
    mean(contrib[match(rownames(idTax), colnames(colMat), nomatch = 0),i]*idTaxNum[,i])
  }) # The mean contribution of the taxa with signal
  allContr = sapply(seq_along(levels(groupFactor)), function(i){
    mean(
      apply(contrib[!colnames(colMat) %in% rownames(idTax),],2,sort, decreasing = TRUE)[seq_len(round(sigFrac*(ncol(colMat)-nrow(idTax)))), ])
...
CodaResList0 = mclapply(datList0mat, CoDa, mc.cores = cores)
...
#Hellinger distance. Actually this is also an svd technique, so also yields taxon scores
...
contrTaxaWrap = function(resListRows, resListCols, IDlist, groupFactor){
dat = mapply(resListRows, resListCols, IDlist, SIMPLIFY = TRUE, FUN = function(row, col, id){
  mapply(row, col, SIMPLIFY = TRUE, MoreArgs = list(idTax = id, groupFactor = groupFactor), FUN = contrTaxa)
})
as.data.frame(t(dat))
}
#-----------------------------#
...
```{r NBrelatedDistrConf, purl = FALSE, eval = FALSE}
p = 1000 #Number of taxa
n = 60 #Number of samples
nPop = 4 #Number of underlying populations
nPerPop = n/nPop #Number of samples per population
...
contrHum = contrTaxaWrap(resListSimSeqHumRows, resListSimSeqHumCols, IDlistHum, groupFactorHum)
contrPlot(contrHum)
```
