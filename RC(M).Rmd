...
The underlying motivation is to develop an unconstrained visualization technique for the microbiome that does not suffer from the defects of currently applied techniques such as PCA, PCoA with Bray-Curtis distance etc. These methods are in particular sensitive to differences in library size.
...
$$\sum_{i=1}^nx_{i.}w_ir_{k'i} = I(k=k')$$
...
Goodman proposes to use $w_i = x_{i.}$ and $z_j = x_{.j}$ thus uses _weighted_ constraints, to retain the relationship with correspondence analysis (see below).

Our intuitive idea to go along with the biological nature of the data was to use uniform weights for the samples ($w_i = 1/n$) but marginal weights for the taxa ($z_j = x_{.j}$). This way we acknowledge the significance of the mean abundance as a relevant measure but the library sizes as a technical artefact.

But actually, does it make sense that the _weighted_ scores are orthogonal, if we fit and plot the unweighted scores in the end?

Becker _et al._ (1989) recommend using uniform weights not to let the marginal distribution affect the model fit. In our case again it makes intuitive sense to make the end result depend on the taxa composition and thus use marginal weights for the taxa.

#### RC(M) margins

The most logical choices for the margins have long seemed to use $u=-log(x_{..}$, $u_i = log(x_{i.})$ and $u_j = log(x{.j})$.

However, the library sizes do not correspond to the maximum likelihood estimate of $u_i$ under the Negative Binomial model. As a result the first dimensional row scores $r_{1i}$ tried to correct for this effect. As a result they were related (linearly correlated) to the library sizes, which we want to avoid absolutley. To avoid this we set $u=0$ and estimate the $u_i$'s, $u_j$'s
and overdispersions iteratively using maximum likelihood in a first step (this converges very quickly).
, and then we'll have to iterate between fitting the overdispersions, the imporance parameters $\psi$, the $r's$ and the $s's$. 
...
RCM_NB_1B1 = function(X, k, rowWeights, colWeights, weightsChar, tol = 1e-3, maxItOut = 500, Psitol = 1e-3, verbose = TRUE, NBRCM=NULL, global ="dbldog", nleqslv.control=list(), method=c("Broyden"), dispFrec=10, convNorm = 2, prior.df=10, marginEst = "marginSums"){
...
MstepZIP_1B1 = function(Z, X, rMat, cMat, tMat, vMat,  k, muMarg,  zeroMarg, psis, chis, lambdaCol, lambdaRow, lambdaColZero, lambdaRowZero, nLambda, rowWeights, colWeights, rMatK, cMatK, vMatK, tMatK, twoCores=TRUE, tol=1e-3, psiTol = 1e-4, chiTol = psiTol, convNorm = 2, maxItMean=5, maxItZeroes= 20, n=n, p=p, global=global, nleqslv.control= nleqslv.control){
...
resList = mclapply(mc.cores=2, c(meanEstZIP_1B1, ZIestZIP_1B1), function(fun){
  ...
  resList = lapply(c(meanEstZIP_1B1, ZIestZIP_1B1), function(fun){ fun(X=X, rMat=rMat, cMat=cMat, tMat=tMat, chis=chis, vMat=vMat, zeroMarg = zeroMarg, lambdaCol=lambdaCol, lambdaRow=lambdaRow, lambdaRowZero=lambdaRowZero, lambdaColZero=lambdaColZero, psiTol=psiTol, chiTol=chiTol, tol=tol, convNorm = convNorm, nleqslv.control = nleqslv.control, global=global, nLambda=nLambda, k=k, Z=Z, muMarg=muMarg,n=n, p=p, psis=psis, maxItMean = maxItMean, maxItZeroes = maxItZeroes, rowWeights=rowWeights, colWeights=colWeights, rMatK = rMatK, cMatK = cMatK, tMatK = tMatK, vMatK = vMatK)})
  }#, meanEstZIP_1B1
...
ZIestZIP_1B1 = function(X, Z, muMarg, k, global, nleqslv.control, tol, chiTol, tMat, vMat, chis, zeroMarg,lambdaColZero, lambdaRowZero, convNorm,n, p, nLambda, psis, rowWeightsZeroNum, colWeightsZeroNum, rMatK, cMatK, tMatK, vMatK, maxItMean = 10, maxItZeroes = 10,...){
...
# Chis
  ...
  chisSol = try(sort(abs(nleqslv(fn = dZipMeanChi_1B1, x = chis, reg=regChis, Z=Z, global=global, control = nleqslv.control, zeroMarg = zeroMarg, jac=ZipJacobianChi_1B1)$x), decreasing=TRUE), silent=TRUE)
  if(!inherits(chisSol,"try-error")){
    chis=chisSol
  }
    ...
    cat("Estimating row scores zeroes \n")
    regRowZero = vMat*chis
    tMatSol = try(nleqslv(fn = dZipMeanTmat_1B1, x = c(tMat, lambdaRowZero),  n=n,k=k, reg=regRowZero, global=global, control = nleqslv.control, zeroMarg = zeroMarg, jac=ZipJacobianTmat_1B1, Z=Z, nLambda=nLambda, rowWeights = rowWeightsZeroNum, tMatK = tMatK)$x, silent=TRUE)
  if(!inherits(tMatSol,"try-error")){
    ...
    vMatSol = try(nleqslv(fn = dZipMeanVmat_1B1, x = c(t(vMat), lambdaColZero), reg=regColZero, p=p,k=k, global=global, control = nleqslv.control, zeroMarg = zeroMarg, jac=ZipJacobianVmat_1B1, Z=Z, nLambda=nLambda, colWeights=colWeightsZeroNum, vMatK = vMatK)$x, silent=TRUE)
  if(!inherits(vMatSol,"try-error")){
...
#--------------------------------------#
dZipMeanLibsizes_1B1 = function(beta, X, Z, reg){
...
#--------------------------------------#
#A jacobian for the psi parameters
ZipJacobianLibsizes_1B1 = function(beta, X, reg, Z){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param X: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param abunds: a vector of length p with the abundance parameters
...
#--------------------------------------#
dZipMeanAbunds_1B1 = function(beta, X, Z, reg){
  # @param beta: a vector of logged library size estimates
  # @param y: the nxp data matrix
  # @param reg: the current logged abundance estimates
  # @param abunds: a vector of length p with the abundance parameters
...
#--------------------------------------#
#A jacobian for the psi parameters
ZipJacobianAbunds_1B1 = function(beta, X, reg, Z){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param X: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param abunds: a vector of length p with the abundance parameters
...
dZipZeroCol_1B1 = function(beta, Z, reg){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param X: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param theta: a vector of length p with the dispersion parameters
...
#--------------------------------------#
ZipJacobianZeroCol_1B1 = function(beta, Z, reg){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param X: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param theta: a vector of length p with the dispersion parameters
...
#--------------------------------------#
#Estimate the offsets for the zeroes
dZipZeroRow_1B1 = function(beta, Z, reg){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param X: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param theta: a vector of length p with the dispersion parameters
...
#--------------------------------------#
ZipJacobianZeroRow_1B1 = function(beta, Z, reg){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param X: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param theta: a vector of length p with the dispersion parameters
...
#--------------------------------------#
dZipMeanChi_1B1 = function(beta, Z, reg, zeroMarg, nLambda){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param X: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param theta: a vector of length p with the dispersion parameters
...
#--------------------------------------#
#A jacobian for the psi parameters
ZipJacobianChi_1B1 = function(beta, Z, reg, zeroMarg, nLambda){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param X: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param theta: a vector of length p with the dispersion parameters
...
#--------------------------------------#
dZipMeanTmat_1B1 = function(beta, reg, k,n, Z, zeroMarg, nLambda, rowWeights, tMatK){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param X: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param theta: a vector of length p with the dispersion parameters
...
#--------------------------------------#
#A jacobian for the psi parameters
ZipJacobianTmat_1B1 = function(beta, reg, k, n, Z, zeroMarg, nLambda, rowWeights, tMatK){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param X: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param k: a scalar, dimension of the RC solution
...
#--------------------------------------#
dZipMeanVmat_1B1 = function(beta, reg, k, p, Z, zeroMarg, nLambda, colWeights, vMatK){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param X: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param theta: a vector of length p with the dispersion parameters
...
#--------------------------------------#
#A jacobian for the psi parameters
ZipJacobianVmat_1B1 = function(beta, reg, k, p, Z, zeroMarg, nLambda, colWeights, vMatK){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param X: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param k: a scalar, dimension of the RC solution
...
RCM_ZIP_1B1 = function(X, k, rowWeights, colWeights, weightsChar, tol = 1e-3, maxItOut = 500, psiTol = 1e-4, chiTol=psiTol, verbose = TRUE, global ="dbldog", nleqslv.control=list(), method="Broyden", twoCores=FALSE, convNorm = 2,  maxItMean = 20, maxItZeroes=30, ZIPRCM=NULL){
  ...
  regZero = outer(logitZeroRows, logitZeroCols, "+")
  Z[X==0] = (1+exp(-regZero-expMu))[X==0]^(-1)
 ...
 libsTmp = try(nleqslv(fn = dZipMeanLibsizes_1B1, x = logLibSizesMLE, X = X, reg=logAbundsMLE, global=global, control = nleqslv.control, jac=ZipJacobianLibsizes_1B1, method=method, Z=Z)$x, silent=TRUE)
 if(class(libsTmp)!="try-error"){ logLibSizesMLE = libsTmp}
  absTmp = try(nleqslv(fn = dZipMeanAbunds_1B1, x = logAbundsMLE, X = X, reg=logLibSizesMLE, global=global, control = nleqslv.control, jac=ZipJacobianAbunds_1B1, method=method, Z=Z)$x, silent=TRUE)
 if(class(absTmp)!="try-error"){ logAbundsMLE = absTmp}
  ...
  zeroRowsTmp = try(nleqslv(fn = dZipZeroRow_1B1, x = logitZeroRows,   reg=logitZeroCols, global=global, control = nleqslv.control, jac=ZipJacobianZeroRow_1B1, method=method, Z=Z)$x, silent=TRUE)
 if(class(zeroRowsTmp)!="try-error"){ logitZeroRows = zeroRowsTmp}
  zeroColsTmp = try(nleqslv(fn = dZipZeroCol_1B1, x = logitZeroCols, reg=logitZeroRows, global=global, control = nleqslv.control, jac=ZipJacobianZeroCol_1B1, method=method, Z=Z)$x, silent=TRUE)
 if(class(zeroColsTmp)!="try-error"){ logitZeroCols = zeroColsTmp}
  ...
  #Otherwise try to use intelligent starting values
      ...
      #Depending on the weighting schemes, use other starting values
   svdX = svd(diag(1/libSizes) %*% (X-muMarg)*(1-zeroMarg) %*% diag(1/colSums(X)))#switch(weightsChar,
#                 "marginalmarginal" = svd(diag(1/libSizes) %*% (X-muMarg) %*% diag(1/colSums(X))),
#                 "marginaluniform" = svd(diag(1/libSizes) %*% (X-muMarg)),
#                 "uniformmarginal" = svd((X-muMarg) %*% diag(1/colSums(X))),
#                 "uniformuniform" = svd(X-muMarg))
...
tMat = apply(tMat, 2, function(rowS){
  ...
  if(!convergence ){
    ...
    return(list(converged = convergence,rMat=rMat, cMat=cMat, psis = psis, X=X,
                rowRec = rowRec, colRec = colRec, psiRec = psiRec, lambdaRow = lambdaRow, lambdaCol = lambdaCol, lambdaRowZero = lambdaRowZero, lambdaColZero = lambdaColZero, chis = chis, tMat = tMat, vMat = vMat, zeroMarg = zeroMarg, chiRec = chiRec, rowRecZero = rowRecZero, colRecZero = colRecZero, iter=iterOut-1, Z=Z, fit="RCM_ZIP_1B1"))
}
```
...
RCM = function(dat, k, round=FALSE, method= c("NB","NB_1B1", "ZIP","ZIP_1B1", "ZINB"), prevCutOff=0.01, rowWeights = "uniform", colWeights = "marginal",...){
  ...
  dat = if (taxa_are_rows(dat)) t(otu_table(dat)@.Data) else otu_table(dat)@.Data
...
plotRCM = function(RCM, X = NULL, thetas = NULL, abunds = NULL, arrowFrac = 0.04, biplot = TRUE,
                   libLoc ="topright", libInset = c(-0.7,-0.1), libCex = 0.85, libInterSp = 0.75, samColour=NULL,
...
Sample plots with the libsizes in colour. The first abbreviation refers to the weighting scheme for the rows(samples), the second to the weighting scheme for the columns. E.g "margunif" means $w_i = x_{i.}$ and $z_j = 1/p$. The "MLE"" epitheton indicates that the independence model was estimated by MLE.

```{r SamplePlotsNoSignal, results='hide'}
par(mfrow=c(2,4))
#unifmarg
lapply(names(solListNS), function(Y){with(solListNS[[Y]], {
  ...
  ggplot(data=dfCol, aes(x=Dim1, y=Dim2, col=col)) +geom_point(size=2) +ggtitle(Y) + scale_colour_continuous(name="Library sizes")
    })
  })
```
...
Weighting scheme for colscore normalization does not affect the samples plot, but the centering weights do. If we estimate the library sizes by MLE the trend seems to disappear. Plot the loadings as functions of the library sizes. 

Dimension 1

```{r loadings vs. library sizes dim1, results='hide'}
par(mfrow=c(2,4))
...
In all weighing schemes, the row scores of the first dimension increase with library size (not in absolute values but they form a gradient). This is likely a result of a discrepancy between the MLE estimates of the sequencing depths under the NB model and the library size (row sums of our count table $\sum_{j=1}^px_{ij}$, which are also just an _estimate_ of the sequencing depth). The first dimension of the MLE row scores estimates then tries to correct for this aberration so that its effect disappears in the higher dimensions. We plot the estimates of the sequencing depths of the two different estimators
...
The taxon plots of the fits with uniform weights for the centering of the taxa are used ($\sum_{j=1}^ps_{kj} = 0$) suffer from outliers when the abundances are estimated as column sums. For the other weighting schemes it is hard to establish anay effect for now. Contrarily to my expectations it are the weights for the centering rather than the normalization requirement that cause the outliers. With MLE estimation of the abundances, the outliers are gone!
...
#Abundances
par(mfrow=c(2,4))
sapply(names(solListNS), function(Y){with(solListNS[[Y]], {plot(main=Y,cMat[1,], colSums(X), log="y", cex=0.5, ylab="Mean  abundances",xlab="Dim1", sub = paste0("Cor = ",round(cor(cMat[1,], colSums(X)),2)))})})
```

Marginal centering weights causes the column scores to correlate with the abundances

```{r taxonScesvsAbunds23D, results='hide'}
par(mfrow=c(2,4))
lapply(names(solListNS), function(Y){with(solListNS[[Y]], {plot(main=Y,cMat[2,], colSums(X), log="y", cex=0.5, ylab="Mean  abundances",xlab="Dim2")})})
par(mfrow=c(2,4))
lapply(names(solListNS), function(Y){try(with(solListNS[[Y]], {plot(main=Y,cMat[3,], colSums(X), log="y", cex=0.5, ylab="Mean  abundances",xlab="Dim3")}))})
```
...
par(mfrow=c(2,4))
lapply(names(solListNS), function(Y){with(solListNS[[Y]], {plot(main=Y, cMat[1,], thetas, log="y", cex=0.5, ylab="Overdispersion", xlab="Dim1")})})
par(mfrow=c(2,4))
lapply(names(solListNS), function(Y){with(solListNS[[Y]], {plot(main=Y, cMat[2,], thetas, log="y", cex=0.5, ylab="Overdispersion", xlab="Dim2")})})
```
...
##### Influence function (very heavy so not evaluated)

Take a look at the influence function values for the first psi parameters

```{r NS influence measures: psis, results="hide", eval=FALSE}
...
par(mfrow=c(3,5))
lapply(names(infl1), function(x){
  IdTmp = sample(seq_along(infl1[[x]]$abunds), 200)
  with(infl1[[x]], plot(rep(abunds[IdTmp], nrow(infl)), c(t(infl[,IdTmp])), log="x", main=x, ylab = "Influence",xlab = "Abundance")) #Very heavy!
})
par(mfrow=c(3,5))
lapply(names(infl1), function(x){
  IdTmp = sample(seq_along(infl1[[x]]$libsizes), 100)
  with(infl1[[x]], plot(rep(libsizes[IdTmp], ncol(infl)), c(infl[IdTmp,]), log="x", main=x, col=(libsizes==libsizes[IdTmp])+1, ylab = "Influence", xlab = "Library sizes")) #Very heavy!
})
```
...
```{r X vs Infl(X), results="hide", eval=FALSE}
#Expectations
par(mfrow=c(3,5))
lapply(names(infl1), function(x){
plot(solListNS[[x]]$X, infl1[[x]]$infl, ylab= "Influence", xlab = "Counts")
})
```

Versus expected abundances

```{r Infl vs observed abundances, results="hide", eval=FALSE}
par(mfrow=c(3,5))
lapply(names(infl1), function(x){
plot(outer(rowSums(solListNS[[x]]$X), colSums(solListNS[[x]]$X)/sum(solListNS[[x]]$X)), infl1[[x]]$infl, ylab = "Influence", xlab = "Expected counts")
})
```

Versus Pearson residuals

```{r vs Pearson residuals, results="hide", eval=FALSE}
par(mfrow=c(3,5))
lapply(names(infl1), function(x){
plot(sqrt(diag(1/rowSums(solListNS[[x]]$X)))%*%(outer(rowSums(solListNS[[x]]$X), colSums(solListNS[[x]]$X)/sum(solListNS[[x]]$X))-solListNS[[x]]$X)%*%sqrt(diag(1/colSums(solListNS[[x]]$X))), infl1[[x]]$infl, ylab = "Influence", xlab = "Pearson residuals")
})
```
...
if(!file.exists("toyDataSigab.RData")){
  ...
  syntNBSigmargmarg_1B1_ab = mccollect(syntNBSigmargmarg_1B1_abJob, FALSE)[[1]] #Check
  ...
  syntNBSigunifunif_1B1_ab = mccollect(syntNBSigunifunif_1B1_abJob, FALSE)[[1]] #Check
 ...
 testAbJob2unif = mcparallel(RCM(dataMatSigNBab, method="NB_1B1", k=3, nleqslv.control= nleqslv.control.ab, maxItOut=1e3, rowWeights = "uniform", colWeights = colWeightsTestAb2, prevCutOff=0.01))
  ...
  #More realistic libsizes
      testAbJobLibs = mcparallel(RCM(dataMatSigNBabag, method="NB_1B1", k=3, nleqslv.control= nleqslv.control.ab, maxItOut=1e3, rowWeights = "uniform", colWeights = "marginal", prevCutOff=0.01))
...
```{r abSignalPlots, results='hide', eval=FALSE}
cols = c("grey", "red","blue","purple","green","brown","cyan","black", "orange", "magenta","yellow","pink")
palette(cols)
par(mfrow=c(2,4))
lapply(names(solListWSab), function(Y){plotRCM(solListWSab[[Y]], samColour = sampleSigNBab, main=Y, biplot=FALSE, Dim = c(1,2), libInset=c(-0.7,0))})
```

```{r abSignalPlots23D, results='hide', eval=FALSE}
par(mfrow=c(2,4))
...
```{r abSignalPlots 13D, results='hide', eval=FALSE}
par(mfrow=c(2,4))
...
lapply(names(solListWSab), function(Y){with(solListWSab[[Y]], {
  ...
  ggplot(data=dfCol, aes(x=Dim1, y=Dim2, col=col)) +geom_point(size=3) +ggtitle(Y) + scale_colour_continuous(name="Library sizes")
    })
  })
```
...
```{r ab libsizes, eval=FALSE, results='hide', eval=FALSE}
#Look at the loadings in function of the library sizes and abunds
...
```{r ab biplots, results='hide', eval=FALSE}
#par(mfrow=c(2,4), mai = c(2,2,2,4))
par(mfrow=c(2,4))
lapply(names(solListWSab), function(Y){plotRCM(solListWSab[[Y]], samColour = sampleSigNBab, main=Y, biplot=TRUE, Dim = c(1,2), taxColour=taxaSigNBab, taxLegPos = "bottomright", abundLeg=TRUE, taxCol  =cols, taxInset = c(-1.35,-0.8), libCex = 0.65, libInset = c(-0.6,-0.3), arrowFrac = 0.05)})
```
...
if(!file.exists("toyDataSigLU.RData")){
    ...
    syntNBSigmargmarg_1B1_LUJob = mcparallel(RCM(dataMatSigNBLU, method="NB_1B1", k=2, nleqslv.control= nleqslv.control.lu, maxItOut=2e4, prevCutOff=0.01, colWeights="marginal", rowWeights = "marginal", marginEst="MLE"))
  syntNBSigmargmarg_1B1_LU = mccollect(syntNBSigmargmarg_1B1_LUJob, FALSE)[[1]] 
  
      syntNBSigunifmarg_1B1_LUJob = mcparallel(RCM(dataMatSigNBLU, method="NB_1B1", k=2, nleqslv.control= nleqslv.control.lu, maxItOut=2e4, prevCutOff=0.01, colWeights="marginal", rowWeights = "uniform", marginEst="MLE"))
  syntNBSigunifmarg_1B1_LU = mccollect(syntNBSigunifmarg_1B1_LUJob, FALSE)[[1]] 
  
      syntNBSigmargunif_1B1_LUJob = mcparallel(RCM(dataMatSigNBLU, method="NB_1B1", k=2, nleqslv.control= nleqslv.control.lu, maxItOut=2e4, prevCutOff=0.01, rowWeights="marginal", colWeights = "uniform", marginEst="MLE"))
  syntNBSigmargunif_1B1_LU = mccollect(syntNBSigmargunif_1B1_LUJob, FALSE)[[1]] 
  
      syntNBSigunifunif_1B1_LUJob = mcparallel(RCM(dataMatSigNBLU, method="NB_1B1", k=2, nleqslv.control= nleqslv.control.lu, maxItOut=2e4, prevCutOff=0.01, colWeights="uniform", rowWeights = "uniform", marginEst="MLE"))
  syntNBSigunifunif_1B1_LU = mccollect(syntNBSigunifunif_1B1_LUJob, FALSE)[[1]] 
...
#   colWeightsTestLU3 = cbind(margWeightsLU, unifWeightsLU, unifWeightsLU)
      ...
      syntNBSigmargmarg_1B1_LUJobMLE = mcparallel(RCM(dataMatSigNBLU, method="NB_1B1", k=3, nleqslv.control= nleqslv.control.lu, maxItOut=2e3, prevCutOff=0.01, colWeights="marginal", rowWeights = "marginal", marginEst = "MLE"))
  syntNBSigmargmarg_1B1_LUMLE = mccollect(syntNBSigmargmarg_1B1_LUJobMLE, FALSE)[[1]] 
  
      syntNBSigunifmarg_1B1_LUJobMLE = mcparallel(RCM(dataMatSigNBLU, method="NB_1B1", k=3, nleqslv.control= nleqslv.control.lu, maxItOut=2e3, prevCutOff=0.01, colWeights="marginal", rowWeights = "uniform", marginEst = "MLE"))
  syntNBSigunifmarg_1B1_LUMLE = mccollect(syntNBSigunifmarg_1B1_LUJobMLE, FALSE)[[1]] 
  
      syntNBSigmargunif_1B1_LUJobMLE = mcparallel(RCM(dataMatSigNBLU, method="NB_1B1", k=3, nleqslv.control= nleqslv.control.lu, maxItOut=2e3, prevCutOff=0.01, rowWeights="marginal", colWeights = "uniform", marginEst = "MLE"))
  syntNBSigmargunif_1B1_LUMLE = mccollect(syntNBSigmargunif_1B1_LUJobMLE, FALSE)[[1]] 
  
      syntNBSigunifunif_1B1_LUJobMLE = mcparallel(RCM(dataMatSigNBLU, method="NB_1B1", k=3, nleqslv.control= nleqslv.control.lu, maxItOut=2e3, prevCutOff=0.01, colWeights="uniform", rowWeights = "uniform", marginEst = "MLE"))
  syntNBSigunifunif_1B1_LUMLE = mccollect(syntNBSigunifunif_1B1_LUJobMLE, FALSE)[[1]] 
 
  save(dataMatSigNBLU, syntNBSigunifmarg_1B1_LU, syntNBSigunifunif_1B1_LU,syntNBSigmargmarg_1B1_LU,syntNBSigmargunif_1B1_LU, sampleSigNBLU, taxaSigNBLU,  file="toyDataSigLU.RData")#syntNBSigunif, syntNBSigmarg, syntNBSigmarg_3,testLU1, testLU2, testLU3, testLU1unif, testLU2unif, testLU3unif,testLUInv,testLULibs,
...
# solListWSLU = list("unifunif" = syntNBSigunifunif_1B1_LU, "unifmarg" = syntNBSigunifmarg_1B1_LU, "margunif" = syntNBSigmargunif_1B1_LU, "margmarg" = syntNBSigmargmarg_1B1_LU, "unifmargunifRmarg" = testLU1,  "margmargunifRmarg"=testLU2, "margunifunifRmarg" = testLU3, "unifmargunifRunif" = testLU1unif, "margmargunifRunif"=testLU2unif, "margunifunifRunif" = testLU3unif, "inverse"= testLUInv, "testEqualLibs" =   testLULibs, "unifunifMLE" = syntNBSigunifunif_1B1_LUMLE, "unifmargMLE" = syntNBSigunifmarg_1B1_LUMLE, "margunifMLE" = syntNBSigmargunif_1B1_LUMLE, "margmargMLE" = syntNBSigmargmarg_1B1_LUMLE )
...
The most influential observations are zero counts in highly LUundant species and high libsizes (i.e. high expectations)! Note that we cen derive this from the influence function that does not even depend on the weights!
...
tolAchPsi = with(syntNBSigmargmarg_1B1_LU, lapply(1:k, function(y){sapply( 2:iter[y], function(x){LUs(1-psiRec[y,x]/psiRec[y,x-1])})}))
par(mfrow=c(1,3))
lapply(tolAchPsi, function(x){plot(x, log="y");LUline(h=0.01, col="red");LUline(h=0.001, col="blue")})

with(syntNBSigmargmarg_1B1_LU,plot(psiRec[3,1:iter[3]])) #Weird jumps, don't stop too early!
```
...
if(!file.exists("syntZIPnoSig.RData")){
  ...
  syntZIPnoSigUnifMargJob = mcparallel(RCM(dataMatZIPnoSig, method = "ZIP_1B1",k=2, nleqslv.control = nleqslv.controlZIP, colWeights = "marginal", rowWeights = "uniform", maxItOut=5e1))
  ...
  syntZIPnoSigMargUnif =  mccollect(syntZIPnoSigMargUnifJob, FALSE)[[1]]
   ...
   syntZIPnoSigMargMargJob = mcparallel(RCM(dataMatZIPnoSig, method = "ZIP_1B1",k=2, nleqslv.control = nleqslv.controlZIP, colWeights = "marginal", rowWeights = "marginal", maxItOut=5e1))
