...
dNBllcol_1B1 = function(beta, X, reg, thetas, muMarg, k, p, n, colWeights, nLambda, cMatK) {
  ...
  lambda3 = ifelse(k==1, 0,beta[(p+3):length(beta)])
...
dNBllrow_1B1 = function(beta, X, reg, thetas, muMarg, k, n , p, rowWeights, nLambda, rMatK) {
  ...
  lambda3 = ifelse(k==1, 0,beta[(n+3):length(beta)])
...
MstepZIP_1B1 = function(Z, X, rMat, cMat, tMat, vMat,  k, muMarg,  Zcol, psis, chis, lambdaCol, lambdaRow, lambdaColZero, lambdaRowZero, nLambda, rowWeights, colWeights, rMatK, cMatK, twoCores=TRUE, tol=1e-3, psiTol = 1e-4, chiTol = psiTol, convNorm = 2, maxItMean=5, maxItZeroes= 20, n=n, p=p, global=global, nleqslv.control= nleqslv.control){
  
#Optimization of the mena and zero-inflated components are independent (see Lambert 1992), so fork here
if(twoCores){
resList = mclapply(mc.cores=2, c(meanEstZIP_1B1, ZIestZIP_1B1), function(fun){
  fun(X=X, rMat=rMat, cMat=cMat, tMat=tMat, chis=chis, vMat=vMat, Zcol=Zcol, lambdaCol=lambdaCol, lambdaRow=lambdaRow, lambdaRowZero=lambdaRowZero, lambdaColZero=lambdaColZero, psiTol=psiTol, chiTol=chiTol, tol=tol, convNorm = convNorm, nleqslv.control = nleqslv.control, global=global, nLambda=nLambda, k=k, Z=Z, muMarg=muMarg,n=n, p=p, psis=psis, maxItMean = maxItMean, maxItZeroes = maxItZeroes, rowWeights=rowWeights, colWeights=colWeights, rMatK = rMatK, cMatK = cMatK)
})
} else{
  resList = lapply(c(meanEstZIP_1B1, ZIestZIP_1B1), function(fun){ fun(X=X, rMat=rMat, cMat=cMat, tMat=tMat, chis=chis, vMat=vMat, Zcol=Zcol, lambdaCol=lambdaCol, lambdaRow=lambdaRow, lambdaRowZero=lambdaRowZero, lambdaColZero=lambdaColZero, psiTol=psiTol, chiTol=chiTol, tol=tol, convNorm = convNorm, nleqslv.control = nleqslv.control, global=global, nLambda=nLambda, k=k, Z=Z, muMarg=muMarg,n=n, p=p, psis=psis, maxItMean = maxItMean, maxItZeroes = maxItZeroes, rowWeights=rowWeights, colWeights=colWeights, rMatK = rMatK, cMatK = cMatK)})
  }
...
meanEstZIP_1B1 = function(X, rMat, cMat, Z, muMarg, k, global, nleqslv.control, tol, psiTol,lambdaCol, lambdaRow, convNorm,  nLambda, n, p, psis, rowWeights, colWeights, maxItMean = 10, maxItZeroes = 10, rMatK, cMatK,...){
 ...
 cat("\n Estimating psis (k=",k,") \n")
  ...
  if(class(rMatSol)!="try-error"){
  rMat = matrix(rMatSol[1:n], byrow=FALSE, ncol=k, nrow=1)
...
rMat = rMat/sqrt(sum(rowWeights * rMat^2))
  ...
  cat("Estimating column scores mean \n")
  regCol = t(t(rMat)*psis)
  cMatSol = try(nleqslv(fn = dZipMeanCmat_1B1, x = c(t(cMat), lambdaCol), X=X, reg=regCol, muMarg=muMarg, p=p, k=k, global=global, control = nleqslv.control, jac=ZipJacobianCmat_1B1, Z=Z, nLambda=nLambda, colWeights=colWeights)$x, silent=TRUE)
    if(class(cMatSol)!="try-error"){
  cMat = matrix(cMatSol[1:p], byrow=TRUE, nrow=k, ncol=1)
...
ZIestZIP_1B1 = function(X, Z, muMarg, k, global, nleqslv.control, tol, chiTol, tMat, vMat, chis, Zcol,lambdaColZero, lambdaRowZero, convNorm,n, p, nLambda, psis, rowWeightsZeroNum, colWeightsZeroNum, rMatK, cMatK, maxItMean = 10, maxItZeroes = 10,...){
...
# Chis
  ...
  if(!inherits(chisSol,"try-error")){
    ...
    cat("Estimating row scores zeroes \n")
    regRowZero = vMat*chis
    tMatSol = try(nleqslv(fn = dZipMeanTmat_1B1, x = c(tMat, lambdaRowZero),  n=n,k=k, reg=regRowZero,p=p, global=global, control = nleqslv.control, Zcol=Zcol, jac=ZipJacobianTmat_1B1, Z=Z, nLambda=nLambda, rowWeights=rowWeightsZeroNum, rMatK = rMatK)$x, silent=TRUE)
  if(!inherits(tMatSol,"try-error")){
    tMat = matrix(tMatSol[1:n], byrow=FALSE, ncol=1, nrow=n)
    lambdaRowZero = tMatSol[(k+1):(p+nLambda)]
      }
    
    ## Column scores
    cat("Estimating column scores zeroes \n")
    regColZero = t(t(tMat)*chis)
    vMatSol = try(nleqslv(fn = dZipMeanVmat_1B1, x = c(t(vMat), lambdaColZero), reg=regColZero, p=p,k=k, global=global, control = nleqslv.control, Zcol=Zcol, jac=ZipJacobianVmat_1B1, Z=Z, nLambda=nLambda, colWeights=colWeightsZeroNum, cMatK = cMatK)$x, silent=TRUE)
  if(!inherits(vMatSol,"try-error")){
...
dZipMeanRmat_1B1 = function(beta, X, reg, muMarg, n, k, Z, nLambda, rowWeights, rMatK){
  ...
  score = c(t(
    tcrossprod(reg ,(1-Z)*(X-mu))) + 
                        rowWeights*(t(lambda1 + lambda2*2*t(rMat)) + (rMatK %*% lambda3))
    )
  center = sum(rMat*rowWeights)
  unitSum = sum(rMat^2*rowWeights)-1
  if(k==1){return(c(score,center, unitSum))}
...
#--------------------------------------#
#A jacobian for the psi parameters
ZipJacobianRmat_1B1 = function(beta, X, reg, muMarg, n,k, Z, nLambda, rowWeights, rMatK){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param X: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param theta: a vector of length p with the dispersion parameters
...
#--------------------------------------#
dZipMeanCmat_1B1 = function(beta, X, muMarg, p,k, Z, nLambda, colWeights, reg){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param X: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param theta: a vector of length p with the dispersion parameters
...
#--------------------------------------#
#A jacobian for the psi parameters
ZipJacobianCmat_1B1 = function(beta, X, psis, rMat, colWeights, k, p, muMarg, Z, nLambda, reg){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param X: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param theta: a vector of length p with the dispersion parameters
...
dZipMeanTmat_1B1 = function(beta, reg, k,n, Z, Zcol, nLambda, rowWeights, tMatK){
  ...
  score = c(t(t(
    tcrossprod((Z-GZero), reg) + 
                         rowWeights*(t(lambda1 + lambda2*2*t(tMat)) + (tMatK %*% lambda3)
    ) )))
...
#--------------------------------------#
#A jacobian for the psi parameters
ZipJacobianTmat_1B1 = function(beta, reg, k, n, Z, Zcol, nLambda, rowWeights){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param X: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param k: a scalar, dimension of the RC solution
...
#--------------------------------------#
dZipMeanVmat_1B1 = function(beta, reg, k,p, Z, Zcol, nLambda, colWeights){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param X: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param theta: a vector of length p with the dispersion parameters
  ...
  score = c(t(
    crossprod(reg,(Z-muZero/(1+muZero))) + 
                        t(colWeights*t(lambda1 + lambda2*2*vMat + (lambda3 %*% vMat)))
    ))
...
#--------------------------------------#
#A jacobian for the psi parameters
ZipJacobianVmat_1B1 = function(beta, reg, k, p, Z, Zcol, nLambda, colWeights){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param X: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param k: a scalar, dimension of the RC solution
...
tMat = apply(tMat, 2, function(rowS){
  ...
  if(KK>1){
    muMarg = muMarg * exp(rMat[,1:(KK-1), drop=FALSE] %*% (cMat[1:(KK-1),, drop=FALSE]*psis[1:(KK-1)]))
    Zcol = Zcol + tMat[,1:(KK-1), drop=FALSE] %*% (vMat[1:(KK-1),, drop=FALSE]*chis[1:(KK-1)])
    }
...
tic = proc.time() #Time the calculation
  ...
  within(tmp, {
    runtimeInMins = (proc.time()-tic)[3]/60
...
NBmargmargSigPlotDF = RCMres2DF(syntNBSigmargmarg_1B1_3,  sampleSigNB, taxaSigNB[colMeans(dataMatNBSig==0)<0.99])
NBunifmargSigPlotDF_1B1 = RCMres2DF(syntNBSigunifmarg_1B1_3,  sampleSigNB, taxaSigNB[colMeans(dataMatNBSig==0)<0.99])
...
if(!file.exists("toyDataSigab.RData")){
  ...
  save(dataMatNBSig3, syntNBSigunifmarg_1B1_ab, syntNBSigunifunif_1B1_ab,syntNBSigmargmarg_1B1_ab,syntNBSigmargunif_1B1_ab,taxaSigNBab, sampleSigNBab, file="toyDataSigab.RData")#syntNBSigunif, syntNBSigmarg, syntNBSigmarg_3,
