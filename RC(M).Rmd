...
This would force the parameter of a single continuous covariate to be 0, the parameters of two continuous variables to be each other's opposite. In short it is not very desirable.
...
nLambda1s = NROW(centMat)
 ...
 lambdasAlpha = rep(0,nLambda1s +KK)
  ...
  #Modify offset if needed
  if(KK>1){modMat = exp(getRowMatConstr(v = v, NB_params = NB_params[,,KK-1], covariates = covariates, alpha = alpha[, KK-1]))
  ...
  rowMat = getRowMatConstr(v = v, NB_params = NB_params[,,KK], covariates = covariates, alpha = alpha[, KK])
...
# A ggplot2-version
PCbiplot = function(RCMfit, Dim=c(1,2), ..., colour = NULL, colLegend = colour, shapeLegend = NULL, size=3, arrowFrac = 0.025, scalingFactor=NULL, Palette=NULL, arrowCol="black", plotArrows =TRUE, plotLabels = FALSE) {
    #Retrieve dots (will be passed on to aes())
    dotList=list(...)
    constrained = !is.null(RCMfit$covariates)
    if(constrained){
      dataSam <- data.frame(RCMfit$covariates %*% RCMfit$alpha[,Dim]) 
...
Palette=rainbow(length(unique(dataSam$colourPlot)))
    ...
    dataTax = data.frame(
      if(constrained) {t(RCMfit$cMat[Dim,]*RCMfit$psis[Dim])}  else {t(RCMfit$cMat[Dim,])})
    arrowLengths = apply(dataTax,1,function(x){sqrt(sum(x^2))})
    id = arrowLengths >= quantile(arrowLengths,1-arrowFrac)
    if(is.null(scalingFactor)){
    scalingFactor = min(abs(apply(dataSam[, paste0("Dim", Dim)],2, range)))/max(abs(dataTax[id,]))*0.99
    }
    #Filter out small arrows
    dataTax = dataTax[id,] * scalingFactor
    names(dataTax)=paste0("Dim", Dim)
    if(plotLabels) dataTax$labels = rownames(dataTax)
    ...
    if(plotArrows){
    plot <- plot + geom_segment(data=dataTax, aes_string(x=0, y=0, xend="Dim1", yend = "Dim2"), arrow=arrow(length=unit(0.2,"cm")), alpha=0.75, color=arrowCol, show.legend=FALSE)
    } else if(plotLabels){
      plot <- plot + geom_text(data=dataTax, aes_string(x="Dim1", y = "Dim2", label = "labels"),  alpha=0.75, color=arrowCol, show.legend=FALSE)
    } else {}
...
dLR_nb <- function(Alpha, X, CC, responseFun = c("linear", "quadratic", "nonparametric"), cMat, psi, NB_params, NB_params_noLab, d, alphaK, k, centMat, nLambda, nLambda1s, thetaMat, muMarg,  ...){
  ...
  lag = switch(responseFun, #The lagrangian depends on the shape of the response function
               ...
               "quadratic" = psi * (
                 # (crossprod(CC, (X-mu)/(1+mu/thetaMat)) %*% (cMat*NB_params[2,])) + 
                 #   (crossprod(CC, (X-mu)/(1+mu/thetaMat)) %*% (cMat*NB_params[3,])) * alpha - 
...
LR_nb_Jac <- function(Alpha, X, CC, responseFun = c("linear", "quadratic", "nonparametric"), cMat, psi, NB_params, NB_params_noLab, d, alphaK, k, nLambda, nLambda1s, centMat, thetaMat, muMarg, n, ...){
  # @param Alpha: a vector of length d + k*(2+(k-1)/2), the environmental gradient plus the lagrangian multipliers
  # @param X: the n-by-p count matrix
  # @param CC: a n-by-d covariate vector
  # @param responseFun: a character string indicating the type of response function
  ...
  Jac[did,did] = switch(responseFun,
                           ...
                           -psi^2 *(colSums(tensor(vapply(did, FUN.VALUE = tmp, function(x){CC[,x]*tmp}), CC,1,1)*alphaArray) - colSums(tensor(vapply(did, FUN.VALUE = tmp, function(x){CC[,x]*tmp0}), CC, 1, 1) )) ,
...
# Cancer clearly dominates the separation, the second dimension splits normal and small adenoma!
```
...
Cancer turns out to be the main driver of explained variability
