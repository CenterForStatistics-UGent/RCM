...
knitr::opts_chunk$set(cache = TRUE, autodep = TRUE,  warning=FALSE, message=FALSE, echo=TRUE, eval=FALSE, tidy = TRUE, fig.width=9, fig.height=6, root.dir=WD, purl=FALSE, fig.show = "hold")
# The required package list:
reqpkg <- c("phyloseq","MASS", "parallel","nleqslv", "edgeR", "VGAM", "HMP", "ggplot2", "pscl", "alabama", "vegan")
...
with $\mathbf{1}$ a vector of ones of length $d$ and $\mathbb{1}_k$ the identity matrix of dimension $k$. However to maintain maximal comparability with the unconstrained RC(M)-model we could also set 
...
A smarter approach is to iterate between estimating $\alpha$ by optimizing $LR(\alpha)$ given $p_j^{(\alpha)}$ and $p^{(\alpha)}$, and estimating $p_j^{(\alpha)}$ and $p^{(\alpha)}$ given $\alpha$. This results in much fewer estimations of $p_j^{(\alpha)}$ and $p^{(\alpha)}$ and allows to optimize $LR(\alpha)$ by finding the roots of $\frac{\partial LR(\alpha)}{d \alpha} = 0$ (and analytically specifying the second order derivative).

Convergence is assumed when every single change in $\alpha$ is below a tolerance level (infinity norm).

In case of linear response functions, the likelihood ration then equals

$$ LR(\boldsymbol{\alpha}_k| \boldsymbol{\beta}_{jk}, \boldsymbol{\beta}_{k}) =  \sum_{i=1}^n  \sum_{j=1}^p s_{jk}x_{ij} \boldsymbol{\beta}_{jk} \boldsymbol{\alpha}_k - \sum_{i=1}^n  \sum_{j=1}^p s_{jk}x_{ij} \boldsymbol{\beta}_{k} \boldsymbol{\alpha}_k$$
$$ =  \sum_{i=1}^n  \sum_{j=1}^p s_{jk}x_{ij} \boldsymbol{\beta}_{jk} \boldsymbol{\alpha}_k - \boldsymbol{\beta}_{k} \boldsymbol{\alpha}_k \sum_{i=1}^n  \sum_{j=1}^p s_{jk}x_{ij} $$

Taking into account the restrictions mentioned above, the associated Lagrangian becomes

$$ Lag_{LR}(\boldsymbol{\alpha}_k| \boldsymbol{\beta}_{jk}, \boldsymbol{\beta}_{k}) =  \psi_k \Big(\sum_{i=1}^n  \sum_{j=1}^p s_{jk}x_{ij} \boldsymbol{\beta}_{jk} \boldsymbol{\alpha}_k - \boldsymbol{\beta}_{k} \boldsymbol{\alpha}_k \sum_{i=1}^n  \sum_{j=1}^p s_{jk}x_{ij} \Big) + \lambda_{1k} \boldsymbol{\alpha}_k^t\mathbf{1} + \lambda_{2k} (\boldsymbol{\alpha}_k^t \boldsymbol{\alpha}_k - 1) + \boldsymbol{\lambda}_{orth,k} \boldsymbol{\alpha}_k^t \boldsymbol{\alpha}_{-k}$$
...
$$\frac{\partial Lag_{LR}(\boldsymbol{\alpha}_k)}{\partial \alpha_{yk}} = \psi_k \Big( \sum_{i=1}^n  \sum_{j=1}^p s_{jk}x_{ij} \beta_{1jyk} - \sum_{i=1}^n  \sum_{j=1}^p s_{jk}x_{ij} \beta_{1.yk} \Big) + \lambda_{1k} + 2\lambda_{2k} \boldsymbol{\alpha}_k + \boldsymbol{\lambda}_{orth,k} \boldsymbol{\alpha}_{-k}$$
Together with the restrictions, this is almost a linear set of equations but for the term $\lambda_{2k} \boldsymbol{\alpha}_k$. The second order derivatives are

$$\frac{\partial^2 Lag_{LR}(\boldsymbol{\alpha}_k)}{\partial \alpha_{yk}^2} =  2\lambda_{2k} $$

$$\frac{\partial^2 Lag_{LR}(\boldsymbol{\alpha}_k)}{\partial \alpha_{yk} \partial \alpha_{y'k}} =  2\lambda_{2k}$$

$$\frac{\partial^2 Lag_{LR}(\boldsymbol{\alpha}_k)}{\partial \alpha_{yk} \partial \lambda_{1k}} =  1$$

$$\frac{\partial^2 Lag_{LR}(\boldsymbol{\alpha}_k)}{\partial \alpha_{yk} \partial \lambda_{2k}} =  2\boldsymbol{\alpha}_k$$
...
estDisp = function (X, cMat, rMat, muMarg, psis, trended.dispersion, prior.df = 10, dispWeights = NULL)
{
  # @param X: the data matrix of dimensions nxp
...
RCM_NB = function(X, k, rowWeights = "uniform", colWeights = "marginal", tol = 1e-3, maxItOut = 500, Psitol = 1e-3, verbose = TRUE, NBRCM = NULL, global = "dbldog", nleqslv.control=list(maxit = 500), jacMethod = "Broyden", dispFrec = 20, convNorm = 2, prior.df=10, marginEst = "MLE", confounders = NULL, prevCutOff = 1e-2, minFraction = 0.1, covariates = NULL, centMat = NULL, responseFun = "linear"){
  ...
  if((!is.null(NBRCM)) ){
    if(NBRCM$fit != "RCM_NB"){
      ...
      libSizesMLE = NBRCM$libSizesMLE
      abundsMLE = NBRCM$abundsMLE
      #Extend the record matrices
      ...
      # Fill out the other columns of the SVD
...
alpha = matrix(0,d,k)
alpha[!colnames(covariates) %in% alias,] = CCA$biplot[,1:k]
alpha = t(t(alpha)-colMeans(alpha))
alpha = t(t(alpha)/sqrt(colSums(alpha^2)))

alphaRec = array(0, dim=c(d,k, maxItOut))
v = switch(responseFun, linear = 2, quadratic = 3, 1)
NB_params = matrix(0,v,p) #Initiate parameters of the response function
NB_params_noLab = matrix(0,v,1)
...
#Overdispersions (not at every iterations to speed things up, doesn't change a lot anyway)
    if((iterOut[KK] %% dispFrec) ==0  || iterOut[KK]==1){
  if (verbose) cat("\n Estimating overdispersions \n")
 thetas = estDisp(X = X, rMat = rMat, cMat = cMat[KK,,drop=FALSE],  muMarg=muMarg, psis = psis[KK], prior.df = prior.df, trended.dispersion = trended.dispersion)
 thetasMat = matrix(thetas, n, p, byrow=TRUE)
  }
...
if (verbose) cat("\n Estimating column scores \n")
regCol = rMat*psis[KK]
tmpCol = nleqslv(fn = dNBllcol, x = c(t(cMat[KK,]), lambdaCol[idK]), thetas =  thetasMat , X = X, reg = regCol, muMarg = muMarg, k = KK,  global = global, control = nleqslv.control, n=n, p=p, jac = NBjacobianCol, method = jacMethod, colWeights = colWeights, nLambda = (KK+1), cMatK = cMat[1:(KK-1),,drop=FALSE])
...
tmpAlpha = estAlpha(Alpha = alpha[, KK,drop=FALSE], X = X, CC = covariates, d = d, ncols = p, v = v, responseFun = responseFun, muMarg = muMarg, thetas =  thetasMat , cReg = regAlpha, alphaK = alpha[,1:(KK-1),drop=FALSE], k=KK , centMat = centMat, n=n, NB_params = NB_params, NB_params_noLab = NB_params_noLab) #Provide previous parameters of the response function since they might provide good estimates

if(verbose)  cat(ifelse(tmpAlpha$converged, "Row scores converged \n", "Row scores DID NOT converge \n"))
  alpha[,KK] = tmpAlpha$Alpha
  NB_params = tmpAlpha$NB_params
  NB_params_noLab = tmpAlpha$NB_params_noLab
...
dLR_nb <- function(Alpha, X, CC, responseFun = c("linear", "quadratic", "nonparametric"), muMarg,   cReg, nleqslv.control = list(trace=FALSE), NB_params, NB_params_noLab, ...){
  # @param Alpha: a vector of length d, the environmental gradient
  # @param X: the n-by-p count matrix
  # @param CC: the n-by-d covariate matrix
  # @param responseFun: a character string indicating the type of response function
  ...
  # @return: a scalar, the evaluation of the log-likelihood ratio at the given alpha
  lag = psi*(outer(cMat*betaJ)*X*alpha-beta*alpha * sum(X %*% cMat)) + lambda1*sum(alpha) + lambda2 * (sum(alpha^2)-1) + lambda3 * alpha %*% alphaT
  center = sum(alpha)
  norm = sum(alpha^2) -1
  if(k>1){
  ortho = alpha %*% alphaT
  return(c(lag, center, norm, ortho))
  } else {
    return(c(lag, center, norm))
  }
}
...
estAlpha = function(Alpha, X, CC, d, k, ncols, centMat, n, v,maxItAlpha = 50, responseFun = c("linear", "quadratic", "nonparametric"), muMarg, thetas, cReg, alphaK, control.outer = list(trace=FALSE), control.optim=list(trace=FALSE), nleqslv.control = list(), convNorm = 2, tol=1e-3, NB_params = NULL, NB_params_noLab = NULL){
...
NB_params_noLab = estNBparamsNoLab(design = design, thetas = thetas, muMarg = muMarg, cReg = cReg, X = X, nleqslv.control = nleqslv.control, initParam = NB_params_noLab, n = n, v = v)
  ...
  return(list(Alpha = Alpha, NB_params = NB_params, NB_params_noLab = NB_params_noLab, convergence = convergenceA))
}
#--------------------------------------#
...
RCM = function(dat, k, round=FALSE, distribution= "NB", prevCutOff = 0.01, minFraction = 0.1, rowWeights = "uniform", colWeights = "marginal", covariates = NULL, confounders = NULL, prevFit = NULL, ...){
...
testLR = LR_nb(alphaT, X, t(regMat), responseFun ="linear", muMarg = muMargObs, thetas = thetasCon[idCol], cReg = cVec2, d= nrow(regMat), ncols=ncol(X),v=2)
...
As ways of generating test datasets we see three different options
  ...
  3) Real datasets with alleged biological signal known
  
  Parametric simulation is needed as a first check but evidently favours our method. SimSeq is a reasonably neutral tool, we will only use WMW to obtain the lfdr. Real datasets will need to complement our analysis but they may not be possible to evaluate in a high-throughput way, this will need to happen empirically.
...
A first intuitive approach would be to compare the within cluster distances on the plot to the distances between the clusters. Say we have $m = 1,...,l$ groups with (multidimensional) means $\mathbf{M}_m$. The between distance between groups m and m' is then defined as $d_{mm'} = d_{euclidean}(\mathbf{M}_m, \mathbf{M}_{m'})$ with $d_{euclidean}$ the euclidean distance. The within distance is then defined as $d_{m, within} = \frac{1}{n_m}\sum_{i=1}^{n_m} d_{euclidean}(\mathbf{M}_m, Y_i)$. The overall distance of a group then equals $d_{m, overall} = \frac{1}{n_m}\sum_{i=1}^{n_m} d_{euclidean}(\mathbf{0}, Y_i)$ with $\mathbf{0}$
...
### Unconstrained
 ...
 - PCoA
   ...
   * UniFrac distance (Mirobial ecology)
