...
knitr::opts_chunk$set(cache = TRUE, autodep = TRUE,  warning=FALSE, message=FALSE, echo=FALSE, eval=TRUE, tidy = TRUE, fig.width=11, fig.height=8, root.dir=WD)
setwd(WD)
...
#Overdispersions (not at every iterations to speed things up, doesn't change a lot anyway)
 ...
 thetas = estDisp(X = X, rMat = rMat,cMat =  cMat,  muMarg=muMarg, psis = psis, k=k, prior.df = prior.df)
  }
...
dNBllcol_1B1 = function(beta, X, reg, thetas, muMarg, k, p, n, colWeights, nLambda, cMatK) {
  ...
  lambda3Mat = matrix(0,ncol=k-1, nrow=k-1)
  lambda3Mat[lower.tri(lambda3Mat)] = lambda3Mat[upper.tri(lambda3Mat)]= beta[(p+3):length(beta)]
  }
  score = c(t(
    crossprod(reg,((X-mu)/(1+t(t(mu)/thetas)))) + 
                        t(colWeights*t(lambda1 + lambda2*2*cMat + (lambda3Mat %*% cMat)))
...
#Orthogonality restrictions sum(abunds*r_{ik}*r_{ik'}) = 0. First all of dimension 1, then the remaining of dimension 2 etc. #+2*k
  if(k==1){lambda3Mat=0} else {
  lambda3Mat = matrix(0,ncol=k-1, nrow=k-1)
  lambda3Mat[lower.tri(lambda3Mat)] = lambda3Mat[upper.tri(lambda3Mat)]= beta[(n+3):length(beta)]
  }
...
NBjacobianRow_1B1 = function(beta, X, reg, thetas, muMarg, k, n ,p, nlambda, rowWeights, nLambda, rMatK){
  ...
  if(k==1) {lambda3Mat=0} else {
  lambda3Mat = matrix(0,ncol=k-1, nrow=k-1)
  lambda3Mat[lower.tri(lambda3Mat)] = lambda3Mat[upper.tri(lambda3Mat)]= beta[(k+3):length(beta)]
  }
  ...
  #dLag²/dr_{ik}dlambda_{3kk'}
  if(k>1){
    Jac[1:n,(n+3):(n+nLambda)] = apply(rMatK, 2, function(x){rMat*rowWeights*x})
  }
  #Symmetrize
  Jac = Jac + t(Jac)
#dLag²/dr_{ik}²
...
rMat = apply(rMat, 2, function(rowS){
  ...
  if(KK>1){muMarg = muMarg * exp(outer(rMat[,1:(KK-1)] ,cMat[1:(KK-1),]*psis[1:(KK-1)]))}
...
#Overdispersions (not at every iterations to speed things up, doesn't change a lot anyway)
  ...
  if (verbose) cat("\n Estimating overdispersions \n")
 thetas = estDisp(X = X, rMat = rMat[,KK,drop=FALSE], cMat = cMat[KK,,drop=FALSE],  muMarg=muMarg, psis = psis[KK], k=1:KK, prior.df = prior.df)
  }
...
llZipMeanRow =  function(beta, X, reg, muMarg, k,p,n,  Z, nLambda, rowWeights){
  rMat = matrix(beta, ncol=2)
  tmp = (rMat %*% reg) +log(muMarg)
  -sum((1-Z)*(X*tmp-exp(tmp)))
}
...
dZipMeanCmat = function(beta, X, muMarg, k,p,n, Z, nLambda, colWeights, reg){
  ...
  centers = colSums(colWeights*t(cMat))
  unitSums = colSums(colWeights*t(cMat^2))-1
  if(k==1){return(c(score,centers, unitSums, orthogons))}
  orthogons = unlist(sapply(1:(k-1), function(K){
    vapply((K+1):k,FUN.VALUE=0, function(Kinner){
      sum(cMat[K,]*cMat[Kinner,]*colWeights)
    })
...
tMat = apply(tMat, 2, function(rowS){
  ...
  if(!convergence ){
    ...
    return(list(converged = convergence,rMat=rMat, cMat=cMat, psis = psis, X=X,
                rowRec = rowRec, colRec = colRec, psiRec = psiRec, lambdaRow = lambdaRow, lambdaCol = lambdaCol, lambdaRowZero = lambdaRowZero, lambdaColZero = lambdaColZero, chis = chis, tMat = tMat, vMat = vMat, chiRec = chiRec, rowRecZero = rowRecZero, colRecZero = colRecZero, iter=iterOut, Z=Z, Zcol=Zcol))
}
```
...
tMat = apply(tMat, 2, function(rowS){
  ...
  if(!convergence ){
    ...
    return(list(converged = convergence, rMat=rMat, cMat=cMat, psis = psis, X=X,
                 rowRec = rowRec, colRec = colRec, psiRec = psiRec, lambdaRow = lambdaRow, lambdaCol = lambdaCol, lambdaRowZero = lambdaRowZero, lambdaColZero = lambdaColZero, chis = chis, tMat = tMat, vMat = vMat, chiRec = chiRec, rowRecZero = rowRecZero, colRecZero = colRecZero, iter=iterOut, Z=Z, Zcol=Zcol, thetas=thetas))
}
```
...
RCM = function(dat, round=FALSE, method= c("NB","NB_1B1", "ZIP", "ZINB"),prevCutOff=0.01,rowWeights = "uniform", colWeights = "marginal",...){
  if (class(dat)=="matrix"){
...
tic = proc.time() #Time the calculation
  tmp = switch(method, NB=RCM_NB(dat, rowWeights=rowWeightsNum, colWeights=colWeightsNum, weightsChar = weightsChar,...),NB_1B1=RCM_NB_1B1(dat, rowWeights=rowWeightsNum, colWeights=colWeightsNum, weightsChar = weightsChar,...), ZIP=RCM_ZIP(dat, rowWeights=rowWeightsNum, colWeights=colWeightsNum,weightsChar = weightsChar, ...), ZINB=RCM_ZINB(dat,  rowWeights=rowWeightsNum, colWeights=colWeightsNum,weightsChar = weightsChar,...))
  tmp$runtime = (proc.time()-tic)[3]
  tmp
}
...
#   syntNBNSunifJob = mcparallel(RCM(dataMatNBNS, method="NB", k=2, nleqslv.control= nleqslv.control, maxItOut=2e3, prevCutOff=0.01, colWeights = "uniform"))
#   syntNBNSunif=mccollect(syntNBNSunifJob, FALSE)[[1]] 
#   
#   syntNBNSunif_1B1_Job = mcparallel(RCM(dataMatNBNS, method="NB_1B1", k=2, nleqslv.control= nleqslv.control, maxItOut=2e3, prevCutOff=0.01, colWeights = "uniform"))
#   syntNBNSunif_1B1=mccollect(syntNBNSunif_1B1_Job, FALSE)[[1]] 
#   #One by one estimation does converge much faster! It also has much larger lambda3 parameters, bu I guess these cannot be direclty compared since they come from fundamentall different estimation problems?
#   
#   id= with(syntNBNSunif,which(abs(cMat)>25, TRUE)[,2])
#     syntNBNSunifJobrefit = mcparallel(RCM(syntNBNSunif$X[,-id], method="NB", k=2, nleqslv.control= nleqslv.control, maxItOut=2e3, prevCutOff=0.1))
#   syntNBNSunifRefit=mccollect(syntNBNSunifJobrefit, FALSE)[[1]] 
#   #Refitting does not help, new taxa become outlying
  
 #With marginal weights
    syntNBNSmargJob = mcparallel(RCM(dataMatNBNS, method="NB", k=2, nleqslv.control= nleqslv.control, maxItOut=2e3, colWeights = "marginal", prevCutOff=0.01))
  syntNBNSmarg=mccollect(syntNBNSmargJob, FALSE)[[1]]
  
      syntNBNSmarg_1B1Job = mcparallel(RCM(dataMatNBNS, method="NB_1B1", k=2, nleqslv.control= nleqslv.control, maxItOut=2e3, colWeights = "marginal", prevCutOff=0.01))
  syntNBNSmarg_1B1=mccollect(syntNBNSmarg_1B1Job, FALSE)[[1]]
  
      syntNBNSmarg_3Job = mcparallel(RCM(dataMatNBNS, method="NB", k=3, nleqslv.control= nleqslv.control, maxItOut=2e3, colWeights = "marginal", prevCutOff=0.01))
  syntNBNSmarg_3=mccollect(syntNBNSmargJob_3, FALSE)[[1]]
  
      syntNBNSmarg_1B1_3Job = mcparallel(RCM(dataMatNBNS, method="NB_1B1", k=3, nleqslv.control= nleqslv.control, maxItOut=2e3, colWeights = "marginal", prevCutOff=0.01))
  syntNBNSmarg_1B1_3=mccollect(syntNBNSmarg_1B1_3Job, FALSE)[[1]]
  
  #Save the results
  save(syntNBNSmarg,syntNBNSmarg_1B1, syntNBNSmarg_1B1_3, syntNBNSmarg_3, file="toyDataUnifNS.RData" ) #syntNBNSunif,syntNBNSunif_1B1
...
```{r Plot results}
with(syntNBNSunif, {plot(rMat)})
with(syntNBNSunif, {plot(t(cMat))}) # We see some outliers for the taxa
#Look at the loadings in function of the library sizes and abunds

with(syntNBNSunif, {plot(cMat[1,], colSums(X), log="y")})
with(syntNBNSunif, {plot(abs(cMat[1,]), colSums(X), log="y")})
with(syntNBNSunif, {plot(cMat[2,], colSums(X), log="y")})
with(syntNBNSunif, {plot(abs(cMat[2,]), colSums(X), log="y")})
#Lowly abundant taxa get higher scores

hist(1/syntNBNSunif$thetas)

with(syntNBNSmarg, {plot(cMat[1,], colSums(X), log="y")})
with(syntNBNSmarg, {plot(abs(cMat[1,]), colSums(X), log="y")})
with(syntNBNSmarg, {plot(cMat[2,], colSums(X), log="y")})
with(syntNBNSmarg, {plot(abs(cMat[2,]), colSums(X), log="y")})

hist(1/syntNBNSmarg$thetas)
#With marginal weighting at least there are no outliers
...
Here the first attempt with modified abundances (not evaluated) 
...
idSig2NB = sample(1:NtaxaNBNS, NtaxaSignal2NB)
#Apply the signals
...
#Generate data
Nref = (NsamplesNBNS-NsamplesSignal1-NsamplesSignal2-NsamplesSignal12)
...
meanMatSig12NB = outer(libSizesNBNS[sample(size=NsamplesSignal12, 1:NsamplesNBNS)], rhosNBSig12)
...
dataMatSig12NB = makeNBdata(meanMatSig12NB, thetaMatSigNB)

dataMatSigNB = rbind(dataMatRefNB, dataMatSig1NB, dataMatSig2NB, dataMatSig12NB)

#Save signals
sampleSigNB = factor(c(rep("Reference", Nref), rep("Signal1", NsamplesSignal1), rep("Signal2", NsamplesSignal2), rep("Signal 1 and 2", NsamplesSignal12)))
taxaSigNBtmp = rep("Reference",ncol(dataMatSigNB))
taxaSigNBtmp[idSig1NB] = "Signal 1"
taxaSigNBtmp[idSig2NB] = "Signal 2"
taxaSigNB =factor(taxaSigNBtmp) 
names(taxaSigNB) = colnames(dataMatSigNB) = names(rhosNBSigref)
names(sampleSigNB) = rownames(dataMatSigNB) = 1:NsamplesNBNS
```

And now the "model-based" apporach

```{r NB model based data with signal}
load("/home/stijn/PhD/American Gut/AGpars.RData")
#Define parameters
Nsamples= 300
Ntaxa = 900
...
#Row scores (add two standard deviations)
rChange1 = c(rep(2,10), rep(0, Nsamples-20), rep(2,10))
rChange2 =  c(rep(0,5),rep(2,15), rep(0, Nsamples-40), rep(2,15), rep(0,5))

#Logical indices for these
rid1 = rChange1!=0
rid2 = rChange2!=0

rowScores1 = rnorm(Nsamples-Nsamples%%2, sd=1)  + rChange1
rowScores2 = rnorm(Nsamples-Nsamples%%2, sd=1) + rChange2

#Column scores
cChange1 = c(rep(0,10),rep(2,20), rep(0, Ntaxa-50), rep(2,20))
cChange2 = c(rep(0,30),rep(2,25), rep(0, Ntaxa-100), rep(2,25), rep(0,20))

cid1 = cChange1!=0
cid2 = cChange2!=0

colScores1 = rnorm(Ntaxa-Ntaxa%%2, sd = 1) + cChange1
colScores2 = rnorm(Ntaxa-Ntaxa%%2, sd = 1) + cChange2
...
```{r fit RC(M) NB with signal}
nleqslv.control = list(trace=TRUE, maxit = 250, cndtol=.Machine$double.eps)
if(!file.exists("toyDataSig.RData")){
#   syntNBSigunifJob = mcparallel(RCM(dataMatNBSig, method="NB", k=2, nleqslv.control= nleqslv.control, maxItOut=2e3, prevCutOff=0.01, colWeights ="uniform"))
#   syntNBSigunif=mccollect(syntNBSigunifJob, FALSE)[[1]] 
  
  syntNBSigmargJob = mcparallel(RCM(dataMatNBSig, method="NB", k=2, nleqslv.control= nleqslv.control, maxItOut=2e3, prevCutOff=0.01, colWeights="marginal"))
  syntNBSigmarg=mccollect(syntNBSigmargJob, FALSE)[[1]] 
  
    syntNBSigmarg_1B1Job = mcparallel(RCM(dataMatNBSig, method="NB_1B1", k=2, nleqslv.control= nleqslv.control, maxItOut=2e3, prevCutOff=0.01, colWeights="marginal"))
  syntNBSigmarg_1B1=mccollect(syntNBSigmarg_1B1Job, FALSE)[[1]] 
  
    syntNBSigmarg_3Job = mcparallel(RCM(dataMatNBSig, method="NB", k=3, nleqslv.control= nleqslv.control, maxItOut=2e3, prevCutOff=0.01, colWeights="marginal"))
  syntNBSigmarg_3 = mccollect(syntNBSigmarg_3Job, FALSE)[[1]] 
  
    syntNBSigmarg_1B1_3Job = mcparallel(RCM(dataMatNBSig, method="NB_1B1", k=3, nleqslv.control= nleqslv.control, maxItOut=2e3, prevCutOff=0.01, colWeights="marginal"))
  syntNBSigmarg_1B1_3 = mccollect(syntNBSigmarg_1B1_3Job, FALSE)[[1]] 
 
  #Not the same as the original solution?? Needs further investigation
  save(dataMatNBSig, dataMatNBSig_1B1, syntNBSigmarg, syntNBSigmarg_3, syntNBSigmarg_1B1_3, taxaSigNB, sampleSigNB, file="toyDataSig.RData")#syntNBSigunif
} else {load("toyDataSig.RData")}

# A function to pour the results into a dataframe
RCMres2DF = function(RCM,  sampleGroups, taxaGroups){
  list(rows =data.frame(r1=RCM$rMat[,1]*RCM$psis[1], r2=RCM$rMat[,2]*RCM$psis[2],  sampleGroups=sampleGroups), columns = data.frame(c1=RCM$cMat[1,], c2=RCM$cMat[2,], taxaGroups=taxaGroups))
}
NBunifSigPlotDF = RCMres2DF(syntNBSigunif,  sampleSigNB, taxaSigNB[colMeans(dataMatNBSig==0)<0.99])
NBmargSigPlotDF = RCMres2DF(syntNBSigmarg,  sampleSigNB, taxaSigNB[colMeans(dataMatNBSig==0)<0.99])
NBmargSigPlotDF_1B1 = RCMres2DF(syntNBSigmarg_1B1,  sampleSigNB, taxaSigNB[colMeans(dataMatNBSig==0)<0.99])
...
ggplot(data=NBmargSigPlotDF$rows, aes(x=r1, y=r2, col=sampleGroups)) + geom_point() + scale_colour_manual(values=c("grey50","blue","purple","red"))
ggplot(data=NBmargSigPlotDF_1B1$rows, aes(x=r1, y=r2, col=sampleGroups)) + geom_point() + scale_colour_manual(values=c("grey50","blue","purple","red"))

NBunifSigPlotDF2 = RCMres2DF(syntNBSigunif2,  sampleSigNB2)
NBmargSigPlotDF2 = RCMres2DF(syntNBSigmarg2,  sampleSigNB2)

with(syntNBSigunif, plot(rMat))
with(syntNBSigunif, plot(t(cMat)))

with(syntNBSigmarg, plot(rMat))
with(syntNBSigmarg, plot(t(cMat), col=taxaSigNB)) #No more outliers, and taxa are correctly identified. To be contibued ...
```

The column scores are very sensitive to high zero frequencies and dito overdispersions. The abundances are normal but very high fraction of zeroes. Try with less prior.df? Or inverse weight by overdispersions?

Not evaluated

```{r Nb sig 2, eval=FALSE}
if(!file.exists("toyDataUnifSig2.RData")){
  syntNBSigunifJob2 = mcparallel(RCM(dataMatSigNB, method="NB", k=2, nleqslv.control= nleqslv.control, maxItOut=2e3, prior.df=3))
  syntNBSigunif2=mccollect(syntNBSigunifJob2, FALSE)[[1]] 
  save(syntNBSigunif, taxaSigNB, sampleSigNB, file="toyDataUnifSig2.RData" )
} else {load("toyDataUnifSig2.RData")}

#Prior df does not matter
```

##### Plot the results

```{r RC(M) Nb with signal results}
with(data4RCM3, plot(rMat, col=rowSignal)); legend("topleft", legend=levels(rowSignal), pch=1, col=1:4)
with(data4RCM3, plot(t(cMat), col=colSignal)); legend("topleft", legend=levels(colSignal), pch=1, col=1:4)
save(data4RCM3, rid1, rid2, cid1, cid2, file="data4RCM3.RData")
```
