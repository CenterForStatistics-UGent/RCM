...
levelsMeth = c("CA","DCA", "Hellinger","BC","BCrel","BCrelNMDS", "JSD","CoDa","RCM", "Control")
labelsMeth = c("CA","DCA","Hellinger","Bray-Curtis-Abs","Bray-Curtis","Bray-Curtis NMDS", "JSD","CoDa","RCM", "Control")
groupsMeth = factor(c("Independence","Independence","Independence","Distance","Distance","Distance","Distance", "CoDa","RCM"), levels = c("Independence","Distance","CoDa","RCM", "Control"), ordered = TRUE)
```
...
$$R^{-1/2}(X-E)J^{-1/2}=U\Sigma V$$

with $\Sigma$ a diagonal matrix with the singular values of $X$ on the diagnonal and $R$ and $J$ diagonal matrices with row and column sums.

In matrix notation the reconstitution formula becomes

$$X = E_{independence} + R^{1/2}U\Sigma VJ^{1/2}$$

with $\mathbf{1'}R^{1/2}U=\mathbf{0}$ and $\mathbf{1'}J^{1/2}V=\mathbf{0}$ (weighted means equal zero) and $U'R^{1/2}U=\mathbf{1}$ and $V'K^{1/2}V=\mathbf{1}$ (weighted variances equal one) (see VanDerHeijden et al., 1985).
...
# @return: the singular value decomposition of the matrix of pearson residuals
  ...
  # 
  # # #Eigenvalue approach (ter Braak 1986)
  # EigGoal = t(X) %*% Y %*% solve(t(Y) %*% diag(R) %*% Y) %*% t(Y) %*% X
  # EigDec = eigen(EigGoal)
  # EigSitesWeighted = diag(1/R) %*% X %*% EigDec$vectors %*% diag(1/C)
  # alpha = solve(t(Y) %*% diag(R) %*% Y) %*% t(Y) %*% diag(R) %*% EigSitesWeighted
  # EigSitesConstr = Y %*% alpha
  # 
  # GeigDec = geigen(t(Y) %*% X %*% diag(1/C) %*% t(X) %*% Y, t(Y) %*% diag(R) %*% Y)
  # 
  # # Singular value approach (ter Braak 2014)
  # Ystar = sqrtm(solve(t(Y) %*% diag(R) %*% Y)) %*% t(Y) %*% X %*% diag(1/sqrt(C))
  # svdY = svd(Ystar)
  # B = sqrtm(solve(t(Y) %*% diag(R) %*% Y)) %*% svdY$u
  # U = diag(1/sqrt(C)) %*% svdY$v %*% diag(svdY$d)
  # sites = Y %*% B
  # 
  # # At least 
  # cor(B[,1:2], alpha[,1:2])
  # cor(sites[,1:2], EigSitesConstr[,1:2])
  # 
  # # Vegan
  # ccaVegan = cca(X, Y)$CCA
  # 
  # par(mfrow = c(2,2))
  # #Species plots
  # tmp = plot(cca(X, Y),display = "sp")
  # plot(U)
  # plot(EigDec$vectors)
  # plot(Svd$v)
  # cor(cbind(U[,1], EigDec$vectors[,1], Svd$v[,1], tmp[[1]][,1] ))
  # 
  # par(mfrow = c(2,2))
  # # Sample plots constrained
  # tmp = plot(cca(X, Y),display = "lc", type = "points")
  # plot(sites)
  # plot(EigSitesConstr)
  # plot(Svd$u)
  # cor(cbind(sites[,1], EigSitesConstr[,1], Svd$u[,1], tmp[[1]][,1] ))
  # 
  # # Ter Braaks eigen- and singular value decompositions are the same, but differ from the chi-square difference approach?
  }
}
#-------------------------------------------------#
cca2 = function (X, Y, Z, ...) 
...
RCM = function(dat, k, round=FALSE, distribution= "NB", prevCutOff = 0.025, minFraction = 0.1, rowWeights = "uniform", colWeights = "marginal", covariates = NULL, confounders = NULL, prevFit = NULL, ...){
  ...
  if(!is.null(covariates)){
    ...
    if(any(sapply(datFrame, is.logical))){
      datFrame[,sapply(datFrame, is.logical)] = lapply(datFrame[,sapply(datFrame, is.logical)], factor) #Convert logicals to factors
...
if(!file.exists(file = "armpitRCM.RData")){
  ...
  save(armpitRCM, armpitRCMconstrLin, armpitRCMconstrQuad, file = "armpitRCM.RData")
...
This is a longitudinal dataset on growth in a water cooling system. It has the unique property of providing relative counts trhough 16S sequencing that can be multiplied by absolute counts from flow cytometry to get truly absolute abundances that reflect the number of cells. This type of data does not qualify for the term _compositional data_ anymore. The advent of these kind of approaches support the use of count models rather than compositional data approaches.
...
```{r analysCMET, purl = FALSE, eval = FALSE}
if(!file.exists(file = "CMETRCM.RData")){
  CMETRCM = RCM(CMETwater, k = 3)
  envVarsCMET = c("ph","temp","Conductivity","cellDensity", "Reactor.phase", "collection_date", "Timepoint", "Reactor.cycle")
  CMETRCMlin = RCM(CMETwater, k = 3, responseFun = "linear", covariates = envVarsCMET)
  CMETRCMquad = RCM(CMETwater, k = 3, responseFun = "quadratic", covariates = envVarsCMET)
  save(CMETRCM, CMETRCMlin, CMETRCMquad, file = "CMETRCM.RData")
...
plotCor = function(scores, datList, Dims = 1:3, scoreDim = "rows", dataMat = TRUE, yloc = 0.2, groupMeth = factor(c(as.character(groupsMeth), "Control"), levels = levels(groupsMeth))){
par(mar = c(4,4,4,4))
if(!dataMat){
...
plotSil = function(resListRows, groupFactor, method = NULL, groupMeth = droplevels(groupsMeth),...){
...
boxplot(Silhouette~Method, main = "Mean silhouettes per Monte Carlo simulation by method", data = meanSil1df, col = groupMeth, ...)
...
#---------------------------------#
distRatioPlot = function(resListRows, groupFactor, method = NULL, groupMeth = droplevels(groupsMeth),...) {
...
boxplot(DistanceRatio~Method, main = "Mean ratios of overal to within distance by method", data = meanDist1df, col = groupMeth, ylim = c(0, ceiling(max(meanDist1df$DistanceRatio))),las = 2,...)
...
contrPlot = function(contr, method = NULL, groupMeth = groupsMeth, ...) {
...
addLegend = function(yloc = 0.2, groupMeth, x = length(groupMeth)+1,...){
legend(legend = levels(groupMeth), col = factor(levels(groupMeth), levels = levels(groupMeth), ordered=TRUE),pch = 16, x = x, y = yloc, xpd = TRUE,...)
}
```
