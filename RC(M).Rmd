...
dNBllcol_constr_mat = function(betas, X, reg, theta, muMarg, cReg) {
  ...
  # @return The evaluation of the score functions, a matrix of size (deg+1)*p

  mu = exp(c(reg %*% betas)* cReg) * muMarg
  score =  crossprod((X-mu)/(1+mu/theta) , reg) * cReg
  return(c(score)) #First all intercepts, then all slopes
}
#--------------------------------------#
# The jacobian of this function
JacCol_constr_mat = function(betas, X, reg, theta, muMarg, cReg) {
  # @param beta: a (deg+1)*p matrix of regression parameters with deg the degree of the response function
  # @param X: the nxp data matrix
  # @param reg: a matrix of regressors with the dimension (deg+1)*n (\mathbf{C} \boldsymbol{\alpha})
  # @param theta: The dispersion parameter
  ...
  # @return The jacobian, a (deg+1)*p x (deg+1)*p matrix

  mu = exp(c(reg %*% betas)* cReg) * muMarg
  tmp = t(t((1+t(t(X)/thetas))*mu/(1+t(t(mu)/thetas))^2) * cReg^2)
  Jac = crossprod(tmp,reg)*reg #Don't forget to square cReg!
  return(-Jac)
}
#--------------------------------------#
