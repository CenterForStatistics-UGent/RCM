...
For the filtering based on the prevalence and abundance, we need a confounder matrix
 ...
 In this case all that matters is to change the offset. We will still return the confoundes' parameters though.
...
RCM = function(dat, k, round=FALSE, distribution= c("NB", "ZIP", "ZINB"), prevCutOff = 0.01, minFraction = 0.1, rowWeights = "uniform", colWeights = "marginal", covariates = NULL, confounders = NULL, ...){
  ...
  ##Build confounder matrix if applicable. For the preliminary trimming, we do not include an intercept, but we do include all the levels of the factors using contrasts=FALSE: we want to do the trimming in every subgroup, so no hidden reference levels. For the filtering we just use a model with anintercept and treatment coding##
  if(!is.null(confounders)){
  ...
  # In this case we will 1) Use treatment coding for the dummies to simplify the estimation 2) Include an intercept
