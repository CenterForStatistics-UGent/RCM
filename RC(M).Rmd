...
reqpkg <- c("phyloseq","MASS", "parallel","nleqslv", "edgeR", "VGAM", "HMP", "ggplot2", "pscl", "alabama")
...
$$ q = 1 + d_{cont} + \sum_{l=1}^{d_{cat}} (m_l-1) $$
rows, or $q$ parameters to be estimated. We can write $\boldsymbol{\alpha}_k$ then as:

$$\boldsymbol{\alpha}_k = (\beta_{0}, \beta_{cont_1}, ... , \beta_{cont_{d_{cont}}}, \beta_{cat_{11}}, \beta_{cat_{12}}, ..., \beta_{cat_{1(m_1-1)}}, ...,  \beta_{cat_{d_{cat}(m_{d_{cat}}-1)}})_k$$
Hereby $\beta_{0k}$ represents the offset of the $k$-th dimension. It represents the log-row score for a sample with all continuous covariates equal to zero and the categorical ones equal to their reference level. 

For imposing the restricitions and for plotting we want a representation whereby all covariate levels are represented so that the parameters sum to 0 within each covariate.

$$\boldsymbol{\alpha}_k' = (\beta_{0}', \beta_{cont_1}', ... , \beta_{cont_{d_{cont}}}', \beta_{cat_{11}}', \beta_{cat_{12}}', ..., \beta_{cat_{1m_1}}', ...,  \beta_{cat_{d_{cat}m_{d_{cat}}}}')_k$$
under the restriction that for all categorical variables $l$

$$\sum_{m=1}^{m_l} \beta_{cat_{lm}k} = 0$$
In this case $\beta_{k0}'$ is the _grand mean_: the average value over all groups of the categorical variables and for all continuous covariates equal to zero. It is this $\boldsymbol{\alpha}_k$ that will need to be centered, normalized and orthogonalized. Note that the categorical covariates are already centered by default. Also this representation will come in handy for plotting, centering the levels of the same covariate around zero. We will call this the __zero-sum__ representation. For a single covariate we go from the treatment (with first level the reference level) to the zero sum representation by solving the following system:
...
#-------------------------------------#
#Fit the library sizes (u_i) by ML
...
dNBpsis = function(beta, X, reg, theta,  muMarg){
  ...
  # @param rMat: a nx1 matrix with row scores
  # @param mumarg: the nxp offset
  # @param cMat: a 1xp matrix with column scores
  # @param theta: a vector of length p with the dispersion parameters

  # @return The evaluation of the score function at beta
...
NBjacobianPsi = function(beta, X, reg, muMarg, theta){
  ...
  # @param theta: a vector of length p with the dispersion parameters
    ...
    # @return The evaluation of the Jacobian function at beta
...
dNBllcol = function(beta, X, reg, thetas, muMarg, k, p, n, colWeights, nLambda, cMatK, estPsis) {
  # @param beta: a matrix of 1*(p+2k+k*(k-1)/2) with p column scores + k within dimensional centering lambda parameters + k within dimensional normalization parameters +  k(k-1)/2 between dimension lambda parameters
  # @param X: the nxp data matrix
  ...
  # @return A vector of length p+2*k+ k(k-1)/2 evaluations of the derivative of lagrangian
  ...
  lambda2 = if(estPsis) beta[p+2] else 0 #normalization restrictions sum(abunds*r^2_{ik}) = 1
  
  lambda3 = if(k==1){0} else {beta[(p + 2 + estPsis):length(beta)]}
...
#-------------------------------------------#
# A function to estimate the overdispersion with empirical Bayes using the edgeR pacakage
...
filterConfounders = function(muMarg, confMat, X, thetas, p, n, nleqslv.control, tol = 1e-3, maxIt = 20){
  ...
  # @return thetas: new theta estimates
  # @return muMarg: a new offset matrix
...
RCM_NB = function(X, k, rowWeights = "uniform", colWeights = "marginal", tol = 1e-3, maxItOut = 500, Psitol = 1e-3, verbose = TRUE, NBRCM = NULL, global ="dbldog", nleqslv.control=list(), method=c("Broyden"), dispFrec=10, convNorm = 2, prior.df=10, marginEst = "MLE", estPsis = TRUE, confoundersFilt = NULL, confounders = NULL, covariates = NULL, responseFun = "linear", prevCutOff = 1e-2, minFraction = 0.1){
  ...
  if(!is.null(confounders)){ #First and foremost: filter on confounders
    Xorig = X
...
#Store intermediate estimates
  ...
  rownames(cMat) = colnames(rMat) = paste0("Dim",1:k)
    ...
    returnList = list(converged = convergence,rMat=rMat, cMat=cMat, thetas = thetas, psis = psis, X=X,
                 rowRec= rowRec, colRec = colRec, psiRec = psiRec, thetaRec = thetaRec, lambdaRow=lambdaRow, lambdaCol=lambdaCol, rowWeights=rowWeights, colWeights=colWeights, iter=iterOut-1, fit="RCM_NB", libSizesMLE = switch(marginEst, "MLE"=exp(logLibSizesMLE), "marginSums" = libSizes), abundsMLE = switch(marginEst, "MLE"=exp(logAbundsMLE), "marginSums"=abunds))
  
  } else { #If covariates provided, do a constrained analysis
  
d = ncol(covariates)

# alpha0 = vapply(1:20L,FUN.VALUE = matrix(0,d,k), function(x){
#   id = sample(seq_len(n),d)
#   Matrix::solve(covariates[id,], rMat[id,]) #invert definition to obtain starting values, nd average the result over 20 instances. This is a bit far-fetched perhaps?
# }) 
# 
# alpha = as.matrix(apply(alpha0, c(1,2), mean))
#   alpha = t(t(alpha) - colMeans(alpha))
#   alpha = t(t(alpha)/sqrt(apply(alpha,2, function(x){sum(x^2)}))) #Center and normalize
...
alphaRec = array(0, dim=c(d,k, maxItOut))
 ...
 if(!is.null(NBRCM)){ #If fit provided, replace lower dimension starting values
  ...
  alphaOld = alpha[,KK]
  cMatOld = cMat[KK,]
  rMat = covariates %*% alpha[,KK,drop=FALSE]
...
tmpAlpha = estAlpha(Alpha = alpha[, KK], X = X, CC = covariates, d = d, ncols = p, responseFun = responseFun, muMarg = muMarg, thetas = thetas, cReg = regAlpha, alphaK = alpha[,1:(KK-1),drop=FALSE], k=KK)
...
#Store intermediate estimates
  ...
  returnList=list(converged = convergence, alpha = alpha, cMat=cMat, thetas = thetas, psis = psis, X=X, alphaRec = alphaRec, covariates = covariates, colRec = colRec, psiRec = psiRec, thetaRec = thetaRec, lambdaCol = lambdaCol, rowWeights = rowWeights, colWeights = colWeights, iter = iterOut-1, fit = "RCM_NB_constr", libSizesMLE = switch(marginEst, "MLE" = exp(logLibSizesMLE), "marginSums" = libSizes), abundsMLE = switch(marginEst, "MLE" = exp(logAbundsMLE), "marginSums" = abunds), confounders = confounders, covariates = covariates, confParams = confParams)
  }
    if(!all(convergence) ){
...
```{r LRnb, purl=FALSE} 
# DON'T USE "p" as variable name, partial matching in the grad-function!!!

# A function that returns the value of the log-likelihood ratio of alpha
LR_nb <- function(Alpha, X, CC, responseFun = c("linear","quadratic","nonparametric"), muMarg, thetas,  cReg, d, ncols, nleqslv.control = list(trace=FALSE), ...){#
...
#    }) #Same output but our function is much faster (10 fold!) and always converges!
   ...
   NB_param_noLab = nleqslv(x = hh , reg = design,  fn = dNBllcol_constr_noLab, thetas = thetas, muMarg = muMarg, cReg = cReg, X = X, control = nleqslv.control, jac = JacCol_constr_noLab)$x
...
#--------------------------------------#
# The jacobian of this function
JacCol_constr_noLab = function(betas, X, reg, thetas, muMarg, cReg) {
  # @param beta: a vector of regression parameters with length v (\boldsymbol{beta}_j)
  # @param X: the nxp data matrix
  # @param reg: a matrix of regressors of dimension nxv (\mathbf{C} \boldsymbol{\alpha})
  # @param thetas: The dispersion parameters (a vector of length p)
  ...
  tmp2 = vapply(1:ncol(reg), FUN.VALUE = tmp, function(x){reg[,x]*tmp})
  Jac = crossprod(reg, apply(tmp2, c(1,3), sum))  
  return(-Jac)
}
#--------------------------------------#
# A function to estimate alpha based on the LR-criterion
estAlpha = function(Alpha, X, CC, d, k, ncols, responseFun = c("linear","quadratic","nonparametric"), muMarg, thetas, cReg, alphaK, control.outer = list(trace=TRUE), control.optim=list(trace=TRUE), nleqslv.control = list() ){
  ...
  alphaNew = constrOptim.nl(par = Alpha, fn =LR_nb, gr = NULL, heq = heq_nb, heq.jac = heq_nb_jac, alphaK = alphaK, X=X, CC=CC, responseFun = c("linear"), muMarg = muMarg, thetas = thetas, cReg = cReg, d = d, ncols=ncols, control.outer = control.outer, control.optim = control.optim, nleqslv.control = nleqslv.control, k = k) #
}
#--------------------------------------#
# A linear equality constraints function for estAlpha (see ?alabama)
heq_nb = function(Alpha, alphaK, d, k, ...){
  # Alpha to AlphaZS
  center = sum(Alpha)
  size = sum(Alpha^2)-1
  if(k==1) { return(c(center, size))}
  ortho = alphaK %*% Alpha
  c(center, size, ortho)
}
#--------------------------------------#
# It's jacobian, returns a matrix with d columns and at least 2 rows
heq_nb_jac = function(Alpha, alphaK, d, k, ...){
  center = rep.int(1,d)
  size = 2*Alpha
  if(k==1) {return(rbind(center, size))}
  ortho = alphaK
  rbind(center, size, ortho)
}
#--------------------------------------#
# A function that goes from the treatment coding to the zero-sum respresentation.
treatToZero = function(params){
  # @param params: a named parameter vector in treatment coding
  
  # @return a named parameter vector in zero-sum coding
}
#--------------------------------------#
# A function that goes from the zero-sum coding to the treatment respresentation.
zeroToTreat = function(params){
  # @param params: a named parameter vector in zero-sum coding
  
  # @return a named parameter vector in treatment coding
}
```
...
RCM = function(dat, k, round=FALSE, distribution= c("NB", "ZIP", "ZINB"), prevCutOff = 0.01, minFraction = 0.1, rowWeights = "uniform", colWeights = "marginal", covariates = NULL, confounders = NULL, ...){
  ...
  # @param covariates (optional): In case "dat" is a phyloseq object, the names of the sample variables to be used as covariates in the constrained analysis. In case dat is a matrix, a nxf matrix or dataframe of covariates. Character variables will be converted to factors, with a warning. Defaults to NULL, in which case an unconstrained analysis is carried out.
  ...
  if(!is.null(confounders)){
    if(class(confounders) %in% c("vector","matrix")){
      ...
      if(class(confounders[,1]) %in% c("character", "factor")){ #If character or factor, build model matrix
        ...
        confModelMatTrim = model.matrix(object = as.formula(paste("~" , paste(colnames(confounders), collapse="+"),"-1")), contrasts.arg = apply(confounders,2,contrasts, contrasts=FALSE)) 
        confModelMat = model.matrix(object = as.formula(paste("~", paste(colnames(confounders), collapse="+"))), contrasts.arg = apply(confounders,2,contrasts, contrasts=TRUE)) 
      } else {
        confModelMat = confModelMatTrim = confounders
      }
    } else if(class(confounders) == "dataframe"){
      if(NROW(X)!=NROW(confounders)){ #Check dimensions
        stop("Data and confounder matrix do not have the same number of samples! \n")
      }
      confModelMatTrim = model.matrix( object = as.formula(paste("~", paste(names(confounders), collapse="+"),"-1")), contrasts.arg = lapply(confounders[,sapply(confounders, is.factor),drop=FALSE],contrasts, contrasts=FALSE))
      confModelMat = model.matrix( object = as.formula(paste("~", paste(names(confounders), collapse="+"))), contrasts.arg = lapply(confounders[,sapply(confounders, is.factor),drop=FALSE],contrasts, contrasts = TRUE))
  ...
  # In this case we will 1) Use treatment coding for the dummies to simplify the estimation 2) Include an intercept. The tricky part lies in the normalization of alpha, see introduction.
  if(!is.null(covariates)){
    if(class(covariates) %in% c("vector","matrix")){
      if(NROW(X)!=NROW(covariates)){ #Check dimensions
        stop("Data and confounder matrix do not have the same number of samples! \n")
      }
      if(is.vector(covariates)){
      covariates = as.matrix(covariates) #Convert to matrix if only 1 variable
      }
      if(is.null(colnames(covariates))){ #assign names if needed
        colnames(covariates) = paste("var",seq_len(NCOL(covariates)))
      }
      if(class(covariates[,1]) %in% c("character", "factor")){ #If character or factor, build model matrix
        if(class(covariates[,1])=="character"){
          warning("Converting character vector to factor! \n")
        }
        covModelMat = model.matrix( ~ paste(colnames(covariates), sep="+")) #Without intercept
      } else {
        covModelMat = covariates
      }
    } else if(class(covariates) == "dataframe"){
      if(NROW(X)!=NROW(covariates)){ #Check dimensions
        stop("Data and covounder matrix do not have the same number of samples! \n")
      }
      covModelMat = model.matrix( ~ paste(names(covariates), sep="+"))
      ...
      covModelMat = model.matrix( ~ paste(names(covariates), sep="+"), data=sample_data(dat))
    } else{
...
confTest = RCM(RCMhmpFilt[["Anterior.nares"]], distribution="NB", k = K, nleqslv.control= nleqslv.control, maxItOut=5e3, prevCutOff=0.01, colWeights="marginal", rowWeights = "uniform", marginEst = "MLE", round=TRUE, confounders = c("sex","RUNCENTER2"))
```
...
testLR = LR_nb(alphaT, X, t(regMat), responseFun ="linear", muMarg = muMargObs, thetas = thetasCon[idCol], cMat = cVec2, psi=1, d= nrow(regMat), ncols=ncol(X))

testAlpha = estAlpha(c(0.01,0.01,0.01), X, t(regMat), responseFun ="linear", muMarg = muMargObs, thetas = thetasCon[idCol], cMat = cVec2, psi=1, d = nrow(regMat), ncols=ncol(X), alphaK = NULL)
...
alphaT = c(0.2,
           ...
           1.5,
           1.2,0.9,-3.1,
           1.1,-2.5,1.4) #The categorical variables with sum coding (they center around zero by default. Remember to estimate with treatment coding but normalize based on sum coding.)
alphaT = alphaT-mean(alphaT)
alphaT = alphaT/sqrt(sum(alphaT^2))
regMat = t(
  cbind(model.matrix(~var1+var2+var3+var4-1, contrasts = list(var4="contr.sum"))
, model.matrix(~var5-1, contrasts = list( var5="contr.sum")))
                  )
...
testRCMconstr2Job = mcparallel(RCM_NB(X = X, k = 2, covariates = t(regMat)[,-c(6,9)] ))
