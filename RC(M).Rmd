...
plot.RCM = function(RCMfit, Dim = c(1,2), 
                    ...
                    taxFrac = 0.025, scalingFactor = NULL, plotTaxa = TRUE, quadDrop = 0.995, nPoints = 1e3,
...
Palette=rainbow(length(unique(dataSam$colourPlot)))
    ...
    dataTax = within(dataTax, { #Scale the arrows
          ...
          meanPeakHeights = apply(RCMfit$NB_params[,,Dim],2, function(x){
            A = x[3,]; B = x[2,]; C = x[1,];
            mean(sapply(exp(B^2 * RCMfit$psis-4*A*C)/(4*A), function(y){max(y,1/y)})) #select largest relative departure
          })
    ...
    plot <- plot + 
      geom_point(data=dataTax, aes_string(x='end1', y='end2', fill="colour", size), pch = 21, show.legend=FALSE, inherit.aes = FALSE) + #The centers
      geom_path(inherit.aes = FALSE, data = dataEllipse, mapping = aes_string(x = "x", y = "y", group = "taxon", colour = "colour"))
    } else {
