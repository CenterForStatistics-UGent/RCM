...
Sample plots with the libsizes in colour
...
par(mfrow=c(3,5))
lapply(names(solListNS), function(Y){with(solListNS[[Y]], {plot(main=Y,rMat[,1] *psis[1] ,rowSums(X), ylab="Library sizes",xlab="Dim1" )})})
```
...
par(mfrow=c(3,5))
lapply(names(solListNS), function(Y){with(solListNS[[Y]], {plot(main=Y,rMat[,2] *psis[2],rowSums(X), ylab="Library sizes",xlab="Dim2" )})})
```
...
par(mfrow=c(3,5))
lapply(names(solListNS), function(Y){try(with(solListNS[[Y]], {plot(main=Y,rMat[,3] *psis[3],rowSums(X), ylab="Library sizes",xlab="Dim3")}))})
```
...
with(solListNS[["MLEunifunif"]], {plot(rowSums(X),libSizesMLE, xlab ="Library sizes", ylab = "MLE sequencing depth",log="xy")})
abline(0,1);abline(h=c(1e4,1e5), col="red");abline(v=c(1e4,1e5), col="red")
MSElibs = mean((rowSums(solListNS[["MLEunifunif"]]$X)-rep(c(1e4,1e5), each=150))^2)
MSEmle = mean((solListNS[["MLEunifunif"]]$libSizesMLE-rep(c(1e4,1e5), each=150))^2)
```
...
par(mfrow=c(2,5))
lapply(names(solListNS), function(Y){try(with(solListNS[[Y]], {plot(main=Y,(rMat %*% diag(psis))[,2:3], xlab = "Dim2",ylab="Dim3")}))})
```
...
#Abundances
par(mfrow=c(3,5))
sapply(names(solListNS), function(Y){with(solListNS[[Y]], {plot(main=Y,cMat[1,], colSums(X), log="y", cex=0.5, ylab="Mean  abundances",xlab="Dim1")})})
```

In Pearson correlations

```{r Taxon scores abunds correlations}
sapply(names(solListNS), function(Y){with(solListNS[[Y]], {cor(cMat[1,], colSums(X))})})
```
...
Take a look at the influence function values for the first psi para
...
palette(cols)
par(mfrow=c(3,4))
lapply(names(solListWSab), function(Y){plotRCM(solListWSab[[Y]], samColour = sampleSigNBab, main=Y, biplot=FALSE, Dim = c(1,2), libInset=c(-0.8,0))})
lapply(names(solListWSab), function(Y){try(plotRCM(solListWSab[[Y]], samColour = sampleSigNBab, main=Y, biplot=FALSE, Dim = c(1,3), libInset=c(-0.6,0)))})
```

Only when the margins are estimated with MLE are the first two dimensions used to separate the signal well. Especially the unirom weighting for the samples and marginal for the columns (as was our first intuition) appears to perform well.

```{r abSignalPlots 13D}
par(mfrow=c(3,4))
lapply(names(solListWSab), function(Y){try(plotRCM(solListWSab[[Y]], samColour = sampleSigNBab, main=Y, biplot=FALSE, Dim = c(2,3), libInset=c(-0.6,0)))})
...
The first dimension does not separate the groups? It seems to be a "library size dimension" if the offsets are estimated using the margins.

```{r ab plots2 marginal taxon weighting, eval=FALSE}
...
```{r ab biplots margTaxWeights, eval=FALSE}
par(mfrow=c(2,2))
...
#   
#   testLUJob1 = mcparallel(RCM(dataMatSigNBLU, method="NB_1B1", k=3, nleqslv.control= nleqslv.control.lu, maxItOut=1e3, rowWeights = "marginal", colWeights = colWeightsTestLU1, prevCutOff=0.01))
#   testLU1 = mccollect(testLUJob1, FALSE)[[1]]
#   
#  testLUJob2 = mcparallel(RCM(dataMatSigNBLU, method="NB_1B1", k=3, nleqslv.control= nleqslv.control.lu, maxItOut=1e3, rowWeights = "marginal", colWeights = colWeightsTestLU2, prevCutOff=0.01))
#   testLU2 = mccollect(testLUJob2, FALSE)[[1]]
#   
#  testLUJob3 = mcparallel(RCM(dataMatSigNBLU, method="NB_1B1", k=3, nleqslv.control= nleqslv.control.lu, maxItOut=1e3, rowWeights = "marginal", colWeights = colWeightsTestLU3, prevCutOff=0.01))
#   testLU3 = mccollect(testLUJob3, FALSE)[[1]]
#   
#   testLUJob1unif = mcparallel(RCM(dataMatSigNBLU, method="NB_1B1", k=3, nleqslv.control= nleqslv.control.lu, maxItOut=1e3, rowWeights = "uniform", colWeights = colWeightsTestLU1, prevCutOff=0.01))
#   testLU1unif = mccollect(testLUJob1unif, FALSE)[[1]]
#   
#  testLUJob2unif = mcparallel(RCM(dataMatSigNBLU, method="NB_1B1", k=3, nleqslv.control= nleqslv.control.lu, maxItOut=1e3, rowWeights = "uniform", colWeights = colWeightsTestLU2, prevCutOff=0.01))
#   testLU2unif = mccollect(testLUJob2unif, FALSE)[[1]]
#   
#   testLUJob3unif = mcparallel(RCM(dataMatSigNBLU, method="NB_1B1", k=3, nleqslv.control= nleqslv.control.lu, maxItOut=1e3, rowWeights = "uniform", colWeights = colWeightsTestLU3, prevCutOff=0.01))
#   testLU3unif = mccollect(testLUJob3unif, FALSE)[[1]]
#   
#   #Try inverse weighting?
#     testLUJobInv = mcparallel(RCM(dataMatSigNBLU, method="NB_1B1", k=3, nleqslv.control= nleqslv.control.lu, maxItOut=1e3, rowWeights = sum(dataMatSigNBLU)/rowSums(dataMatSigNBLU), colWeights =1/margWeightsLU, prevCutOff=0.01))
#   testLUInv = mccollect(testLUJobInv, FALSE)[[1]]
#   
#   #More realistic libsizes
#       testLUJobLibs = mcparallel(RCM(dataMatSigNBLUag, method="NB_1B1", k=2, nleqslv.control= nleqslv.control.lu, maxItOut=1e3, rowWeights = "uniform", colWeights = "marginal", prevCutOff=0.01))
#   testLULibs = mccollect(testLUJobLibs, FALSE)[[1]]
  ...
  save(dataMatSigNBLU, syntNBSigunifmarg_1B1_LU, syntNBSigunifunif_1B1_LU,syntNBSigmargmarg_1B1_LU,syntNBSigmargunif_1B1_LU,taxaSigNBLU, sampleSigNBLU, dataMatSigNBLU,  file="toyDataSigLU.RData")#syntNBSigunif, syntNBSigmarg, syntNBSigmarg_3,testLU1, testLU2, testLU3, testLU1unif, testLU2unif, testLU3unif,testLUInv,testLULibs,
