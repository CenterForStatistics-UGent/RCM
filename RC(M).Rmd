...
```{r turnmice}
if(!file.exists("turntMiceRCM.RData")){
...
##### Other improved versions
   ...
   * Hellinger distance. Hellinger distance may be a robust alternative to correspondence analysis, although it is designed for distances between distributions rather than between real data ratios. It has been used in publications on microbiome, e.g. Kreisinger _et al._, 2015, Hollister _et al._ 2015, Vital _et al._, 2015.
...
location and dispersion effects
 ...
 - CoDa: PCA of log-ratio transformed data (Gloor _et al._ (2016), Gloor and Reid(2016)
...
mapply(RCMlist, CAresList0, CodaResList0, CodaScaledResList0, BCresList0, JSDresList0, BCresList0Rel, HelDistresList, FUN = function(RCM, CA, CoDa,  BC, JSD, BCrel, Hell){
                    list(RCM = RCM, CA = CA, 
                         CoDa = CoDa,
                         BC = BC, JSD = JSD, 
...
weightedTaxonScores = function(X, rowScores){
  crossprod(rowScores, X %*% diag(1/colSums(X)))
}
#-----------------------------------------#
...
plotCor = function(scores, datList, Dims = 1:3, scoreDim = "rows", dataMat = FALSE){
par(mar = c(4,4,4,4))
if(!dataMat){
...
#---------------------------------#
distRatioPlot = function(resListRows) {
...
An objective simulation approach would be to use non-parametric resampling from a true dataset, as in _SimSeq_
