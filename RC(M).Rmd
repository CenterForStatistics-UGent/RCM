...
#A sample biplot
resList0Rows = lapply(RCMresList0, extractRows)
...
```{r resList0Evalsil, purl = FALSE, fig.cap = "Mean silhouettes by method, results of 100 simulations shown for first two dimensions. Red diamonds indicate means. A silhouette of 1 point to perfect classification, 0 to borderline cases, -1 to perfect wrong classification \\label{fig:silhouette1}"}
sil0  = lapply(resList0Rows, function(x){
  sapply(x, silhouette, clusters = groupFactor)
})
meanSil0 = sapply(sil0, function(x){
  colMeans(x)
})
meanSil0df = melt(meanSil1, value.name = "Silhouette", varnames = c("Method"))
boxplot(Silhouette~Method, main = "Mean silhouettes by method", data = meanSil0df)
meansSil0 = tapply(meanSil0df$Silhouette, meanSil0df$Method, mean)
points(meansSil0, col="red", pch=18, cex = 1.6)
```
...
par(mar = c(4,4,4,4))
cor1 = lapply(1:3, function(Dim){mapply(resList0Rows, datList1, FUN = function(x,z){
  libSizes = rowSums(z)
...
resList1 = mapply(RCMresList1, CAresList1, CodaResList1, CodaScaledResList1, BCresList1, JSDresList1,
                  FUN = function(RCM, CA, CoDa, CoDaScale, BC, JSD){
                    list(RCM = RCM, CA = CA, CoDa = CoDa, CoDaScale = CoDaScale, BC = BC, JSD = JSD)
                  }, 
                  SIMPLIFY = FALSE
)
...
datList2mat = lapply(RCMresList2, function(x){x$X})
...
obj = resList2Rows[[ID]]
lapply(names(obj),function(x){plot(obj[[x]], col = groupFactor2, main=x, xlab = "Dim 1",ylab = "Dim 2", type = "s")})
rm(ID, obj)
par(mfrow = c(1,1))
...
ID = sample(size=1, seq_along(resList1))
obj = resList2Rows[[ID]]
lapply(names(obj),function(x){plot(obj[[x]][,c(1,3)], col = groupFactor2, main=x, xlab = "Dim 1",ylab = "Dim 3", type = "s")})
# plot(resList1$RCM[[id]]$rMat[,1:2], col = groupFactor, main = "RC(M)")
# plot(resList1$CA[[id]]$u[,1:2], col = groupFactor, main = "CA", xlab = "Dim 1", ylab ="Dim2")
# plot(resList1$CoDA[[id]]$rotation[,1:2], col = groupFactor, main = "CoDA")
# plot(resList1$BC[[id]]$vectors[,1:2], col = groupFactor, main = "PCoA: Bray-Curtis")
...
par(mar = c(4,4,4,4))
cor2 = lapply(1:3, function(Dim){mapply(resList2Rows, datList2mat, FUN = function(x,z){
  libSizes = rowSums(z)
  sapply(x, function(y){libCor(y, libSizes = libSizes, Dim = Dim)})
}, SIMPLIFY = TRUE)})
...
The CoDa method is clearly affected by the library sizes
