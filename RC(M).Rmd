...
One restriction I intended to include but whcih turns out to be a very bad idea is to center the $\alpha$'s:

$$\boldsymbol{\alpha}_k^t\mathbf{1} = \mathbf{0}$$

This would force the parameter of a single continuous covariate to be 0, the parameters of two continuous variables to be each other's opposite
...
Filtering of rare taxa is not needed in this case, as all covariates are summarized in one "continuous" row score. For $\psi_k$ and $\boldsymbol{\alpha}_k$ we apply an infinity norm, for $\mathbf{s}_k$ the default is an L2-norm.
...
distanceFun = function(coord, clusters, Dim = c(1,2)){
  ...
  list(withinDist = withinDist, overalDist =  overalDist)
...
extractRows = function(x){
  with(x, list(
    ...
    BC = BC$vectors %*% diag(BC$values$Eigenvalues),
...
```{r resList1Evalsil, purl = FALSE, fig.cap = "Mean silhouettes by method, results of 100 simulations shown. Red diamonds indicate means. A silhouette of 1 point to perfect classification, 0 to borderline cases, -1 to perfect wrong classification \\label{fig:silhouette1}"}
...
```{r resList1Evaldist, purl = FALSE, fig.cap = "Mean silhouettes by method, results of 100 simulations shown. Red diamonds indicate means. A silhouette of 1 point to perfect classification, 0 to borderline cases, -1 to perfect wrong classification \\label{fig:silhouette1}"}
dist1  = lapply(resList1Rows, function(x){
  sapply(x, silhouette, clusters = groupFactor)
})
meanSil1 = sapply(sil1, function(x){
  colMeans(x)
...
save(resList1, file = "RCMsim/resList2.RData")
