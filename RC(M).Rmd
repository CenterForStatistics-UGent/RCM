...
dNBllcol_1B1 = function(beta, X, reg, thetas, muMarg, k, p, n, colWeights, nLambda, cMatK) {
  ...
  score = crossprod(reg,((X-mu)/(1+t(t(mu)/thetas)))) + colWeights*(lambda1 + lambda2*2*cMat + (lambda3 %*% cMatK))
...
dNBllrow_1B1 = function(beta, X, reg, thetas, muMarg, k, n , p, rowWeights, nLambda, rMatK) {
  ...
  score = c(tcrossprod(reg, (X-mu)/(1+t(t(mu)/thetas)))) + rowWeights*(lambda1 + lambda2* 2*rMat + rMatK %*% lambda3)
  #
...
RCM_NB_1B1 = function(X, k, rowWeights, colWeights, weightsChar, tol = 1e-3, maxItOut = 500, Psitol = 1e-3, verbose = TRUE, libSizes = NULL, NBRCM=NULL, global ="dbldog", nleqslv.control=list(), method=c("Broyden"), dispFrec=15, convNorm = 2, prior.df=10){
...
### Negative binomial: influence function 

To know which observation affect the obtained scores most we use the influence function

```{r Influence function NB}
...
RCM_ZIP_1B1 = function(X, k, rowWeights, colWeights, weightsChar, tol = 1e-3, maxItOut = 500, psiTol = 1e-4, chiTol=psiTol, verbose = TRUE, global ="dbldog", nleqslv.control=list(), method=c("Broyden"), twoCores=TRUE, convNorm = 2,  maxItMean = 20, maxItZeroes=30, ZIPRCM=NULL){
...
#                 "uniformuniform" = svd(X-muMarg))
  ...
  psis = sapply(psis, min, 10) #Temporary solution
...
tMat = apply(tMat, 2, function(rowS){
  ...
  rowRec[,KK, iterOut] = rMat[,KK] = Mlist$rMat
  colRec[KK,, iterOut] = cMat[KK,] = Mlist$cMat
  rowRecZero[,KK, iterOut] = tMat[,KK] = Mlist$tMat
  colRecZero[KK,, iterOut] = vMat[KK,] = Mlist$vMat
  psiRec[KK, iterOut] = psis[KK] = Mlist$psis
  chiRec[KK, iterOut] = chis[KK] = Mlist$chis
  
  ## 2)f. Change iterator
    iterOut[KK] = iterOut[KK] + 1
    
  ##Check convergence  (any numbered norm for row and column scores)
  convergence = ((iterOut[KK] <= maxItOut) && 
                   (all(abs(1-psis/psiOld) < psiTol)) &&
                   ((sum((1-rMatOld/rMat)^convNorm)/n)^(1/convNorm) < tol) && 
                   ((sum((1-cMatOld/cMat)^convNorm)/p)^(1/convNorm) < tol) ) && (all(abs(1-chis/chiOld) < chiTol)) &&  (sum(abs(1-tMat/tMatOld)^convNorm)/n)^(1/convNorm) < tol &&  (sum(abs(1-vMat/vMatOld)^convNorm)/p)^(1/convNorm) < tol 
} # END while-loop until convergence
...
```{r 3 plots2 marginal taxon weighting}
...
```{r 3D plots2 marginal taxon weighting}
...
if(!file.exists("toyDataSigab.RData")){
  ...
  save(dataMatSigNBab, syntNBSigunifmarg_1B1_ab, syntNBSigunifunif_1B1_ab,syntNBSigmargmarg_1B1_ab,syntNBSigmargunif_1B1_ab,taxaSigNBab, sampleSigNBab, file="toyDataSigab.RData")#syntNBSigunif, syntNBSigmarg, syntNBSigmarg_3,
...
The first dimension does not separate the groups?

```{r ab plots2 marginal taxon weighting}
subListab = solListWSab[c("unifmarg","margmarg")]
mapply(list(c(1,2), c(1,3),c(1,2), c(1,3)),rep(names(subListab),each=2), FUN = function(x,Y){plotRCM(solListWSab[[Y]], Colour = sampleSigNB3, main=Y, biplot=FALSE, Dim = x)})
palette(palStore)
```
...
```{r ab biplots}
par(mfrow=c(2,2))
lapply(names(solListWSab), function(Y){plotRCM(solListWSab[[Y]], samColour = sampleSigNBab, main=Y, biplot=TRUE, Dim = c(1,2), taxColour=taxaSigNBab)})
```
...
par(mfrow=c(2,2))
mapply(list(c(1,2), c(1,3),c(1,2), c(1,3)),rep(names(subListab),each=2), FUN = function(x,Y){plotRCM(subListab[[Y]], samColour = sampleSigNB3, main=Y, biplot=TRUE, Dim = x, taxColour=taxaSigNB3)})
par(mfrow=c(1,1))
Y="unifmarg"
plotRCM(subListab[[Y]], samColour = sampleSigNB3, main=Y, biplot=TRUE, Dim = c(1,3), taxColour=taxaSigNB3)
```
...
logP="y"
#Libsizes
lapply(names(solListWSab), function(Y){with(solListWSab[[Y]], {plot(main=Y,rMat[,1] *psis[1],rowSums(X), log=logP )})})
#lapply(names(solListWSab), function(Y){with(solListWSab[[Y]], {lines(lowess(rMat[,1] *psis[1], rowSums(X)))})})
lapply(names(solListWSab), function(Y){with(solListWSab[[Y]], {plot(main=Y,rMat[,2] *psis[2],rowSums(X), log=logP )})})
#lapply(names(solListWSab), function(Y){with(solListWSab[[Y]], {lines(lowess(rMat[,2] *psis[2], rowSums(X)))})})
lapply(names(solListWSab), function(Y){with(solListWSab[[Y]], {plot(main=Y,rMat[,3] *psis[3],rowSums(X), log=logP )})})
#lapply(names(solListWSab), function(Y){with(solListWSab[[Y]], {lines(lowess(rMat[,3] *psis[3], rowSums(X)))})})
```

The first dimension scores are related to the library sizes in all weighting schemes. This may explain why it is not used to discriminate between the groups.
...
if(!file.exists("syntZIPnoSig.RData")){
  syntZIPnoSigUnifUnifJob = mcparallel(RCM(dataMatZIPnoSig, method = "ZIP_1B1",k=3, nleqslv.control = nleqslv.controlZIP, colWeights = "uniform", rowWeights = "uniform", maxItOut=5e1))
  syntZIPnoSigUnifUnif =  mccollect(syntZIPnoSigUnifUnifJob, FALSE)[[1]] 
  
  syntZIPnoSigUnifMargJob = mcparallel(RCM(dataMatZIPnoSig, method = "ZIP_1B1",k=3, nleqslv.control = nleqslv.controlZIP, colWeights = "marginal", rowWeights = "uniform"))
  syntZIPnoSigUnifMarg =  mccollect(syntZIPnoSigUnifMargJob, FALSE)[[1]]
  
  syntZIPnoSigMargUnifJob = mcparallel(RCM(dataMatZIPnoSig, method = "ZIP_1B1",k=3, nleqslv.control = nleqslv.controlZIP, colWeights = "uniform", rowWeights = "marginal"))
  syntZIPnoSigMargUnif =  mccollect(syntZIPnoSigMargUnifJob, FALSE)[[1]]
  
   syntZIPnoSigMargMargJob = mcparallel(RCM(dataMatZIPnoSig, method = "ZIP_1B1",k=3, nleqslv.control = nleqslv.controlZIP, colWeights = "uniform", rowWeights = "uniform"))
