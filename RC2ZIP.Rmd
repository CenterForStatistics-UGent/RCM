...
reqpkg <- c("phyloseq","MASS", "parallel","nleqslv", "HMP", "pscl", "VGAM")
...
See the RC2NB.Rmd document for elaborate theoretical background.
...
Unlike the RC(2)NB model, $f_j$ has to be estimated as well, there is no obvious candidate here.
...
Estep  = function(y, rMat, cMat, tMat, vMat, abunds, k, libSizes,  Zcol, psis, chis){
  
  # @return: The values of Z
  expMu = exp(rMat %*% (cMat*psis)) * outer(libSizes, abunds)
  regZero =  t(Zcol + t(tMat %*% (vMat * chis)))
  
  Z = y
  Z[y>0] = 0
  Z[y==0] = (1+exp(-regZero-expMu))[y==0]^(-1)
  Z
}
...
Mstep = function(Z, y, rMat, cMat, tMat, vMat, abunds, k, libSizes,  Zcol, psis, chis, lambdaCol, lambdaRow, lambdaColZero, lambdaRowZero, twoCores=TRUE, tol=1e-3, psiTol = 1e-4, chiTol = psiTol, convNorm = 2 , maxIt = 200,n=n, p=p, global=global, nleqslv.control= nleqslv.control, nLambda){
  
#Optimization of the mena and zero-inflated components are independent (see Lambert 1992), so fork here
if(twoCores){
resList = mclapply(mc.cores=2, c(meanEst, ZIest), function(fun){
  fun(y=y, rMat=rMat, cMat=cMat, tMat=tMat, chis=chis, vMat=vMat, Zcol=Zcol, lambdaCol=lambdaCol, lambdaRow=lambdaRow, lambdaRowZero=lambdaRowZero, lambdaColZero=lambdaColZero, psiTol=psiTol, chiTol=chiTol, tol=tol, convNorm = convNorm, nleqslv.control = nleqslv.control, global=global, nLambda=nLambda, k=k, Z=Z, abunds=abunds, libSizes = libSizes,n=n, p=p, psis=psis, maxItMean = 5, maxItZeroes = 20)
})
} else{
  resList = lapply(c(meanEst, ZIest), function(fun){  fun(y=y, rMat=rMat, cMat=cMat, tMat=tMat, chis=chis, vMat=vMat, Zcol=Zcol, lambdaCol=lambdaCol, lambdaRow=lambdaRow, lambdaRowZero=lambdaRowZero, lambdaColZero=lambdaColZero, psiTol=psiTol, chiTol=chiTol, tol=tol, convNorm = convNorm, nleqslv.control = nleqslv.control, global=global, nLambda=nLambda, k=k, Z=Z,p=p, n=n, abunds=abunds, libSizes = libSizes, psis=psis, maxItMean = 5, maxItZeroes = 20)})
  }
...
meanEst = function(y, rMat, cMat, Z, abunds, libSizes, k, global, nleqslv.control, tol, psiTol, chiTol, tMat, vMat, chis, Zcol,lambdaCol, lambdaRow, lambdaColZero, lambdaRowZero,convNorm,  nLambda,n,p,psis, maxItMean = 10, maxItZeroes = 10){
 #Mean component
  
    iter = 1
  while((iter==1 || !converged) && iter<maxIt){
  ...
  cat("Estimating psis \n")
  psis = sort(abs(nleqslv(fn = dZipMeanPsi, x = psis, y = y, rMat = rMat, cMat = cMat, Z=Z, abunds=abunds, libSizes = libSizes, k=k, global=global, control = nleqslv.control, jac=ZipJacobianPsi)$x), decreasing=TRUE)
  
  #As weighted Poisson regression
#   regMean = sapply(1:k, simplify="array", function(K){
#     outer(rMat[,K], cMat[K,])
#   })
  
    ## Row scores
  cat("Estimating row scores mean \n")
  rMatSol = nleqslv(fn = dZipMeanRmat, x = c(rMat, lambdaRow), y = y, psis = psis, cMat = cMat, abunds =abunds, libSizes = libSizes, k=k, n=n, p=p, global=global, control = nleqslv.control, jac=ZipJacobianRmat, Z=Z, nLambda=nLambda)$x
  ...
  cMatSol = nleqslv(fn = dZipMeanCmat, x = c(t(cMat), lambdaCol), y = y, psis = psis, rMat = rMat,abunds =abunds, libSizes = libSizes, k=k, n=n, p=p, global=global, control = nleqslv.control, jac=ZipJacobianCmat, Z=Z, nLambda=nLambda)$x
...
ZIest = function(y, rMat, cMat, Z, abunds, libSizes, k, global, nleqslv.control, tol, psiTol, chiTol, tMat, vMat, chis, Zcol,lambdaCol, lambdaRow, lambdaColZero, lambdaRowZero,convNorm,n, p,  nLambda,psis, maxItMean = 10, maxItZeroes = 10){
  ...
  chis = sort(abs(nleqslv(fn = dZipMeanChi, x = chis, y = y, tMat = tMat, vMat = vMat, Z=Z, abunds =abunds, libSizes = libSizes, k=k, global=global, control = nleqslv.control, Zcol=Zcol, jac=ZipJacobianChi)$x), decreasing=TRUE)
  
    ## Row scoers
    cat("Estimating row scores zeroes \n")
    tMatSol = nleqslv(fn = dZipMeanTmat, x = c(tMat, lambdaRowZero), y = y, chis = chis, vMat = vMat,  abunds =abunds, libSizes = libSizes, k=k, n=n, p=p, global=global, control = nleqslv.control, Zcol=Zcol, jac=ZipJacobianTmat, Z=Z, nLambda=nLambda)$x
    ...
    vMatSol = nleqslv(fn = dZipMeanVmat, x = c(t(vMat), lambdaColZero), y = y, chis = chis, tMat = tMat, abunds =abunds, libSizes = libSizes, k=k, n=n, p=p, global=global, control = nleqslv.control, Zcol=Zcol, jac=ZipJacobianVmat, Z=Z, nLambda=nLambda)$x
...
dZipMeanPsi = function(beta, y, abunds, k, libSizes, Z, rMat, cMat){
  ...
  mu = exp(arrayprod(regMean, beta)) * outer(libSizes, abunds)
  
  vapply(1:k,FUN.VALUE=0, function(u){
  sum((1 - Z)*(y - mu)*regMean[,,u])
  })
}

#--------------------------------------#
#A jacobian for the psi parameters
ZipJacobianPsi = function(beta, y, rMat, cMat, abunds, k, libSizes, Z){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param y: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param theta: a vector of length p with the dispersion parameters
  ...
  sapply(1:k, function(K){
    ...
    sum(-mu*regMean[,,Kinner]*regMean[,,K]*(1-Z))
  })})
}

#--------------------------------------#
dZipMeanRmat = function(beta, y, psis,cMat, abunds, k,p,n, libSizes, Z, nLambda){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param y: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param theta: a vector of length p with the dispersion parameters
  ...
  regMean = cMat*psis #kxp
  mu = exp(rMat %*% regMean) * outer(libSizes, abunds)
  ...
  score = c(t(
    tcrossprod(regMean ,(1-Z)*(y-mu)) + 
                        lambda1) + t(lambda2*2*t(rMat)) + (rMat %*% lambda3Mat)
    )
...
#--------------------------------------#
#A jacobian for the psi parameters
ZipJacobianRmat = function(beta, y, psis, cMat, abunds, k, p, n, libSizes, Z, nLambda){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param y: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param theta: a vector of length p with the dispersion parameters
  ...
  for (K in 1:(k-1)){
      for(Kinner in (K+1):k){
        #dLag²/dr_{ik}dr_{ik'}
      diag(Jac[(n*(K-1)+1):(n*K),(n*(Kinner-1)+1):(n*Kinner)]) =  ((1-Z)*(-t(t(mu)* regMean[Kinner,]))) %*% regMean[K,] +
         lambda3Mat[Kinner, K]
      }
  }
  ...
  #Symmetrize
  Jac = Jac + t(Jac)
    diag(Jac[1:(n*k),1:(n*k)]) = c(tcrossprod(-mu*(1-Z), regMean^2) + 
                                     2*rep(beta[((n+1)*k+1):((n+2)*k)], each = n))
  Jac
...
# image(JacSpa)
#--------------------------------------#
dZipMeanCmat = function(beta, y, psis,rMat, abunds, k,p,n, libSizes, Z, nLambda){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param y: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param theta: a vector of length p with the dispersion parameters
  ...
  regMean = t(t(rMat)*psis)
  mu = exp(regMean %*% cMat) * outer(libSizes, abunds)
...
#--------------------------------------#
#A jacobian for the psi parameters
ZipJacobianCmat = function(beta, y, psis, rMat, abunds, k, p, n, libSizes, Z, nLambda){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param y: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param theta: a vector of length p with the dispersion parameters
  ...
  for (K in 1:(k-1)){
      for(Kinner in (K+1):k){
        #dLag²/dr_{ik}dr_{ik'}
      diag(Jac[(p*(K-1)+1):(p*K),(p*(Kinner-1)+1):(p*Kinner)]) =  crossprod(regMean[,K],(1-Z)*(-(mu* regMean[,Kinner])))  +
         lambda3Mat[Kinner, K]
      }
  }
  ...
  Jac = Jac + t(Jac)
         ...
         diag(Jac[1:(p*k),1:(p*k)]) = c(crossprod(-mu*(1-Z), regMean^2) + 
                                     2*rep(beta[((p+1)*k+1):((p+2)*k)], each = p)*abunds)
  Jac
}

#--------------------------------------#

dZipMeanChi = function(beta, y, abunds, k, libSizes, Z, tMat, vMat, Zcol, nLambda){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param y: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param theta: a vector of length p with the dispersion parameters
  ...
  GZero = expit(t(t(tMat %*% (vMat*beta))+Zcol))
  
  vapply(1:k,FUN.VALUE=0, function(u){
  sum((Z-GZero)*regMean[,,u])
  })
  
# crossprod(regMean,(Z-muZero/(1+muZero)))
}

#--------------------------------------#
#A jacobian for the psi parameters
ZipJacobianChi = function(beta, y, tMat, vMat, abunds, k, libSizes, Z, Zcol){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param y: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param theta: a vector of length p with the dispersion parameters
  ...
  expZero = exp(t(t(tMat %*% (vMat*beta))+Zcol))
  
  sapply(1:k, function(K){
    vapply(1:k,FUN.VALUE=0, function(Kinner){
    -sum(regMean[,,Kinner]*regMean[,,K]*expZero/(1+expZero)^2)
  })})
}

#--------------------------------------#
dZipMeanTmat = function(beta, y, chis,vMat, abunds, k,p,n, libSizes, Z, Zcol, nLambda){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param y: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param theta: a vector of length p with the dispersion parameters
  ...
  score = c(t(t(
    tcrossprod((Z-GZero), regMean) + 
                         t(lambda2*2*t(tMat)) + (tMat %*% lambda3Mat)
    ) + lambda1))
  
  centers = colSums(tMat)
  unitSums = colSums(tMat^2)-1
  orthogons = unlist(sapply(1:(k-1), function(K){
    vapply((K+1):k,FUN.VALUE=0, function(Kinner){
      sum(tMat[,K]*tMat[,Kinner])
    })
  }))
    return(c(score,centers, unitSums, orthogons))
}
...
#--------------------------------------#
#A jacobian for the psi parameters
ZipJacobianTmat = function(beta, y, chis, vMat, abunds, k, p, n, libSizes, Z, Zcol, nLambda){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param y: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param theta: a vector of length p with the dispersion parameters
  ...
  muZero = exp(t(t(tMat %*% (vMat*chis))+Zcol))
  ...
  Jac[1:(n*k),(n*k+1):((n+1)*k)] = c(rep.int(c(rep.int(1,n), rep.int(0,n*k)),k-1), rep.int(1,n)) 
  #dLag²/dr_{ik}dlambda_{2k}
  Jac[1:(n*k),((n+1)*k+1):((n+2)*k)] = vapply(1:k,FUN.VALUE = vector("numeric",n*k), function(K){c(rep.int(0,(K-1)*n),2 *tMat[,K],rep.int(0,(k-K)*n))})

  for (K in 1:(k-1)){
      for(Kinner in (K+1):k){
        #dLag²/dr_{ik}dr_{ik'}
      diag(Jac[(n*(K-1)+1):(n*K),(n*(Kinner-1)+1):(n*Kinner)]) = -tcrossprod(regMean[Kinner,],t(regMean[K,]*t(muZero/(1+muZero)^2))) + lambda3Mat[Kinner, K]
      }
  }
  #dLag²/dr_{ik}dlambda_{3kk'}
   Jac[1:(n*k),((n+2)*k+1):(n*k+nLambda)] = unlist(sapply(1:(k-1), function(K){
        vapply((K+1):k,FUN.VALUE = vector("numeric",n*k), function(Kinner){
          c(rep.int(0, n*(K-1)), tMat[,Kinner], rep.int(0, n*(Kinner-K-1)), tMat[,K],rep.int(0, n*(k-Kinner)))
        })
      }))
  
  #Symmetrize
  Jac = Jac + t(Jac)
    diag(Jac[1:(n*k),1:(n*k)]) = c(-tcrossprod(muZero/(1+muZero)^2, regMean^2) + 2*rep(beta[((n+1)*k+1):((n+2)*k)], each = n))
    
  Jac
}
#--------------------------------------#
dZipMeanVmat = function(beta, y, chis,tMat, abunds, k,p,n, libSizes, Z, Zcol, nLambda){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param y: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param theta: a vector of length p with the dispersion parameters
  ...
  score = c(t(
    crossprod(regMean,(Z-muZero/(1+muZero))) + 
                        t(abunds*t(lambda1 + lambda2*2*vMat + (lambda3Mat %*% vMat)))
    ))
  
  centers = colSums(abunds*t(vMat))
  unitSums = colSums(abunds*t(vMat^2))-1
  orthogons = unlist(sapply(1:(k-1), function(K){
    vapply((K+1):k,FUN.VALUE=0, function(Kinner){
      sum(vMat[K,]*vMat[Kinner,]*abunds)
    })
  }))
    return(c(score,centers, unitSums, orthogons))
}
...
#--------------------------------------#
#A jacobian for the psi parameters
ZipJacobianVmat = function(beta, y, chis, tMat, abunds, k, p, n, libSizes, Z, Zcol, nLambda){
  # @param beta: a vector of r regression parameters to optimize: the r psi parameters
  # @param y: the nxp data matrix
  # @param reg: a nxpxr regressor array with r the number of regressors
  # @param theta: a vector of length p with the dispersion parameters
  ...
  Jac[1:(p*k),(p*k+1):((p+1)*k)] = c(rep(c(abunds, rep.int(0,p*k)),k-1), abunds)
  #Jac[1:(p*k),(p*k+1):((p+1)*k)] = sapply(1:k, function(K){c(rep(0,(K-1)*p),abunds,rep(0,(k-K)*p))})
  Jac[1:(p*k),((p+1)*k+1):((p+2)*k)] = vapply(1:k,FUN.VALUE = vector("numeric", p*k), function(K){c(rep.int(0,(K-1)*p),abunds*2 *vMat[K,],rep.int(0,(k-K)*p))})

  for (K in 1:(k-1)){
      for(Kinner in (K+1):k){
        #dLag²/dr_{ik}dr_{ik'}
      diag(Jac[(p*(K-1)+1):(p*K),(p*(Kinner-1)+1):(p*Kinner)]) = -regMean[,Kinner]%*%(regMean[,K]*muZero/(1+muZero)^2) + lambda3Mat[Kinner, K]*abunds
      }
  }
    #dLag²/ds_{ik}dlambda_{3kk'}
    Jac[1:(p*k),((p+2)*k+1):(p*k+nLambda)] = unlist(sapply(1:(k-1), function(K){
        vapply((K+1):k,FUN.VALUE = vector("numeric",p*k), function(Kinner){
          c(rep.int(0, p*(K-1)), abunds*vMat[Kinner,], rep.int(0, p*(Kinner-K-1)), abunds*vMat[K,],rep.int(0, p*(k-Kinner)))
        })
      }))
  
  #Symmetrize
  Jac = Jac + t(Jac)
    diag(Jac[1:(p*k),1:(p*k)]) = c(t( -crossprod(regMean^2,muZero/(1+muZero)^2)) + abunds *2*rep(beta[((p+1)*k+1):((p+2)*k)], each = p))
    
  Jac
}
# #--------------------------------------#
# dZipMeanZcol = function(beta, y, chis,tMat, abunds, k,p,n, libSizes, Z, vMat){
#   # @param beta: a vector of r regression parameters to optimize: the r psi parameters
#   # @param y: the nxp data matrix
#   # @param reg: a nxpxr regressor array with r the number of regressors
#   # @param theta: a vector of length p with the dispersion parameters
...
# #--------------------------------------#
# #A jacobian for the psi parameters
# ZipJacobianZcol = function(beta, y, psis, cMat, abunds, k, p, n, libSizes, Za){
#   # @param beta: a vector of r regression parameters to optimize: the r psi parameters
#   # @param y: the nxp data matrix
#   # @param reg: a nxpxr regressor array with r the number of regressors
#   # @param theta: a vector of length p with the dispersion parameters
...
#     Zcol = beta[1:p]
# #regMean = 0
#   muZero = t(t(exp(tMat %*% (vMat*chis)))+Zcol)
# 
# #   lambda3Mat = matrix(0,ncol=k, nrow=k)
# #   lambda3Mat[lower.tri(lambda3Mat)] = lambda3Mat[upper.tri(lambda3Mat)]= beta[(p+2*k+1):length(beta)]
#   
#   #The suymmetric jacobian matrix, equal to the fisher information. The upper part is filled first, then mirror image is taken for lower triangle
# 
# #   #dLag²/dr_{ik}dlambda_{1k}
...
# #       for(Kinner in (K+1):k){
# #         #dLag²/dr_{ik}dr_{ik'}
# #       diag(Jac[(p*(K-1)+1):(p*K),(p*(Kinner-1)+1):(p*Kinner)]) = -muzero/(1+muZero)^2 + lambda3Mat[Kinner, K]
# #       }
# #   }
# #     #dLag²/ds_{ik}dlambda_{3kk'}
# #     Jac[1:(p*k),((p+2)*k+1):(p*k+nLambda)] = unlist(sapply(1:(k-1), function(K){
...
# #   #Symmetrize
# #   Jac = Jac + t(Jac)
#      diag(c(colSums(-muZero /(1+muZero)^2)))
#     
# }
# #---------------------------------------#

ZIPouterLoop = function(X, k, tol = 1e-3, maxItOut = 500, psiTol = 1e-3, chiTol=psiTol, verbose = TRUE, libSizes = NULL, rMat = NULL, cMat = NULL, psis = NULL, chis=NULL, vMat =NULL, tMat = NULL, Zcol = NULL, global ="dbldog", nleqslv.control=list(),method=c("Broyden"), lambdaRow=NULL, lambdaCol = NULL, twoCores=TRUE, convNorm = 2, cMatMax=6, lambdaRowZero=NULL, lambdaColZero=NULL){
 ...
 ## 1) Initialization
  ...
  if(is.null(psis)){
  psis = svdX$d[1:k]#sort(, decreasing=TRUE) #*sqrt(colSums(rMatInit^2)*rowSums(t(t(cMatInit^2)*abunds)))
    # psiInit = assoc$phi[1,]
  }
  ...
  cMat[cMat>cMatMax] = cMatMax
  cMat[cMat< -cMatMax] = -cMatMax
  
  if(is.null(tMat)){
  tMat = matrix(rowMeans(X==0)-mean(rowMeans(X==0)), ncol = k, nrow=n)
    # rMatInit = assoc$row[,,1]
  }
  if(is.null(vMat)){
  vMat = matrix(colMeans(X==0)-mean(colMeans(X==0)), ncol = p, nrow=k)
    # cMatInit = t(assoc$col[,,1])
  }
    if(is.null(Zcol)){
Zcol = unlist(mclapply(1:ncol(X), mc.cores=detectCores()-1,function(i){
 tmp= try(zeroinfl(X[,i]~log(libSizes)|1)$coef$zero, silent=TRUE)
 if(class(tmp)=="try-error"){
   tmp = -Inf #If no zeroes => No chance on a structurla zero
 }
 tmp
}))
    # cMatInit = t(assoc$col[,,1])
  ...
  repZeroes = rep.int(0,2*k+k*(k-1)/2)
  if(is.null(lambdaRow)) lambdaRow =  repZeroes
  if (is.null(lambdaCol)) lambdaCol =  repZeroes
  if(is.null(lambdaRowZero)) lambdaRowZero =  repZeroes
  if (is.null(lambdaColZero)) lambdaColZero =  repZeroes

  iterOut = 1
  rowRec = rowRecZero = array(0,dim=c(NROW(X),k, maxItOut+1))
  colRec = colRecZero = array(0,dim=c(k,NCOL(X), maxItOut+1))
  psiRec = chiRec = matrix(0,ncol=maxItOut+1, nrow=k)
  ...
  #Expectation
  Z = Estep (X, rMat, cMat, tMat, vMat, abunds, k, libSizes, Zcol, psis, chis)

  #Maximization
  Mlist = Mstep(Z = Z, y = X, rMat = rMat, cMat = cMat, tMat = tMat, vMat = vMat, abunds=  abunds, k = k, n=n, p=p, libSizes = libSizes, Zcol=Zcol, psis = psis,chis = chis, twoCores=twoCores, tol = tol, psiTol = psiTol, chiTol = chiTol, convNorm = convNorm, global=global, nLambda = nLambda, nleqslv.control = nleqslv.control, lambdaCol = lambdaCol, lambdaRow=lambdaRow, lambdaColZero = lambdaColZero, lambdaRowZero = lambdaRowZero)
  ...
  convergence = ((iterOut <= maxItOut) && 
                   ...
                   ((sum((1-cMatOld/cMat)^convNorm)/p)^(1/convNorm) < tol) ) && all ((chiOld-chis) < chiTol) &&  (sum(abs(1-tMat/tMatOld)^convNorm))^(1/convNorm) < tol &&  (sum(abs(1-vMat/vMatOld)^convNorm))^(1/convNorm) < tol 
} # END while-loop
  ...
  if(!convergence ){
    ...
    return(list(rMat=rMat, cMat=cMat, psis = psis, X=X,
                converged = convergence, rowRec = rowRec, colRec = colRec, psiRec = psiRec, lambdaRow = lambdaRow, lambdaCol = lambdaCol, lambdaRowZero = lambdaRowZero, lambdaColZero = lambdaColZero, chis = chis, iterOut = iterOut, tMat = tMat, vMat = vMat, chiRec = chiRec, rowRecZero = rowRecZero, colRecZero = colRecZero, iter=iterOut))
}
#-------------------------------------------#
...
Nsamples= 300
Ntaxa = 900
lambdas = sample(ZIPmeans, Ntaxa)
zeroes = sample(zeroProbs, Ntaxa)

libSizesZIP =c(rep(1e4, floor(Nsamples/2)), rep(1e5, floor(Nsamples/2)))
psi1 = 7
psi2 = 5
...
idSignal1tax = sample(c(rep(TRUE, NtaxaSignal1), rep(FALSE, Ntaxa-NtaxaSignal1)))
idSignal2tax = sample(c(rep(TRUE, NtaxaSignal2), rep(FALSE, Ntaxa-NtaxaSignal2)))

idSignal1taxzero = sample(c(rep(TRUE, NtaxaSignal1zero), rep(FALSE, Ntaxa-NtaxaSignal1zero)))
idSignal2taxzero = sample(c(rep(TRUE, NtaxaSignal2zero), rep(FALSE, Ntaxa- NtaxaSignal2zero )))

colScores1 = rnorm(Ntaxa-Ntaxa%%2, sd=8)  + ifelse(idSignal1tax, Signal1tax,0)
colScores2 = rnorm(Ntaxa-Ntaxa%%2, sd=5) + ifelse(idSignal2tax, Signal2tax,0)

colScores1zero = rnorm(Ntaxa-Ntaxa%%2, sd=8)  + ifelse(idSignal1taxzero, Signal1zeroTax,0)
colScores2zero = rnorm(Ntaxa-Ntaxa%%2, sd=5) + ifelse(idSignal2taxzero, Signal2zeroTax,0)

NsamplesSignal1 = 20
NsamplesSignal2 = 20

NsamplesSignal1zero = 20
NsamplesSignal2zero = 20
...
idSignal1sam = sample(c(rep(TRUE, NsamplesSignal1), rep(FALSE, Nsamples-NsamplesSignal1)))
idSignal2sam = sample(c(rep(TRUE, NsamplesSignal2), rep(FALSE, Nsamples-NsamplesSignal2)))

idSignal1samzero = sample(c(rep(TRUE, NsamplesSignal1zero), rep(FALSE, Nsamples-NsamplesSignal1zero)))
idSignal2samzero = sample(c(rep(TRUE, NsamplesSignal2zero), rep(FALSE, Nsamples- NsamplesSignal2zero )))

rowScores1 = rnorm(Nsamples-Nsamples%%2, sd=8)  + ifelse(idSignal1sam, Signal1sam,0)
rowScores2 = rnorm(Nsamples-Nsamples%%2, sd=5) + ifelse(idSignal2sam, Signal2sam,0)

rowScores1zero = rnorm(Nsamples-Nsamples%%2, sd=8)  + ifelse(idSignal1samzero, Signal1zerosam,0)
rowScores2zero = rnorm(Nsamples-Nsamples%%2, sd=5) + ifelse(idSignal2samzero, Signal2zerosam,0)
...
colScoresMat = normalize(rbind(colScores1, colScores2),dim=1,weights=lambdas)

rowScoresMatzero = normalize(cbind(rowScores1zero, rowScores2zero),dim=2,weights=rep(1, length(rowScores1)))
colScoresMatzero = normalize(rbind(colScores1zero, colScores2zero),dim=1,weights=lambdas)


meanMat = outer(libSizesZIP, lambdas) * exp(rowScoresMat %*% (c(psi1, psi2)*colScoresMat))
zeroMat = expit(logit(matrix(zeroes, nrow=nrow(meanMat),ncol=ncol(meanMat), byrow = TRUE)) +rowScoresMatzero %*% (c(chi1, chi2)*colScoresMatzero))

dataMatZIP = rzipois(n=prod(dim(meanMat)),lambda = meanMat, pstr0 = zeroMat)
# mean(dataMatZIP==0) #Correct frequency of zeroes
dataMatZIPtrim = dataMatZIP[rowSums(dataMatZIP)>0, colSums(dataMatZIP)>0]

idSignal1sam = idSignal1sam[rowSums(dataMatZIP)>0]
idSignal2sam = idSignal2sam[rowSums(dataMatZIP)>0]
idSignal1samzero = idSignal1samzero[rowSums(dataMatZIP)>0]
idSignal2samzero = idSignal2samzero[rowSums(dataMatZIP)>0]

idSignal1tax = idSignal1tax[colSums(dataMatZIP)>0]
idSignal2tax = idSignal2tax[colSums(dataMatZIP)>0]
idSignal1taxzero = idSignal1taxzero[colSums(dataMatZIP)>0]
idSignal2taxzero = idSignal2taxzero[colSums(dataMatZIP)>0]
```

Fit the algorithm to the toy ZIP data

```{r fit Toy ZIP data}
maxItOut = 200
toyZIPresJob = mcparallel(ZIPouterLoop(X=dataMatZIPtrim, k = 2,nleqslv.control = nleqslv.control, maxItOut = maxItOut ))
toyZIPres=mccollect(toyZIPresJob, FALSE)[[1]]

#Commment out
rMat= rowScoresMat[rowSums(dataMatZIP)>0,]
cMat = colScoresMat[,colSums(dataMatZIP)>0]
tMat = rowScoresMatzero[rowSums(dataMatZIP)>0,]
vMat = colScoresMatzero[,colSums(dataMatZIP)>0]
k = 2
X = y = dataMatZIPtrim
library(pscl)
libSizes = rowSums(X)
...
psis = c(psi1, psi2)
global = "dbldog"
nleqslv.control=list(trace=TRUE, chkjac=FALSE, cndtol = .Machine$double.ep, allowSingular=TRUE, maxit = 150)
lambdaRow=lambdaCol=lambdaRowZero=lambdaColZero = rep(0,5)
n=nrow(X)
p=ncol(X)
Z=Estep(y = y, rMat = rMat, cMat = cMat, tMat = tMat, vMat = vMat, abunds=abunds, k=k, libSizes=libSizes, Zcol = Zcol, psis = psis, chis = chis)
chiTol = psiTol = 1e-4
```
