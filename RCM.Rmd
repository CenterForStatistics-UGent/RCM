...
reqpkg <- c("phyloseq", "MASS", "parallel", "nleqslv", "edgeR", "VGAM", "HMP", "ggplot2", "pscl", "vegan", "zCompositions", "DirFactor", "alabama", "reshape2", "tensor", "locfit", "cluster", "SpiecEasi", "ape")
...
groupsMeth = factor(c("Independence","Independence","Independence","Independence","Independence","DPCOA","Distance","Distance","Distance","Distance","Distance","Distance", "CoDa", "RCM"), levels = c("Independence", "Distance", "CoDa", "DPCOA", "RCM"), ordered = TRUE)
...
```{r libSizeCorr0b, purl = FALSE, fig.cap = "Correlations of row scores with library sizes, results of 10b0b simulations shown for first two dimensions. Red diamonds indicate means.\\label{fig:libCor1}", eval =FALSE}
...
```{r AbsCorr0b, purl = FALSE, fig.cap = "Correlations of column scores with average relative abundances, results of 10b0b simulations shown for first two dimensions. Red diamonds indicate means.\\label{fig:libCor1}", eval =FALSE}
plotCor(resList0bCols, datList = datList0b,scoreDim = "columns", dataMat = TRUE)
rm(resList0bRows, resList0b, datList0b)
```
...
rm(resList1Rows, resList1, datList1, resList1Cols)
```
...
contrPlot(contr2)
rm(resList2Rows, datList2, resList2Cols, contr2)
```
...
contrPlot(contr3)
rm(resList3Rows, resList3, datList3, resList3Cols, contr3)
```
...
rm(resList4Rows, resList4, datList4, resList4Cols)
```
...
contrPlot(contr5)
rm(resList5Rows, resList5Cols, contr5, datList5, IDlist5)
```
...
rm(resListPhyRows, resListPhy, datListPhy, resListPhyCols, contrPhy)
```
...
contrPlot(contrZINB)
rm(resListZINBRows, resListZINB, datListZINB, resListZINBCols, contrZINB)
```
...
```{r resListDM sampleBiplot, purl = FALSE, include = FALSE}
#A sample biplot
...
contrPlot(contrDM)
rm(resListDMRows, resListDM, datListDM, resListDMCols, contrDM)
```
...
```{r resListSimSeqHum sampleBiplot, purl = FALSE, include = FALSE}
#A sample biplot
...
```{r libSizeCorrZeller, purl = FALSE, fig.cap = "Correlations of row scores with library sizes, results of 1ZellerZeller simulations shown for first two dimensions for simulation with the Dirichlet-multinomial distribution with 10% DA taxa. Red diamonds indicate means.\\label{fig:libCor1}", eval = FALSE}
...
```{r resListSimSeqZellercols sampleBiplot, purl = FALSE, include = FALSE, eval = FALSE}
#A sample biplot
randomScatter(resListSimSeqZellerCols)
```

```{r AbsCorrZeller, purl = FALSE, fig.cap = "Correlations of column scores with average relative abundances, results of 1ZellerZeller simulations shown for first two dimensions for simulation with the Dirichlet-multinomial distribution with 10% DA taxa. Red diamonds indicate means.\\label{fig:libCor1}", eval = FALSE}
plotCor(resListSimSeqZellerCols, datListZeller, scoreDim = "columns", dataMat = TRUE)
```

Taxa scores of correspondence analysis tend to correlate with taxon abundances in some cases

```{r resListSimSeqZellerEvalsil, purl = FALSE, fig.cap = "Mean silhouettes by method, results of 100 simulations shown for first two dimensions for simulation with the Dirichlet-multinomial distribution with 10% DA taxa. Red diamonds indicate means. A silhouette of 1 point to perfect classification, 0 to borderline cases, -1 to perfect wrong classification \\label{fig:silhouette1}", eval = FALSE}
plotSil(resListSimSeqZellerRows, groupFactorZeller)
```

In the non-parametric setting RCM achieves by far the highest silhouette!

```{r resListSimSeqZellerEvaldist, purl = FALSE, fig.cap = "Mean ratios of within to overal distance by method, results of 100 simulations shown for first two dimensions for simulation with the Dirichlet-multinomial distribution with 10% DA taxa. Red diamonds indicate means.  \\label{fig:dist1}", eval = FALSE}
distRatioPlot(resListSimSeqZellerRows, groupFactorZeller)
```

In the non-parametric setting, RCM achieves the lowest distance ratios, CA the highest

```{r AbCorrZeller, purl = FALSE, fig.cap = "Correlations of column scores with abundances, results of 100 simulations shown for first two dimensions for simulation with the zero-inflated negative binomial distribution with 10% DA taxa. Red diamonds indicate means. 'Control' is a negative control setting of an independent standard normal variable. \\label{fig:abCorZeller}", eval = FALSE}
plotCor(resListSimSeqZellerCols, datListZeller, dataMat = TRUE, scoreDim = "columns")
```

CA suffers from correlation with relative abundances, but not on average.

```{r taxRatZeller, purl = FALSE, fig.cap = "Mean ratio of signal taxa to top 50% non-signal taxa in terms of contribution to cluster separation, results of 100 simulations shown for first two dimensions for simulation with the zero-inflated negative binomial distribution with 10% DA taxa. Red diamonds indicate means. \\label{fig:taxRatZeller}", eval = FALSE}
...
rm(resListZellerRows, resListZeller, datListZeller, resListZellerCols, contrZeller)
```
