...
estNPresp = function(sampleScore, muMarg, X, ncols, thetas, n, coefInit, coefInitOverall, dfSpline, vgamMaxit, colWeights, verbose,...){
 ...
 #No intercept needed: if the spline equals zero there is no departure from independence. Also we have only one additive term
      tmp = try(vgam(data = df,x ~ bs(sampleScore, degree = dfSpline), offset = logMu, family = negbinomial.size(lmu = "loge", size = thetas[i]), coefstart = coefInit[,i], maxit = vgamMaxit,...), silent = TRUE)
    # }
  ...
  overall = vgam(c(X) ~ bs(samRep, degree = dfSpline), offset = c(log(muMarg)), family = negbinomial.size(lmu = "loge", size = rep(thetas, each = n)), coefstart = coefInitOverall, maxit = vgamMaxit,...)
