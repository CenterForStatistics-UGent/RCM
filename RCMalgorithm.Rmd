...
reqpkg <- c("phyloseq", "ggplot2", "reshape2", "parallel", "RCM")
for(i in reqpkg)
{
...
```{r HMP filter out variable effect, purl=FALSE, fig.cap="Biplot of the RC(M) ordination of the HMP Anterior nares dataset after conditioning on main sequencing center. Colours indicate sequencing center. The effect of sequencing center has been largely filtered out. \\label{fig:samHMP}"}
...
```{r ZellerConstrainedResp, purl=FALSE, eval = FALSE, fig.cap="Non-parametric response functions of constrained RC(M) analysis on 16S Zeller data. Only the 8 most strongly reacting taxa are shown. \\label{fig:ZellerNPrespFun}"}
...
```{r humMicePlot, purl = FALSE, fig.cap = "RC(M) biplot of Turnbaugh B data. This dataset appears to have a very strong signal, we can distinguish three or four clusters. \\label{supfig:HumMice}"}
plot(HumMiceRCM)
```

```{r humMicePlotSecondLook, purl = FALSE, fig.cap = "RC(M) biplot of Turnbaugh B data. The separation in the first dimension is clearly an effect of Diet, the second dimension seems to be dominated by the date of collection \\label{supfig:HumMiceCol}"}
