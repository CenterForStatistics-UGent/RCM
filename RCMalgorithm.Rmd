...
```{r ZellerConstrainedBiplot, purl=FALSE, fig.cap="Triplot of constrained RC(M) analysis on metagenomic Zeller data with linear response functions. Cancer diagnosis and country appear to be the most important variables. \\label{fig:triZellerMGDiagnosis}"}
plot(zellerMphyRCMconstrLin, samColour = "Diagnosis", samShape = "Country", xInd = 2.5)
```

Some OTU seems completely out of touch with the rest. Plot the deviances of the taxa.

```{r ZellerConstrainedBiplotDev, purl=FALSE, fig.cap="Taxon plot of constrained RC(M) analysis on metagenomic Zeller data with linear response functions, with taxa coloured by deviances \\label{fig:taxZellerDiagnosis}"}
...
```{r ZellerConstrainedBiplotNonParam, purl=FALSE, eval = FALSE, fig.cap="Triplot of constrained RC(M) analysis on metagenomic Zeller data with non-parametric response functions. The ordination is very similar to the one for the 16S data. \\label{fig:triZellerDiagnosisNP}"}
plot.RCM(zellerMphyRCMconstrNonParam, samColour = "Diagnosis", samShape = "Country")
```

```{r ZellerConstrainedResp, purl=FALSE, eval = FALSE, fig.cap="Non-parametric response functions of constrained RC(M) analysis on metagenomic Zeller data. Only the 8 most strongly reacting taxa are shown. The outlier from the linear case is also among them. The most strongly reacting taxa decrease expected abundance with smaller environmental score \\label{fig:ZellerNPrespFunMG}"}
plotRespFun(zellerMphyRCMconstrNonParam, yLoc = -100)
```
