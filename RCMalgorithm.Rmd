...
reqpkg <- c("phyloseq", "ggplot2", "parallel", "RCM")
for(i in reqpkg)
{
  library(i, quietly=TRUE, verbose=FALSE, warn.conflicts=FALSE, character.only=TRUE)
} 
par(mar = c(4,7,4,4)) #Make sure the biplots are square!, cex.main = 0.9
palStore = palette()
# funFiles = dir("RCM/R")
funFilesPub = dir("pubFun")
# for (i in funFiles) {source(file.path("RCM/R",i))}
...
The algorithm requires the following inputs:
 ...
 The algorithm allows to supply the following optional inputs
  ...
  - a n-by-q design matrix __G__ of confounding variables, containing $z-1$ dummies for categorical variables with $z$ levels, and an intercept. The entry $g_{il}$ then represents the value of confounder variable $l$ in sample $i$.
...
The independence model is fitted as follows (see discussion below for explanation):

 1. Find starting values $u_{i, init} = log(\sum_{j=1}^p x_{ij})$ and $u_{j, init} = log(\frac{\sum_{i=1}^n x_{ij}}{\sum_{i=1}^n \sum_{j=1}^p x_{ij}})$ for $u_{i}$ and $u_j$ respectively
 2. Estimate a mean-dispersion trend using the estimateGLMTrendedDisp() function of the _edgeR_ package (version `r packageVersion("edgeR")`) \cite{Robinson2010edgeR}, given $u_i$ and $u_i$. This estimate is very insensitive to slight changes in the mean structure and will never be reestimated throughout the whole fitting procedure to save computation time.
...
5. Estimate the $u_{j,new}$'s using maximum likelihood, keeping the $\theta_j$'s and $u_i$'s constant
 ...
 $$\Big(\frac{1}{n}\sum_{i=1}^n |1-\frac{u_{i,new}}{u_{i,old}}|^a\Big)^{\frac{1}{a}} < 0.001 $$
 
 and
 
 $$\sqrt{\Big(\frac{1}{p}\sum_{j=1}^p (1-\frac{u_{j,new}}{u_{j,old}})^2\Big)} < 0.001$$
Once the independence model has converged, the estimates $u_i$ and $u_j$ are kept constant throughout the remainder of the fitting process. All maximum likelihood estimation is under the negative binomial model.
...
Again this step is performed iteratively by alternating between estimating the $\zeta_{jl}$ parameters and re-estimating the overdispersion parameters as in step 3 of the independence model. Convergence is then assumed when $\sqrt{\frac{1}{pq} \sum_{l=1}^q \sum_{j=1}^p (1-\frac{\zeta_{jl,new}}{\zeta_{jl,old}})^2}$ drops below a tolerance level of 0.001.
...
1. Obtain a singular value decomposition as $R^{-1}(X-E)C^{-1} = U\Sigma V$, with __E__ the matrix of expected values fitted so far, and __R__ and __C__ diagonal matrices with row and column sums of __X__ on the diagonal respectively. This gives us initial values $[r_{i1}^{SVD}, r_{i2}^{SVD},..., r_{iK}^{SVD}] = U_{(1.)}$, the first row of $U$, and correspondingly $\Sigma$ and $V$ give initial values for $\boldsymbol{\psi}^{SVD}$ and $\mathbf{s}^{SVD}$. For these initial values we still need to ensure that the (weighted) variances equal 1, and transfer these weights to the importance parameters $\mathbf{\psi}^{SVD}$ so we set
...
b) Estimate the importance parameter $\psi_k$ by full ML, forcing it to be positive and keeping the sample and taxon scores and overdispersions fixed.
c) Estimate the sample scores $r_{ik}$ by restricted ML, keeping the taxon scores, overdispersions and importance parameters fixed. Lagrangian multipliers are used to ensure that

$$ \sum_{i=1}^n r_{ik} = 0$$
and 

$$ \forall_{k'<k} k': \sum_{i=1}^n r_{ik}r_{ik'} = \delta_{kk'}$$
...
$$ \forall_{k'<k} k': \sum_{j=1}^p z_js_{jk}s_{jk'} = \delta_{kk'}$$
...
with the term between [] being optional. For this model four components need to be fitted iteratively:
 ...
 1. Standardize the covariate matrix. To render the values of the continuous variables in the environmental gradient comparable, it is clear that they need to be centered and scaled prior to model fitting, as in PCA. This means that their corresponding elements of $\boldsymbol{\alpha}$ represent the contribution to the environmental score of one standard deviation away from the mean of this variable. A perfect quantitative comparison to the magnitude of the values of the paramaters of the dummies of the categorical variables will never be possible. In our case, with 0-1 dummy coding, equal parameters for a dummy and a continuous variable imply that this level of the categorical variable contributes as much to the environmental score as one standard deviation away from the overall mean of the continuous variable.
 
 2. Starting values for $\boldsymbol{\alpha}$ are obtained from a constrained correspondence analysis by the $cca()$ function in the $vegan$ package \cite{vegan}. Next they are normalized to fulfil the $\boldsymbol{\alpha}^t_k\boldsymbol{\alpha}_k = 1$ requirement by setting

$$\boldsymbol{\alpha}_k = \frac{\boldsymbol{\alpha}_k^{cca}}{\sqrt{\sum_{y=1}^d \alpha_{yk}^2}} $$ 
 
Starting values for $\boldsymbol{\psi}$ are the eigenvalues of the constrained correspondence analysis. For the response functions no starting values are calculated.
 ...
 For non-parametric response functions this relies on cubic splines as defined by the $s()$ function of the $VGAM$ package \cite{VGAM}. Also for non-parametric response functions the area between the response functions and the x-axis over the range of environmental scores $\boldsymbol{\alpha}_k^t\mathbf{C}_i$ is calculated as $a_{jk}$. The importance parameter psi is now defined to be 
 ...
 with $g_{NB}$ the density function of the negative binomial distribution, and under the restrictions that:
  ...
  - $\boldsymbol{\alpha}_k^t\boldsymbol{\alpha}_k' = \delta_{kk'}$
  - Components of $\alpha_k$ belonging to dummies of the same categorical variable sum to zero

This approach encourages maximal niche separation between the species. The latter restriction is convenient for later plotting and also assures avoids dependence of the solution on the choice of reference level, in the light of the normalization step in e).
 
However, the separated niche concept is not accepted by all ecologists. If niches are really maximally separated, how can species co-occur then? An alternative option is therefore to estimate $\alpha$ by maximizing only the numerator in the previous equation, but under the same restriction. Surprisingly, the solutions of both approaches are very similar, although not exactly identical. The full ML solution is also much faster.
 ...
 $$\forall w: \sqrt{\sum_{j=1}^p \Big(1-\frac{\beta_{wjk}^{new}}{\beta_{wjk}^{old}}\Big)^2} < 0.001$$
...
However, the library sizes $x_{i.}$ do not correspond to the maximum likelihood estimate of $u_i$ under the negative binomial model. As a result the first dimensional row scores $r_{1i}$ tried to correct for this discrepancy and became related (linearly correlated) to the library sizes. This effect of sequencing depth on the sample ordination is something we want to avoid absolutely. Therefor we estimate the $u_i$'s, $u_j$'s iteratively using maximum likelihood, which also implies dispersion estimation as outlined above.

In practice, the marginal sums differ more from the MLE for the library sizes than for the taxon abundances, as can be seen from Supplementary Figures \ref{supfig:libSizes} and \ref{supfig:abunds}. 

```{r libSizes, fig.show = "hold", echo = FALSE, fig.cap = "Scatterplots on the log-scale of row sums (library sizes)  versus maximum likelihood estimates of sequencing depth for the Kostic dataset \\label{supfig:libSizes}", fig.width = 5, fig.height = 4}
load("/home/stijn/PhD/Biplots/kosticRCM.RData")
par(mar = c(4 , 9, 4, 4), cex.main = 0.9, cex.lab = 0.9)
libSizes = rowSums(kosticRCM$X)
abunds = colSums(kosticRCM$X)/sum(kosticRCM$X)
plot(log = "xy",libSizes, kosticRCM$libSizes, ylab = "log(Maximum likelihood estimates \nof sequencing depths)", xlab = "log(Library sizes)", main = paste("Pearson correlation on log-scale: ",round(cor(log(libSizes), log(kosticRCM$libSizes)),3))); abline(0,1)
```

```{r Abunds, fig.show = "hold", echo = FALSE, fig.cap = "Scatterplots on the log-scale of column sums (marginal taxon abundances) versus maximum likelihood estimates of abundance respectively for the Kostic dataset \\label{supfig:abunds}", fig.width = 5, fig.height = 4}
par(mar = c(4 , 9, 4, 4), cex.main = 0.9, cex.lab = 0.9)
plot(abunds, kosticRCM$abunds, ylab = "log(Maximum likelihood estimates \nof taxon abundances)", xlab = "log(Marginal taxon abundances)", log = "xy", main = paste("Pearson correlation on log-scale: ", round(cor(log(abunds), log(kosticRCM$abunds)),3))); abline(0,1)
```
...
Note that our definition of the response function differs from the response function most common one \cite{terBraak1986, Zhu2005, Yee2006}: it no longer models mean abundance but mean departure from independence.
...
## Relationship between unconstrained RC(M) and existing methods
...
There exist many variation so of correspondence analysis, but all are concerned with the difference between the observed count __X__ and the expected counts based on the margins (the independence model) __E__: $X-E$. This means that the signal can come from observations that are either smaller or larger than expected. This two-way mode of deviations will give rise to the unimodality of correspondence analysis.
...
with $a_i = c_i = x_{i.} = \sum_{j=1}^px_{ij}$ and $b_j = d_j = \frac{x_{.j}}{x_{..}}$ with $x_{.j} = \sum_{i=1}^nx_{ij}$. K = min(n,p) and the terms are ordered such that $\omega_1 >\omega_2>...>\omega_K$. We can then write this model alternatively as

$$E(X_{ij}) = \frac{x_{i.}x_{.j}}{x_{..}} \big(1+\sum_{k=1}^K \omega_k v_{ki} w_{jk}\big)$$

When fitted, the second series of terms will attempt to repair discrepancies between __X__ and __E__ and as such capture departures from independence. This is called the *reconstitution formula* since it decomposes the observed average count into its expectation under independence and a residual. The residual is then further decomposed into $k$ orthogonal pieces.
...
Correspondence analysis performs a singular value decomposition of the following matrix
...
with $\mathbf{1'}R^{1/2}U=\mathbf{0}$ and $\mathbf{1'}J^{1/2}V=\mathbf{0}$ (weighted means of rows and columns equal zero) and $U'R^{1/2}U=\mathbf{1}$ and $V'K^{1/2}V=\mathbf{1}$ (weighted variances equal one) \cite{VanDerHeijden1985}.

#### Link to the RC(M)-model

If $a =\sum_{k=1}^K \omega_k v_{ik} w_{jk}$ is small (i.e. the deviation from independence is small) then $log(1+a) \approx a$, and the counts are Poisson distributed then

$$log(E(x_{ij})) = log(x_{i.}) + log(x_{.j}) - log(x_{..}) + log\big(1 + \sum_{k=1}^K \omega_k v_{ik} w_{jk}\big) \approx log(x_{i.}) + log(x_{.j}) - log(x_{..}) + \sum_{k=1}^K \omega_k v_{ik} w_{jk}$$

i.e. an equivalence exists between the RC(M)-model and correspondence analysis \cite{Escoufier1982, VanDerHeijden1994}.

If the same restricitions apply to the scores $v_{ik}$ and $w_{jk}$ as to U and V, we can state that  $\psi_k \approx \omega_k$, $v_{ik} \approx r_{ik}$ and $w_{jk} \approx s_{jk}$. The assumption that the departure from independence is small seems unlikely for microbiome data, but it does provide useful starting values for the fitting of the RC(M) model.

### Gomms

The $gomms$ package \cite{Sohn2017} implements a similar mean model to the RC(M) model, but the differences lie mainly in the error distribution. Our RC(M) model assumes a negative binomial model with unique dispersion parameters for every taxon. The $gomms$ methods employs a zero-inflated quasi Poisson distribution with a common overdispersion parameter. It is well known however, that taxa overdispersion differ a lot in sequencing data \cite{Robinson2007, Anders2010}. Also, the zero-inflated component raises the need for an EM-algorithm which is numerically unstable. The $gomms$ package does not exploit the taxon scores for making biplots, and no diagnostic plots are available.

### Row-column interaction models

Our RC(M) method is an application of row-column interaction models (RCIM) proposed in \cite{Yee2014}, namely the model already introduced by \cite{Goodman1979}. We have written our own, faster implementation and improved dispersion estimation for the negative binomial error model.

## Relationship between constrained RC(M) and other methods

### Constrained correspondence analysis

The solution of constrained correspondence analysis (CCA) corresponds to that of quadratic response curves with tolerances equal for all taxa and augmented with a Poisson distribution. Whereas the constrained RC(M) model models the departure from independence in a multiplicative way by estimating an independence model, correspondence analysis captures departure from independence in an additive way by modelling the residuals. In CCA the environmental gradients are not made orthogonal as in the RC(M)-model, but the sample scores are \cite{terBraak1986}.

### Environmental gradient estimation

Most existing methods to estimate response functions and environmental gradients use linear, quadratic and non-parametric response function as in our RC(M) methods \cite{Zhu2005, Yee2015}. They fail however to account explicitly for differences in sequencing depth and taxon abundance, as our method does by estimating the independence model. Our RC(M) method has borrowed the log-ratio approach of estimating the environmental gradient based on niche separation from \cite{Zhu2005}.
...
The deviance residuals $d_{ij}$ of the negative binomial distribution are defined as \cite{Zwilling2013}:
...
The Human Microbiome Project (HMP) aimed to characterize the healthy human microbiome \cite{Group2009}.
...
```{r HMP filter out variable effect, purl=FALSE, fig.cap="Biplot of the RC(M) ordination of the HMP Anterior nares dataset after conditioning on main sequencing center. Colours indicate sequencing center. The effect of sequencing center has been largely filtered out. \\label{fig:samHMPfilt}"}
...
```{r AGPplots, purl = FALSE, fig.cap="Biplot of the RC(M) ordination of the American gut dataset. As expected the dataset is very noisy and we do not find a clear signal. \\label{fig:samAGP}"}
plot(resListRCM[["AGP"]])
```
...
```{r plotZellerSphyDiagnosis, purl = FALSE, fig.cap="Biplot of the RC(M) ordination of the 16S Zeller data. We see some gradient in function of cancer diagnosis, but there is still a lot of remaining variability. \\label{fig:samZeller16S}"}
plot(zellerSphyRCM, samColour = "Diagnosis", samShape = "Country")
```

```{r plotZellerMphy, purl = FALSE, fig.cap = "Biplot of the RC(M) ordination of the metagenomics Zeller data. No clear gradient related to the diagnosis is visible. \\label{fig:samZellerMGS}"}
plot(zellerMphyRCM, samColour = "Diagnosis", samShape = "Country")
```
...
In the constrained analysis we use all the available covariates to construct the environmental gradient: age, gender, diagnosis, BMI and country.
...
```{r ZellerConstrainedBiplot, purl=FALSE, fig.cap="Triplot of constrained RC(M) analysis on 16S Zeller data with linear response functions. Cancer diagnosis and country appear to be the most important variables. \\label{fig:triZellerDiagnosis}"}
plot(zellerSphyRCMconstrLin, samColour = "Diagnosis", samShape = "Country")
```

```{r ZellerConstrainedBiplotNonParam, purl=FALSE, eval = FALSE, fig.cap="Triplot of constrained RC(M) analysis on 16S Zeller data with non-parametric response functions. The environmental gradient is similar as to the linear case, only age is more important according to this model. \\label{fig:triZellerDiagnosisNP}"}
plot(zellerSphyRCMconstrNonParam, samColour = "Diagnosis", samShape = "Country")
```

```{r ZellerConstrainedResp, purl=FALSE, fig.cap="Non-parametric response functions of constrained RC(M) analysis on 16S Zeller data. Only the 8 most strongly reacting taxa are shown. \\label{fig:ZellerNPrespFun}"}
plotRespFun(zellerSphyRCMconstrNonParam)
```

```{r ZellerConstrainedBiplotMGS, purl=FALSE, fig.cap="Triplot of constrained RC(M) analysis on metagenomic Zeller data with linear response functions. Cancer diagnosis and country appear to be the most important variables. \\label{fig:triZellerMGDiagnosis}"}
plot(zellerMphyRCMconstrLin, samColour = "Diagnosis", samShape = "Country", xInd = 2.5)
```

Some mOTU seems completely out of touch with the rest. Plot the deviances of the taxa.

```{r ZellerConstrainedBiplotDev, purl=FALSE, fig.cap="Taxon plot of constrained RC(M) analysis on metagenomic Zeller data with linear response functions, with taxa coloured by deviances \\label{fig:taxZellerMGSDiagnosis}"}
plot(zellerMphyRCMconstrLin, plotType = "species", taxCol = "Deviance", taxDots = TRUE, square = FALSE, arrowSize = 0)
```

The taxon in question does not exhibit an exceptionally poor fit, but is definitely worth further investigation.

```{r ZellerConstrainedBiplotNonParamMGS, purl=FALSE, eval = FALSE, fig.cap="Triplot of constrained RC(M) analysis on metagenomic Zeller data with non-parametric response functions. The ordination is very similar to the one for the 16S data. \\label{fig:triZellerMGSDiagnosisNP}"}
plot(zellerMphyRCMconstrNonParam, samColour = "Diagnosis", samShape = "Country")
```

```{r ZellerConstrainedRespMGS, purl=FALSE, eval = FALSE, fig.cap="Non-parametric response functions of constrained RC(M) analysis on metagenomic Zeller data. Only the 8 most strongly reacting taxa are shown. The outlier from the linear case is also among them. The most strongly reacting taxa decrease expected abundance with smaller environmental score \\label{fig:ZellerNPrespFunMG}"}
plotRespFun(zellerMphyRCMconstrNonParam, yLocVar = -100)
```
...
In this study the fecal microbiome of monozygotic and dizygotic twins was characterized \cite{Turnbaugh2009} (Turnbaugh A data). On of the aims was to find a link with obesity.

```{r turnTwins, purl = FALSE}
...
```{r turnBaughAplot, fig.cap = "RC(M) biplot of the Turnbaugh A data. No signal in function of weight status is evident, with such small datasets it is hard to draw conclusions.\\label{supfig:Twins}"}
plot(turntwinsRCm, samColour = "Status_p", colLegend = "Weight status")
rm(turntwinsRCm)
```
...
In this group, 15 gnotobiotic mice were inoculated with human feces, and for one group the diet was switched to a Western diet after one month \cite{Turnbaugh2009b} (Turnbaugh B data). Then a second generation of mice was inoculated with cecal samples from the previous groups, and here also diet was varied. The variables recorded were:
...
HumMiceRCMlin = RCM(HumMicePhy, k=3, covariates = c("Diet","Generation_p","TimeSinceStart"))
HumMiceRCMnonParam = RCM(HumMicePhy, k=3, covariates = c("Diet","Generation_p","TimeSinceStart"), responseFun = "nonparametric")
gommsHum = gommsPhy(HumMicePhy)
save(HumMiceRCM, HumMiceRCMnonParam, gommsHum, file = "turnMiceRCM.RData")
} else {load("turnMiceRCM.RData")}
```

```{r humMicePlotSecondLook, purl = FALSE, fig.cap = "RC(M) biplot of Turnbaugh B data. This dataset appears to have a very strong signal, we can distinguish three or four clusters. The separation in the first dimension is clearly an effect of Diet, the second dimension seems to be dominated by the date of collection \\label{supfig:HumMiceCol}"}
plot(HumMiceRCM, samColour = "TimeSinceStart", samShape = "Diet", colLegend = "Days since \nfirst collection")
```
...
```{r KosticPlots, fig.cap="RC(M) biplot of Kostic data. Country of data collection seems to have a strong impact on the composition \\label{supfig:Kostic}"}
plot(kosticRCM, samColour = "COUNTRY", samShape = "RADIATION_THERAPY_POOLED")
```

Country, chemotherapy and radiotherapy seem to be related to microbiome composition

```{r KosticPlotsConstrained, fig.cap="Constrained RC(M) triiplot of Kostic data. Country of data collection and cancer status are clearly the main drivers of the ordination. This is surprising since in the unconstrained ordination the role of cancer status was lost in the noise \\label{supfig:KosticLin}"}
plot(kosticRCMlin, samColour = "COUNTRY", samShape = "TREATMENT", samLegend = "Cancer status", colLegend = "Country")
```
...
This is a longitudinal dataset on microbial growth in a water cooling system of a nuclear facility \cite{Props2016}.
...
```{r plotCMET, purl = FALSE, fig.cap = "RC(M) biplot of the Props data. The first dimension mainly tracks the change of the cooling water throughout the different phases. There is a certain arch effect visible, which may indicate samples from the startup and shutdown are somewhat similar. \\label{supgfig:Props}"}
plot(CMETRCM, samColour = "Reactor.phase", samShape = "Reactor.cycle", taxNum = 6)
```

```{r plotCMETlin, purl = FALSE, fig.cap = "Constrained RC(M) triplot with linear response functions of the Props data. The constrained analysis confirms phase and cycle as being the most important drivers. Also, the most important taxa are seen to react to the gradient dominated by reactor cycle. \\label{supgfig:PropsConstr}"}
plot(CMETRCMlin, samColour = "Reactor.phase", samShape = "Reactor.cycle")
```

```{r plotCMETnp, purl = FALSE, fig.cap = "Constrained RC(M) biplot with non-parametric response functions of the Props data. This confirms the dominant role of reactor cycle and phase in shaping the ordination. The sample ordination looks very similar to the linear response case, suggesting a good fit for linear response functions \\label{supgfig:PropsConstrNP}"}
