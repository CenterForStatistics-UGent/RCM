...
```{r plotDevResp}
residualPlot(zellerSphyRCMconstrLin, whichTaxa ="response")
```

The same taxa but with Pearson residuals 

```{r plotPearResp}
residualPlot(zellerSphyRCMconstrLin, whichTaxa = "response", resid = "Pearson")
```
...
```{r plotDevRuns}
residualPlot(zellerSphyRCMconstrLin, whichTaxa = "runs", resid = "Deviance")
```

```{r plotPearRuns}
residualPlot(zellerSphyRCMconstrLin, whichTaxa = "runs", resid = "Pearson")
```

For these taxa we see no signal at all. The runs test statistic may be dominated by the number of zero counts, and not be informative at all.
