...
contrPlot = function(contr, method = NULL, las = 2, expFac = 1.2, groupMeth = droplevels(groupsMeth[names(groupsMeth) %in% levels(factorMeth)[levelsMeth %in% names(contr)] & if(is.null(method)) TRUE else names(groupsMeth) %in% method]), ylim = c(0,max(sapply(contr, quantile, 0.95, na.rm = TRUE))), diamCol = "orange",...) {
  ...
  boxplot(contrRatio~Method, main = "Mean contribution of DA taxa relative to \n the 50% most contributing non-DA taxa by method", data = moltenContr, ylab = "Mean contribution ratio", col = groupMeth, las = las, at = at, ylim = ylim,...)
  points(y = meansDist, x = at, col = diamCol, pch=18, cex = 1.6)
  abline(h=1, lty = "dashed")
  addLegend(groupMeth = groupMeth, expFac = expFac, yloc = 5)
}