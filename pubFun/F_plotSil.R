...
plotSil = function(resListRows, groupFactor, method = NULL,
                   groupMeth = droplevels(groupsMeth[names(groupsMeth) %in% levels(factorMeth)[levelsMeth %in% names(resListRows[[1]])] & if(is.null(method)) TRUE else names(groupsMeth) %in% method]),
                   las = 2, diamColor = "orange",...){
