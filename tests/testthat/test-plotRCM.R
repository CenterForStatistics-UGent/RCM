...
test_that("Correct plotting commands yield ggplot2 objects", {
    expect_s3_class(
        plot(unconstrRcm, samColour = "Influence", inflVar = "psi"),
        "ggplot"
    )
    expect_s3_class(
        plot(constrRcm,
            inflVar = "DiagnosisNormal", samColour = "Influence",
            samShape = "Diagnosis"
        ),
        "ggplot"
    )
