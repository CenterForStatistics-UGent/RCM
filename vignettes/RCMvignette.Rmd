...
By default, only 2 dimensions are fitted, but more can be requested throught the _k_ argument.
...
The simplest axis labels are simply the importance parameters $\psi_m$ from the RC(M)-model. They directly reflect the impact on the mean counts in the respective dimension, as the factors to which they are multplied are normalized. Hence the $\psi_m$'s are the only components that can grow in size to add more weight to a dimension.

On the other hand, the sample scores _have_ already been scaled with respect to these parameters, and they do not add addtional information to the plot itself. Also, their order is often but not necessarily strictly decreasing. Only the ratios of the different $\psi_m$'s are direclty interpretable. Still plotting the $\psi_m$'s is the default option of the _RCM_ package.
...
Another way to quantify the importance of the dimensions is to compare their differences in log-likelihoods of model m ($ll_m$) with respect to the saturated model ("deviances"). The log-likelihood saturared model ($ll_{sat}$) is calculated using the Poisson density, setting mean and variance equal to the observed counts. These differences in log-likelihood are then normalized with respect to the difference in loglikelihood between the independence model ($ll_{independence}$) and the saturated model. The terms obtained for dimension $m$ is then:
...
with $x_{ij}$ the observed count and $e_{ijm}$ the expected count under the model with $m$ dimensions. The inertia has the advantage that also the variance explained by the filtering on confouders step can be plotted, and that there is a measure of residual variance.
